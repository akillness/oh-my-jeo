...
description: [omj] Hermes Scheduled Ops Blueprint workflow: design recurring Hermes operations with schedule, delivery, silence policy, context chain, and prepared-vs-observed status.
metadata:
  hermes:
    tags: [workflow, oh-my-jeo, operations]
    category: operations
...
`automation-blueprint` exists so Hermes can make recurring operational work feel native and scheduled without OMJ becoming a hidden cron runner, transport bot, source retriever, or executor.
...
```sh
omj runtime record --skill automation-blueprint --harness scheduled-ops-blueprint --status started
