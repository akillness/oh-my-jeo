...
description: [omj] Hermes Research Department workflow pack: prepare Scout, Analyst, and Briefer research operations with source inbox and briefing status boundaries.
metadata:
  hermes:
    tags: [workflow, oh-my-jeo, research]
    category: research
...
`research-department` exists so Hermes users can start complex research-ops patterns without manually designing profiles, cron, knowledge storage, synthesis tooling, and delivery glue, while OMJ keeps every runtime claim observed-only.
...
```sh
omj runtime record --skill research-department --harness research-department --status started
