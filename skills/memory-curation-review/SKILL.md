...
description: [omj] Hermes memory curation workflow: review stale, conflicting, duplicate, or risky memories and skill notes through approve/reject/update actions.
metadata:
  hermes:
    tags: [workflow, oh-my-jeo, memory]
    category: memory
...
`memory-curation-review` exists so Hermes users can ask for this workflow in chat and receive a structured, evidence-bounded OMJ operating surface instead of ad hoc narration.
...
```sh
omj runtime record --skill memory-curation-review --harness memory-curation-review --status started
