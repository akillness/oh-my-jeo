---
name: ultragoal
description: [omj] Hermes Ultragoal workflow: file-backed durable goal ledgers.
metadata:
  hermes:
    tags: [workflow, oh-my-jeo, execution]
    category: execution
    phase: durable-goals
    role: handoff-guide
...
Use Hermes to maintain .omj/goals goal_ledger/v1 state, show goal_status_card/v1 / goal_continuation/v1 next actions, and route coding milestones to the selected runtime profile with only observed runtime evidence.
...
```sh
omj runtime record --skill ultragoal --harness goal-execution --status started
