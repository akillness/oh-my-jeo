...
description: [omj] Hermes agent ops review workflow: help a manager inspect AI-agent research, coding, review, status, blockers, quality gates, and throughput levers.
metadata:
  hermes:
...
`agent-ops-review` exists so Hermes users can ask for this workflow in chat and receive a structured, evidence-bounded OMJ operating surface instead of ad hoc narration.
...
```sh
omj runtime record --skill agent-ops-review --harness agent-ops-review --status started
