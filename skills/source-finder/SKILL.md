---
name: source-finder
description: [omj] Hermes Source Finder workflow: prepare typed source candidates and acquisition status before downstream work.
metadata:
  hermes:
...
`source-finder` exists so Hermes can turn vague source discovery requests into typed candidates, acquisition status, and downstream workflow choice without pretending OMJ searched, downloaded, or verified the material.
...
```sh
omj runtime record --skill source-finder --harness source-finder --status started
