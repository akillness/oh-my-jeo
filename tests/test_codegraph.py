...
""".lstrip(),
    ...
    _write(
        root / "src" / "plugin_bundle" / "omj" / "tools" / "demo_tool.py",
        """
...
class CodegraphTests(unittest.TestCase):
    def test_build_graph_json_shape_and_internal_edges(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            from omj.codegraph import build_codegraph

            graph = build_codegraph(repo, generated_at="2026-01-01T00:00:00Z")

        self.assertEqual(graph["schema_version"], "omj_codegraph/v1")
        ...
        self.assertIn("plugin_tool", files["src/plugin_bundle/omj/tools/demo_tool.py"]["entrypoint_tags"])
    ...
    def test_build_command_writes_artifact_and_prints_json(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            artifact_path = repo.resolve() / ".omj" / "codegraph" / "codegraph.json"
            self.assertTrue(artifact_path.exists())
            written = json.loads(artifact_path.read_text(encoding="utf-8"))

        self.assertEqual(payload["schema_version"], "omj_codegraph/v1")
        self.assertEqual(written["schema_version"], "omj_codegraph/v1")
    ...
    def test_summary_human_output_keeps_boundary_visible(self) -> None:
        ...
        self.assertEqual(status, 0, stderr)
        self.assertEqual(stderr, "")
        self.assertIn("OMJ codegraph summary", stdout)
    ...
    def test_handoff_context_is_compact_prepared_context(self) -> None:
        ...
        self.assertEqual(stderr, "")
        payload = json.loads(stdout)
        self.assertEqual(payload["schema_version"], "omj_codegraph_context/v1")
    ...
    def test_src_layout_package_dir_does_not_invent_omj_modules(self) -> None:
        with TemporaryDirectory() as tmp:
            repo = Path(tmp)
            _write(
                repo / "pyproject.toml",
...
""".lstrip(),
        ...
        self.assertNotIn("omj.realpkg.cli.main", files["src/realpkg/cli.py"]["defines"])
        self.assertIn(
...
""".lstrip(),
        ...
        files = {record["path"]: record for record in graph["files"]}
        self.assertIn("omj.codegraph.scanner.build", files["src/codegraph/scanner.py"]["defines"])
    ...
    def test_build_write_filesystem_error_is_cli_error_not_traceback(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            (repo / ".omj").write_text("not a directory", encoding="utf-8")

            status, stdout, stderr = run_cli(["codegraph", "build", "--repo", str(repo), "--write"])

        self.assertNotEqual(status, 0)
        self.assertEqual(stdout, "")
        self.assertIn("omj:", stderr)
