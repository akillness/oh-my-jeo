...
from omj.coding_lifecycle import start_codex_delegation_lifecycle
from omj.memory import (
...
class MemoryContractTests(unittest.TestCase):
    ...
    def test_apply_batch_is_dry_run_idempotent_and_path_safe(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertFalse((paths.omj_home / "memory").exists())
            ...
            memory_file = paths.omj_home / "memory" / "scopes" / "project.json"
    ...
    def test_memory_inspection_ignores_symlink_scope_escapes(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            outside.write_text(
                json.dumps(
                    {
                        "schema_version": "omj_memory_scope/v1",
                        "scope": {"kind": "project", "ref": "default"},
