...
load_local_package()
from omj.coding_lifecycle import record_codex_dispatch, record_codex_result, record_codex_verification, start_codex_delegation_lifecycle
from omj.codex_progress import summarize_codex_jsonl_text
from omj.paths import resolve_paths
from omj.profiles.setup import write_setup_profile
from omj.runtime_artifacts import (
    create_run,
    export_runtime,
    validate_runtime,
    write_ci_record,
...
from omj.runtime_records import validate_wrapper_session_record
from omj.wrapper.executor_sessions import (
...
from omj.wrapper_sessions import (
    WrapperSessionError,
...
class WrapperSessionTests(unittest.TestCase):
    ...
    def test_session_start_is_scoped_by_hermes_target_metadata(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            paths = resolve_paths(root / ".omj", root / ".hermes-a")
    ...
    def test_plan_acceptance_gates_handoff_and_links_run_ledger(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertTrue(accepted["status"]["chat_response"]["headline"].startswith("[omj] handoff - "))
            self.assertEqual(accepted["status"]["chat_response"]["usage_trace"]["visible_prefix"], "[omj] handoff")
    ...
    def test_coding_briefing_is_metadata_only_and_keeps_cards_compact(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            context_pack = {
                ...
                "source_refs": [{"source": "omj_memory", "truth_level": "approved_context", "precedence": 60, "item_count": 1}],
                "included_context": [
                    {
                        "item_id": "ctx-1",
    ...
    def test_hermes_selection_prepares_runtime_handoff_without_executor_run(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(prepared["handoff"]["runtime_handoff"]["runtime_profile"]["runtime_family"], "omj")
    ...
    def test_codex_wrapper_session_exposes_open_attach_record_actions(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertTrue(codex_launch["not_omj_backend_execution"])
            ...
            self.assertTrue(codex_launch["resume_capability"]["not_omj_backend_execution"])
    ...
    def test_non_terminal_executor_launches_are_prompt_only(self) -> None:
        for executor in ("generic", "hermes", "omx-runtime", "omo-runtime", "omc-runtime"):
            with self.subTest(executor=executor):
                ...
                self.assertTrue(launch["not_omj_backend_execution"])
    ...
    def test_export_runtime_run_scope_includes_only_linked_wrapper_sessions(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            first = create_or_resume_wrapper_session(
                paths,
                "risky refactor first scoped runtime fix in src/omj/runtime/artifacts.py",
                source="discord",
            ...
            second = create_or_resume_wrapper_session(
                paths,
                "risky refactor second unrelated runtime fix in src/omj/runtime/artifacts.py",
                source="discord",
