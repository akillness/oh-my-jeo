...
class ApplicationCaseArtifactTests(unittest.TestCase):
    def _record_case(self, omj_home: Path, skill: str, harness: str, status: str) -> dict:
        code, stdout, _ = run_cli(["--omj-home", str(omj_home), "runtime", "record", "--skill", skill, "--harness", harness, "--status", status])
        ...
        self.assertEqual(code, 0)
        code, stdout, _ = run_cli(["--omj-home", str(omj_home), "runtime", "show", run["run_id"]])
        self.assertEqual(code, 0)
    ...
    def test_three_application_cases_create_runtime_artifacts(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            cases = [
                self._record_case(omj_home, "oh-my-jeo", "coding-handling", "started"),
                self._record_case(omj_home, "ultragoal", "goal-execution", "started"),
                self._record_case(omj_home, "code-review", "critic", "completed"),
            ]
            ...
            for case in cases:
                ...
                self.assertTrue((omj_home / "runtime" / "runs" / case["run"]["run_id"] / "run.json").exists())
    ...
    def test_docs_describe_artifact_backed_cases(self) -> None:
        ...
        self.assertIn("omj runtime show", cases)
        self.assertIn("omj cases artifact --all --write", cases)
        self.assertIn(".omj/use-cases/artifacts/", cases)
        self.assertIn("omj cases replay --json", cases)
        self.assertIn("omj cases readiness --json", cases)
    ...
    def test_g1_to_g10_demo_fixture_is_wrapper_renderable(self) -> None:
        ...
        self.assertEqual(fixture["schema_version"], "omj_use_case_demo_collection/v1")
        ...
        for card in fixture["cards"]:
            with self.subTest(card=card["goal"]):
                ...
                self.assertEqual(card["wrapper_card"]["component"], "omj_use_case_card")
    ...
    def test_g1_to_g10_use_case_artifacts_are_local_runbooks(self) -> None:
        ...
        self.assertEqual(collection["schema_version"], "omj_use_case_artifact_collection/v1")
    ...
    def test_use_case_artifacts_can_be_written_and_validated(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            code, stdout, stderr = run_cli(["--omj-home", str(omj_home), "cases", "artifact", "--all", "--write", "--json"], output_json=False)
            ...
            self.assertTrue((omj_home / "use-cases" / "index.json").exists())
            self.assertEqual(len(list((omj_home / "use-cases" / "artifacts").glob("*.json"))), 10)

            code, stdout, stderr = run_cli(["--omj-home", str(omj_home), "cases", "artifact-validate", "--json"], output_json=False)
    ...
    def test_g1_to_g10_use_case_readiness_rollup_separates_optional_store(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            code, stdout, stderr = run_cli(["--omj-home", str(omj_home), "cases", "readiness", "--json"], output_json=False)
            ...
            code, _, stderr = run_cli(["--omj-home", str(omj_home), "cases", "artifact", "--all", "--write", "--json"], output_json=False)
            self.assertEqual(code, 0, stderr)
