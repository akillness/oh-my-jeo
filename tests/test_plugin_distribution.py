...
from omj.plugin_pack import inspect_plugin_bundle
from omj.plugin_bundle.omj.metadata import PROVIDED_HOOKS, PROVIDED_TOOLS, TOOL_FILE_STEMS
...
def load_installed_plugin(plugin_dir: Path):
    module_name = "_test_omj_installed_plugin"
...
class PluginDistributionTests(unittest.TestCase):
    def test_bundled_plugin_resource_is_packaged(self) -> None:
        root = resources.files("omj.plugin_bundle.omj")
        ...
        self.assertIn("omj.plugin_bundle.omj", packages)
        self.assertEqual(package_dir["omj.plugin_bundle"], "src/plugin_bundle")
        self.assertTrue((Path("src") / "plugin_bundle" / "omj" / "__init__.py").is_file())
        self.assertTrue((Path("src") / "plugin_bundle" / "omj" / "references" / "__init__.py").is_file())
        self.assertIn("omj.plugin_bundle.omj", pyproject["tool"]["setuptools"]["package-data"])
        self.assertIn(
            "*.md",
            pyproject["tool"]["setuptools"]["package-data"]["omj.plugin_bundle.omj.references"],
        )
    ...
    def test_probe_tool_falls_back_only_when_package_is_absent(self) -> None:
        from omj.plugin_bundle.omj.tools import probe_tool

        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            omj_home = root / ".omj"
            hermes_home = root / ".hermes"
            args = {"omj_home": str(omj_home), "hermes_home": str(hermes_home)}
            package_missing = ModuleNotFoundError("No module named 'omj'", name="omj")
            backend_broken = ModuleNotFoundError("No module named 'omj.probe'", name="omj.probe")

            with mock.patch.object(probe_tool, "_package_probe", side_effect=package_missing):
                fallback_payload = json.loads(probe_tool.omj_probe_handler(args))
            ...
            with mock.patch.object(probe_tool, "_package_probe", side_effect=backend_broken):
                with self.assertRaises(ModuleNotFoundError):
                    probe_tool.omj_probe_handler(args)
    ...
    def test_setup_default_installs_plugin(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertTrue((hermes_home / "plugins" / "omj").exists())
    ...
    def test_setup_with_plugin_installs_and_registers_smoke(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            status, stdout, stderr = run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "setup", "--with-plugin"])
            ...
            self.assertTrue((plugin_dir / ".omj-plugin-manifest.json").exists())
            self.assertEqual(
            ...
            inspection = inspect_plugin_bundle(resolve_paths(omj_home, hermes_home))
    ...
    def test_setup_with_plugin_dry_run_writes_nothing(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            status, stdout, stderr = run_cli(
                ["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "setup", "--with-plugin", "--dry-run"]
            )
    ...
    def test_setup_with_plugin_refuses_dirty_managed_files_without_force(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            plugin_yaml = hermes_home / "plugins" / "omj" / "plugin.yaml"
            plugin_yaml.write_text(plugin_yaml.read_text(encoding="utf-8") + "\n# local edit\n", encoding="utf-8")

            status, _, stderr = run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "setup", "--with-plugin"])

            self.assertEqual(status, 2)
            self.assertIn("OMJ status helper files were changed outside OMJ", stderr)
            self.assertIn("omj setup --force", stderr)
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "setup", "--with-plugin", "--force"])[0], 0)
    ...
    def test_doctor_reports_stale_plugin_bundle_as_plain_setup_refresh(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            manifest_path = plugin_dir / ".omj-plugin-manifest.json"
            ...
            self.assertEqual(inspection["missing_registered_tools"], ["omj_probe", "omj_recommend"])
            self.assertIn("stale relative to the installed OMJ package", "; ".join(inspection["errors"]))
            ...
            self.assertIn("omj setup", checks["plugin_register_smoke"]["next_action"])
            self.assertNotIn("--force", checks["plugin_register_smoke"]["next_action"])
            self.assertIn("omj_probe", checks["plugin_register_smoke"]["message"])
    ...
    def test_doctor_fails_for_malformed_installed_plugin(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            hermes_home = root / ".hermes"
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "setup", "--with-plugin"])[0], 0)
            (hermes_home / "plugins" / "omj" / "__init__.py").unlink()

            status, stdout, stderr = run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "doctor"])
    ...
    def test_installed_plugin_status_tool_and_hook_keep_evidence_boundary(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            module = load_installed_plugin(hermes_home / "plugins" / "omj")
            ctx = FakeHermesContext()
            module.register(ctx)
            self.assertIn("omj_capabilities", ctx.tools)
            self.assertIn("omj_context", ctx.tools)
            self.assertIn("omj_gather_evidence", ctx.tools)
            self.assertIn("omj_hud", ctx.tools)
            self.assertIn("omj_probe", ctx.tools)
            self.assertIn("omj_role", ctx.tools)
            self.assertIn("omj_status", ctx.tools)
            self.assertIn("on_session_end", ctx.hooks)
            self.assertIn("pre_llm_call", ctx.hooks)
            self.assertIn("pre_tool_call", ctx.hooks)

            hud_handler = ctx.tools["omj_hud"]["args"][2]
            hud_payload = json.loads(hud_handler({"omj_home": str(omj_home), "hermes_home": str(hermes_home), "limit": 1}))
            self.assertEqual(hud_payload["schema_version"], "omj_hud/v1")
            self.assertIn("[omj]", hud_payload["display"]["line"])
            ...
            probe_handler = ctx.tools["omj_probe"]["args"][2]
            probe_payload = json.loads(
                probe_handler(
                    {
                        "omj_home": str(omj_home),
            ...
            self.assertEqual(probe_payload["plugin_tool"], "omj_probe")
            self.assertIn("capability_gap_roadmap", probe_payload)
            self.assertEqual(probe_payload["capability_gap_roadmap"]["schema_version"], "omj_capability_gap_roadmap/v1")
            ...
            handler = ctx.tools["omj_status"]["args"][2]
            payload = json.loads(handler({"omj_home": str(omj_home), "limit": 1}))
            self.assertEqual(payload["schema_version"], "omj_status/v1")
            ...
            recommend_handler = ctx.tools["omj_recommend"]["args"][2]
            recommendation = json.loads(
                recommend_handler({"message": "make an image card for this PR with secret-token-123", "limit": 2})
            )
            self.assertEqual(recommendation["schema_version"], "omj_recommend_result/v1")
            ...
            context_handler = ctx.tools["omj_context"]["args"][2]
            ...
            self.assertEqual(context_brief["plugin_tool"], "omj_context")
            ...
            evidence_handler = ctx.tools["omj_gather_evidence"]["args"][2]
            evidence = json.loads(
                evidence_handler(
                    {
            ...
            self.assertEqual(evidence["schema_version"], "omj_evidence_probe/v1")
            ...
            role_handler = ctx.tools["omj_role"]["args"][2]
            roles = json.loads(role_handler({"action": "list"}))
            self.assertEqual(roles["schema_version"], "omj_role_catalog/v1")
            ...
            self.assertEqual(role_payload["schema_version"], "omj_role_context/v1")
            ...
            self.assertIn("OMJ Role Context", role_payload["context"])
            self.assertIn("OMJ workflow-layer responsibility context", role_payload["context"])
            ...
            role_hook_payload = ctx.hooks["pre_llm_call"](
                omj_home=str(omj_home),
                user_message="[omj-role:planning-lead] do not leak this exact sentence",
                is_first_turn=True,
            )
            self.assertIsNotNone(role_hook_payload)
            role_context = role_hook_payload["context"]
            self.assertIn("[OMJ Role: planner]", role_context)
            ...
            self.assertIsNone(
                ctx.hooks["pre_tool_call"](
                    ...
                    tool_input={"goal": "[omj-role:planning-lead] prepare a plan"},
                )
            )
            tool_warning = ctx.hooks["pre_tool_call"](
                tool_name="delegate_task",
                tool_input={"goal": "[omj-role:nope] prepare a plan"},
            )
            ...
            self.assertIn("[OMJ Tool Checkpoint]", tool_checkpoint["context"])
            self.assertIn("schema=omj_generic_tool_checkpoint/v1", tool_checkpoint["context"])
            ...
            checkpoint_payload = tool_checkpoint["omj_generic_tool_checkpoint"]
            self.assertEqual(checkpoint_payload["schema_version"], "omj_generic_tool_checkpoint/v1")
            ...
            session_checkpoint = ctx.hooks["on_session_end"](omj_home=str(omj_home))
            self.assertEqual(session_checkpoint["status"], "checkpoint_written")
            checkpoint = json.loads((omj_home / "runtime" / "plugin-session-end.json").read_text(encoding="utf-8"))
            self.assertEqual(checkpoint["schema_version"], "omj_plugin_session_end/v1")
            self.assertEqual(checkpoint["privacy"], "metadata_only")
            payload_after_checkpoint = json.loads(handler({"omj_home": str(omj_home), "limit": 1}))
            self.assertEqual(
                payload_after_checkpoint["plugin_session_end"]["schema_version"],
                "omj_plugin_session_end/v1",
            )
            ...
            context_brief = hook_payload["omj_context_brief"]
            self.assertIn("[OMJ Awareness]", context)
            self.assertIn("Hermes-native workflow guidance", context)
            self.assertIn("consider OMJ before generic tools", context)
            self.assertIn("Use message-specific route hints", context)
            self.assertIn("not observed execution", context)
            self.assertIn("omj_capabilities", context)
            self.assertIn("omj_context", context)
            self.assertIn("omj_status/omj_hud", context)
            self.assertIn("[omj]", context)
            ...
            empty_first_turn_context = ctx.hooks["pre_llm_call"](
                omj_home=str(root / ".empty-omj"),
                user_message="make an image summary card for this PR",
                is_first_turn=True,
            )
            self.assertIsNotNone(empty_first_turn_context)
            self.assertIn("[OMJ Awareness]", empty_first_turn_context["context"])
            self.assertIn("[OMJ Route Hint]", empty_first_turn_context["context"])
            ...
            self.assertIn("[OMJ Awareness]", mid_session_visual_context["context"])
            self.assertEqual(
                mid_session_visual_context["omj_context_brief"]["route_hint"]["primary_workflow"],
                "img-summary",
            )
            ...
            self.assertNotIn(
                ...
                json.dumps(mid_session_visual_context["omj_context_brief"], sort_keys=True),
            )
            ...
            mid_session_role_context = ctx.hooks["pre_llm_call"](
                omj_home=str(root / ".empty-omj"),
                user_message="[omj-role:planner] do not leak this mid-session prompt",
                is_first_turn=False,
                include_omj_awareness=False,
            )
            self.assertIsNotNone(mid_session_role_context)
            self.assertIn("[OMJ Role: planner]", mid_session_role_context["context"])
