...
class WorktreeCreatorTests(unittest.TestCase):
    def test_worktree_prepare_dry_run_does_not_create_path(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(payload["schema_version"], "omj_worktree_prepare/v1")
    ...
    def test_worktree_prepare_creates_git_worktree_and_records_ledger(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("omj runtime observe", payload["runtime_observation_followup"]["record_with"])
            worktree_list = _git(repo, "worktree", "list", "--porcelain")
            self.assertIn(str(target), worktree_list.stdout)

            status, stdout, stderr = run_cli(["--omj-home", str(home), "worktree", "list", "--limit", "1"])
            self.assertEqual(stderr, "")
            self.assertEqual(status, 0)
            records = json.loads(stdout)["records"]
            self.assertEqual(records[0]["schema_version"], "omj_worktree_observation/v1")
    ...
    def test_worktree_bind_builds_codex_launch_recipe_for_observed_worktree(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertTrue(payload["worktree"]["observed_in_omj_ledger"])
            ...
            self.assertIn("omj chat session open-executor session-1 --observed", actions["record_executor_opened"]["backend_command"])
    ...
    def test_worktree_bind_runtime_profile_adds_runtime_observation_recipe(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("omj runtime observe --session session-3", actions["record_worktree_runtime_observation"]["backend_command"])
...
def _init_repo(path: Path) -> Path:
    ...
    _git(path, "-c", "user.name=OMJ Test", "-c", "user.email=omj@example.com", "commit", "-m", "init")
    return path
