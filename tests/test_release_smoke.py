...
class ReleaseSmokeTests(unittest.TestCase):
    def test_release_readiness_checklist_is_plan_only_and_names_required_gates(self) -> None:
        ...
        self.assertEqual(items["context_brief_coverage"]["command"], "/tmp/omj demo context-brief-coverage --summary")
        self.assertEqual(items["context_brief_coverage"]["phase"], "contract-quality")
        self.assertTrue(items["context_brief_coverage"]["required"])
        self.assertIn("recognized OMJ route-hint workflow", items["context_brief_coverage"]["evidence_required"])
        self.assertIn("does not prove live Hermes chat behavior", items["context_brief_coverage"]["proof_boundary"])
        self.assertIn("skill_manifest_coverage", items)
        self.assertEqual(items["skill_manifest_coverage"]["command"], "/tmp/omj demo skill-manifest-coverage --summary")
        self.assertEqual(items["skill_manifest_coverage"]["phase"], "contract-quality")
        self.assertTrue(items["skill_manifest_coverage"]["required"])
        self.assertIn("hardcoded credentials", items["skill_manifest_coverage"]["evidence_required"])
        self.assertIn("credential-hygiene contracts", items["skill_manifest_coverage"]["proof_boundary"])
    ...
    def test_hermes_smoke_plan_is_non_mutating_until_live(self) -> None:
        ...
        self.assertEqual(commands[0], ["hermes", "skills", "tap", "add", "akillness/oh-my-jeo"])
        self.assertEqual(
            commands[1],
        ...
        self.assertIn(["hermes", "skills", "inspect", "akillness/oh-my-jeo/skills/oh-my-jeo"], commands)
    ...
    def test_live_smoke_records_successful_command_results(self) -> None:
        ...
        self.assertEqual(seen[0], ["hermes", "skills", "tap", "add", "akillness/oh-my-jeo"])
        self.assertEqual(seen[-1], ["hermes", "skills", "inspect", "akillness/oh-my-jeo/skills/oh-my-jeo"])
