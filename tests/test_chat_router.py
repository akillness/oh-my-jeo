...
from omj.routing.intent import classify_workflow_intent
from omj.skills.catalog import primary_harness_for_skill
...
class ChatRouterTests(unittest.TestCase):
    ...
    def test_low_signal_chat_falls_back_to_router(self) -> None:
        ...
        self.assertEqual(decision["selected_skill"], "oh-my-jeo")
        self.assertEqual(decision["candidate_skill"], "oh-my-jeo")
    ...
    def test_operations_surfaces_dispatch_to_dedicated_harnesses(self) -> None:
        cases = (
            ...
            (
                "Can OMJ run a release reliability review before launch?",
                "reliability-review",
                "reliability-review",
            ),
            (
            ...
            (
                "Can OMJ make an automation blueprint for a daily research digest?",
    ...
    def test_operator_surface_good_examples_route_without_skill_names(self) -> None:
        cases = (
            ...
            (
                "after omj update says setup is next but Hermes skills still look stale.",
                "doctor",
            ),
            (
            ...
            (
                "omj setup says done but Hermes cannot see the skills.",
                "doctor",
            ),
            (
            ...
            (
                "I installed it but omj is not found in a new terminal.",
                "doctor",
            ),
            (
                "Hermes skills list does not show OMJ after setup.",
                "doctor",
            ),
            (
    ...
    def test_paper_learning_routes_paper_explanation_without_stealing_related_lanes(self) -> None:
        explanation_cases = (
            ...
            "Can OMJ help me summarize a paper PDF?",
            "Can OMJ help with paper summaries?",
            "¿Puede OMJ ayudar con un resumen de paper PDF?",
            "OMJで論文PDFを説明できる？",
            "OMJ可以解释论文PDF吗？",
        )
    ...
    def test_explicit_workflow_learning_feedback_wins_over_domain_terms(self) -> None:
        cases = (
            ...
            "Can OMJ help me improve a workflow that went wrong?",
        )
    ...
    def test_runtime_portability_task_card_sits_above_workflow_route(self) -> None:
        ...
        self.assertEqual(task_card["schema_version"], "omj_task_card/v1")
    ...
    def test_short_omj_maintenance_commands_use_operator_task_card(self) -> None:
        cases = (
            ("omj update", "update", "oh-my-jeo"),
            ("omj setup", "setup", "oh-my-jeo"),
            ("omj doctor", "doctor", "doctor"),
            ("omj install", "install", "oh-my-jeo"),
            ("omj list", "list", "oh-my-jeo"),
            ("omj 업데이트해줘", "update", "oh-my-jeo"),
            ("omj 닥터 돌려줘", "doctor", "doctor"),
            ("omj 셋업해줘", "setup", "oh-my-jeo"),
            ("./omj update", "update", "oh-my-jeo"),
        )
        ...
        for message, command, selected_skill in cases:
            with self.subTest(message=message):
                ...
                self.assertIn("task_card:omj_cli_maintenance", decision["recommendations"][0]["matched"])
                self.assertIn("operator_maintenance_command", decision["recommendations"][0]["matched"])
                self.assertEqual(task_card["schema_version"], "omj_task_card/v1")
                self.assertEqual(task_card["task_type"], "omj_cli_maintenance")
                self.assertEqual(task_card["route_level"], "operator_maintenance_command")
                self.assertEqual(task_card["command"], command)
                self.assertEqual(task_card["command_argv"], ["omj", command])
                self.assertEqual(task_card["recommended_next_action"], f"run_omj_{command}")
    ...
    def test_omj_maintenance_guard_does_not_swallow_code_change_request(self) -> None:
        decision = route_chat_message("fix the omj update router implementation", source="discord")

        self.assertIsNone(decision["task_card"])
        self.assertNotIn("task_card:omj_cli_maintenance", decision["recommendations"][0]["matched"])
        ...
        explanation = route_chat_message("omj update 설명해줘", source="discord")
        self.assertIsNone(explanation["task_card"])
        self.assertNotIn("task_card:omj_cli_maintenance", explanation["recommendations"][0]["matched"])
    ...
    def test_issue_251_representative_routes_stay_compact_and_correct(self) -> None:
        cases = (
            ...
            ("OMJ 업데이트해줘", "oh-my-jeo", "omj_cli_maintenance"),
            ("왜 OMJ 라우팅 틀렸어?", "workflow-learning", "router_design_feedback"),
    ...
    def test_router_design_feedback_prefers_workflow_learning_over_feedback_triage(self) -> None:
        cases = (
            ...
            "Why did OMJ route this wrong? Fix the router design.",
            "피드백인데 OMJ 라우팅 설계가 잘못됐어.",
            "더 상위레벨에서 마이그레이션이라는 개념을 한 task로 인식해야지.",
            "이번 작업 회고하면서 OMJ가 얼마나 관여했는지 봐줘.",
        )
    ...
    def test_workflow_vocabulary_meta_discussion_does_not_select_delivery(self) -> None:
        cases = (
            ...
            "가상 프로젝트로 OMJ 라우팅을 테스트해보자. 아직 요구사항은 없어.",
            "OMJ developer note: one-cycle delivery is only vocabulary in this setup test.",
            "OMJ developer test: Codex handoff vocabulary, not asking to implement.",
            "OMJ route: `$ultraprocess` ≠ ejecutar; Codex handoff token only.",
        )
    ...
    def test_pasted_omj_awareness_evaluation_routes_learning_before_delivery(self) -> None:
    ...
    def test_workflow_intent_prefers_structural_cues_over_language_tables(self) -> None:
        intent = classify_workflow_intent("OMJ route: `$ultraprocess` ≠ ejecutar; Codex handoff token only.")
    ...
    def test_generic_korean_omj_feedback_does_not_become_router_design_card(self) -> None:
        decision = route_chat_message("피드백인데 OMJ 문서가 좋아요.", source="discord")
    ...
    def test_domain_work_with_pasted_status_does_not_become_omj_quality_loop(self) -> None:
        message = """Sionic 위키 페이지 사용성 평가하고 더 보기 좋게 만들어줘.

[omj] v1.0.1 | plugin:ready | target:single | coding-agent:runtime(codex)
...
"""
    ...
    def test_status_context_before_user_instruction_is_not_discarded(self) -> None:
        message = """[omj] v1.0.1 | plugin:ready | target:single | coding-agent:runtime(codex)
[OMJ] Native bridge status context.
Evidence boundary: prepared handoffs are not execution evidence.
Latest runtime run: 20260625T090917585910Z-loop-goal-loop-8b5bec.

Sionic 작업 말고 OMJ 라우터 강화 플랜으로 잡아줘.
"""
        ...
        same_line = route_chat_message("[OMJ] 라우터 강화 플랜으로 잡아줘", source="discord")
    ...
    def test_bounded_omj_usage_review_cues_survive_without_domain_stealing(self) -> None:
        status = """
[OMJ Awareness]
status=prepared_not_observed; Evidence boundary: pasted status is diagnostic evidence.
selected_workflow=ultraprocess
"""
        prefixes = (
            "이번 작업에서 OMJ를 덜 쓴 이유를 분석해줘.",
            "이번 작업에서 OMJ를 덜 썼는지 분석해줘.",
            "이번 작업에서 OMJ 관여도 분석해줘.",
            "이번 작업에서 OMJ가 부족했던 점을 분석해줘.",
            "status=prepared_not_observed; 이번 작업에서 OMJ를 덜 쓴 이유를 분석해줘.",
        )
    ...
    def test_specific_capability_catalog_questions_dispatch_to_matching_workflow(self) -> None:
        cases = (
            ("does OMJ support scheduled automation?", "automation-blueprint"),
            ("can OMJ help with MCP setup?", "toolbelt-readiness"),
            ("does OMJ support memory cleanup?", "memory-curation-review"),
            ("does OMJ support voice commands?", "voice-operator"),
            ("OMJ로 GitHub issue webhook 처리 가능해?", "github-event-ops"),
        )
        ...
        for message, selected_skill in cases:
            with self.subTest(message=message):
                ...
                self.assertEqual(decision["recommendations"][0]["skill"], selected_skill)
                self.assertEqual(decision["confidence"], "high")
                self.assertIn("Specific OMJ capability question", decision["reason"])
    ...
    def test_file_lookup_does_not_dispatch_to_workflow_keyword(self) -> None:
        for message in (
            ...
            "what does OMJ do in docs/ARCHITECTURE.md?",
        ):
            with self.subTest(message=message):
                decision = route_chat_message(message, source="discord")
    ...
    def test_delivery_cycle_chat_beats_research_department_for_pr_requests(self) -> None:
        cases = (
            ...
            "Can OMJ do one full research plan implementation review cycle?",
        )
    ...
    def test_risky_refactor_route_plan_keeps_ralplan_before_delivery(self) -> None:
        capability_question = route_chat_message("What OMJ workflow should I use for a risky refactor?", source="discord")
    ...
    def test_memory_context_chat_dispatches_to_curation_review(self) -> None:
        ...
        cross_channel = route_chat_message(
            "시간이 지나며 메모리 압축되고 다른 채널 용어가 겹쳐서 OMJ 맥락 관리가 필요해",
            source="discord",
        )
