...
class WorkflowStateCliTests(unittest.TestCase):
    def test_state_start_writes_authoritative_workflow_state(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            status, stdout, stderr = run_cli(["--omj-home", str(omj_home), "state", "start", "--workflow", "deep-interview", "--note", "clarify"])
            ...
            self.assertTrue((omj_home / "state" / "deep-interview-state.json").exists())
    ...
    def test_state_status_survives_new_cli_invocation(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "state", "start", "--workflow", "deep-interview"])[0], 0)
            status, stdout, stderr = run_cli(["--omj-home", str(omj_home), "state", "status"])
    ...
    def test_allowed_transition_auto_completes_source_state(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "state", "start", "--workflow", "deep-interview"])[0], 0)
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "state", "start", "--workflow", "ralplan"])[0], 0)

            deep = json.loads((omj_home / "state" / "deep-interview-state.json").read_text(encoding="utf-8"))
            ralplan = json.loads((omj_home / "state" / "ralplan-state.json").read_text(encoding="utf-8"))
    ...
    def test_conflicting_transition_is_denied_with_diagnostic(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "state", "start", "--workflow", "deep-interview"])[0], 0)
            status, _, stderr = run_cli(["--omj-home", str(omj_home), "state", "start", "--workflow", "team"])
    ...
    def test_terminal_outcomes_are_recorded(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            status, stdout, stderr = run_cli(
                ["--omj-home", str(omj_home), "state", "finish", "--workflow", "ralplan", "--outcome", "question_pending", "--note", "waiting"]
            )
    ...
    def test_state_clear_removes_only_target_workflow(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "state", "start", "--workflow", "deep-interview"])[0], 0)
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "state", "start", "--workflow", "ralplan"])[0], 0)
            status, stdout, stderr = run_cli(["--omj-home", str(omj_home), "state", "clear", "--workflow", "ralplan"])

            self.assertEqual(stderr, "")
            self.assertEqual(status, 0)
            self.assertTrue(json.loads(stdout)["removed"])
            self.assertTrue((omj_home / "state" / "deep-interview-state.json").exists())
            self.assertFalse((omj_home / "state" / "ralplan-state.json").exists())

    def test_state_reports_malformed_state_without_crashing(self) -> None:
        with TemporaryDirectory() as tmp:
            omj_home = Path(tmp) / ".omj"
            state_dir = omj_home / "state"
