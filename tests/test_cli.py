...
from omj.cli import OmjError, cmd_runtime_merge
from omj.commands import setup as setup_commands
from omj.commands.main import build_parser
from omj.commands.language import LANGUAGE_CODES, MESSAGES
from omj.config_adapter import external_dirs
from omj.paths import resolve_paths
from omj.routing.intent import classify_omj_quality_intent
from omj.skill_pack import builtin_skill_reference_templates, builtin_skill_templates
class CliTests(unittest.TestCase):
    def test_no_arg_cli_shows_welcome_instead_of_error(self) -> None:
        ...
        self.assertIn("If `omj` is not found", stdout)
        self.assertIn("If this screen appears after `omj uninstall`", stdout)
        self.assertIn("omj --help", stdout)

    def test_root_help_explains_command_lanes(self) -> None:
        help_text = build_parser().format_help()

        self.assertIn("Install OMJ once, then use Hermes chat.", help_text)
        ...
        self.assertIn("`omj` was not found", help_text)
        self.assertIn("setup", help_text)
        self.assertIn("Connect OMJ workflows to the target Hermes profile", help_text)
        self.assertIn("quickstart", help_text)
        self.assertIn("Show the first-use OMJ/Hermes path", help_text)
        self.assertIn("chat", help_text)
        self.assertIn("wrapper chat events", help_text)
        self.assertIn("omj cases recommend", help_text)
        self.assertIn("omj ops list", help_text)
        self.assertIn("omj materials list", help_text)
    ...
    def test_context_brief_exposes_omj_mental_model_and_route_hint(self) -> None:
        ...
        self.assertEqual(stderr, "")
        self.assertIn("OMJ context brief", stdout)
        ...
        self.assertIn("Do not skip OMJ merely because a generic tool", stdout)

        status, stdout, stderr = run_cli(
            ["context", "brief", "--source", "discord", "what OMJ workflows are available?"],
            output_json=False,
        )

        self.assertEqual(status, 0, stderr)
        self.assertEqual(stderr, "")
        self.assertIn("Catalog question: show_workflow_picker via omj_capabilities", stdout)
        ...
        self.assertEqual(payload["schema_version"], "omj_context_brief/v1")
        ...
        self.assertIn("[OMJ Awareness]", payload["prompt_context"])
        self.assertIn("[OMJ Route Hint]", payload["prompt_context"])
        ...
        self.assertIn("omj_context", payload["prompt_context"])
    ...
    def test_context_brief_does_not_mask_capability_family_projection_errors(self) -> None:
        from omj.plugin_bundle.omj.context_brief import build_context_brief

        with patch("omj.capabilities.families.capability_family_cards", side_effect=RuntimeError("projection boom")):
            with self.assertRaisesRegex(RuntimeError, "projection boom"):
                build_context_brief("what can OMJ do?")
        ...
        status, stdout, stderr = run_cli(
            [
                ...
                "discord",
                "--json",
                "what OMJ workflows are available with secret-token-123?",
            ],
            output_json=False,
        )
        ...
        self.assertIn("./omj", catalog_question["direct_invocation_aliases"])
        self.assertNotIn("secret-token-123", stdout)
    ...
    def test_context_brief_routes_omj_quality_loops_to_process_before_feedback_triage(self) -> None:
        cases = (
            "updated OMJ로 라우터/맥락/컨텍스트 손실/성능균형/코딩 handoff 유사버그를 계속 찾아 개선하는 루프 실행",
            "Run a loop to find and fix OMJ router context-loss and coding handoff bugs with progress evidence.",
            "OMJ 라우팅 품질과 진행상태 보고 버그를 계속 찾아서 코딩 handoff 신뢰성을 개선해줘",
        )
    ...
    def test_omj_router_prefix_reference_does_not_trigger_quality_loop(self) -> None:
        message = "What is the OMJ router prefix syntax?"

        intent = classify_omj_quality_intent(message)

        self.assertFalse(intent.applies)
        self.assertNotIn("improve:fix", intent.matched_cues)

    def test_recommend_omj_router_prefix_reference_does_not_select_quality_loop(self) -> None:
        message = "What is the OMJ router prefix syntax?"
        ...
        guarded_process_routes = [
            ...
            and "guard:omj_quality_improvement_loop" in recommendation["matched"]
        ]
        self.assertEqual(guarded_process_routes, [])

    def test_recommend_path_like_fragments_do_not_score_as_command_triggers(self) -> None:
        for message in (
            "investigate https://example.com/omj/status for market context",
            r"inspect C:\work\omj\routing\policy.py before the weekly review",
        ):
            with self.subTest(message=message):
                status, stdout, stderr = run_cli(["recommend", message, "--limit", "3"])
                ...
                trigger_matches = [
                    ...
                    if matched in {"trigger:/", "trigger:./", "trigger:/omj", "trigger:./omj"}
                ]
    ...
    def test_chat_interact_routes_workflow_learning_to_audit_actions(self) -> None:
        ...
        for message in (
            "missed route: OMJ was not used",
            "OMJ 안 썼어",
            "missed route: Hermes skipped OMJ for my image request",
            "Hermes did not use OMJ for my image request; record this as workflow learning",
            "이미지 생성 요청에서 OMJ 안 썼어. workflow-learning으로 기록해줘",
        ):
    ...
    def test_chat_interact_omj_next_action_question_uses_quickstart_card(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            base = ["--omj-home", str(root / ".omj"), "--hermes-home", str(root / ".hermes")]
            status, stdout, stderr = run_cli(
                base + ["chat", "interact", "--source", "discord", "omj 상태랑 다음 액션 알려줘"],
                output_json=False,
            )
            ...
            self.assertTrue(payload["chat_response"]["headline"].startswith("[omj] quickstart - "))
            state = payload["chat_response"]["state"]
            self.assertEqual(state["status_source"], "omj_quickstart")
            self.assertEqual(state["quickstart_card"]["schema_version"], "omj_quickstart_card/v1")
            self.assertIn("Local status:", payload["chat_response"]["body"])
            self.assertIn("Next in Hermes:", payload["chat_response"]["body"])
            self.assertIn("Use OMJ request-to-handoff", payload["chat_response"]["body"])
            ...
            self.assertEqual(state["capability_gap_roadmap"]["schema_version"], "omj_capability_gap_roadmap/v1")
            self.assertEqual(state["roadmap_next_actions"][0]["id"], "run_setup")
            self.assertEqual(state["roadmap_next_actions"][0]["id"], "run_setup")
            self.assertIn("omj setup", state["roadmap_next_actions"][0]["command"])

    def test_chat_interact_omj_status_question_uses_sectioned_status_card(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            base = ["--omj-home", str(root / ".omj"), "--hermes-home", str(root / ".hermes")]
            status, stdout, stderr = run_cli(
                base + ["chat", "interact", "--source", "discord", "show OMJ status"],
                output_json=False,
            )
            ...
            state = payload["chat_response"]["state"]
            self.assertEqual(state["status_source"], "omj_probe")
            self.assertEqual(state["capability_gap_roadmap"]["schema_version"], "omj_capability_gap_roadmap/v1")
    ...
    def test_learning_record_eval_and_regression_replay(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(review_payload["next_action"], f"omj learning proposal {candidate_id}")
            ...
            self.assertFalse(regression["regression_case"]["fixture"]["privacy"]["redaction_provable_by_omj"])
            ...
            self.assertTrue(export_payload["learning_export_ref"].startswith("omj-learning-export:"))
            ...
            index_path = root / ".omj" / "learning" / "index.json"
    ...
    def test_setup_and_doctor_default_to_human_summary_with_json_escape_hatch(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            with patch("omj.command_path.shutil.which", return_value="/usr/local/bin/omj"):
                status, stdout, stderr = run_cli(base + ["setup"], output_json=False)

            self.assertEqual(status, 0, stderr)
            self.assertEqual(stderr, "")
            self.assertIn("[1/", stdout)
            self.assertIn("Installing OMJ workflows", stdout)
            self.assertIn("OMJ setup complete.", stdout)
            self.assertIn("OMJ workflows:", stdout)
            ...
            self.assertIn("If Hermes cannot see OMJ yet", stdout)
            ...
            self.assertEqual(status, 0, stderr)
            self.assertEqual(stderr, "")
            self.assertIn("OMJ doctor complete.", stdout)
            ...
            self.assertIn("Use OMJ request-to-handoff", stdout)
    ...
    def test_quickstart_card_shows_first_use_path_without_recording_runtime_evidence(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("For machine-readable output, rerun with `--json`.", stdout)
            self.assertFalse((root / ".omj" / "runtime" / "wrapper_sessions").exists())
            ...
            self.assertEqual(stderr, "")
            payload = json.loads(stdout)
            self.assertEqual(payload["schema_version"], "omj_quickstart_card/v1")
    ...
    def test_setup_recovers_bare_null_external_dirs_shape(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertFalse(omj_home.exists())
            ...
            self.assertEqual(external_dirs(config_text), [str((omj_home / "skills").resolve())])
    ...
    def test_doctor_warns_when_omj_command_is_not_on_path(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            base = ["--omj-home", str(root / ".omj"), "--hermes-home", str(root / ".hermes")]
            ...
            with patch("omj.command_path.shutil.which", return_value=None):
                status, stdout, stderr = run_cli(base + ["doctor"], output_json=False)
    ...
    def test_operator_catalog_commands_default_to_human_summary_with_json_escape_hatch(self) -> None:
        cases = (
            (["recommend", "risky", "refactor"], "OMJ recommendation", "recommendations"),
            (
                ["playbook", "recommend", "turn", "this", "issue", "into", "a", "PR"],
                "OMJ playbook recommendation",
                "recommendations",
            ),
            (["cases", "recommend", "daily", "competitor", "digest"], "OMJ use-case recommendation", "recommendations"),
            (["cases", "list"], "OMJ Hermes use cases", "use_cases"),
            (["cases", "inspect", "G10"], "OMJ use case:", "use_case"),
            (["cases", "demo", "G10"], "OMJ use-case demo card:", "wrapper_card"),
            (["cases", "demo", "--all"], "OMJ G1-G10 use-case demo cards", "cards"),
            (["cases", "artifact", "G10"], "OMJ use-case artifact:", "artifact_id"),
            (["cases", "artifact", "--all"], "OMJ G1-G10 use-case artifacts", "artifacts"),
            (["cases", "readiness"], "OMJ G1-G10 use-case readiness", "gates"),
            (["cases", "replay"], "OMJ G1-G10 use-case replay", "results"),
            (["cases", "validate"], "OMJ G1-G10 feature surface validation", "validated"),
            (["playbook", "list"], "OMJ playbooks", "playbooks"),
            (["playbook", "inspect", "safe-feature-change"], "OMJ playbook:", "playbook"),
            (["profile", "list"], "OMJ profile packs", "packs"),
            (["profile", "inspect", "cto-loop"], "OMJ profile pack:", "pack"),
            (["profile", "inspect", "coding-runtime-team"], "OMJ operating model:", "model"),
        )
    ...
    def test_cases_catalog_exposes_g1_to_g10_and_recommends_real_situations(self) -> None:
        ...
        self.assertEqual(stderr, "")
        payload = json.loads(stdout)
        self.assertEqual(payload["schema_version"], "omj_use_case_catalog/v1")
        ...
        self.assertIn("Use OMJ ops-observability-card", inspected["hermes_chat_prompt"])
        ...
        self.assertEqual(demo["schema_version"], "omj_use_case_demo_card/v1")
        ...
        self.assertEqual(demo["wrapper_card"]["component"], "omj_use_case_card")
        ...
        self.assertIn("omj playbook inspect scheduled-ops-blueprint", demo["operator_commands"][1])
        ...
        self.assertEqual(demo_collection["schema_version"], "omj_use_case_demo_collection/v1")
        ...
        for card in demo_collection["cards"]:
            with self.subTest(demo_card=card["goal"]):
                self.assertEqual(card["schema_version"], "omj_use_case_demo_card/v1")
                ...
                self.assertTrue(card["chat_surface"]["headline"].startswith("[omj] "))
        ...
        self.assertEqual(artifact["schema_version"], "omj_use_case_artifact/v1")
        ...
        self.assertIn("omj cases validate --json", artifact["proof_surfaces"])
        ...
        self.assertEqual(artifact_collection["schema_version"], "omj_use_case_artifact_collection/v1")
        ...
        with TemporaryDirectory() as tmp:
            base = ["--omj-home", str(Path(tmp) / ".omj")]
            ...
            self.assertEqual(write_payload["schema_version"], "omj_use_case_artifact_write/v1")
            self.assertEqual(write_payload["count"], 10)
            self.assertTrue(Path(write_payload["index_path"]).exists())
            self.assertEqual(len(list((Path(tmp) / ".omj" / "use-cases" / "artifacts").glob("*.json"))), 10)
        ...
        self.assertEqual(stderr, "")
        replay = json.loads(stdout)
        self.assertEqual(replay["schema_version"], "omj_use_case_replay/v1")
        self.assertEqual(replay["status"], "passed")
        ...
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(readiness["schema_version"], "omj_use_case_readiness/v1")
            ...
            self.assertEqual(gates["demo_cards"]["command"], "omj cases demo --all --json")
    ...
    def test_local_operator_commands_default_to_human_summary_with_json_escape_hatch(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            cases = (
                (["apply"], "OMJ apply complete.", "config"),
                (["list"], "OMJ managed skills", "skills"),
                (["probe"], "OMJ capability probe", "capabilities"),
            )
            ...
            self.assertEqual(stderr, "")
            self.assertEqual(status, 0)
            self.assertIn("OMJ workspace snippet written", stdout)
    ...
    def test_list_exposes_catalog_context_for_chat_answers(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("In chat, ask Hermes what OMJ can do", stdout)
            ...
            self.assertEqual(stderr, "")
            payload = json.loads(stdout)
            self.assertEqual(payload["catalog_context"]["schema_version"], "omj_installed_skill_catalog_context/v1")
    ...
    def test_probe_parity_matrix_maps_common_agent_runtime_gaps(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(matrix["schema_version"], "omj_parity_matrix/v1")
            ...
            self.assertEqual(matrix["probe_alignment"]["omj_plugin_bundle"], "available")
            ...
            self.assertEqual(status, 0, stderr)
            self.assertEqual(stderr, "")
            self.assertIn("OMJ capability probe", stdout)
    ...
    def test_runtime_team_readiness_reports_contract_without_claiming_execution(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(stderr, "")
            payload = json.loads(stdout)
            self.assertEqual(payload["schema_version"], "omj_team_worker_readiness/v1")
            ...
            self.assertIn("omj setup", " ".join(payload["next_actions"]))
    ...
    def test_runtime_team_readiness_reflects_worker_observations(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            home_args = ["--omj-home", str(root / ".omj"), "--hermes-home", str(root / ".hermes")]
    ...
    def test_setup_interactive_wizard_records_user_choices(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertNotIn("every OMJ workflow is still installed", stdout)
            self.assertIn("OMJ setup complete.", stdout)
            self.assertIn("OMJ status helper:", stdout)
            ...
            state = json.loads((omj_home / "runtime" / "state.json").read_text(encoding="utf-8"))
    ...
    def test_setup_interactive_wizard_uses_defaults_on_eof(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            with patch(
                "omj.commands.setup._try_star_github_repo",
    ...
    def test_setup_project_scope_uses_project_local_paths(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            with patch("omj.paths.Path.cwd", return_value=root):
            ...
            self.assertEqual(payload["operator_summary"]["paths"]["omj_home"], str((root / ".omj").resolve()))
            self.assertEqual(payload["operator_summary"]["paths"]["hermes_home"], str((root / ".hermes").resolve()))
            self.assertTrue((root / ".omj" / "skills").exists())
            self.assertTrue((root / ".hermes" / "config.yaml").exists())
            self.assertIn(str((root / ".omj" / "skills").resolve()), (root / ".hermes" / "config.yaml").read_text(encoding="utf-8"))
    ...
    def test_setup_with_mcp_records_unobserved_bridge_preference(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(mcp["schema_version"], "omj_mcp_setup/v1")
            self.assertTrue(mcp["requested"])
            self.assertFalse(mcp["observed"])
            self.assertEqual(mcp["bridge"]["manifest_command"], "omj mcp manifest")
            self.assertEqual(mcp["bridge"]["host_config_recipes_command"], "omj mcp config-recipe --host <host>")
            self.assertEqual(
                mcp["bridge"]["known_recipe_hosts"],
                ["generic", "claude-code", "codex", "opencode", "cursor"],
            )
            self.assertEqual(mcp["bridge"]["server_command"], "omj mcp serve")
            self.assertIn("observe-host", mcp["bridge"]["host_observation_command"])
            self.assertIn("omj_probe", mcp["bridge"]["tools"])
            self.assertIn("does not prove an MCP host loaded OMJ", mcp["claim_boundary"])
            state = json.loads((omj_home / "runtime" / "state.json").read_text(encoding="utf-8"))
            self.assertEqual(state["last_setup"]["mcp_setup"]["mode"], "bridge_requested")
    ...
    def test_setup_and_install_support_localized_human_output(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(status, 0, stderr)
            self.assertEqual(stderr, "")
            self.assertIn("OMJ doctor가 완료되었습니다.", stdout)
            ...
            status, stdout, stderr = run_cli(["--omj-home", str(install_root / ".omj"), "install", "--language", "zh"], output_json=False)

            self.assertEqual(status, 0, stderr)
            self.assertEqual(stderr, "")
            self.assertIn("刷新 OMJ 工作流包", stdout)
            self.assertIn(f"已准备 {len(builtin_skill_templates())} 个工作流", stdout)
            self.assertIn("OMJ install 已完成。", stdout)
    ...
    def test_setup_reports_status_helper_conflict_in_plain_language(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(status, 2)
            self.assertIn("Installing OMJ workflows", stdout)
            self.assertIn("OMJ status helper location already exists", stderr)
            self.assertIn("omj setup --force", stderr)
    ...
    def test_install_and_update_default_to_human_summary_with_json_escape_hatch(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(stderr, "")
            self.assertIn("Installing OMJ workflows", stdout)
            self.assertIn("OMJ install complete.", stdout)
            self.assertIn(f"OMJ workflows: {len(builtin_skill_templates())} ready", stdout)
            self.assertIn("Run `omj setup`", stdout)
            with self.assertRaises(json.JSONDecodeError):
                json.loads(stdout)
            ...
            self.assertEqual(status, 0, stderr)
            self.assertEqual(stderr, "")
            self.assertIn("OMJ update preview complete.", stdout)
            self.assertIn("Rerun without `--dry-run`", stdout)
            ...
            self.assertEqual(status, 0, stderr)
            self.assertEqual(stderr, "")
            self.assertIn("OMJ update complete.", stdout)
            ...
            self.assertIn("OMJ command: not updated (workflows only)", stdout)
            self.assertIn("State log:", stdout)
            self.assertIn("last_update", stdout)
            self.assertIn("Run `omj doctor` to verify health", stdout)
            self.assertIn("To update the `omj` command itself", stdout)
            ...
            self.assertEqual(stderr, "")
            payload = json.loads(stdout)
            self.assertEqual(payload["package"], "oh-my-jeo")
            ...
            state = json.loads((root / ".omj" / "runtime" / "state.json").read_text(encoding="utf-8"))
    ...
    def test_update_self_update_reenters_with_command_package_marker(self) -> None:
        ...
        plan = {"release": Namespace(package_url="https://example.invalid/omj.zip"), "python": sys.executable}
        first = subprocess.CompletedProcess(args=["pip"], returncode=0, stdout="", stderr="")
        second = subprocess.CompletedProcess(args=["omj"], returncode=0, stdout="", stderr="")

        with patch.object(setup_commands.subprocess, "run", side_effect=[first, second]) as run:
            with patch.object(setup_commands.sys, "argv", ["omj", "update", "--json"]):
        ...
        self.assertIn("https://example.invalid/omj.zip", pip_args)
        reentry_args = run.call_args_list[1].args[0]
        self.assertEqual(reentry_args[:3], [sys.executable, "-m", "omj.cli"])
    ...
    def test_update_self_update_skips_local_channel_without_package_source(self) -> None:
        ...
        with patch.object(
            ...
            return_value={"managed": True, "python": sys.executable, "venv_dir": "/tmp/omj-venv"},
        ):
    ...
    def test_update_self_update_recognizes_venv_python_symlink_path(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            venv_bin = root / ".local" / "share" / "omj" / "venv" / "bin"
    ...
    def test_source_checkout_exposes_omj_cli_module(self) -> None:
        repo_root = Path(__file__).resolve().parents[1]
        result = subprocess.run(
            [sys.executable, "-m", "omj.cli", "recommend", "risky refactor", "--limit", "1", "--json"],
            cwd=repo_root,
    ...
    def test_release_hermes_smoke_cli_defaults_to_plan_mode(self) -> None:
        ...
        self.assertEqual(commands[0], ["omj-dev", "--omj-home", str(omj_home.resolve()), "--hermes-home", str(hermes_home.resolve()), "setup"])
    ...
    def test_release_checklist_defaults_to_human_summary_with_json_escape_hatch(self) -> None:
        status, stdout, stderr = run_cli(
            ["release", "checklist", "--version", "v1.0.0", "--omj-command", "/tmp/omj"],
            output_json=False,
        )

        self.assertEqual(status, 0, stderr)
        self.assertEqual(stderr, "")
        self.assertIn("OMJ release checklist for 1.0.0 (v1.0.0)", stdout)
        ...
        self.assertIn("/tmp/omj --help", stdout)
    ...
    def test_release_product_readiness_cli_summarizes_product_story(self) -> None:
        status, stdout, stderr = run_cli(
            ["release", "product-readiness", "--version", "v1.0.1", "--omj-command", "/tmp/omj"],
            output_json=False,
        )

        self.assertEqual(status, 0, stderr)
        self.assertEqual(stderr, "")
        self.assertIn("OMJ product readiness for 1.0.1", stdout)
        self.assertIn("Status: ready", stdout)
        ...
        self.assertEqual(status, 0, stderr)
        self.assertEqual(stderr, "")
        self.assertIn("OMJ skill content smoke", stdout)
        self.assertIn("Status: ok", stdout)
    ...
    def test_release_evidence_bundle_cli_packages_optional_write_artifact(self) -> None:
        with TemporaryDirectory() as tmp:
            omj_home = Path(tmp) / ".omj"
            hermes_home = Path(tmp) / ".hermes"
            base = ["--omj-home", str(omj_home), "--hermes-home", str(hermes_home)]

            status, stdout, stderr = run_cli(
                base + ["release", "evidence-bundle", "--version", "v1.0.1", "--omj-command", "/tmp/omj"],
                output_json=False,
            )

            self.assertEqual(status, 0, stderr)
            self.assertEqual(stderr, "")
            self.assertIn("OMJ release evidence bundle for 1.0.1", stdout)
            self.assertIn("Status: ready", stdout)
            self.assertIn("Written: no", stdout)
            self.assertIn("Local artifact store: not_written", stdout)
            self.assertFalse((omj_home / "runtime" / "release-evidence" / "index.json").exists())
            ...
            self.assertTrue((omj_home / "runtime" / "release-evidence" / "index.json").exists())
    ...
    def test_release_install_smoke_defaults_to_human_plan_with_json_escape_hatch(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(status, 0, stderr)
            self.assertEqual(stderr, "")
            self.assertIn("OMJ install smoke", stdout)
    ...
    def test_release_hermes_smoke_cli_can_observe_installed_command_without_live_profile_mutation(self) -> None:
        ...
        self.assertEqual(payload["installed_command_smoke"]["results"][0]["command"], [str(fake_omj), "--help"])
        self.assertEqual(
            payload["installed_command_smoke"]["results"][1]["command"],
            [str(fake_omj), "release", "skill-content-smoke", "--json"],
        )
        self.assertFalse(payload["first_use_status_smoke"]["observed"])
    ...
    def test_release_hermes_smoke_cli_fails_when_installed_command_is_not_on_path(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            omj_home = root / ".omj"
            hermes_home = root / ".hermes"
            missing_command = "omj-definitely-not-on-path-for-test"
            status, stdout, stderr = run_cli(
                [
                    "--omj-home",
                    str(omj_home),
    ...
    def test_memory_cli_inspects_packs_and_applies_review_updates(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            status, stdout, stderr = run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "memory", "inspect", "--fixture", str(fixture)])
            ...
            status, stdout, stderr = run_cli(["--omj-home", str(omj_home), "memory", "apply", "--batch", str(batch_path), "--dry-run"])
            ...
            self.assertFalse((omj_home / "memory").exists())

            self.assertEqual(run_cli(["--omj-home", str(omj_home), "memory", "apply", "--batch", str(batch_path)])[0], 0)

            status, stdout, stderr = run_cli(["--omj-home", str(omj_home), "memory", "pack", "--executor", "codex"])
    ...
    def test_ops_cli_writes_lists_shows_validates_and_exports_artifacts(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            omj_home = root / ".omj"
            base = ["--omj-home", str(omj_home), "ops"]
            ...
            self.assertEqual(rhythm["schema_version"], "omj_ops_write_result/v1")
            ...
            self.assertTrue((omj_home / "operations" / "index.json").exists())
            ...
            self.assertEqual(listing["schema_version"], "omj_ops_list/v1")
            ...
            self.assertEqual(exported["schema_version"], "omj_ops_export_result/v1")
    ...
    def test_ops_blueprint_cli_prepares_scheduled_ops_without_runtime_claims(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertFalse((omj_home / "hermes-ops").exists())
            ...
            self.assertTrue((omj_home / "hermes-ops" / "blueprints" / f"{blueprint_id}.json").exists())

            status, stdout, stderr = run_cli(base + ["blueprint-list"])
            self.assertEqual(status, 0, stderr)
            listing = json.loads(stdout)
            self.assertEqual(listing["schema_version"], "omj_ops_blueprint_list/v1")
            ...
            self.assertEqual(status, 0, stderr)
            self.assertEqual(json.loads(stdout)["schema_version"], "omj_ops_blueprint_show/v1")
    ...
    def test_ops_research_department_cli_prepares_workflow_without_runtime_claims(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            dry = json.loads(stdout)
            self.assertFalse(dry["store"]["written"])
            self.assertFalse((omj_home / "research-department").exists())
            plan = dry["plan"]
            ...
            self.assertTrue((omj_home / "research-department" / "plans" / f"{plan_id}.json").exists())

            status, stdout, stderr = run_cli(base + ["research-department-list"])
            self.assertEqual(status, 0, stderr)
            listing = json.loads(stdout)
            self.assertEqual(listing["schema_version"], "omj_ops_research_department_list/v1")
            self.assertEqual(listing["plans"][0]["plan_id"], plan_id)

            status, stdout, stderr = run_cli(base + ["research-department-show", plan_id])
            self.assertEqual(status, 0, stderr)
            self.assertEqual(json.loads(stdout)["schema_version"], "omj_ops_research_department_show/v1")
    ...
    def test_ops_cli_lists_and_exports_are_bounded_by_default(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            base = ["--omj-home", str(root / ".omj"), "ops"]
    ...
    def test_materials_cli_records_generation_handoff_and_observed_export_boundaries(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            base = ["--omj-home", str(root / ".omj"), "materials"]
            ...
            self.assertEqual(artifact["schema_version"], "omj_material_artifact/v1")
    ...
    def test_visual_cli_prepares_prompt_cards_and_records_observations(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            base = ["--omj-home", str(root / ".omj"), "img-summary"]
            legacy_base = ["--omj-home", str(root / ".omj"), "visual"]
            ...
            self.assertTrue((root / ".omj" / "visual" / "observations" / "index.json").exists())
    ...
    def test_recommend_omj_quality_loop_routes_to_process_despite_bug_terms(self) -> None:
        cases = (
            "updated OMJ로 라우터/맥락/컨텍스트 손실/성능균형/코딩 handoff 유사버그를 계속 찾아 개선하는 루프 실행",
            "Run a loop to find and fix OMJ router context-loss and coding handoff bugs with progress evidence.",
            "OMJ 라우팅 품질과 진행상태 보고 버그를 계속 찾아서 코딩 handoff 신뢰성을 개선해줘",
        ...
        for message in cases:
            with self.subTest(message=message):
                ...
                self.assertEqual(recommendations[0]["skill"], "ultraprocess")
                self.assertEqual(recommendations[0]["next_action"], "start_ultraprocess")
                self.assertIn("guard:omj_quality_improvement_loop", recommendations[0]["matched"])
    ...
    def test_recommend_independent_operations_surfaces(self) -> None:
        cases = (
            ...
            (
                "회의록 요약을 부탁했는데 OMJ 안 쓰고 일반 답변했어",
                "operating-rhythm",
                "prepare_operating_record",
                "meeting, scrum, sprint",
                "prepared",
    ...
    def test_recommend_weak_query_uses_fallback(self) -> None:
        ...
        self.assertEqual(status, 0)
        recommendations = json.loads(stdout)["recommendations"]
        self.assertEqual(recommendations[0]["skill"], "oh-my-jeo")
        self.assertIn("oh-my-jeo", {recommendation["skill"] for recommendation in recommendations})
    ...
    def test_playbook_inspect_shows_owners_and_evidence_boundaries(self) -> None:
        ...
        self.assertTrue({"hermes", "omj", "wrapper"} <= owners)
    ...
    def test_chat_route_dispatches_image_capability_questions_to_img_summary(self) -> None:
        cases = (
            "이미지 생성 기능 뭐 있어?",
            "what image generation features does OMJ have?",
            "does OMJ support image generation?",
            "이미지 생성해줘",
            "이미지 만들어줘",
            "generate an image",
            "generate an image.",
    ...
    def test_chat_route_dispatches_specific_capability_questions_to_cards(self) -> None:
        cases = (
            ("does OMJ support scheduled automation?", "automation-blueprint", "prepare_scheduled_ops_blueprint"),
            ("can OMJ help with MCP setup?", "toolbelt-readiness", "prepare_toolbelt_readiness"),
            ("does OMJ support memory cleanup?", "memory-curation-review", "prepare_memory_curation_review"),
            ("does OMJ support voice commands?", "voice-operator", "prepare_voice_operator_card"),
            ("OMJ로 GitHub issue webhook 처리 가능해?", "github-event-ops", "prepare_github_event_ops_card"),
        )
        ...
        for message, selected_skill, next_action in cases:
            with self.subTest(message=message):
                ...
                self.assertIn("Specific OMJ capability question", route["reason"])
    ...
    def test_chat_route_hint_exposes_wrapper_card_without_recording_route(self) -> None:
        ...
        self.assertEqual(payload["route_hint"]["schema_version"], "omj_route_hint/v1")
        self.assertEqual(payload["route_hint"]["primary_workflow"], "img-summary")
        ...
        self.assertEqual(payload["generic_tool_checkpoint"]["schema_version"], "omj_generic_tool_checkpoint/v1")
    ...
    def test_chat_route_hint_can_emit_manual_prompt_context(self) -> None:
        status, stdout, stderr = run_cli(
            ["chat", "route-hint", "--source", "discord", "--prompt-context", "missed route: OMJ was not used"]
        )
    ...
    def test_chat_route_file_lookup_does_not_emit_workflow_clarification(self) -> None:
        ...
        self.assertEqual(route["selected_skill"], "oh-my-jeo")
    ...
    def test_chat_route_and_interact_guard_short_omj_maintenance_command(self) -> None:
        status, stdout, stderr = run_cli(["chat", "route", "--source", "discord", "omj update"])

        self.assertEqual(stderr, "")
        self.assertEqual(status, 0)
        route = json.loads(stdout)["route"]
        self.assertEqual(route["selected_skill"], "oh-my-jeo")
        self.assertEqual(route["task_card"]["task_type"], "omj_cli_maintenance")
        self.assertEqual(route["task_card"]["route_level"], "operator_maintenance_command")
        self.assertEqual(route["task_card"]["recommended_next_action"], "run_omj_update")
        self.assertIn("avoid_repo_mutation", route["task_card"]["operation_primitives"])

        status, stdout, stderr = run_cli(["chat", "interact", "--source", "discord", "omj update"])

        self.assertEqual(stderr, "")
        self.assertEqual(status, 0)
        payload = json.loads(stdout)
        self.assertEqual(payload["next_action"], "run_omj_update")
    ...
    def test_chat_interact_safe_feature_presents_plan_and_disabled_handoff(self) -> None:
        ...
        self.assertTrue(response["headline"].startswith("[omj] ralplan - "))
    ...
    def test_chat_interact_routes_grounded_operator_examples(self) -> None:
        cases = (
            ...
            ("회의록 요약을 부탁했는데 OMJ 안 쓰고 일반 답변했어", "operating-rhythm", "ack", "prepare_operating_record"),
    ...
    def test_chat_interact_direct_picker_aliases(self) -> None:
        for message in ("./omj", "/omj", "./skills", "/skills"):
            with self.subTest(message=message):
                ...
                self.assertEqual(payload["route"]["selected_skill"], "oh-my-jeo")
                self.assertEqual(payload["chat_response"]["kind"], "skill_picker")
                picker = payload["chat_response"]["state"]["skill_picker"]
                self.assertEqual(picker["schema_version"], "omj_skill_picker/v1")
                ...
                self.assertTrue({"oh-my-jeo", "deep-interview", "ralplan", "loop", "ultraprocess"} <= option_ids)
                self.assertEqual(picker["featured_options"][0]["id"], "oh-my-jeo")
                ...
                self.assertEqual(action_payload["featured_options"][0]["id"], "oh-my-jeo")
    ...
    def test_chat_interact_catalog_questions_open_picker_without_shell(self) -> None:
        for message in (
            ...
            with self.subTest(message=message):
                ...
                self.assertEqual(capability_summary["schema_version"], "omj_capability_summary/v1")
    ...
    def test_chat_interact_omj_intro_questions_use_context_brief(self) -> None:
        for message in ("what is OMJ and how do I use it?", "OMJ가 뭐야? 어떻게 써?"):
            with self.subTest(message=message):
                status, stdout, stderr = run_cli(["chat", "interact", "--source", "discord", message])
                ...
                self.assertTrue(payload["chat_response"]["headline"].startswith("[omj] context - "))
                ...
                state = payload["chat_response"]["state"]
                self.assertEqual(state["context_brief"]["schema_version"], "omj_context_brief/v1")
    ...
    def test_chat_interact_partial_prefix_preview_shows_only_omj(self) -> None:
        cases = {
            "./": "./omj",
            "/": "/omj",
            "./o": "./omj",
        ...
        for message, insert_text in cases.items():
            with self.subTest(message=message):
                ...
                self.assertEqual(preview["schema_version"], "omj_command_preview/v1")
                self.assertEqual([suggestion["label"] for suggestion in preview["suggestions"]], ["omj"])
    ...
    def test_chat_native_command_exports_platform_registration_contracts(self) -> None:
        ...
        for source, registration_schema in cases.items():
            with self.subTest(source=source):
                ...
                self.assertEqual(status, 0)
                payload = json.loads(stdout)
                self.assertEqual(payload["schema_version"], "omj_native_command_surface/v1")
                self.assertEqual(payload["source"], source)
                self.assertEqual(payload["command"], "omj")
                self.assertEqual(payload["registration"]["schema_version"], registration_schema)
                self.assertEqual(payload["preview_contract"]["only_top_level_suggestions"], ["omj"])
                self.assertEqual(payload["fallback_card"]["primary_action"]["label"], "Open omj")
    ...
    def test_loop_cli_start_feedback_permit_and_status(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            home = ["--omj-home", str(root / ".omj"), "--hermes-home", str(root / ".hermes")]
            ...
            status, stdout, stderr = run_cli(
                ...
                + [
                    ...
                    "loop-cli",
                    "--goal-summary",
                    "Make OMJ release-ready for ambitious teams",
                    "--goal-reframe",
            ...
            self.assertIn("Continue OMJ loop", queue_handoff["handoff_text"])
    ...
    def test_loop_status_lists_invalid_local_artifacts_without_crashing(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            home = ["--omj-home", str(root / ".omj"), "--hermes-home", str(root / ".hermes")]
            bad_loop = root / ".omj" / "loops" / "bad-loop"
    ...
    def test_chat_route_records_runtime_routing_artifact(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            status, stdout, stderr = run_cli(["--omj-home", str(root / ".omj"), "--hermes-home", str(root / ".hermes"), "runtime", "show", run_id])
    ...
    def test_chat_session_flow_persists_plan_decision_and_links_handoff_run(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertNotIn("omj ", json.dumps(session_status["chat_response"]).lower())
    ...
    def test_coding_delegate_weak_query_falls_back(self) -> None:
        ...
        self.assertEqual(delegation["recommended_workflow"], "oh-my-jeo")
    ...
    def test_coding_delegate_records_metadata_only_artifact(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            status, stdout, stderr = run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "runtime", "validate"])
            self.assertEqual(stderr, "")
            self.assertEqual(status, 0)
            self.assertTrue(json.loads(stdout)["ok"])
    ...
    def test_runtime_observe_records_journal_events_for_prepared_codex_runs(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            omj_home = root / ".omj"
            home_args = ["--omj-home", str(omj_home), "--hermes-home", str(root / ".hermes")]
            ...
            status, stdout, stderr = run_cli(
                ...
                + [
                    ...
                    "--executor",
                    "codex",
                    "implement safe runtime feature in src/omj/runtime/artifacts.py without overclaiming",
                ]
            )
            self.assertEqual(stderr, "")
            self.assertEqual(status, 0)
            ...
            self.assertFalse((omj_home / "runtime" / "runs" / run_id / "runtime_observations.jsonl").exists())
        ...
        with TemporaryDirectory() as tmp:
            ...
            raw_log = "\n".join(
                [
                    ...
                    json.dumps({"type": "tool_call", "tool": "apply_patch", "message": "modified src/omj/coding/codex_progress.py"}),
    ...
    def test_coding_executor_readiness_dry_run_is_first_use_contract(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertFalse((omj_home / "runtime" / "executor-readiness.json").exists())
    ...
    def test_coding_delegate_records_codex_executor_handoff_without_raw_message(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            omj_home = root / ".omj"
            hermes_home = root / ".hermes"
            hostile = "implement cleanup of duplicated routing code in src/omj/routing/policy.py with secret-token-123"
            ...
            status, stdout, stderr = run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "runtime", "show", run_id])
            self.assertEqual(stderr, "")
            self.assertEqual(status, 0)
            shown = json.loads(stdout)
    ...
    def test_coding_delegate_record_blocks_meta_vocabulary_without_runtime_mutation(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            hermes_home = root / ".hermes"
            message = (
                "OMJ developer test: Codex coding handoff and ultraprocess one-cycle delivery "
            ...
            forced_vague_home = root / ".omj-vague-forced"
            status, stdout, stderr = run_cli(
                [
                    "--omj-home",
        ...
        with TemporaryDirectory() as tmp:
            ...
            status, stdout, stderr = run_cli(
                [
                    ...
                    "--executor",
                    "codex",
                    "implement risky status migration in src/omj/runtime/status.py",
                ]
            )
            ...
            status, stdout, stderr = run_cli(
                ["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "runtime", "delegation-status", "--run", run_id]
            )
    ...
    def test_runtime_review_ci_merge_commands_advance_status_ladder(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertTrue(chat["chat_response"]["headline"].startswith("[omj] status - "))
            self.assertNotIn("omj ", json.dumps(chat["chat_response"]).lower())
    ...
    def test_runtime_merge_rejects_conflicting_status_options(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            status, stdout, stderr = run_cli(base + ["runtime", "record", "--skill", "oh-my-jeo", "--harness", "coding-handling"])
            self.assertEqual(stderr, "")
            self.assertEqual(status, 0)
            run_id = json.loads(stdout)["run"]["run_id"]
            ...
            with self.assertRaisesRegex(OmjError, "accepts only one"):
        ...
        with TemporaryDirectory() as tmp:
            ...
            status, stdout, stderr = run_cli(
                [
                    ...
                    "--executor",
                    "codex",
                    "implement status badge in src/omj/runtime/status.py",
                ]
            )
            self.assertEqual(stderr, "")
            self.assertEqual(status, 0)
            run_id = json.loads(stdout)["runtime"]["run"]["run_id"]
            (omj_home / "runtime" / "runs" / run_id / "coding_delegation.json").unlink()
    ...
    def test_hermes_plan_returns_review_gated_scaffold(self) -> None:
        ...
        self.assertIn("omj coding delegate --executor codex --record --from-plan <accepted-plan.md>", plan["execution_handoff"])
    ...
    def test_install_apply_doctor_and_list(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            with patch("omj.command_path.shutil.which", return_value="/usr/local/bin/omj"):
                ...
                self.assertEqual(run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "list"])[0], 0)
                self.assertEqual(run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "doctor"])[0], 0)
                list_status, list_stdout, list_stderr = run_cli(
                    ["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "list"],
                    output_json=False,
                )
            ...
            manifest = json.loads((omj_home / "manifest.json").read_text(encoding="utf-8"))
            names = {skill["name"] for skill in manifest["skills"]}
            self.assertIn("oh-my-jeo", names)
            self.assertIn("ralph", names)
            self.assertIn("ultragoal", names)
            self.assertIn(str(omj_home / "skills"), (hermes_home / "config.yaml").read_text(encoding="utf-8"))
            state = json.loads((omj_home / "runtime" / "state.json").read_text(encoding="utf-8"))
            self.assertEqual(state["installed_skills"], len(manifest["skills"]))
            self.assertEqual(state["last_applied_skills_dir"], str((omj_home / "skills").resolve()))
            self.assertEqual(state["release_channel"], "preview")

            with patch("omj.command_path.shutil.which", return_value="/usr/local/bin/omj"):
                _, doctor_stdout, _ = run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "doctor"])
    ...
    def test_setup_runs_install_and_apply(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            status, stdout, stderr = run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "setup"])
            ...
            self.assertEqual(payload["operator_summary"]["command_path"]["schema_version"], "omj_command_path/v1")
            self.assertEqual(payload["hermes_native"]["mode"], "omj_bootstrap")
            ...
            self.assertEqual(state["last_setup"]["hermes_native"]["skills_dir"], str((omj_home / "skills").resolve()))

            doctor_status, doctor_stdout, doctor_stderr = run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "doctor"])
            self.assertEqual(doctor_stderr, "")
            self.assertEqual(doctor_status, 0)
            self.assertTrue(json.loads(doctor_stdout)["ok"])
    ...
    def test_uninstall_defaults_to_full_managed_cleanup_and_preserves_unrelated_hermes_files(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(stderr, "")
            self.assertEqual(status, 0)
            self.assertTrue((omj_home / "skills").exists())
            self.assertTrue((hermes_home / "plugins" / "omj" / "plugin.yaml").exists())
            managed_agent = hermes_home / "agents" / "omj-cto-loop-cto.md"
            ...
            self.assertIn(str(omj_home.resolve()), preview["would_remove"])
            self.assertIn(str((hermes_home / "plugins" / "omj").resolve()), preview["would_remove"])
            ...
            self.assertFalse(omj_home.exists())
            self.assertFalse((hermes_home / "plugins" / "omj").exists())
            self.assertFalse(managed_agent.exists())
            self.assertTrue(unrelated_agent.exists())
            self.assertTrue(unrelated_plugin.exists())
            self.assertNotIn(str(omj_home / "skills"), (hermes_home / "config.yaml").read_text(encoding="utf-8"))
    ...
    def test_uninstall_terminal_summary_explains_when_command_remains_on_path(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(stderr, "")
            self.assertEqual(status, 0)
            self.assertIn("OMJ uninstall complete.", stdout)
            self.assertIn("Command package: not removed", stdout)
            self.assertIn("If `omj` still runs after uninstall", stdout)
            with self.assertRaises(json.JSONDecodeError):
                json.loads(stdout)
    ...
    def test_uninstall_removes_install_sh_managed_command_package_when_detected(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            fake_omj = fake_venv_bin / "omj"
            fake_omj.write_text("# fake omj\n", encoding="utf-8")
            fake_bin = root / "bin"
            fake_bin.mkdir()
            fake_link = fake_bin / "omj"
            fake_link.symlink_to(fake_omj)
            base = ["--omj-home", str(omj_home), "--hermes-home", str(hermes_home)]

            self.assertEqual(run_cli(base + ["setup"])[0], 0)
            env = {**os.environ, "OMJ_VENV_DIR": str(fake_venv), "OMJ_BIN_DIR": str(fake_bin)}
    ...
    def test_setup_and_chat_detect_persisted_hermes_target_topology_drift(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            base = ["--omj-home", str(omj_home), "--hermes-home", str(hermes_a)]
            ...
            registry = json.loads((omj_home / "targets.json").read_text(encoding="utf-8"))
            ...
            self.assertIn(str(omj_home / "skills"), (hermes_b / "config.yaml").read_text(encoding="utf-8"))
            registry = json.loads((omj_home / "targets.json").read_text(encoding="utf-8"))
    ...
    def test_setup_profile_can_set_prompt_only_runtime_default(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertTrue((omj_home / "setup-profile.json").exists())
    ...
    def test_setup_records_operating_model_without_installing_profile_packs(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            written_profile = json.loads((omj_home / "setup-profile.json").read_text(encoding="utf-8"))
            self.assertEqual(written_profile["operating_model_id"], "coding-runtime-team")
            self.assertNotIn("operating_model", written_profile)
            self.assertFalse((hermes_home / "agents" / "omj-engineering-delivery-planner.md").exists())
    ...
    def test_setup_github_star_helper_uses_gh_best_effort(self) -> None:
        ...
        with patch("omj.commands.setup.subprocess.run", return_value=completed) as run:
    ...
    def test_setup_github_star_helper_reports_missing_gh_without_blocking(self) -> None:
        with patch("omj.commands.setup.subprocess.run", side_effect=FileNotFoundError):
            result = setup_commands._try_star_github_repo()
    ...
    def test_setup_github_star_helper_reports_broken_gh_without_blocking(self) -> None:
        with patch("omj.commands.setup.subprocess.run", side_effect=PermissionError("bad gh")):
            result = setup_commands._try_star_github_repo()
    ...
    def test_setup_github_star_prompt_uses_localized_copy(self) -> None:
        output = io.StringIO()
        with patch("omj.commands.setup._ask_yes_no", return_value=False) as ask, patch("sys.stdout", output):
    ...
    def test_setup_github_star_prompt_yes_records_star(self) -> None:
        output = io.StringIO()
        with (
            patch("omj.commands.setup._ask_yes_no", return_value=True),
            patch(
                "omj.commands.setup._try_star_github_repo",
                return_value={"ok": True, "reason": "starred_or_already_starred"},
            ) as star,
    ...
    def test_setup_github_star_prompt_dry_run_does_not_call_github(self) -> None:
        ...
        with (
            patch("omj.commands.setup._ask_yes_no", return_value=True),
            patch("omj.commands.setup._try_star_github_repo") as star,
            patch("sys.stdout", output),
        ):
    ...
    def test_setup_choice_menu_uses_cursor_not_checkbox(self) -> None:
        from omj.commands.setup import _choice_menu_lines
    ...
    def test_setup_choice_menu_counts_wrapped_terminal_rows(self) -> None:
        from omj.commands.setup import _rendered_terminal_rows, _visible_text_width
    ...
    def test_setup_executor_copy_uses_simple_coding_agent_names(self) -> None:
        from omj.commands.language import tr
        from omj.commands.setup import _executor_summary
    ...
    def test_optional_team_profile_packs_are_listed_and_installed_on_request(self) -> None:
        ...
        self.assertIn("omj setup --profile-pack cto-loop", profile["install_command"])
        ...
        with TemporaryDirectory() as tmp:
            ...
            self.assertFalse((hermes_home / "agents").exists())
            self.assertFalse((omj_home / "team-profile-packs" / "research-strategy.json").exists())
            ...
            self.assertEqual(installed["schema_version"], "omj_team_profile_pack/v1")
            ...
            cto_file = hermes_home / "agents" / "omj-cto-loop-cto.md"
            pm_file = hermes_home / "agents" / "omj-cto-loop-pm.md"
    ...
    def test_setup_dry_run_marks_bootstrap_state_unobserved(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            status, stdout, stderr = run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "setup", "--dry-run"])
    ...
    def test_install_is_idempotent_and_detects_local_modifications(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "install"])[0], 0)
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "install"])[0], 0)
            skill_file = omj_home / "skills" / "ralph" / "SKILL.md"
            skill_file.write_text(skill_file.read_text(encoding="utf-8") + "\nlocal edit\n", encoding="utf-8")

            status, _, stderr = run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "install"])
            self.assertEqual(status, 2)
            self.assertIn("local modifications detected", stderr)

            doctor_status, doctor_stdout, _ = run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "doctor"])
            ...
            self.assertIn("omj install --force", checks["local_modifications"]["next_action"])
            self.assertIn("ralph/SKILL.md", checks["local_modifications"]["message"])
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "install", "--force"])[0], 0)
    ...
    def test_doctor_reports_wrong_runtime_home(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "--hermes-home", str(installed_hermes_home), "install"])[0], 0)
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "--hermes-home", str(installed_hermes_home), "apply"])[0], 0)

            status, stdout, _ = run_cli(["--omj-home", str(omj_home), "--hermes-home", str(other_hermes_home), "doctor"])
            ...
            self.assertIn("omj setup", checks["runtime_context"]["next_action"])
            self.assertIn("omj setup", doctor["recommended_next_action"])
    ...
    def test_convert_from_local_skill_fixture(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "convert", "--from-skills-dir", str(root / "local-skills")])[0], 0)
            converted = (omj_home / "skills" / "ralph" / "SKILL.md").read_text(encoding="utf-8")
            self.assertIn("description: [omj] Hermes Ralph workflow", converted)
    ...
    def test_mocked_source_install_update(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "install", "--source", str(root / "release-archive")])[0], 0)
            first_manifest = json.loads((omj_home / "manifest.json").read_text(encoding="utf-8"))
            ...
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "update", "--source", str(root / "release-archive")])[0], 0)
            updated = (omj_home / "skills" / "team" / "SKILL.md").read_text(encoding="utf-8")
    ...
    def test_release_channel_metadata_and_validation(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            status, stdout, stderr = run_cli(["--omj-home", str(omj_home), "install", "--dry-run", "--channel", "stable", "--version", "1.0.0"])
            self.assertEqual(stderr, "")
            self.assertEqual(status, 0)
            dry_run = json.loads(stdout)
            ...
            status, _, stderr = run_cli(["--omj-home", str(omj_home), "install", "--dry-run", "--channel", "stable"])
            self.assertEqual(status, 2)
            self.assertIn("stable channel requires", stderr)

            status, _, stderr = run_cli(["--omj-home", str(omj_home), "update", "--channel", "local"])
            self.assertEqual(status, 2)
    ...
    def test_installer_reported_update_records_version_and_ref_movement(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("OMJ command: main@new -> main@human (updated)", stdout)
            self.assertNotIn("To update the `omj` command itself", stdout)
            ...
            self.assertIn("Release state: updated", stdout)
            self.assertIn("OMJ command: 1.0.1 -> 1.0.2 (updated)", stdout)
            ...
            self.assertIn("Release state: updated", stdout)
            self.assertIn("OMJ command: 1.0.2 (v1.0.2 -> v1.0.2@rerun) (updated)", stdout)
            ...
            self.assertIn("Release state: updated", stdout)
            self.assertIn("OMJ command: 1.0.2 -> 1.0.2 (updated)", stdout)
    ...
    def test_direct_update_source_ref_metadata_does_not_claim_command_package_update(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            status, stdout, stderr = run_cli(
                ["--omj-home", str(omj_home), "update", "--source-ref", "main@manual"],
                output_json=False,
            )
            ...
            status, stdout, stderr = run_cli(
                ["--omj-home", str(omj_home), "update", "--source-ref", "main@manual", "--json"],
                output_json=False,
            )
            self.assertEqual(status, 0, stderr)
            self.assertEqual(stderr, "")
    ...
    def test_first_direct_update_source_ref_records_metadata_without_package_update(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            status, stdout, stderr = run_cli(
                ["--omj-home", str(omj_home), "update", "--source-ref", "main@first"],
                output_json=False,
            )
    ...
    def test_latest_release_update_state_wins_over_stale_last_update(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "update", "--source-ref", "main@old"])[0], 0)
            self.assertEqual(
                run_cli(
                    [
                        "--omj-home",
            ...
            status, stdout, stderr = run_cli(
                ["--omj-home", str(omj_home), "update", "--source-ref", "main@new", "--json"],
                output_json=False,
            )
    ...
    def test_update_tolerates_malformed_previous_runtime_state(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            omj_home = root / ".omj"
            state_path = omj_home / "runtime" / "state.json"
    ...
    def test_runtime_commands_record_show_and_delegate(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "runtime", "status"])[0], 0)
            status, stdout, _ = run_cli(
                [
                    "--omj-home",
                    str(omj_home),
            ...
            status, stdout, _ = run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "runtime", "runs"])
            self.assertEqual(status, 0)
            ...
            status, stdout, _ = run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "runtime", "runs", "--limit", "1"])
            self.assertEqual(status, 0)
            self.assertEqual([item["run_id"] for item in json.loads(stdout)["runs"]], [second_run["run_id"]])

            status, stdout, stderr = run_cli(
                ["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "runtime", "export", "--limit", "1", "--summary"]
            )
            self.assertEqual(stderr, "")
            self.assertEqual(status, 0)
            ...
            status, stdout, _ = run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "runtime", "show", run["run_id"]])
            self.assertEqual(status, 0)
            shown = json.loads(stdout)
            ...
            status, stdout, stderr = run_cli(
                ["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "runtime", "progress-status"]
            )
            self.assertEqual(stderr, "")
            self.assertEqual(status, 0)
            ...
            status, stdout, stderr = run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "runtime", "show", run["run_id"]])
            self.assertEqual(stderr, "")
            self.assertEqual(status, 0)
            shown = json.loads(stdout)
            ...
            status, stdout, stderr = run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "runtime", "export"])
            self.assertEqual(stderr, "")
            self.assertEqual(status, 0)
            exported = json.loads(stdout)
        ...
        with TemporaryDirectory() as tmp:
            ...
            codex_log.write_text(
                "\n".join(
                    [
                        json.dumps({"type": "tool_call", "tool": "rg", "args": "rg runtime progress src tests"}),
                        json.dumps({"type": "tool_call", "tool": "apply_patch", "message": "modified src/omj/coding/executor_progress.py"}),
                    ]
                ),
                encoding="utf-8",
            )
    ...
    def test_runtime_progress_observe_reports_malformed_binding_without_traceback(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(status, 0)
            run = json.loads(stdout)["run"]
            paths = resolve_paths(omj_home, hermes_home)
    ...
    def test_docs_workflows_command_prints_writes_and_checks_generated_reference(self) -> None:
        ...
        self.assertIn("### oh-my-jeo", stdout)
    ...
    def test_docs_workflows_json_exposes_machine_readable_quality_contract(self) -> None:
        ...
        skills = {skill["name"]: skill for skill in payload["skills"]}
        self.assertIn("why_this_exists", skills["oh-my-jeo"])
        self.assertIn("do_not_use_when", skills["oh-my-jeo"])
        self.assertIn("good_example", skills["oh-my-jeo"])
        self.assertIn("bad_example", skills["oh-my-jeo"])
        self.assertIn("Use OMJ request-to-handoff", skills["oh-my-jeo"]["good_example"]["prompt"])
    ...
    def test_doctor_reports_unwritable_runtime_artifact_path_without_crashing(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            omj_home = root / ".omj"
            omj_home.mkdir()
            (omj_home / "runtime").write_text("not a directory", encoding="utf-8")

            status, stdout, stderr = run_cli(["--omj-home", str(omj_home), "doctor"])
            ...
            self.assertIn("writable --omj-home", checks["runtime_artifacts"]["next_action"])
    ...
    def test_doctor_reports_malformed_runtime_state_without_crashing(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            omj_home = root / ".omj"
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "install"])[0], 0)
            state_path = omj_home / "runtime" / "state.json"
    ...
    def test_runtime_status_and_record_tolerate_malformed_state(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            status, stdout, stderr = run_cli(["--omj-home", str(omj_home), "runtime", "status"])
            ...
            self.assertEqual(status, 0)
            run_id = json.loads(stdout)["run"]["run_id"]
            self.assertTrue((omj_home / "runtime" / "runs" / run_id / "run.json").exists())
