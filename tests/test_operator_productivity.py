...
load_local_package()
from omj.operator_productivity import (
...
from omj.skills.catalog import harness_definition, installable_skill_names, primary_harness_for_skill
...
class OperatorProductivityTests(unittest.TestCase):
    ...
    def test_ops_cli_prepares_stores_lists_and_validates_agent_review_cards(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertFalse((root / ".omj" / "agent-ops").exists())
            ...
            self.assertTrue((root / ".omj" / "agent-ops" / "reviews" / f"{card_id}.json").exists())
    ...
    def test_chat_interact_renders_manager_card_without_shell_catalog_approval(self) -> None:
        ...
        self.assertNotIn("omj list", json.dumps(payload))
