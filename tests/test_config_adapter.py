...
load_local_package()
from omj.config_adapter import ensure_external_dir, external_dirs, remove_external_dir
...
class ConfigAdapterTests(unittest.TestCase):
    ...
    def test_ensure_external_dir_creates_empty_config(self) -> None:
        change = ensure_external_dir("", "/tmp/omj/skills")

        self.assertTrue(change.changed)
        self.assertEqual(external_dirs(change.text), ["/tmp/omj/skills"])

    def test_ensure_external_dir_preserves_existing_keys_and_is_idempotent(self) -> None:
        original = "model: test\nskills:\n  disabled:\n    - old\n"
        first = ensure_external_dir(original, "/tmp/omj/skills")
        second = ensure_external_dir(first.text, "/tmp/omj/skills")

        self.assertTrue(first.changed)
        self.assertFalse(second.changed)
        self.assertIn("model: test", first.text)
        self.assertIn("disabled:", first.text)
        self.assertEqual(external_dirs(first.text), ["/tmp/omj/skills"])

    def test_external_dirs_accepts_yaml_sequence_at_key_indent(self) -> None:
        original = "model: test\nskills:\n  external_dirs:\n  - /tmp/omj/skills\n"

        self.assertEqual(external_dirs(original), ["/tmp/omj/skills"])
        change = ensure_external_dir(original, "/tmp/omj/skills")
    ...
    def test_ensure_external_dir_preserves_key_indent_sequence_style(self) -> None:
        ...
        self.assertTrue(change.changed)
        self.assertEqual(external_dirs(change.text), ["/keep", "/tmp/omj/skills"])
        self.assertIn("  - /keep\n  - /tmp/omj/skills\n", change.text)

    def test_remove_external_dir_only_removes_managed_entry(self) -> None:
        original = "skills:\n  external_dirs:\n    - /keep\n    - /tmp/omj/skills\n"
        change = remove_external_dir(original, "/tmp/omj/skills")
    ...
    def test_remove_external_dir_from_key_indent_sequence(self) -> None:
        ...
        self.assertEqual(external_dirs(change.text), ["/keep"])
        self.assertIn("  - /keep\n", change.text)
        self.assertNotIn("/tmp/omj/skills", change.text)
    ...
    def test_ensure_external_dir_rejects_unsupported_scalar_shapes(self) -> None:
        for value in ("'null'", '"null"', "null # comment", "~ # comment", "/tmp/omj/skills"):
            with self.subTest(value=value):
                original = f"skills:\n  external_dirs: {value}\n"
                self.assertEqual(external_dirs(original), [])
                with self.assertRaises(ValueError):
                    ensure_external_dir(original, "/tmp/omj/skills")
    ...
    def test_remove_external_dir_from_inline_list(self) -> None:
        original = "skills:\n  external_dirs: [/keep, /tmp/omj/skills]\n"
        change = remove_external_dir(original, "/tmp/omj/skills")
