...
def run_cli(args: list[str], *, output_json: bool = True, stdin_text: str = "") -> tuple[int, str, str]:
    ...
    previous_output = os.environ.get("OMJ_OUTPUT")
    if output_json:
        os.environ["OMJ_OUTPUT"] = "json"
    else:
        os.environ.pop("OMJ_OUTPUT", None)
    with _redirect_stdin(stdin), redirect_stdout(stdout), redirect_stderr(stderr):
        try:
            ...
            if previous_output is None:
                ...
                os.environ["OMJ_OUTPUT"] = previous_output
