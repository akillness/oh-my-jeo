...
from omj.capabilities.families import capability_family_projection, family_for_workflow
from omj.capabilities.registry import capability_snapshot, capability_summary, inspect_capability, list_capabilities
from omj.capabilities.schema import normalize_capability_section
from omj.skills.catalog import builtin_definitions, installable_skill_names
...
class CapabilityManifestTests(unittest.TestCase):
    def test_capability_snapshot_is_deterministic_and_boundary_safe(self) -> None:
        ...
        self.assertEqual(first["schema_version"], "omj_capability_manifest/v1")
        self.assertEqual(first["determinism"], "static_projection_no_runtime_clock")
        self.assertEqual(first["omj_awareness"]["schema_version"], "omj_awareness/v1")
        self.assertIn("workflow-shaped requests", first["omj_awareness"]["purpose"])
        self.assertIn("Hermes-native workflow pack", first["omj_awareness"]["product_context"])
        self.assertIn("every OMJ skill", first["omj_awareness"]["all_skill_context_rule"])
        self.assertIn("generic tool can render or execute", first["omj_awareness"]["all_skill_context_rule"])
        self.assertIn("Every generated workflow skill", first["omj_awareness"]["skill_coverage"])
        self.assertIn("meeting notes -> meeting-brief", " ".join(first["omj_awareness"]["cross_lane_examples"]))
        self.assertIn("img-summary", json.dumps(first["omj_awareness"], sort_keys=True))
        ...
        self.assertIn("Prepared OMJ capability", first["evidence_boundaries"]["prepared_is_not"])

    def test_awareness_lanes_cover_every_catalog_workflow_surface(self) -> None:
        awareness = capability_snapshot()["omj_awareness"]
        lane_skills = {
            str(skill)
    ...
    def test_capability_families_are_user_facing_front_door_projection(self) -> None:
        ...
        self.assertEqual(projection["schema_version"], "omj_capability_families/v1")
        self.assertEqual(
    ...
    def test_capability_sections_have_expected_ids(self) -> None:
        ...
        self.assertIn("omj_awareness", sections["omj_awareness"])
        self.assertIn("handoff-guide", sections["agent_roles"])
        self.assertIn("ultragoal", sections["skills"])
        self.assertIn("omj_capabilities", sections["hooks"])
    ...
    def test_capability_summary_is_human_facing_catalog_context(self) -> None:
        ...
        self.assertIn("Prepared OMJ capability", summary["evidence_boundary"])
    ...
    def test_capability_inspect_finds_skill_and_role_without_runtime_claim(self) -> None:
        ...
        awareness = inspect_capability("omj_awareness", section="omj_awareness")["capability"]
        ...
        self.assertIn("every OMJ skill", skill["workflow_context_rule"])
        ...
        self.assertIn("omj_context", awareness["tool_hints"][0])
        self.assertIn("omj_capabilities", " ".join(awareness["tool_hints"]))
        ...
        self.assertEqual(awareness["schema_version"], "omj_awareness/v1")
        ...
        self.assertIn("OMJ workflow-layer responsibility context", role["workflow_context_rule"])
        ...
        self.assertIn("Prepared OMJ capability", playbook["prepared_is_not"])
    ...
    def test_package_metadata_includes_capabilities_package(self) -> None:
        ...
        self.assertIn("omj", packages)
        self.assertIn("omj.capabilities", packages)
        self.assertIn("omj.routing", packages)
        self.assertEqual(package_dir["omj"], "src/omj")
        self.assertEqual(package_dir["omj.capabilities"], "src/capabilities")
        self.assertEqual(package_dir["omj.routing"], "src/routing")
        for package_path in (
            "capabilities",
            "coding",
            "install",
        ...
        self.assertTrue((source_root / "omj" / "cli" / "__init__.py").is_file())
        for compatibility_module in (
            ...
            with self.subTest(compatibility_module=compatibility_module):
                self.assertTrue((source_root / "omj" / compatibility_module).is_file())
