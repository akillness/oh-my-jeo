...
from omj.research_department import (
...
class OperationsArtifactTests(unittest.TestCase):
    ...
    def test_prepared_operating_rhythm_artifact_round_trips(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(written["schema_version"], "omj_operation_artifact/v1")
    ...
    def test_observed_operating_rhythm_requires_supplied_evidence(self) -> None:
        errors = validate_operation_artifact(
            {
                "schema_version": "omj_operation_artifact/v1",
                "artifact_id": "a",
                "surface": "operating-rhythm",
                "kind": "retro",
                "title": "Sprint retro",
    ...
    def test_validate_store_reports_malformed_artifacts(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("schema_version must be omj_operation_artifact/v1", " ".join(validation["errors"]))
    ...
    def test_show_rejects_path_traversal_artifact_ids(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            secret.write_text(json.dumps({"schema_version": "omj_operation_artifact/v1", "title": "secret"}), encoding="utf-8")
...
def _paths_from_tmp(tmp: str) -> OmjPaths:
    root = Path(tmp)
    return OmjPaths(root / ".omj", root / ".hermes")
