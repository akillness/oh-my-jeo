...
from omj.chat_router import route_chat_message, routing_record_payload
from omj.coding_delegation import build_coding_delegation_payload, coding_delegation_record_payload
from omj.executor_progress import build_progress_binding, build_safe_progress_signal, observe_executor_progress, write_progress_binding
...
from omj.runtime.records import (
...
class RuntimeArtifactTests(unittest.TestCase):
    ...
    def test_journal_events_project_run_lifecycle_without_legacy_observation_file(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            run_dir = paths.runtime_runs_dir / run["run_id"]
            message = "implement safe runtime feature in src/omj/runtime/artifacts.py without overclaiming"
            payload = build_coding_delegation_payload(
                message,
                source="discord",
                executor_target="codex",
        ...
        with TemporaryDirectory() as tmp:
            ...
            with paths.runtime_journal_events_path.open("a", encoding="utf-8") as handle:
                handle.write(
                    json.dumps(
                        {
                            "schema_version": "omj_observation_event/v1",
                            "event_id": "bad-merge",
                            "target_type": "run",
                            "target_id": run["run_id"],
                            "run_id": run["run_id"],
    ...
    def test_show_run_and_export_drop_invalid_executor_progress_payloads(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            (progress_dir / "events.jsonl").write_text(
                json.dumps(
                    {
                        "schema_version": "omj_progress_event/v1",
            ...
            (progress_dir / "reports.jsonl").write_text(
                json.dumps(
                    {
                        "schema_version": "omj_progress_report/v1",
                        "binding_id": binding["binding_id"],
                        "executor_profile": "codex",
                        "event_type": "diff_started",
                        "status": "running",
