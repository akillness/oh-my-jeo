...
load_local_package()
from omj.context_safety import CONTEXT_ARTIFACT_REF_SCHEMA_VERSION
from omj.paths import OmjPaths, resolve_paths
from omj.profiles.setup import write_setup_profile
from omj.wrapper_contract import (
...
from omj.wrapper.native_commands import build_native_command_surface, render_native_command_response
from omj.wrapper.route_hints import build_chat_route_hint_payload
...
class WrapperContractTests(unittest.TestCase):
    ...
    def test_chat_interaction_renders_task_abstraction_card_without_cli_copy(self) -> None:
        ...
        self.assertEqual(payload["route"]["task_card"]["schema_version"], "omj_task_card/v1")
        ...
        self.assertNotIn("omj migration", json.dumps(payload).lower())
        self.assertNotIn(message, json.dumps(payload))

    def test_chat_interaction_renders_omj_maintenance_card_without_coding_handoff(self) -> None:
        payload = build_chat_interaction_payload("omj update", source="discord")

        self.assertEqual(payload["schema_version"], "chat_interaction/v1")
        self.assertEqual(payload["mode"], "route")
        self.assertEqual(payload["route"]["selected_skill"], "oh-my-jeo")
        self.assertEqual(payload["route"]["task_card"]["task_type"], "omj_cli_maintenance")
    ...
    def test_chat_interaction_surfaces_target_change_notice_without_overclaiming(self) -> None:
        notice = {
            "schema_version": "omj_target_change_notice/v1",
            ...
            "topology": {
                "schema_version": "omj_target_topology/v1",
            ...
            "apply_payload": {
                "schema_version": "omj_target_apply_payload/v1",
                "source": "chat:discord",
                "source_metadata": {
                    "agent_ref": "agent-b",
                ...
                "persistence_contract": "Pass this source_metadata back to the OMJ wrapper backend with target-change apply enabled; no raw chat prompt is required.",
            },
    ...
    def test_delegate_mode_uses_setup_default_codex_executor_when_available(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            payload = build_chat_interaction_payload(
                "implement a focused parser fix in src/omj/parser.py and update tests",
                source="discord",
                mode="delegate",
                paths=paths,
            )
    ...
    def test_delegate_mode_can_prepare_hermes_coding_team_path(self) -> None:
        ...
        self.assertEqual(payload["next_action"], "show_runtime_handoff")
        self.assertEqual(runtime["runtime_profile"]["runtime_family"], "omj")
    ...
    def test_chat_interaction_renders_learning_candidate_card_with_learn_prompt(self) -> None:
        ...
        self.assertEqual(card["scope"]["project_ref"], "omjm")
    ...
    def test_route_mode_exposes_visible_omj_usage_trace_for_web_research(self) -> None:
        payload = build_chat_interaction_payload(
            "web-research로 Hermes Agent와 Oh My Codex/OpenCode 계열을 비교해서 OMJM 포지셔닝 근거를 찾아줘.",
            source="discord",
        )
        ...
        self.assertEqual(trace["schema_version"], "omj_usage_trace/v1")
        self.assertEqual(trace["visible_prefix"], "[omj] web-research")
        ...
        self.assertEqual(explanation["schema_version"], "omj_workflow_explanation/v1")
        ...
        self.assertTrue(response["headline"].startswith("[omj] web-research - "))
        self.assertEqual(response["plain_headline"], "I know which workflow should handle this.")
        self.assertEqual(rendering["schema_version"], "omj_messenger_rendering/v1")
    ...
    def test_messenger_rendering_converts_markdown_tables_to_bullets(self) -> None:
        ...
        rendering = messenger_rendering_contract(
            visible_prefix="[omj] web-research",
            first_line="[omj] web-research - 비교 결과입니다.",
            body=body,
        ...
        self.assertIn("- Hermes Agent: 강점: Skills, memory, gateway; OMJ 포지션: Workflow layer", rendering["body_text"])
        self.assertIn("- OpenCode: 강점: TUI coding loop; OMJ 포지션: Handoff target", rendering["body_text"])
    ...
    def test_messenger_rendering_keeps_tables_inside_code_fences(self) -> None:
        ...
        rendering = messenger_rendering_contract(
            visible_prefix="[omj] web-research",
            first_line="[omj] web-research - 예제입니다.",
            body=body,
            claim_boundary="Research summary is not execution evidence.",
        )
        ...
        rendered = render_native_command_response(
            {
                ...
                "chat_response": {
                    "kind": "plan",
                    "headline": "[omj] web-research - 비교 결과입니다.",
                    "body": body,
        ...
        rendered = render_native_command_response(
            {
                ...
                "chat_response": {
                    ...
                    "body": body,
                    "state": {"phase": "planning"},
                    "messenger_rendering": {"schema_version": "omj_messenger_rendering/v1"},
                    "actions": [],
                },
            },
            source="discord",
    ...
    def test_route_mode_exposes_visible_omj_usage_trace_for_multiple_workflows(self) -> None:
        cases = (
        ...
        for message, workflow in cases:
            with self.subTest(workflow=workflow):
                ...
                self.assertEqual(response["usage_trace"]["visible_prefix"], f"[omj] {workflow}")
                self.assertTrue(response["headline"].startswith(f"[omj] {workflow} - "))
                self.assertNotIn("[omj]", response["body"])
    ...
    def test_blocker_status_uses_status_usage_trace_label(self) -> None:
        ...
        self.assertTrue(response["headline"].startswith("[omj] status - "))
        self.assertEqual(response["usage_trace"]["visible_prefix"], "[omj] status")
        self.assertEqual(response["usage_trace"]["evidence_state"], "observed_partial")

    def test_omj_first_use_questions_render_quickstart_card_without_shell(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            for message in (
                ...
                with self.subTest(message=message):
                    ...
                    self.assertEqual(payload["route"]["selected_skill"], "oh-my-jeo")
                    self.assertEqual(payload["route"]["recommendations"][0]["matched"], ["omj_quickstart_question"])
                    ...
                    self.assertIn("OMJ quickstart is", payload["chat_response"]["body"])
    ...
    def test_explicit_omj_status_questions_can_still_render_probe_roadmap(self) -> None:
        with TemporaryDirectory() as tmp:
            paths = OmjPaths(omj_home=Path(tmp) / ".omj", hermes_home=Path(tmp) / ".hermes")
            payload = build_chat_interaction_payload("show OMJ status", source="discord", paths=paths)

            self.assertEqual(payload["mode"], "status")
            self.assertEqual(payload["next_action"], "show_status")
            self.assertEqual(payload["route"]["selected_skill"], "oh-my-jeo")
            self.assertEqual(payload["chat_response"]["kind"], "status")
            self.assertTrue(payload["chat_response"]["headline"].startswith("[omj] status - "))
            self.assertIn("Current status:", payload["chat_response"]["body"])
            self.assertIn("OMJ setup gaps:", payload["chat_response"]["body"])
    ...
    def test_direct_omj_invocation_exposes_skill_picker(self) -> None:
        payload = build_chat_interaction_payload("./omj", source="discord")
        ...
        self.assertEqual(actions["choose_skill"]["payload"]["schema_version"], "omj_skill_picker/v1")
        self.assertEqual(actions["choose_skill"]["payload"]["featured_options"][0]["id"], "oh-my-jeo")
    ...
    def test_natural_catalog_questions_open_picker_without_shell(self) -> None:
        for message in (
            ...
            with self.subTest(message=message):
                ...
                picker = payload["chat_response"]["state"]["skill_picker"]
                option_ids = {option["id"] for option in picker["options"]}
                self.assertTrue({"oh-my-jeo", "loop", "ultraprocess"} <= option_ids)
                self.assertIn("source-finder", option_ids)
                self.assertIn("paper-learning", option_ids)
                self.assertEqual(picker["featured_options"][0]["id"], "oh-my-jeo")
                picker_groups = {group["id"]: group for group in picker["groups"]}
                ...
                self.assertEqual(primer["schema_version"], "omj_context_primer/v1")
                ...
                self.assertTrue(
                    any("Prepared OMJ capability" in boundary for boundary in capability_summary["evidence_boundary"])
                )
                self.assertNotIn("run_local_operator_check", json.dumps(payload))

    def test_omj_intro_questions_render_context_brief_before_picker(self) -> None:
        for message in (
            "what is OMJ and how do I use it?",
            "explain OMJ for a new Hermes user",
            "OMJ가 뭐야? 어떻게 써?",
            "OMJ 사용법 소개해줘",
        ):
            with self.subTest(message=message):
                ...
                self.assertEqual(payload["route"]["selected_skill"], "oh-my-jeo")
                self.assertEqual(payload["route"]["recommendations"][0]["matched"], ["omj_intro_question"])
                response = payload["chat_response"]
                self.assertEqual(response["kind"], "context_brief")
                self.assertTrue(response["headline"].startswith("[omj] context - "))
                ...
                self.assertEqual(state["status_source"], "omj_context_brief")
    ...
    def test_partial_dot_slash_invocation_exposes_omj_command_preview_only(self) -> None:
        cases = {
            "./": "./omj",
            "/": "/omj",
            "./o": "./omj",
    ...
    def test_native_command_surfaces_cover_major_messenger_registration_paths(self) -> None:
        ...
        for source, registration_schema in cases.items():
            with self.subTest(source=source):
                ...
                self.assertEqual(surface["schema_version"], "omj_native_command_surface/v1")
                self.assertEqual(surface["source"], source)
                self.assertEqual(surface["command"], "omj")
                self.assertEqual(surface["preview_contract"]["only_top_level_suggestions"], ["omj"])
                self.assertEqual(surface["fallback_card"]["schema_version"], "omj_command_fallback_card/v1")
                self.assertEqual(surface["fallback_card"]["primary_action"]["label"], "Open omj")
    ...
    def test_command_preview_can_render_platform_native_fallback_card(self) -> None:
        ...
        self.assertEqual(rendered["schema_version"], "omj_native_command_render/v1")
        self.assertEqual(rendered["source"], "discord")
        self.assertEqual(rendered["response_kind"], "command_preview")
        self.assertEqual(rendered["usage_trace"]["schema_version"], "omj_usage_trace/v1")
        self.assertEqual(rendered["messenger_rendering"]["schema_version"], "omj_messenger_rendering/v1")
        self.assertEqual(rendered["render_kind"], "fallback_card")
        self.assertEqual(rendered["card"]["primary_action"]["submit_text"], "./omj")
        self.assertEqual(rendered["component"]["kind"], "discord_message_components")
        self.assertEqual(rendered["component"]["buttons"][0]["label"], "Open omj")
        self.assertIn("workflow selected", rendered["not_evidence"])

    def test_native_render_exposes_grouped_skill_picker(self) -> None:
        interaction = build_chat_interaction_payload("./omj", source="discord")

        rendered = render_native_command_response(interaction, source="discord")

        self.assertEqual(rendered["schema_version"], "omj_native_command_render/v1")
        self.assertEqual(rendered["render_kind"], "workflow_picker")
        self.assertEqual(rendered["picker_schema"], "omj_skill_picker/v1")
        self.assertEqual(rendered["featured_options"][0]["id"], "oh-my-jeo")
        ...
        self.assertEqual(component["featured_options"][0]["value"], "oh-my-jeo")
    ...
    def test_loop_interaction_exposes_start_card_without_raw_goal_by_default(self) -> None:
        message = "./loop make OMJ a 10k-star quality Hermes-native project"
    ...
    def test_terse_loop_invocation_starts_agentic_loop_without_picker(self) -> None:
        payload = build_chat_interaction_payload("loop reinforcement omj", source="discord")
        ...
        korean = build_chat_interaction_payload("OMJ 루프 강화 ㄱㄱ", source="discord")
        ...
        help_command = build_chat_interaction_payload("omj loop help", source="discord")
    ...
    def test_loop_mentions_do_not_force_explicit_loop_dispatch(self) -> None:
        lookup = build_chat_interaction_payload("look up OMJ loop docs", source="discord")
        ...
        loopback = build_chat_interaction_payload("loopback interface on OMJ is broken", source="discord")
    ...
    def test_generic_catalog_question_still_uses_picker(self) -> None:
        payload = build_chat_interaction_payload("OMJ 기능 뭐 있어?", source="discord")
    ...
    def test_ack_workflow_chat_copy_stays_human_friendly(self) -> None:
        cases = (
            ...
            (
                "can OMJ help with MCP setup?",
                "toolbelt-readiness",
                "prepare_toolbelt_readiness",
                "MCP, CLI, API",
            ),
            (
                "does OMJ support memory cleanup?",
                "memory-curation-review",
                "prepare_memory_curation_review",
                "approve, reject, or update",
            ),
            (
                "does OMJ support voice commands?",
                "voice-operator",
                "prepare_voice_operator_card",
                "confirmation before risky actions",
            ),
            (
                "OMJ로 GitHub issue webhook 처리 가능해?",
                "github-event-ops",
                "prepare_github_event_ops_card",
                "triage, review, label",
            ),
    ...
    def test_recommendation_policy_next_actions_are_visible_ack_actions(self) -> None:
        from omj.routing import recommend
        from omj.wrapper import contract
    ...
    def test_status_card_preserves_generic_event_progress_without_raw_payloads(self) -> None:
        ...
        self.assertEqual(event["schema_version"], "omj_progress_event/v1")
