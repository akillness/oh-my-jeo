...
class GoalLoopTests(unittest.TestCase):
    def test_loop_start_card_redacts_goal_and_exposes_start_contract(self) -> None:
        card = build_loop_start_card(
            "Make OMJ a 10k-star quality Hermes-native project",
            source="discord",
        ...
        visible = build_loop_start_card("Make OMJ public launch-ready", include_goal=True)
        self.assertEqual(visible["goal_summary"], "Make OMJ public launch-ready")

    def test_explicit_loop_invocation_is_agentic_and_does_not_stop_at_interview(self) -> None:
        card = build_loop_start_card(
            "loop reinforcement omj",
            source="discord",
        ...
        help_command = build_loop_start_card("omj loop help")
    ...
    def test_loop_executor_capability_selects_native_or_omj_managed_loop(self) -> None:
        ...
        self.assertEqual(codex["loop_mode"], "omj_managed")
        self.assertEqual(codex["dispatch_owner"], "omj_wrapper")
        self.assertIn("codex_progress_summary/v1", codex["observability"])
        self.assertEqual(hermes["loop_mode"], "omj_managed")
    ...
    def test_loop_queue_lifecycle_lists_handoffs_observes_and_blocks_items(self) -> None:
        ...
        self.assertIn("Continue OMJ loop", handoff["handoff_text"])
