...
load_local_package()
from omj.skill_pack import builtin_definitions, builtin_skill_templates
from omj.routing import recommend as recommend_module
from omj.skills import render as render_module
...
from omj.capabilities.skills import skill_capabilities
from omj.plugin_bundle.omj.tools.capability_tool import standalone_skill_capability_items
from omj.plugin_bundle.omj.awareness import (
...
class EfficiencyContractTests(unittest.TestCase):
    ...
    def test_generated_skill_pack_keeps_memory_guidance_under_budget(self) -> None:
        ...
        compact_budget = (
            sum(
                ...
                if definition.name != "oh-my-jeo" and memory_context_policy_for_skill(definition.name) == "compact"
            )
            + 1
        )
    ...
    def test_omj_awareness_context_is_strong_but_bounded(self) -> None:
        workflow_skill_names = {template.name for template in builtin_skill_templates()} - {"oh-my-jeo"}
        ...
        self.assertIn("consider OMJ before generic tools", primer_context)
        self.assertIn("Use message-specific route hints", primer_context)
        self.assertIn("not observed execution", primer_context)
        self.assertIn("omj_context", primer_context)
        self.assertIn("omj_capabilities", primer_context)
        self.assertIn("omj_status/omj_hud", primer_context)
        ...
        self.assertIn("check OMJ prep/status/learning", awareness_primer_markdown())
        self.assertIn("Generic tool map", awareness_primer_markdown())
        self.assertEqual(combined.count("## OMJ Context Rail"), len(workflow_skill_names))
        self.assertEqual(combined.count("## OMJ Awareness Primer"), 1)
        for name in workflow_skill_names:
            self.assertIn(
    ...
    def test_omj_awareness_lanes_cover_installable_skills(self) -> None:
        ...
        self.assertEqual(
            ...
            "omj_generic_tool_checkpoint/v1",
        )
    ...
    def test_workflow_context_cards_cover_installable_workflow_families(self) -> None:
        workflow_skills = {definition.name for definition in builtin_definitions()} - {"oh-my-jeo", "cancel"}
