...
class MenubarStatusTests(unittest.TestCase):
    def test_menubar_status_keeps_hermes_agents_and_external_executors_separate(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            status, stdout, stderr = run_cli(
                ["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "menubar", "status"]
            )
            ...
            self.assertEqual(payload["versions"]["omj"]["value"], "1.0.1")
    ...
    def test_external_executor_overlay_requires_run_identity_when_multiple_runs_exist(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(run_cli(["--omj-home", str(omj_home), "--hermes-home", str(hermes_home), "setup"])[0], 0)
            for message in (
                "implement first coding task in src/omj/runtime/first.py",
                "implement second coding task in src/omj/runtime/second.py",
            ):
                self.assertEqual(
                    run_cli(
                        [
    ...
    def test_local_process_observation_applies_real_hermes_pid_without_false_path_matches(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            with patch(
                "omj.menubar_status.subprocess.run",
