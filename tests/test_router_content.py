...
load_local_package()
from omj.routing import recommend as recommend_module
from omj.capabilities.orchestration import orchestration_patterns
from omj.wrapper.contract import VISIBLE_ACTIONS
from omj.roles import role_definitions, role_file_markdown, roles_reference_markdown
from omj.skill_pack import (
    builtin_definitions,
    builtin_harnesses,
...
from omj.playbooks import inspect_playbook, list_playbooks
from omj.plugin_bundle.omj.awareness import ROUTER_KEYWORD_SKILLS, router_keyword_summary
from omj.runtime.records import validate_harness_quality
from omj.skills.catalog import (
    SkillDefinition,
...
from omj.skills.render import workflow_reference_markdown, workflow_reference_payload
from omj.snippet import WORKSPACE_SNIPPET
from omj.use_cases import USE_CASES, list_use_cases
...
class RouterContentTests(unittest.TestCase):
    ...
    def test_router_documents_best_effort_and_recovery(self) -> None:
        router = next(skill for skill in builtin_skill_templates() if skill.name == "oh-my-jeo")
        references = {
            str(Path(template.skill_name) / template.relative_path): template.content
            for template in builtin_skill_reference_templates()
        }
        workflow_registry = references["oh-my-jeo/references/workflow-registry.md"]
        wrapper_routing = references["oh-my-jeo/references/wrapper-routing.md"]
        maintenance = references["oh-my-jeo/references/operator-maintenance.md"]
        evidence = references["oh-my-jeo/references/evidence-boundaries.md"]
        ...
        self.assertIn("OMJ Awareness Primer", router.content)
        ...
        self.assertIn("omj chat route", router.content)
        self.assertIn("omj coding delegate", wrapper_routing)
        ...
        self.assertIn("show a command preview with exactly one top-level suggestion: `omj`", router.content)
        self.assertIn("omj chat native-command --source discord", router.content)
        self.assertIn("omj_command_fallback_card/v1", router.content)
        self.assertIn("`./omj`, `/omj`, `./skills`, or `/skills`", router.content)
        ...
        self.assertIn("Do not make the user approve `omj list`", router.content)
        ...
        self.assertIn("omj_target_topology/v1", evidence)
    ...
    def test_context_surfaces_stay_within_compact_budgets(self) -> None:
        ...
        from omj.plugin_bundle.omj.tools.context_tool import OMJ_CONTEXT_SCHEMA
        from omj.plugin_bundle.omj.tools.evidence_tool import OMJ_EVIDENCE_SCHEMA
        from omj.plugin_bundle.omj.tools.hud_tool import OMJ_HUD_SCHEMA
        from omj.plugin_bundle.omj.tools.probe_tool import OMJ_PROBE_SCHEMA
        from omj.plugin_bundle.omj.tools.recommend_tool import OMJ_RECOMMEND_SCHEMA
        from omj.plugin_bundle.omj.tools.role_tool import OMJ_ROLE_SCHEMA
        from omj.plugin_bundle.omj.tools.status_tool import OMJ_STATUS_SCHEMA
        from omj.wrapper.contract import build_chat_interaction_payload
        ...
        maintenance_payload = build_chat_interaction_payload("omj update", source="discord")
        route_payload = build_chat_interaction_payload("웹서치해서 최신 자료 정리해줘", source="discord")
        context_payload = build_chat_interaction_payload("what can OMJ do?", source="discord")
    ...
    def test_role_surface_docs_match_catalog_and_avoid_runtime_claims(self) -> None:
        ...
        self.assertIn("OMJ Role Context", roles_doc)
        self.assertIn("OMJ workflow-layer responsibility context", roles_doc)
        ...
        for role in role_definitions():
            ...
            self.assertIn("OMJ Role Context", text)
    ...
    def test_g1_to_g10_use_case_catalog_is_complete_and_boundary_safe(self) -> None:
        ...
        self.assertIn("omj cases list --json", playbook_doc)
    ...
    def test_router_renders_representative_harness_registry(self) -> None:
        ...
        harness_registry = references["oh-my-jeo/references/harness-registry.md"]
        harnesses = {harness.name for harness in builtin_harnesses()}
    ...
    def test_catalog_definitions_expose_required_metadata_fields(self) -> None:
        for definition in builtin_definitions():
            self.assertTrue(definition.description.startswith("[omj] "), definition.name)
        ...
        for template in builtin_skill_templates():
            self.assertIn("description: [omj] ", template.content.split("---", 2)[1], template.name)
    ...
    def test_workflow_skills_refer_to_harness_discipline(self) -> None:
        ...
        self.assertIn("OMJ Context Rail", skills["ultragoal"].content)
        self.assertIn("part of OMJ's Hermes workflow layer", skills["ultragoal"].content)
        ...
        self.assertIn("hand back to `oh-my-jeo`", skills["ultragoal"].content)
        ...
        self.assertIn("Prepared OMJ routing", skills["ultragoal"].content)
        self.assertIn("omj_target_topology/v1", skills["ultragoal"].content)
        self.assertIn("active_agent_count", skills["ultragoal"].content)
        self.assertIn("omj runtime record --skill ultragoal --harness goal-execution --status started", skills["ultragoal"].content)
        self.assertIn("goal_completion_gate/v1", skills["ultragoal"].content)
        self.assertIn("inspect .omj/goals", skills["ultragoal"].content)
        ...
        for name, skill in skills.items():
            ...
            with self.subTest(skill=name):
                self.assertIn("OMJ Context Rail", skill.content)
    ...
    def test_workflow_reference_payload_exposes_quality_contracts(self) -> None:
        ...
        self.assertEqual(skills["oh-my-jeo"]["quality_tier"], "routing-gated")
        self.assertIn("Keep users command-agnostic", " ".join(skills["oh-my-jeo"]["quality_bar"]))
        self.assertIn("direct workflow selection", " ".join(skills["oh-my-jeo"]["quality_bar"]))
        ...
        self.assertIn("final_checklist", skills["oh-my-jeo"])
        self.assertIn("recovery_notes", skills["oh-my-jeo"])
        self.assertGreaterEqual(len(skills["oh-my-jeo"]["final_checklist"]), 2)
        self.assertGreaterEqual(len(skills["oh-my-jeo"]["recovery_notes"]), 2)
        self.assertIn("routing conservative", skills["oh-my-jeo"]["why_this_exists"])
        ...
        self.assertIn("inspect .omj/goals", ultragoal_recovery)
    ...
    def test_generated_workflow_reference_matches_catalog(self) -> None:
        ...
        self.assertIn("omj_target_topology/v1", reference)
    ...
    def test_first_release_trust_surfaces_are_present(self) -> None:
        required_paths = [
            ...
            Path("site/assets/omj-img-summary-card.png"),
        ]
        ...
        profile_map = Path("assets/omj-profile-interaction-map.svg").read_text(encoding="utf-8")

        self.assertIn("Hermes skill tap path", readme)
        self.assertIn("hermes skills tap add akillness/oh-my-jeo", readme)
        self.assertIn("hermes skills install akillness/oh-my-jeo/skills/oh-my-jeo --yes", readme)
        self.assertIn("hermes skills tap add akillness/oh-my-jeo", installation)
        self.assertIn("hermes skills install akillness/oh-my-jeo/skills/oh-my-jeo --yes", installation)
        self.assertIn("curl -fsSL https://raw.githubusercontent.com/akillness/oh-my-jeo/main/install.sh | sh", readme)
        ...
        self.assertIn("assets/oh-my-jeo-architecture.svg", readme)
        self.assertIn("## Project Structure", readme)
        self.assertIn("assets/omj-core-workflows.png", readme)
        self.assertIn("img.shields.io/badge/github", readme)
        self.assertNotIn("### Img Summary Skill", readme)
        self.assertNotIn("assets/omj-img-summary-card.png", readme)
        self.assertNotIn("## Organization Patterns", readme)
        self.assertNotIn("assets/omj-profile-interaction-map.svg", readme)
        self.assertIn("../assets/omj-profile-interaction-map.svg", roles_doc)
        self.assertIn("OMJ request-to-handoff interaction map", profile_map)
        self.assertIn("Plain request", profile_map)
        self.assertIn("Hermes Agent", profile_map)
        self.assertIn("OMJ workflow layer", profile_map)
        ...
        self.assertIn("omj profile list", installation)
        self.assertIn("omj profile inspect cto-loop", installation)
        self.assertIn("omj setup --profile-pack cto-loop", installation)
        self.assertIn("CTO, PM, Dev, QA, Security, and Ops", installation)
        self.assertIn("not installed by default", installation)
        self.assertIn("OMJ_CHANNEL=stable OMJ_VERSION=<version>", installation)
        ...
        self.assertIn("```sh\nomj update\nomj doctor\n```", update_section)
        self.assertIn("Most users should run only `omj update`.", update_section)
        self.assertLess(update_section.index("omj update\nomj doctor"), update_section.index("Advanced operators"))
        self.assertNotIn("omj update --channel preview", update_section)
        self.assertIn("omj setup", readme)
        self.assertIn("omj doctor", readme)
        self.assertNotIn("## Why OMJ", readme)
        ...
        self.assertIn("curl -fsSL https://raw.githubusercontent.com/akillness/oh-my-jeo/main/install.sh | sh", quick_start)
        self.assertIn("omj setup", quick_start)
        self.assertLess(quick_start.index("omj setup"), quick_start.index("omj doctor"))
        self.assertIn("Use OMJ request-to-handoff for: I want to safely add a feature to this repo.", quick_start)
        ...
        self.assertNotIn("omj_interact", quick_start)
        ...
        self.assertNotIn("omj docs workflows --json", readme)
        ...
        self.assertNotIn("OMJ_WITH_PLUGIN=1", installation)
        self.assertIn("omj setup --profile-pack cto-loop --profile-pack startup-delivery", installation)
        self.assertIn("omj setup --default-executor claude-code", installation)
        self.assertIn("OMJ_RUN_SETUP=1 OMJ_SETUP_ARGS=\"--dry-run\"", installation)
        self.assertIn("The installer also prints the installed `omj` command path", installation)
        self.assertIn("isolated OMJ virtual environment", installation)
        self.assertIn("add the printed directory to", installation)
        self.assertIn("Use OMJ request-to-handoff for: I want to safely add a feature to this repo.", installation)
        self.assertIn("omj chat native-command --source discord", installation)
        self.assertIn("omj_command_fallback_card/v1", installation)
        self.assertIn("chat_response.state.command_preview.suggestions", installation)
        self.assertIn("Open omj", installation)
        self.assertIn("`./omj`, `/omj`, `./skills`, or `/skills`", installation)
        ...
        self.assertIn("approve `omj list`", installation)
        self.assertIn("omj coding executor-readiness --executor <profile>", installation)
        ...
        self.assertIn("OMJ Agent Install Protocol", install_for_agents)
        self.assertIn("curl -fsSL https://raw.githubusercontent.com/akillness/oh-my-jeo/main/install.sh | sh", install_for_agents)
        self.assertIn("omj setup", install_for_agents)
        self.assertIn("omj doctor", install_for_agents)
        self.assertIn("hermes skills tap add akillness/oh-my-jeo", install_for_agents)
        self.assertNotIn("OMJ_WITH_PLUGIN=1", install_for_agents)
        self.assertIn("recommended_next_action", install_for_agents)
        self.assertIn("Use OMJ request-to-handoff for: I want to safely add a feature to this repo.", install_for_agents)
        self.assertIn("omj release hermes-smoke", install_for_agents)
        self.assertIn("install, list, check, and inspect OMJ", install_for_agents)
        ...
        self.assertIn("omj chat interact", installation)
        self.assertIn("harness_quality/v1", installation)
        self.assertIn("omj docs workflows --json", installation)
        ...
        self.assertIn('--language "$OMJ_LANG"', install_sh)
        self.assertIn("OMJ_SOURCE_REF", install_sh)
        self.assertIn('--source-ref "$OMJ_SOURCE_REF"', install_sh)
        self.assertIn("--command-package-updated", install_sh)
        self.assertIn('OMJ_INSTALL_MODE="${OMJ_INSTALL_MODE:-venv}"', install_sh)
        self.assertIn("step_create_venv", install_sh)
        self.assertIn("OMJ_BIN_DIR", install_sh)
        self.assertIn("OMJ_INSTALL_MODE=python", installation)
        self.assertIn("find_omj_command", install_sh)
        self.assertIn("Expose the omj command", install_sh)
        self.assertIn("not on PATH", install_sh)
        self.assertIn("OMJ_DEFAULT_EXECUTOR", install_sh)
        self.assertIn("OMJ_SETUP_PROFILES", install_sh)
        self.assertIn("OMJ_SETUP_ARGS", install_sh)
        self.assertIn('if [ "$OMJ_CHANNEL" = "local" ] && [ -d "$OMJ_PACKAGE_URL" ]; then', install_sh)
        self.assertIn('set -- "$@" --source "$OMJ_PACKAGE_URL"', install_sh)
        self.assertIn("Install OMJ package", install_sh)
        ...
        self.assertIn("This is an operator reference, not an `omj` command.", runbook)
        ...
        self.assertIn("omj release checklist --version 1.0.1 --json", release)
        ...
        self.assertIn("omj release hermes-smoke --live --install-path tap --target-confirmed", release)
        self.assertIn("--hermes-home /tmp/hermes-smoke release hermes-smoke --live --install-path setup", release)
        self.assertIn("hermes skills check oh-my-jeo", release)
        self.assertIn("uv build", release)
        self.assertIn("oh_my_jeo-1.0.1-py3-none-any.whl", release)
        self.assertIn("/tmp/omj-wheel-smoke/bin/omj --help", release)
        self.assertIn("OMJ_VENV_DIR=/tmp/omj-installer-venv", release)
        ...
        self.assertIn("OMJ", site)
        self.assertIn('href="docs/">Read the docs</a>', site)
        self.assertIn("Hermes-native workflow contracts", site)
        self.assertIn("Hermes chat in. Renderable workflow contract out.", site)
        self.assertIn("The fit layer between plain Hermes requests and trustworthy work.", site)
        self.assertIn("Modern OMJ is a contract system, not a command catalog.", site)
        self.assertIn("omj_interact", site)
        ...
        self.assertIn("assets/omj-img-summary-card.png", site)
        self.assertIn("Meetings, PRs, issues, releases, and research become source-specific prompt cards.", site)
        ...
        self.assertIn("../assets/omj-loop-engineering.png", site_docs)
        self.assertIn("../assets/omj-img-summary-card.png", site_docs)
        ...
        self.assertIn("In OMJ, Loop Engineering is the operating surface", site_loop)
        self.assertNotIn("하네스 엔지니어링", site_loop)
        self.assertIn("../../assets/omj-loop-engineering.png", site_loop)
        ...
        self.assertIn("omj loop run-once --loop", site_loop)
        ...
        expected_source_paths = (
            ...
            "src/mcp_bridge.py",
            "src/plugin_bundle/omj/tools/chat_tool.py",
            "src/plugin_bundle/omj/tools/recommend_tool.py",
            "src/hermes_ops.py",
            "src/commands/ops.py",
            "src/skills/catalog.py",
            "src/skills/render.py",
            "skills/oh-my-jeo/SKILL.md",
        )
        ...
        self.assertNotIn("OMJ Layer", article_body)
        self.assertNotIn("OMJ 포지션", article_body)
        self.assertNotIn("Use OMJ", article_body)
        ...
        self.assertIn('href="https://github.com/akillness/oh-my-jeo"', topbar)
        ...
        self.assertIn('href="https://github.com/akillness/oh-my-jeo"', docs_topbar)
        self.assertNotIn(">GitHub<", docs_topbar)
        hero_command = site.split('aria-label="OMJ quick start commands"', 1)[1].split("</code>", 1)[0]
        self.assertIn("curl -fsSL https://raw.githubusercontent.com/akillness/oh-my-jeo/main/install.sh | sh", hero_command)
        self.assertIn("omj setup", hero_command)
        self.assertIn("omj doctor", hero_command)
        self.assertIn("assets/omj-loop-engineering.png", site)
        self.assertTrue(Path("site/assets/omj-loop-engineering.png").is_file())
        self.assertTrue(Path("site/assets/omj-img-summary-card.png").is_file())
        self.assertNotIn("github.com/akillness/oh-my-jeo/tree/main/docs", site)
        self.assertIn("OMJ Documentation", site_docs)
        self.assertIn("hermes skills tap add akillness/oh-my-jeo", site_docs)
        self.assertIn("OMJ_CHANNEL=stable OMJ_VERSION=&lt;version&gt; sh", site_docs)
        self.assertIn("OMJ_SOURCE_REF=main@&lt;sha&gt; sh", site_docs)
        self.assertIn("omj chat interact --source discord", site_docs)
        self.assertIn("chat_response/v1", site_docs)
        self.assertIn("harness_progress/v1", site_docs)
        self.assertIn("omj harness validate", site_docs)
    ...
    def test_direction_and_agent_contract_lock_product_boundary(self) -> None:
        ...
        self.assertIn("OMJ is a Hermes-native wrapper orchestration layer.", direction)
        ...
        self.assertIn("OMJ owns:", direction)
        ...
        self.assertIn("Do not turn OMJ into a hidden Hermes runtime patch", agents)
    ...
    def test_application_cases_document_representative_flows(self) -> None:
        ...
        self.assertIn("omj playbook recommend", text)
        self.assertIn("safe-feature-change", text)
        self.assertIn("결제 실패 피드백을 모아서 회의 주제와 다음 전략을 정리해줘", text)
        self.assertIn("omj demo grounded-score", text)
        self.assertIn("omj cases demo --all --json", text)
        self.assertIn("omj_use_case_demo_card/v1", text)
        ...
        self.assertIn("omj docs workflows --json", text)
        self.assertIn("omj probe", text)
    ...
    def test_playbook_docs_are_discoverable(self) -> None:
        ...
        self.assertNotIn("omj playbook recommend", readme)
        self.assertIn("omj playbook recommend", playbooks)
    ...
    def test_discord_example_uses_wrapper_native_flow(self) -> None:
        ...
        self.assertIn("omj chat interact --source discord --event-json event.json", text)
        self.assertIn("omj chat session start", text)
        self.assertIn("omj chat session prepare-handoff", text)
        self.assertIn("omj coding lifecycle dispatch", text)
        self.assertIn("omj coding lifecycle report", text)
        self.assertIn("omj runtime show", text)
    ...
    def test_chat_wrapper_examples_include_grounded_operator_transcripts(self) -> None:
        ...
        self.assertIn("## Messenger-Native OMJ Entry", text)
        self.assertIn("omj_native_command_surface/v1", text)
        self.assertIn("omj_native_command_render/v1", text)
        ...
        self.assertIn("Telegram can register the `omj` bot command menu", text)
        ...
        self.assertIn("omj demo grounded-score", site_docs)
        self.assertIn("omj cases demo --all --json", site_docs)
        self.assertIn("omj_use_case_demo_card/v1", site_docs)
    ...
    def test_architecture_docs_include_visual_system_view(self) -> None:
        ...
        self.assertIn("OMJ local contract layer", architecture)
