...
load_local_package()
from omj.coding_delegation import build_coding_delegation_payload
from omj.hermes_planning import build_hermes_plan_payload
from omj.ingress import extract_message_text, extract_source_metadata
from omj.skills.render import workflow_reference_payload
...
class WrapperGoldenExampleTests(unittest.TestCase):
    def test_status_ladder_golden_examples_cover_required_scenarios(self) -> None:
        ...
        for item in payload["scenarios"]:
            ...
            self.assertNotIn("omj ", json.dumps(item).lower())
    ...
    def test_harness_quality_examples_cover_wrapper_visible_quality_gates(self) -> None:
        ...
        for item in payload["examples"]:
            ...
            self.assertNotIn("omj ", serialized)
    ...
    def test_hermes_surface_shims_render_fixture_events(self) -> None:
        ...
        for script, fixture, source, raw_message in cases:
            with self.subTest(script=script):
                ...
                self.assertTrue(payload["response"]["headline"].startswith("[omj] "))
                self.assertEqual(payload["usage_trace"]["schema_version"], "omj_usage_trace/v1")
                self.assertEqual(payload["messenger_rendering"]["schema_version"], "omj_messenger_rendering/v1")
    ...
    def test_messenger_command_preview_shims_render_open_omj_cards(self) -> None:
        cases = (
            (
                "examples/discord-adapter-shim.py",
        ...
        for script, fixture, source, component_kind in cases:
            with self.subTest(source=source):
                ...
                self.assertEqual(native_render["schema_version"], "omj_native_command_render/v1")
                self.assertEqual(native_render["render_kind"], "fallback_card")
                self.assertEqual(native_render["card"]["primary_action"]["label"], "Open omj")
                self.assertEqual(native_render["component"]["kind"], component_kind)
                self.assertEqual(registration["schema_version"], "omj_native_command_surface/v1")
                self.assertEqual(registration["preview_contract"]["only_top_level_suggestions"], ["omj"])
    ...
    def test_route_hint_shims_render_workflow_hint_cards(self) -> None:
        ...
        for script, fixture, source, workflow, next_action, raw_message in cases:
            with self.subTest(source=source):
                ...
                self.assertEqual(payload["usage_trace"]["schema_version"], "omj_usage_trace/v1")
                self.assertEqual(payload["usage_trace"]["visible_prefix"], f"[omj] {workflow}")
                self.assertEqual(payload["messenger_rendering"]["schema_version"], "messenger_route_hint_rendering/v1")
                self.assertEqual(payload["wrapper_contract"]["schema_version"], "omj_route_hint_wrapper_contract/v1")
    ...
    def test_plugin_interact_golden_examples_render_plugin_tool_sessions(self) -> None:
        ...
        for item in payload["examples"]:
            ...
            with self.subTest(item["scenario"]):
                ...
                self.assertEqual(plugin["schema_version"], "omj_plugin_interact_render/v1")
