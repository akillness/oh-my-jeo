...
from omj.capabilities.families import capability_family_projection
from omj.capabilities.registry import capability_summary
from omj.capabilities.schema import CAPABILITY_SECTIONS
from omj.plugin_bundle.omj.metadata import PROVIDED_HOOKS, PROVIDED_TOOLS
...
class PluginCapabilitiesTests(unittest.TestCase):
    def test_installed_plugin_registers_capabilities_tool_and_returns_bounded_json(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            handler = ctx.tools["omj_capabilities"]["args"][2]
            payload = json.loads(handler({"action": "export", "section": "keywords"}))
            self.assertEqual(payload["schema_version"], "omj_capability_manifest/v1")
            ...
            self.assertEqual(summary["schema_version"], "omj_capability_summary/v1")
    ...
    def test_plugin_tool_and_hook_can_self_record_host_observation_metadata(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            interact_handler = ctx.tools["omj_interact"]["args"][2]
                ...
                interact_handler(
                    {
                        ...
                        "observation": {
                            ...
                            "evidence_ref": "host-tool-call:omj_interact",
                        },
                    }
                )
            )
            ...
            self.assertEqual(interaction["plugin_host_observation"]["tool"], "omj_interact")
                ...
                probe_handler(
                    {
                        ...
                        "observation": {
                            ...
                            "evidence_ref": "host-tool-call:omj_probe",
                        },
                    }
                )
            )
            ...
            self.assertEqual(probe["plugin_host_observation"]["tool"], "omj_probe")
            ...
            self.assertIn("host-tool-call:omj_probe", serialized)
    ...
    def test_plugin_interact_backend_errors_are_redacted(self) -> None:
        from omj.plugin_bundle.omj.tools.chat_tool import omj_interact_handler

        with patch(
            "omj.plugin_bundle.omj.tools.chat_tool._package_interaction",
        ...
        self.assertEqual(payload["schema_version"], "omj_interact_result/v1")
    ...
    def test_installed_plugin_capabilities_tool_loads_without_installed_omj_package(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            omj_home = root / ".omj"
            hermes_home = root / ".hermes"
            ...
            sionic_prompt_fixture = """이번 Sionic 작업에서 OMJ가 얼마나 관여했는지 사용성 평가하고,
...
"""
            script = textwrap.dedent(
                ...
                observed_probe = json.loads(
                    probe_handler({{
                        ...
                        "observation": {{
                            ...
                            "evidence_ref": "standalone-host-call:omj_probe",
                        }},
                    }})
                )
                ...
                observation_text = ({str(omj_home / "runtime" / "plugin_host_observations.jsonl")!r})
                observation_text = open(observation_text, encoding="utf-8").read() if __import__("os").path.exists(observation_text) else ""
                state_path = {str(omj_home / "runtime" / "state.json")!r}
                ...
                print(json.dumps({{
                    "tool_registered": "omj_capabilities" in ctx.tools,
            ...
            self.assertEqual(payload["summary_schema"], "omj_capability_summary/v1")
            ...
            self.assertEqual(payload["recommend_schema"], "omj_recommend_result/v1")
            ...
            self.assertEqual(payload["context_schema"], "omj_context_brief/v1")
            self.assertEqual(payload["context_source"], "standalone_plugin_bundle_fallback")
            self.assertEqual(payload["context_plugin_tool"], "omj_context")
            ...
            self.assertEqual(payload["context_catalog_tool"], "omj_capabilities")
            ...
            self.assertEqual(payload["probe_tool"], "omj_probe")
            self.assertFalse(payload["probe_runtime_observed"])
            self.assertFalse(payload["probe_native_ready"])
            self.assertIn("omj_plugin_bundle", payload["probe_capability_names"])
            self.assertIn("plugin_tool_context", payload["probe_capability_names"])
            self.assertEqual(payload["probe_roadmap_schema"], "omj_capability_gap_roadmap/v1")
            ...
            self.assertEqual(payload["observed_probe_public"]["tool"], "omj_probe")
            self.assertEqual(payload["observed_probe_public"]["host"], "hermes-agent")
            self.assertIn("standalone-host-call:omj_probe", payload["standalone_observation_text"])
            ...
            self.assertIn("Prepared OMJ capability", payload["prepared_boundary"])
            ...
            self.assertIn("every OMJ skill", payload["loop_context_rule"])
            ...
            self.assertIn("Prepared OMJ capability", payload["playbook_prepared_boundary"])
