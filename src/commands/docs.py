...
def cmd_docs_workflows(args: argparse.Namespace) -> int:
    if args.json:
        if args.check:
            raise OmjError("docs workflows --json cannot be combined with --check")
        if args.output:
            raise OmjError("docs workflows --json cannot be combined with --output")
    ...
    if args.check:
        try:
            ...
            raise OmjError(f"workflow docs check failed: {exc}") from exc
        if current != content:
            raise OmjError(f"workflow docs are stale: {output}")
        ...
        if not tap_skills["ok"]:
            ...
            raise OmjError(f"tap skills are stale: {stale}")
...
def cmd_harness_inspect(args: argparse.Namespace) -> int:
    try:
        ...
        raise OmjError(f"unknown harness: {args.name}") from exc
    return 0
...
def _add_docs_commands(sub) -> None:
    docs = sub.add_parser("docs", help="Render or check generated OMJ workflow reference docs.")
