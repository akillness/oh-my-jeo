...
from ..command_path import COMMAND_PATH_MISSING_NEXT_ACTION, inspect_omj_command_path
...
MCP_SETUP_SCHEMA_VERSION = "omj_mcp_setup/v1"
SELF_UPDATE_REENTRY_ENV = "OMJ_UPDATE_COMMAND_PACKAGE_REENTERED"
SELF_UPDATE_SKIP_ENV = "OMJ_SKIP_COMMAND_PACKAGE_UPDATE"
...
def cmd_install(args: argparse.Namespace) -> int:
    ...
    if _wants_json(args):
        ...
        progress.header(f"OMJ {operation}", tr(language, "install_subtitle"))
...
def _install_result(args: argparse.Namespace) -> dict[str, object]:
    ...
    if args.channel == "local" and not (args.from_skills_dir or args.source):
        raise OmjError("local channel requires --from-skills-dir or --source")
...
def _run_command_package_self_update(args: argparse.Namespace, plan: dict[str, object]) -> int:
    ...
    if not package_url:
        raise OmjError("cannot update command package because no package URL is available")
    ...
    progress.header("OMJ update", "Refresh the OMJ command package and workflow pack.")
    progress.step(1, 2, "Updating omj command package", detail=package_url)
    ...
    if completed.returncode != 0:
        ...
        raise OmjError(f"command package update failed: {detail}")
    progress.done("command package updated")
    if not wants_json:
        progress.step(2, 2, "Refreshing OMJ workflows with the updated command")
    argv = _reentry_argv_with_command_package_updated()
    env = dict(os.environ)
    env[SELF_UPDATE_REENTRY_ENV] = "1"
    rerun = subprocess.run([python, "-m", "omj.cli", *argv], env=env)
...
def _managed_command_runtime() -> dict[str, object]:
    ...
    if venv_dir is None:
        return {"managed": False, "reason": "HOME or OMJ_VENV_DIR is not available"}
    executable = Path(sys.executable).expanduser()
    if not _is_relative_to_without_resolving_symlinks(executable, venv_dir):
        return {
            "managed": False,
            "reason": "current omj command is not running from the install.sh-managed OMJ venv",
...
def _managed_command_venv_dir() -> Path | None:
    explicit = os.environ.get("OMJ_VENV_DIR")
    if explicit:
        return Path(explicit).expanduser().resolve()
    xdg_data_home = os.environ.get("XDG_DATA_HOME")
    if xdg_data_home:
        return (Path(xdg_data_home).expanduser() / "omj" / "venv").resolve()
    home = os.environ.get("HOME")
    if home:
        return (Path(home).expanduser() / ".local" / "share" / "omj" / "venv").resolve()
    return None
...
def _command_package_status_for_install(
    *,
    ...
    if command_package_updated:
        status = "would_update" if dry_run else "updated"
        updated = not dry_run
        reason = "the installer reported that it refreshed the OMJ command package before running this command"
    elif dry_run:
        ...
        status = "not_updated"
        reason = "managed skills were refreshed, but the omj command package was not updated in this run"
    elif source != "builtin":
...
def _setup_operator_summary(
    ...
    summary = {
        ...
        "paths": {
            "omj_home": str(paths.omj_home),
            "hermes_home": str(paths.hermes_home),
            "skills_dir": str(paths.skills_dir),
            "hermes_config_path": str(paths.hermes_config_path),
        },
        "command_path": inspect_omj_command_path(),
        "state_log": {},
    }
    if not dry_run:
...
def _setup_scope(args: argparse.Namespace) -> str:
    if getattr(args, "omj_home", None) or getattr(args, "hermes_home", None):
        return "custom"
...
def cmd_uninstall(args: argparse.Namespace) -> int:
    ...
    if args.registration_only and (args.remove_files or args.all or args.purge):
        raise OmjError("--registration-only cannot be combined with --remove-files, --all, or --purge")
    paths = _paths(args)
    current = read_config(paths.hermes_config_path)
    try:
...
def _list_catalog_context(skills: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "schema_version": "omj_installed_skill_catalog_context/v1",
        "purpose": (
            "Hermes-facing context for answering what installed OMJ workflows are available "
        ...
        "direct_response_guidance": [
            ...
            "Offer `./omj` or the matching clean skill name when the user wants to choose manually.",
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    if setup_menubar:
        progress.step(step_index, total_steps, tr(language, "step_menubar"), detail=str(paths.omj_home / "menubar"))
    ...
    if args.dry_run:
        bootstrap_final_state = (
            "dry run would install generated skills and register the managed OMJ skills directory for Hermes discovery"
        ...
        bootstrap_final_state = "generated skills are installed in the managed OMJ skills directory and registered for Hermes discovery"
    discovery_status = (
...
def _setup_should_attempt_menubar(args: argparse.Namespace) -> bool:
    ...
    if getattr(args, "with_menubar", False):
        return True
    if os.environ.get("OMJ_MENUBAR", "1") == "0":
        return False
    if _wants_json(args) or getattr(args, "dry_run", False):
        return False
...
def _language_was_explicit(args: argparse.Namespace) -> bool:
    return bool(getattr(args, "language", None) or os.environ.get("OMJ_LANG") or os.environ.get("OMJ_LANGUAGE"))


def _setup_paths_were_explicit(args: argparse.Namespace) -> bool:
    return bool(getattr(args, "omj_home", None) or getattr(args, "hermes_home", None))
...
def _keyboard_menu_available() -> bool:
    return (
        ...
        and os.environ.get("OMJ_NO_TUI", "") != "1"
    )
...
class _HumanProgress:
    ...
    def _brief_tty_pause() -> None:
        if sys.stdout.isatty() and os.environ.get("OMJ_PROGRESS", "1") != "0":
            time.sleep(0.04)
...
def _print_setup_summary(payload: dict[str, object], *, language: str = "en") -> None:
    ...
    if isinstance(command_path, dict):
        if command_path.get("found"):
            print(f"  {tr(language, 'command_path_found', path=command_path.get('path', 'omj'))}")
        else:
...
def _print_apply_summary(payload: dict[str, object]) -> None:
    ...
    title = "OMJ apply preview complete." if dry_run else "OMJ apply complete."
    print(_color(title, "1;36", use_color))
...
def _print_list_summary(payload: dict[str, object], *, manifest_path: Path, skills_dir: Path) -> None:
    ...
    if not isinstance(skills, list):
        skills = []
    print(_color("OMJ managed skills", "1;36", use_color))
    print(_color("Summary", "1;32", use_color))
    if not skills:
        ...
        print(_color("Next", "1;32", use_color))
        print("  Run `omj setup` to install managed Hermes skills.")
        print(f"  {tr('en', 'machine_readable')}")
        return
    package = str(payload.get("package", "oh-my-jeo"))
    ...
    print(_color("Next", "1;32", use_color))
    print("  Run `omj doctor` to verify Hermes registration.")
    if skills:
        print("  In chat, ask Hermes what OMJ can do or type `./omj` to open the workflow picker.")
    print(f"  {tr('en', 'machine_readable')}")
...
def _print_recommend_summary(payload: dict[str, object]) -> None:
    ...
    if not isinstance(recommendations, list):
        recommendations = []
    print(_color("OMJ recommendation", "1;36", use_color))
...
def _print_profile_list_summary(payload: dict[str, object]) -> None:
    ...
    if not isinstance(models, list):
        models = []
    print(_color("OMJ profile packs", "1;36", use_color))
    print(_color("Summary", "1;32", use_color))
    ...
    for pack in packs:
        ...
        if summary:
            print(f"    {summary}")
    print(_color("Next", "1;32", use_color))
    print("  Inspect a model or pack with `omj profile inspect <id>`.")
    print("  Install one with `omj setup --profile-pack <id>`.")
    print(f"  {tr('en', 'machine_readable')}")
...
def _print_profile_inspect_summary(payload: dict[str, object]) -> None:
    ...
    if "model" in payload:
        ...
        if not isinstance(model, dict):
            model = {}
        model_id = str(model.get("id", "unknown"))
        print(_color(f"OMJ operating model: {model.get('title', model_id)}", "1;36", use_color))
        print(_color("Summary", "1;32", use_color))
        print(f"  ID: {model_id}")
        ...
        if isinstance(guidance, list) and guidance:
            ...
            for item in guidance:
                print(f"  - {item}")
        print(_color("Next", "1;32", use_color))
        print(f"  {model.get('setup_command', f'omj setup --operating-model {model_id}')}")
    ...
    if not isinstance(roles, list):
        roles = []
    pack_id = str(pack.get("id", "unknown"))
    print(_color(f"OMJ profile pack: {pack.get('title', pack_id)}", "1;36", use_color))
    print(_color("Summary", "1;32", use_color))
    print(f"  ID: {pack_id}")
...
def _print_probe_summary(payload: dict[str, object]) -> None:
    ...
    print(_color("OMJ capability probe", "1;36", use_color))
    print(_color("Summary", "1;32", use_color))
    print(f"  OMJ home: {payload.get('omj_home', '')}")
...
def _plugin_setup_result(args: argparse.Namespace, paths) -> dict[str, object]:
    try:
        ...
        raise OmjError(_friendly_plugin_error(paths, str(exc))) from exc
    if not args.dry_run:
...
def _friendly_plugin_error(paths, message: str) -> str:
    if "exists without an OMJ plugin manifest" in message:
        return (
            "OMJ status helper location already exists, but it does not look like an OMJ-managed install: "
            f"{paths.hermes_plugin_dir}. Run `omj setup --force` to replace only the OMJ status helper files."
        )
    if "managed plugin files changed" in message:
        return (
            "OMJ status helper files were changed outside OMJ. Run `omj setup --force` to refresh the helper, "
...
def _mcp_setup_result(args: argparse.Namespace, paths) -> dict[str, object]:
    ...
    return {
        ...
        "bridge": {
            "manifest_command": "omj mcp manifest",
            "host_config_recipes_command": "omj mcp config-recipe --host <host>",
            "known_recipe_hosts": ["generic", "claude-code", "codex", "opencode", "cursor"],
            "server_command": "omj mcp serve",
            "host_observation_command": (
                "omj mcp observe-host --host <host> --session <session-id> "
        ...
        "claim_boundary": (
            ...
            "loaded OMJ, called a tool, or observed runtime evidence."
        ),
        "next_action": (
            "Use Hermes skills as the normal surface. If the host supports MCP, export `omj mcp manifest` or "
            "`omj mcp config-recipe --host <host>` and wire the stdio `omj mcp serve` bridge. Treat host load as unobserved until a Hermes/MCP host "
            "records a concrete load or tool-call event with `omj mcp observe-host`."
        ),
    }
...
def cmd_recommend(args: argparse.Namespace) -> int:
    if args.limit < 1:
        raise OmjError("recommend --limit must be at least 1")
    query = " ".join(args.task).strip()
    if not query:
        raise OmjError("recommend requires a task description")
    ...
    selected_skill = str(full_recommendations[0].get("skill", "oh-my-jeo")) if full_recommendations else "oh-my-jeo"
...
def cmd_snippet(args: argparse.Namespace) -> int:
    ...
    if _wants_json(args):
        ...
        print(f"OMJ workspace snippet written: {output}")
        print(f"  {tr('en', 'machine_readable')}")
    return 0
...
def _add_top_level_commands(sub) -> None:
    setup = sub.add_parser("setup", help="Connect OMJ workflows to the target Hermes profile.")
    _add_common_install_options(setup)
    setup.add_argument(
        "--scope",
        choices=("user", "project"),
        default=argparse.SUPPRESS,
        help="Install to user-wide ~/.omj/~/.hermes or project-local ./.omj/./.hermes paths.",
    )
    ...
    setup.add_argument(
        ...
        default=None,
        help="Configure OMJ project memory: off, review-first, or auto-safe. Defaults to review-first.",
    )
    setup.add_argument(
        "--with-plugin",
        action="store_true",
    ...
    setup.add_argument(
        "--with-menubar",
        action="store_true",
        help="Install and start the native macOS OMJ menu bar helper when supported.",
    )
    setup.add_argument(
        "--no-menubar",
        action="store_true",
        help="Do not start the native macOS OMJ menu bar helper during setup.",
    )
    setup.add_argument(
        "--with-mcp",
        action="store_true",
        help="Record an optional OMJ MCP bridge preference without claiming MCP host runtime load.",
    )
    setup.add_argument(
        "--profile-pack",
        action="append",
    ...
    update = sub.add_parser("update", help="Refresh OMJ from a preview, stable, local, or explicit package source.")
    ...
    convert = sub.add_parser("convert", help="Import a local skills directory into the managed OMJ skill pack.")
    ...
    apply = sub.add_parser("apply", help="Register the managed OMJ skills directory in Hermes config.")
    apply.add_argument("--dry-run", action="store_true")
    apply.add_argument("--json", action="store_true", help="Print the machine-readable apply payload.")
    apply.set_defaults(func=cmd_apply)

    uninstall = sub.add_parser("uninstall", help="Remove OMJ-managed registration, local files, and optional command package.")
    uninstall.add_argument("--registration-only", action="store_true", help="Only remove the OMJ skills.external_dirs registration from Hermes config.")
    uninstall.add_argument("--remove-files", action="store_true", help="Legacy mode: remove Hermes registration and the managed OMJ home directory.")
    uninstall.add_argument("--all", action="store_true", help="Remove all OMJ-managed local state, plugin bundle, and generated team role files.")
    uninstall.add_argument("--purge", action="store_true", help="Alias for --all.")
    uninstall.add_argument("--keep-command", action="store_true", help="Keep the install.sh-managed omj command venv/link during full cleanup.")
    uninstall.add_argument("--force", action="store_true", help="Also remove an unmanaged ~/.hermes/plugins/omj directory when using --all.")
    ...
    doctor = sub.add_parser("doctor", help="Check local OMJ install health and Hermes skill registration.")
    ...
    recommend = sub.add_parser("recommend", help="Map a task description to likely OMJ workflow skills.")
    recommend.add_argument("task", nargs="+", help="Task description to map to OMJ workflow skills.")
    ...
    probe = sub.add_parser("probe", help="Inspect observable OMJ/Hermes capability surfaces.")
    probe.add_argument(
        "--parity",
        action="store_true",
        help="Include the OMJ parity matrix for common oh-my agent runtime capability axes.",
    )
    probe.add_argument(
        "--roadmap",
        action="store_true",
