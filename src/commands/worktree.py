...
def cmd_worktree_list(args: argparse.Namespace) -> int:
    ...
    _print_json(
        {
            "schema_version": "omj_worktree_observations/v1",
            "records": records,
            "errors": errors,
            "claim_boundary": (
                "OMJ worktree records are workspace-isolation evidence only. They do not prove executor "
...
def _add_worktree_commands(sub) -> None:
    ...
    prepare.add_argument("--branch", default="", help="Branch name for the new worktree. Defaults to omj/<task>-<timestamp>.")
    ...
    listing = worktree_sub.add_parser("list", help="List recent OMJ worktree preparation records.")
