...
def cmd_mcp_sessions(args: argparse.Namespace) -> int:
    ...
    if limit < 1:
        raise OmjError("--limit must be at least 1")
    sessions, errors = read_mcp_host_sessions(_paths(args), limit=limit)
    _print_json(
        {
            "schema_version": "omj_mcp_host_sessions/v1",
            "sessions": sessions,
            "errors": errors,
            "claim_boundary": (
...
def _add_mcp_commands(sub) -> None:
    mcp = sub.add_parser(
        "mcp",
        help="Expose the optional OMJ MCP bridge manifest or stdio server for capable hosts.",
    )
    mcp_sub = mcp.add_subparsers(dest="mcp_command", required=True)

    manifest = mcp_sub.add_parser(
        "manifest",
        help="Print the OMJ MCP bridge manifest/config snippet for a host that supports stdio MCP servers.",
    )
    manifest.add_argument(
        "--command",
        default="omj",
        help="Command path the MCP host should launch. Use an absolute installed omj path when needed.",
    )
    manifest.add_argument(
        "--portable",
        action="store_true",
        help="Omit absolute --omj-home/--hermes-home args so the host inherits its environment.",
    )
