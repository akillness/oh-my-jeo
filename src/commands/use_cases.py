...
def cmd_cases_inspect(args: argparse.Namespace) -> int:
    try:
        ...
        raise OmjError(f"unknown use case: {args.id}") from exc
    if _wants_json(args):
        _print_json(payload)
    else:
...
def cmd_cases_demo(args: argparse.Namespace) -> int:
    try:
        if args.all:
            if args.id:
                raise OmjError("use either `omj cases demo <id>` or `omj cases demo --all`, not both")
            payload = demo_all_use_cases()
        else:
            if not args.id:
                raise OmjError("use-case id required unless --all is passed")
...
def cmd_cases_artifact(args: argparse.Namespace) -> int:
    try:
        if args.all:
            if args.id:
                raise OmjError("use either `omj cases artifact <id>` or `omj cases artifact --all`, not both")
            payload = (
...
def _print_cases_list_summary(payload: dict[str, object]) -> None:
    ...
    print("Boundary")
    print("  These are implemented OMJ feature surfaces, not runtime execution evidence.")
    print("Next")
    print("  Inspect one with `omj cases inspect G10`.")
    print("  Recommend one with `omj cases recommend <task>`.")
    print("  For machine-readable output, rerun with `--json`.")


def _print_case_summary(case: dict[str, object]) -> None:
    print(f"OMJ use case: {case.get('goal')} - {case.get('title')}")
    print("Situation")
...
def _print_case_demo_summary(card: dict[str, object]) -> None:
    ...
    if not isinstance(actions, list):
        actions = []
    print(f"OMJ use-case demo card: {card.get('goal')} - {card.get('title')}")
    print("Route")
    print(
...
def _print_cases_demo_collection_summary(payload: dict[str, object]) -> None:
    ...
    if not isinstance(cards, list):
        cards = []
    print("OMJ G1-G10 use-case demo cards")
    print("Summary")
    ...
    print("Next")
    print("  Inspect one with `omj cases demo G10`.")
    print("  Export all cards with `omj cases demo --all --json`.")
    print("  For machine-readable output, rerun with `--json`.")


def _print_cases_artifact_summary(payload: dict[str, object]) -> None:
    schema = payload.get("schema_version")
    if schema == "omj_use_case_artifact_write/v1":
        mode = str(payload.get("mode", ""))
        print("OMJ use-case artifact write")
        print("Summary")
        if mode == "write_all":
        ...
        print("  For machine-readable output, rerun with `--json`.")
        return
    if schema == "omj_use_case_artifact_collection/v1":
        artifacts = payload.get("artifacts", [])
        if not isinstance(artifacts, list):
            artifacts = []
        print("OMJ G1-G10 use-case artifacts")
        print("Summary")
        ...
        print(f"  {payload.get('boundary')}")
        print("Next")
        print("  Write them with `omj cases artifact --all --write`.")
        print("  For machine-readable output, rerun with `--json`.")
        return
    ...
    if not isinstance(evidence, dict):
        evidence = {}
    print(f"OMJ use-case artifact: {payload.get('goal')} - {payload.get('title')}")
    print("Route")
    ...
    print(f"  {evidence.get('claim_boundary')}")
    print("Next")
    print(f"  Write it with `omj cases artifact {payload.get('goal')} --write`.")
    print("  For machine-readable output, rerun with `--json`.")


def _print_cases_artifact_validate_summary(payload: dict[str, object]) -> None:
    print("OMJ use-case artifact validation")
    print("Summary")
...
def _print_cases_readiness_summary(payload: dict[str, object]) -> None:
    print("OMJ G1-G10 use-case readiness")
    print("Summary")
...
def _print_cases_replay_summary(payload: dict[str, object]) -> None:
    print("OMJ G1-G10 use-case replay")
    print("Summary")
    print(f"  Status: {payload.get('status')}")
...
def _print_cases_recommend_summary(payload: dict[str, object]) -> None:
    ...
    if not isinstance(recommendations, list):
        recommendations = []
    print("OMJ use-case recommendation")
...
def _print_cases_validate_summary(payload: dict[str, object]) -> None:
    ...
    if not isinstance(validated, list):
        validated = []
    print("OMJ G1-G10 feature surface validation")
    print("Summary")
    print(f"  OK: {payload.get('ok')}")
...
def _add_cases_commands(sub) -> None:
    ...
    recommend_cmd.add_argument("task", nargs="+", help="Natural-language request to map to a representative OMJ use case.")
    ...
    artifact_cmd.add_argument("--write", action="store_true", help="Write prepared use-case artifacts under the OMJ home.")
