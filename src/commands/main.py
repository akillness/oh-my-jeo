...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="omj",
        description=(
            "Install OMJ once, then use Hermes chat.\n"
        ...
        epilog=(
            ...
            "If your shell says `omj` was not found after install, use the absolute\n"
            "command path printed by the installer or add that directory to PATH.\n\n"
            "Operator examples:\n"
            "  omj recommend \"risky refactor\"\n"
            "  omj cases recommend \"daily competitor digest\"\n"
            "  omj cases readiness\n"
            "  omj cases demo --all\n"
            ...
            "  omj release evidence-bundle --version 1.0.1 --write\n"
            "  omj plugin observe-host --host hermes-agent --session <session-id> --event plugin_load --evidence-ref <host-log>\n"
            "  omj loop status\n"
            "  omj ops list\n"
            "  omj materials list\n"
            "  omj img-summary prompt-card --kind github_pr --visual-format auto --section summary:What_changed:Safer_setup_copy\n"
            "  omj img-summary prompt-card --kind report --aspect-ratio long_scroll --section summary:Executive_summary:Weekly_metrics_changed\n"
            "  omj worktree prepare --repo . --task \"risky refactor\" --dry-run\n"
            "  omj worktree bind --path .worktrees/risky-refactor --executor codex --session <session-id>\n"
            "  omj runtime status\n\n"
            "  omj runtime team-readiness\n\n"
            "Human-facing maintenance, catalog, and operator checklist commands print summaries by default;\n"
            "pass --json or set OMJ_OUTPUT=json when a wrapper needs full payloads.\n"
    ...
    parser.add_argument("--omj-home", default=None, help="Override the managed OMJ home directory (default: ~/.omj).")
    parser.add_argument("--hermes-home", default=None, help="Override the target Hermes home directory (default: ~/.hermes).")
    parser.add_argument(
        "--scope",
        choices=("user", "project"),
        default=None,
        help="Choose default OMJ/Hermes paths when explicit homes are not supplied.",
    )
...
Useful operator commands:
  omj quickstart         Show first-use prompts and evidence boundaries
  omj recommend "risky refactor"
  omj cases recommend "daily competitor digest"
  omj cases readiness   Check the G1-G10 readiness rollup
  omj cases demo --all  Show wrapper-ready G1-G10 use-case cards
  omj cases artifact --all --write
                          Write local G1-G10 runbook artifacts
  omj cases replay        Replay G1-G10 routing fixtures
  omj context brief "make an image card for this PR"
  ...
  omj mcp manifest       Print the optional stdio MCP bridge manifest
  omj release evidence-bundle --version 1.0.1 --write
                          Write local deterministic release evidence
  omj plugin observe-host --host hermes-agent --session <session-id> --event plugin_load --evidence-ref <host-log>
  omj loop status        Show loopable goal cycle state
  omj ops list           List local operations artifacts
  omj materials list     List material-processing artifacts
  omj img-summary prompt-card Prepare image-generation-ready summary cards
  omj worktree prepare --repo . --task "risky refactor" --dry-run
  omj worktree bind --path .worktrees/risky-refactor --executor codex --session <session-id>
  omj runtime status     Show local evidence artifacts

After setup, restart or reload Hermes Agent and try:
  Use OMJ request-to-handoff for: I want to safely add a feature to this repo.

If `omj` is not found in a new terminal, use the absolute command path printed
by the installer or add that directory to PATH. Run `omj doctor` after that to
verify Hermes registration.

Run `omj --help` for the full command list."""
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"omj: {exc}", file=sys.stderr)
        return 2
