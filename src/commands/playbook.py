...
def cmd_playbook_inspect(args: argparse.Namespace) -> int:
    try:
        ...
        raise OmjError(f"unknown playbook: {args.id}") from exc
    if _wants_json(args):
        _print_json(payload)
    else:
...
def _print_playbook_list_summary(payload: dict[str, object]) -> None:
    ...
    for playbook in playbooks:
        ...
        if summary:
            print(f"    {summary}")
    print("Next")
    print("  Inspect a playbook with `omj playbook inspect <id>`.")
    print("  Recommend one with `omj playbook recommend <task>`.")
...
def _print_playbook_inspect_summary(payload: dict[str, object]) -> None:
    ...
    if not isinstance(stages, list):
        stages = []
    playbook_id = str(playbook.get("id", "unknown"))
    print(f"OMJ playbook: {playbook.get('title', playbook_id)}")
    print("Summary")
...
def _print_playbook_recommend_summary(payload: dict[str, object]) -> None:
    ...
    if not isinstance(recommendations, list):
        recommendations = []
    print("OMJ playbook recommendation")
...
def _add_playbook_commands(sub) -> None:
    ...
    playbook_recommend.add_argument("task", nargs="+", help="Natural-language request to map to an OMJ playbook.")
