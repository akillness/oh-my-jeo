...
def _validate_runtime_names(skill: str, harness: str) -> None:
    if skill not in _valid_skill_names():
        raise OmjError(f"unknown skill for runtime record: {skill}")
    if harness not in _valid_harness_names():
        raise OmjError(f"unknown harness for runtime record: {harness}")
...
def cmd_runtime_delegate(args: argparse.Namespace) -> int:
    ...
    if not (run_dir / "run.json").exists():
        raise OmjError(f"runtime run not found: {args.run_id}")
...
def cmd_runtime_review(args: argparse.Namespace) -> int:
    ...
    if args.status in {"passed", "not_required"} and preflight.get("next_action") != "record_review_evidence":
        raise OmjError(f"cannot record review {args.status} while next_action is {preflight.get('next_action')}")
    ...
    if args.status == "not_required" and isinstance(review_status, dict) and review_status.get("required"):
        raise OmjError("cannot mark required review as not_required")
    try:
...
def cmd_runtime_ci(args: argparse.Namespace) -> int:
    ...
    if args.status == "passed" and preflight.get("next_action") != "record_ci_evidence":
        raise OmjError(f"cannot record passed CI while next_action is {preflight.get('next_action')}")
    ci_status = preflight.get("ci", {})
    if args.status == "not_required" and isinstance(ci_status, dict) and ci_status.get("required"):
        raise OmjError("cannot mark required CI as not_required")
    try:
        ci = write_ci_record(
            run_dir,
            {
...
def cmd_runtime_merge(args: argparse.Namespace) -> int:
    ...
    if len(selected_statuses) > 1:
        raise OmjError("runtime merge accepts only one of --ready, --merged, --blocked, or --status")
    status = selected_statuses[0] if selected_statuses else None
    if not status:
        raise OmjError("runtime merge requires --ready, --merged, --blocked, or --status")
    preflight = summarize_delegated_coding_status(paths, args.run_id)
    allowed_preflight = {
    ...
    if status in allowed_preflight and preflight.get("next_action") not in allowed_preflight[status]:
        raise OmjError(f"cannot record merge {status} while next_action is {preflight.get('next_action')}")
    try:
...
def cmd_runtime_observe(args: argparse.Namespace) -> int:
    ...
    if not target_id:
        raise OmjError("runtime observe requires --run or --session")
    target_dir = (
    ...
    if not required_file.exists():
        raise OmjError(f"runtime {target_type} not found: {target_id}")
...
def cmd_runtime_progress_observe(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        raise OmjError(f"executor progress binding could not be read: {exc}") from exc
    if not binding:
        raise OmjError(f"executor progress binding not found for {target_type}: {target_id}")
    try:
...
def _validate_runtime_observation_target(target_dir, target_type: str, runtime_profile: str) -> bool:
    ...
    if not expected_profile:
        if target_type == "wrapper_session":
            raise OmjError("runtime observe requires a runtime_handoff_prepared wrapper session")
        ...
        if (
            ...
            return False
        raise OmjError("runtime observe cannot record runtime events for this non-runtime handoff run")
    if runtime_profile != expected_profile:
        raise OmjError(f"runtime observation profile mismatch: expected {expected_profile}, got {runtime_profile}")
    return True
...
def _progress_target(args: argparse.Namespace) -> tuple[str, str]:
    ...
    raise OmjError("progress command requires --run or --session")
...
def _require_progress_target(paths, target_type: str, target_id: str) -> None:
    if target_type == "run":
        if not (paths.runtime_runs_dir / target_id / "run.json").exists():
            raise OmjError(f"runtime run not found: {target_id}")
        return
    if target_type == "wrapper_session":
        if not (paths.runtime_wrapper_sessions_dir / target_id / "session.json").exists():
            raise OmjError(f"runtime wrapper_session not found: {target_id}")
        return
    raise OmjError(f"unsupported progress target type: {target_type}")
...
def cmd_runtime_team_readiness(args: argparse.Namespace) -> int:
    ...
    if target_limit is not None and target_limit < 1:
        raise OmjError("--limit must be at least 1 unless --all is set")
