...
def cmd_capabilities_summary(args: argparse.Namespace) -> int:
    ...
    if _wants_json(args):
        _print_json(payload)
        return 0
    print("OMJ capability summary")
    print("Use this when Hermes needs to explain what OMJ can do without shell catalog approval.")
...
def cmd_capabilities_inspect(args: argparse.Namespace) -> int:
    ...
    print(f"OMJ capability: {payload['id']}")
...
def _add_capabilities_commands(sub) -> None:
    capabilities = sub.add_parser("capabilities", help="Inspect OMJ capability manifests for Hermes/plugin/wrapper use.")
    capabilities_sub = capabilities.add_subparsers(dest="capabilities_command", required=True)

    export = capabilities_sub.add_parser("export", help="Export the deterministic OMJ capability manifest.")
    ...
    summary = capabilities_sub.add_parser("summary", help="Summarize OMJ lanes, representative skills, and playbooks.")
