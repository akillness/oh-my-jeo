...
def cmd_plugin_observations(args: argparse.Namespace) -> int:
    ...
    _print_json(
        {
            "schema_version": "omj_plugin_host_observations/v1",
...
def _add_plugin_commands(sub) -> None:
    plugin = sub.add_parser(
        "plugin",
        help="Record host-observed OMJ plugin load/use evidence without claiming execution.",
    )
    plugin_sub = plugin.add_subparsers(dest="plugin_command", required=True)

    observe_host = plugin_sub.add_parser(
        "observe-host",
        help="Record Hermes host or wrapper evidence that the OMJ plugin was loaded or used.",
    )
