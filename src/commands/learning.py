...
def cmd_learning_missed_route(args: argparse.Namespace) -> int:
    try:
        ...
        payload = record_missed_route(
            ...
            title=args.title or "Review missed OMJ workflow route",
...
def cmd_learning_show(args: argparse.Namespace) -> int:
    try:
        ...
        raise OmjError(f"learning trace not found: {args.trace_id}") from exc
    except (OSError, WorkflowLearningError) as exc:
        raise OmjError(str(exc)) from exc
    _print_json(trace)
    return 0
...
def cmd_learning_proposal(args: argparse.Namespace) -> int:
    try:
        ...
        raise OmjError(f"improvement candidate not found: {args.candidate_id}") from exc
    except (OSError, WorkflowLearningError) as exc:
        raise OmjError(str(exc)) from exc
    _print_json(
        {
            ...
            "proposal": proposal,
            "claim_boundary": (
                "The patch proposal is handoff material only. OMJ did not edit source files, "
    ...
    _print_json(
        {
            ...
            "decision": args.decision,
            "candidate": candidate,
            "next_action": f"omj learning proposal {args.candidate_id}" if args.decision == "approve" else "review_improvement",
            "claim_boundary": (
                "The candidate review records a human gate decision only. OMJ did not apply source edits, "
    ...
    _print_json(
        {
            ...
            "claim_boundary": (
                ...
                "OMJ cannot prove redaction and does not treat the fixture as observed workflow execution."
            ),
        }
    )
    return 0
...
def _add_learning_commands(sub) -> None:
    ...
    missed_route = learning_sub.add_parser(
        "missed-route",
        help="Capture a reported missed OMJ workflow route as trace, eval, regression, and review material.",
    )
    ...
    review_candidate.add_argument(
        "--review-note",
        default="",
        help="Optional operator note; OMJ stores only its hash and length, not the note text.",
    )
