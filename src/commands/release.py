...
def cmd_release_checklist(args: argparse.Namespace) -> int:
    try:
        payload = release_readiness_checklist(version=args.version or __version__, omj_command=args.omj_command)
...
def cmd_release_hermes_smoke(args: argparse.Namespace) -> int:
    if args.timeout < 1:
        raise OmjError("release hermes-smoke --timeout must be at least 1")
    ...
    if args.live:
        payload = run_hermes_release_smoke(
            ...
            skill=args.skill,
            tap=args.tap,
            omj_command=args.omj_command,
            omj_home=args.omj_home,
...
def cmd_release_product_readiness(args: argparse.Namespace) -> int:
    try:
        payload = product_readiness_report(version=args.version or __version__, omj_command=args.omj_command)
    except ValueError as exc:
        raise OmjError(str(exc)) from exc
    if _wants_json(args):
        _print_json(payload)
...
def _add_release_commands(sub) -> None:
    ...
    checklist.add_argument("--omj-command", default="omj", help="Installed OMJ executable name or path to show in smoke commands.")
    ...
    smoke.add_argument("--omj-command", default="omj", help="OMJ executable to use for the setup install path.")
    smoke.add_argument(
        "--include-command-smoke",
        action="store_true",
        help="Also execute installed `omj --help` and installed setup-path plan rendering without mutating Hermes.",
    )
    ...
    install_smoke.add_argument("--repo-root", default=None, help="OMJ source checkout to install from; defaults to the current checkout.")
    ...
    install_smoke.add_argument("--python", default="", help="Python executable passed as OMJ_PYTHON; defaults to this Python.")
    install_smoke.add_argument(
        "--run-setup",
        action="store_true",
        help="Set OMJ_RUN_SETUP=1 for an advanced one-shot installer/setup compatibility smoke.",
    )
    install_smoke.add_argument(
        "--setup-args",
        nargs="*",
        default=None,
        help="Extra OMJ_SETUP_ARGS passed only when --run-setup is used.",
    )
    install_smoke.add_argument(
        "--skip-doctor",
        action="store_true",
        help="Set OMJ_RUN_DOCTOR=0 when --run-setup is used.",
    )
    ...
    skill_content = release_sub.add_parser(
        "skill-content-smoke",
        help="Verify generated OMJ skill guidance in the current command package.",
    )
    ...
    product_readiness = release_sub.add_parser(
        "product-readiness",
        help="Summarize deterministic OMJ product readiness gates.",
    )
    product_readiness.add_argument("--version", default="", help="Release version to summarize, such as 1.0.1 or v1.0.1.")
    product_readiness.add_argument("--omj-command", default="omj", help="Installed OMJ executable name/path shown in release gate commands.")
    ...
    evidence_bundle.add_argument("--omj-command", default="omj", help="Installed OMJ executable name/path shown in release gate commands.")
    evidence_bundle.add_argument("--write", action="store_true", help="Write the bundle under .omj/runtime/release-evidence/.")
...
def _print_release_checklist_summary(payload: dict[str, object]) -> None:
    print(f"OMJ release checklist for {payload['version']} ({payload['tag']})")
...
def _print_product_readiness_summary(payload: dict[str, object]) -> None:
    print(f"OMJ product readiness for {payload.get('version')}")
    print("Summary")
...
def _print_release_evidence_bundle_summary(payload: dict[str, object]) -> None:
    ...
    print(f"OMJ release evidence bundle for {payload.get('version')}")
    print("Summary")
    print(f"  Status: {payload.get('status')}")
