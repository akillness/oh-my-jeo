...
def cmd_materials_plan(args: argparse.Namespace) -> int:
    ...
    _print_json(
        {
            "schema_version": "omj_materials_plan_result/v1",
            "artifact": written,
            "store": {
...
def cmd_materials_list(args: argparse.Namespace) -> int:
    ...
    _print_json(
        {
            "schema_version": "omj_materials_list/v1",
...
def cmd_materials_show(args: argparse.Namespace) -> int:
    try:
        ...
        raise OmjError(str(exc)) from exc
    _print_json({"schema_version": "omj_materials_show/v1", "artifact": artifact})
    return 0
...
def cmd_materials_export(args: argparse.Namespace) -> int:
    ...
    _print_json(
        {
            "schema_version": "omj_materials_export_result/v1",
            "format": args.format,
            "kind": args.kind or "all",
            "limit": limit if limit is not None else "all",
