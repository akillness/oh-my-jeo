...
TEAM_PROFILE_SCHEMA_VERSION = "omj_team_profile_pack/v1"
...
class TeamProfilePack:
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "install_command": f"omj setup --profile-pack {self.id}",
...
class OperatingModel:
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "setup_command": f"omj setup --operating-model {self.id}",
            "claim_boundary": (
                "An operating model records OMJ routing and narration defaults only. It does not install role files "
...
def install_team_profile_pack(paths: OmjPaths, pack_id: str, *, force: bool = False, dry_run: bool = False) -> dict[str, object]:
    pack = _pack(pack_id)
...
def _pack_files(paths: OmjPaths, pack: TeamProfilePack) -> list[tuple[Path, str]]:
    return [
        (paths.hermes_agents_dir / f"omj-{pack.id}-{role.id}.md", render_team_role_markdown(pack, role))
