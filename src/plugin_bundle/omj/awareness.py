...
_OMJ_DIAGNOSTIC_EVALUATION_CUES = (
    "usage evaluation",
...
try:  # Keep installed plugin bundles usable even when the full package is absent.
    from ...routing.intent import META_OR_FEEDBACK_INTENTS, classify_omj_quality_intent, classify_workflow_intent
    ...
    @dataclass(frozen=True)
    class _FallbackOmjQualityIntent:
        applies: bool
        ...
        matched_label: str = "semantic:omj_quality_improvement_loop"
    ...
    def _fallback_matched_omj_quality_cues(cues: tuple[str, ...], text: str, compact: str) -> tuple[str, ...]:
        matches = []
        for cue in cues:
    ...
    def _fallback_diagnostic_omj_evaluation_context(text: str, compact: str) -> bool:
        ...
        has_user_omj_subject = bool(re.search(r"(?<![a-z0-9])omj(?![a-z0-9])", user_region))
        has_user_router_subject = bool(_matched_text_cues(("router", "routing", "route hint", "라우터", "라우팅"), user_region, user_compact))
        if not (has_user_omj_subject or has_user_router_subject):
            return False
        return bool(
            _fallback_matched_omj_quality_cues(
                _OMJ_DIAGNOSTIC_EVALUATION_CUES,
                user_region,
                user_compact,
            )
    ...
    def classify_omj_quality_intent(message: str) -> _FallbackOmjQualityIntent:
        ...
        improvement_cues = _fallback_matched_omj_quality_cues(
            (
                "improve",
                "improvement",
                "fix",
        ...
        quality_cues = _fallback_matched_omj_quality_cues(
            (
                "bug",
                "bugs",
                "failure",
        ...
        loop_cues = _fallback_matched_omj_quality_cues(
            ("loop", "continuous", "keep", "keeps", "keeping", "루프", "계속", "반복"),
            text,
            compact,
        )
        handoff_cues = _fallback_matched_omj_quality_cues(("coding handoff", "handoff", "코딩"), text, compact)
        customer_feedback_cues = _fallback_matched_omj_quality_cues(
        ...
        if customer_feedback_cues and not system_target_cues:
            applies = False
        return _FallbackOmjQualityIntent(
            applies=applies,
    ...
    def classify_workflow_intent(message: str) -> _FallbackWorkflowIntent:
        text = unicodedata.normalize("NFKC", message).casefold()
        compact = text.replace(" ", "")
        diagnostic_evaluation = _fallback_diagnostic_omj_evaluation_context(text, compact)
...
OMJ_AWARENESS_SCHEMA_VERSION = "omj_awareness/v1"
OMJ_ROUTE_HINT_SCHEMA_VERSION = "omj_route_hint/v1"
OMJ_GENERIC_TOOL_CHECKPOINT_SCHEMA_VERSION = "omj_generic_tool_checkpoint/v1"
GENERIC_TOOL_CHECKPOINT_TEXT = "Before generic tools, check OMJ prep/status/learning; if relevant, name the workflow first."
...
WORKFLOW_CONTEXT_CARDS = (
    {
        ...
        "omj_pattern": "clarify or plan first, then move to ultragoal, ultraprocess, loop, or handoff only when concrete",
    ...
    },
    {
        ...
        "omj_pattern": "classify source acquisition when needed, collect evidence, separate source notes from synthesis, then create a brief, decision, or status artifact",
    ...
    },
    {
        ...
        "omj_pattern": "shape the deliverable contract, prepare prompts or package metadata, then record generation and QA only when observed",
    ...
    },
    {
        ...
        "omj_pattern": "prepare a schedule/status/repair/learning card, name required tools or evidence, then keep observed runtime state separate",
    ...
    },
    {
        ...
        "omj_pattern": "choose the coding owner, prepare executor-neutral handoff or Hermes coding team path, then track dispatch and result evidence",
...
_ROUTE_HINT_RULES = (
    {
        ...
        "reason": "The user is reporting that OMJ or the expected workflow was not used.",
...
def awareness_lane_examples(lane_id: str) -> list[str]:
    """Return compact examples relevant to one OMJ awareness lane."""
    return list(LANE_CROSS_LANE_EXAMPLES.get(lane_id, []))


def workflow_context_cards() -> list[dict[str, object]]:
    """Return compact workflow cards that teach Hermes when OMJ should help."""
    return [
        {
            "id": card["id"],
            "label": card["label"],
            "user_signal": card["user_signal"],
            "omj_pattern": card["omj_pattern"],
...
def workflow_context_card_for_workflow(workflow: str) -> dict[str, object]:
    """Return the OMJ pattern card that should frame one selected workflow."""
...
def awareness_context_matches_message(message: str) -> bool:
    """Return true when a non-first-turn message should refresh OMJ context."""
    text = unicodedata.normalize("NFKC", message).casefold()
    if not text.strip():
        return False
...
def awareness_route_hint(message: str, *, max_hints: int = 2) -> dict[str, object]:
    ...
    omj_quality_intent = classify_omj_quality_intent(message)
    diagnostic_learning_first = diagnostic_eval
    hints: list[dict[str, object]] = []
    if normalized.strip() and hint_limit:
        if omj_quality_intent.applies and not diagnostic_learning_first:
            hints.append(_omj_quality_improvement_hint(omj_quality_intent))
        if len(hints) < hint_limit and (
            diagnostic_learning_first
            or (_prefers_workflow_learning_hint(intent) and not omj_quality_intent.applies)
        ):
            hints.append(_workflow_vocabulary_reference_hint(intent))
        for rule in _ROUTE_HINT_RULES:
            if len(hints) >= hint_limit:
                break
            if _rule_suppressed_by_omj_quality_intent(rule, omj_quality_intent):
                continue
    ...
    return {
        "schema_version": OMJ_ROUTE_HINT_SCHEMA_VERSION,
        ...
        "claim_boundary": (
            "OMJ route hints are local deterministic prompt context only. They are not workflow execution, "
...
def awareness_route_hint_context(message: str, *, max_hints: int = 2) -> str:
    ...
    lines = [
        "[OMJ Route Hint]",
        "Use this message-specific OMJ hint before generic chat/tools when it fits the user intent.",
        (
...
def awareness_primer_payload() -> dict[str, object]:
    """Return the compact OMJ mental model shared by hooks, tools, and skills."""
    lanes = [
        {
            "id": "intent_to_plan",
            "label": "Intent -> plan",
    ...
    return {
        "schema_version": OMJ_AWARENESS_SCHEMA_VERSION,
        "id": "omj_awareness",
        "purpose": "Give Hermes a compact first-turn mental model for using OMJ across all workflow-shaped requests.",
        "product_context": (
            "OMJ is a Hermes-native workflow pack: it helps Hermes choose skills, shape work, prepare artifacts, "
        ...
        "first_turn_rule": (
            ...
            "review, status, or long loops, consider OMJ before generic chat or generic tools."
        ),
        "all_skill_context_rule": (
            "Across every OMJ skill: match intent to a lane, name adjacent workflows, "
            "and do not dismiss OMJ because a generic tool can render or execute."
        ),
        "generic_tool_checkpoint": GENERIC_TOOL_CHECKPOINT_TEXT,
        "skill_coverage": "Every generated workflow skill carries this rail.",
        "chat_rule": "Normal users talk to Hermes; OMJ CLI is backend, setup, verification, and wrapper infrastructure.",
        "lanes": lanes,
        ...
        "tool_hints": [
            "Use omj_context when Hermes needs the compact OMJ mental model, generic-tool checkpoint, and message-specific route hint in one payload.",
            "Use omj_capabilities action=summary when the user asks what OMJ can do or which workflows are available.",
            "Use omj_recommend when the user gives a natural-language request and Hermes needs the nearest OMJ workflow without shell approval.",
            "Use omj_capabilities for detailed workflow catalog and capability manifest lookup.",
            "Use omj_probe when the user asks whether OMJ is installed, what is missing, or what the next setup/runtime evidence step should be.",
            "Use omj_status or omj_hud for metadata-only runtime state.",
            "Use omj_role for responsibility context when a role marker is present.",
            "Use wrapper cards/actions for user-facing choices instead of asking users to approve shell catalog commands.",
        ],
        "evidence_boundary": (
            "Prepared OMJ routing, prompts, cards, handoffs, or artifacts are not observed execution, image generation, "
        ...
        "source_refs": [
            "src/plugin_bundle/omj/awareness.py",
...
def awareness_generic_tool_checkpoint_payload() -> dict[str, object]:
    return {
        "schema_version": OMJ_GENERIC_TOOL_CHECKPOINT_SCHEMA_VERSION,
        "label": "OMJ-first checkpoint",
        ...
        "operating_rule": (
            "If a route matches, show the preferred OMJ workflow/action first. "
...
def generic_tool_checkpoint_routes() -> list[dict[str, object]]:
    """Return compact OMJ-first route hints for common generic tool families."""
    return [
        {
...
def awareness_primer_context() -> str:
    ...
    return "\n".join(
        [
            "[OMJ Awareness]",
            "OMJ is Hermes-native workflow guidance, not hidden execution or a transport/runtime patch.",
            (
                "For planning, research, files, visuals, automation, coding, review, status, or loops, "
                "consider OMJ before generic tools."
            ),
            ...
            "Expand only when needed with omj_context, omj_capabilities, and omj_status/omj_hud.",
        ]
    )
    ...
    return "\n".join(
        [
            ...
            *lane_lines,
            ...
            "",
            "Tools:",
            "- Tools: `omj_interact`; `omj_context`; `omj_recommend`; `omj_capabilities`; `omj_probe`; `omj_status`/`omj_hud`; `omj_role`.",
            "",
...
def awareness_workflow_context_markdown(skill_name: str) -> str:
    ...
    lane_line = "Use `omj_recommend` or the `oh-my-jeo` router for workflow choice, and `omj_capabilities` for manifest detail."
    if lane:
    ...
    return "\n".join(
        [
            "## OMJ Context Rail",
            "",
            f"- This skill is part of OMJ's Hermes workflow layer, not a standalone executor.",
...
def _prefers_diagnostic_workflow_learning_hint(message: str, intent: object) -> bool:
    ...
    has_user_omj_subject = re.search(r"(?<![a-z0-9])omj(?![a-z0-9])", user_region) is not None
    has_user_router_subject = bool(_matched_text_cues(("router", "routing", "route hint", "라우터", "라우팅"), user_region, compact))
    if not (has_user_omj_subject or has_user_router_subject):
        return False
    return bool(_matched_text_cues(_OMJ_DIAGNOSTIC_EVALUATION_CUES, user_region, compact))
...
def _without_diagnostic_status_lines(text: str) -> str:
    ...
    for line in text.splitlines() or [text]:
        ...
        if not stripped:
            continue
        if stripped.startswith("[omj"):
            stripped = re.sub(r"^\[omj(?:\s+awareness|\s+route\s+hint)?\]\s*", "", stripped).strip()
            if not stripped:
                continue
...
def _omj_quality_improvement_hint(intent: object) -> dict[str, object]:
    workflow = "ultraprocess"
    context_card = workflow_context_card_for_workflow(workflow)
    return {
        "id": "omj_quality_improvement_loop",
        "workflow": workflow,
        "lane": "coding_handoff",
        "next_action": "prepare_one_cycle_delivery",
        "reason": (
            "The whole request is about improving OMJ routing, context, progress, or coding-handoff quality; "
...
def _workflow_vocabulary_reference_hint(intent: object) -> dict[str, object]:
    ...
    return {
        ...
        "reason": "The message discusses OMJ workflow or coding-handoff vocabulary rather than asking to execute it.",
...
def _rule_suppressed_by_omj_quality_intent(rule: dict[str, object], intent: object) -> bool:
