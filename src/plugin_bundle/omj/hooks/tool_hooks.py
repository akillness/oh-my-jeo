...
from ..awareness import OMJ_GENERIC_TOOL_CHECKPOINT_SCHEMA_VERSION, generic_tool_checkpoint_routes
from ..host_observation import observe_plugin_hook_call
from ..omj_roles import extract_role_marker, resolve_role_name, role_aliases, role_names


def pre_tool_call(**kwargs) -> dict[str, object] | None:
    """Inject bounded OMJ context before tool calls without exposing tool input."""
    ...
    if checkpoint:
        ...
        if structured_checkpoint:
            payload["omj_generic_tool_checkpoint"] = structured_checkpoint
...
def _delegate_role_warning(kwargs: dict) -> str:
    ...
    if marker in available or resolve_role_name(marker) in available:
        return ""
    return (
        f"[OMJ Role Warning] Unknown role '{marker}' in delegate_task goal. "
        f"Available roles: {', '.join(available) or '(none)'}. "
        f"Legacy aliases: {', '.join(sorted(aliases)) or '(none)'}. "
        "No OMJ role context will be injected for that subagent."
    )


def _generic_tool_checkpoint_context(kwargs: dict) -> str:
    if kwargs.get("include_omj_tool_checkpoint", True) is False:
        return ""
    ...
    return "\n".join(
        [
            "[OMJ Tool Checkpoint]",
            "schema=omj_generic_tool_checkpoint/v1",
            f"tool_name={_redacted_label(tool_name)}; tool_family={route.get('tool_family')}.",
            f"Before this generic tool ({applies_before}), consider OMJ workflow={route.get('primary_workflow')} first.",
