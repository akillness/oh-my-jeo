...
def on_session_end(**kwargs) -> dict[str, str] | None:
    """Record a metadata-only plugin checkpoint when OMJ runtime state exists."""
    observe_plugin_hook_call("on_session_end", kwargs)
    home = _expand_path(str(kwargs.get("omj_home", "") or "") or os.environ.get("OMJ_HOME", "~/.omj"))
    ...
    if not state and run_count == 0:
        return None
    payload = {
        "schema_version": "omj_plugin_session_end/v1",
        "observed_at": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "runtime_state_present": bool(state),
        "latest_run_id": str(state.get("last_run_id", "")) if isinstance(state, dict) else "",
        "run_count": run_count,
        "privacy": "metadata_only",
        "claim_boundary": "This checkpoint proves only that the local OMJ plugin hook ran; it is not execution, review, CI, merge, or Hermes reload evidence.",
    }
