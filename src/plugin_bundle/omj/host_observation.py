...
PLUGIN_HOST_OBSERVATION_CLAIM_BOUNDARY = (
    "An OMJ plugin host observation is metadata supplied by a Hermes host or wrapper "
...
OBSERVATION_SCHEMA: dict[str, object] = {
    "type": "object",
    "description": (
        "Optional host-supplied metadata for recording that Hermes actually invoked this OMJ plugin "
...
def _observation_metadata(args: dict[str, Any], kwargs: dict[str, Any]) -> dict[str, Any]:
    ...
    for source in (args.get("observation"), kwargs.get("observation"), kwargs.get("omj_observation")):
        if isinstance(source, dict):
            metadata.update(source)
    for key in ("host", "session_id", "source", "message", "omj_home", "hermes_home"):
...
def _record_observation(metadata: dict[str, Any], *, event: str, tool: str, hook: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        from omj.plugin_observations import record_plugin_host_observation

        paths = resolve_paths(
            omj_home=str(metadata.get("omj_home", "") or "") or None,
...
def _default_message(*, event: str, tool: str, hook: str) -> str:
    if event == "tool_call":
        return f"OMJ plugin tool {tool} observed with host/session metadata."
    if event == "hook_call":
        return f"OMJ plugin hook {hook} observed with host/session metadata."
    return "OMJ plugin host observation recorded with metadata-only context."
...
def _record_standalone_observation(
    ...
    *,
    ...
    omj_home = _expand_path(str(metadata.get("omj_home", "") or "") or os.environ.get("OMJ_HOME", "~/.omj"))
    runtime_dir = omj_home / "runtime"
...
def _observation_error(
    *,
    ...
    return {
        "schema_version": "omj_plugin_host_observation_error/v1",
        "event": event,
        "status": "not_recorded",
        "host": host,
        "session_id": session_id,
