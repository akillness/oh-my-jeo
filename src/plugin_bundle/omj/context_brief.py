...
OMJ_CONTEXT_BRIEF_SCHEMA_VERSION = "omj_context_brief/v1"
...
def build_context_brief(
    ...
    *,
    ...
    """Build bounded Hermes-facing OMJ operating context."""
    text = str(message or "")
    ...
    payload: dict[str, object] = {
        "schema_version": OMJ_CONTEXT_BRIEF_SCHEMA_VERSION,
        "source": source,
        "purpose": (
            "Give Hermes a compact OMJ mental model before generic chat, image, file, search, or coding tools."
        ),
        "message": {
        ...
        "normal_response_contract": {
            "schema_version": "omj_context_response_contract/v1",
            "when_user_asks_capabilities": "Summarize lanes and offer the workflow picker instead of requesting shell approval.",
            "when_request_matches_lane": "Name the OMJ workflow, the reason, the next action, and what is not evidence yet.",
            "when_generic_tool_is_available": (
                "Do not skip OMJ merely because a generic tool can render, search, edit files, or run code."
            ),
        ...
        "claim_boundary": (
            "OMJ context is advisory routing and status context only. It is not workflow execution, image generation, "
...
def _capability_family_cards() -> list[dict[str, object]]:
    try:
        from omj.capabilities.families import capability_family_cards
    except ImportError:
...
def _catalog_question_hint(message: str) -> dict[str, object]:
    if not _is_catalog_question(message):
        return {}
    return {
        "schema_version": "omj_catalog_question_hint/v1",
        "status": "matched",
        "next_action": "show_workflow_picker",
        "recommended_tool": "omj_capabilities",
        "recommended_tool_args": {"action": "summary"},
        "wrapper_contracts": ["omj_skill_picker/v1", "omj_capability_summary/v1"],
        "direct_invocation_aliases": ["./omj", "/omj", "./skills", "/skills"],
        "response_rule": (
            "Answer in chat by summarizing OMJ lanes and offering the workflow picker; "
            "do not ask the user to approve `omj list` for catalog discovery."
        ),
        "claim_boundary": (
...
def _is_catalog_question(message: str) -> bool:
    ...
    if not text:
        return False
    try:
        from omj.routing.catalog_questions import is_skill_catalog_question
    except Exception:
