...
ROLE_CONTEXT_SCHEMA_VERSION = "omj_role_context/v1"
ROLE_NAME_RE = re.compile(r"^[a-zA-Z0-9_-]+$")
ROLE_MARKER_RE = re.compile(r"\[omj-role:([a-zA-Z0-9_-]+)\]")
...
def role_context_payload(role: str) -> dict[str, object]:
    ...
    if not prompt:
        return {
            ...
            "context": "",
            "claim_boundary": "OMJ role context is prompt guidance only; it is not runtime delegation or execution evidence.",
        }
