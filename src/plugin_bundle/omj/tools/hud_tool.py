...
from ..runtime_reader import read_omj_hud

OMJ_HUD_SCHEMA = {
    "name": "omj_hud",
    "description": (
        "Read the compact OMJ metadata-only HUD payload for Hermes TUI/status surfaces. "
...
def omj_hud_handler(args: dict[str, Any], **kwargs) -> str:
    observation = observe_plugin_tool_call("omj_hud", args, kwargs)
    token_metadata = {
        key: args.get(key)
        for key in (
            "tokens_remaining",
