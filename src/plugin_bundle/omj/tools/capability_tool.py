...
OMJ_CAPABILITIES_SCHEMA = {
    "name": "omj_capabilities",
    "description": (
        "Read OMJ agent, skill, hook, keyword, orchestration, tool, and evidence capability manifests. "
...
def omj_capabilities_handler(args: dict, **kwargs) -> str:
    observation = observe_plugin_tool_call("omj_capabilities", args, kwargs)
...
def _load_package_registry() -> dict[str, object]:
    try:
        from omj.capabilities.registry import capability_summary, filtered_capability_snapshot, inspect_capability, list_capabilities
    except ImportError:
        return {}
    return {
...
def _standalone_capability_summary() -> dict[str, object]:
    ...
    return {
        "schema_version": "omj_capability_summary/v1",
        "source": "standalone_plugin_bundle_fallback",
        "degraded": True,
        "determinism": "static_plugin_metadata_no_runtime_clock",
        "purpose": (
            "Compact Hermes-facing summary for answering what OMJ can do, choosing the nearest workflow, "
        ...
        "direct_response_guidance": [
            "When a user asks what OMJ can do, summarize capability families and offer the workflow picker.",
...
def _standalone_capability_snapshot(section: str | None = None) -> dict[str, object]:
    ...
    payload: dict[str, object] = {
        "schema_version": "omj_capability_manifest/v1",
        "source": "standalone_plugin_bundle_fallback",
        "degraded": True,
        "determinism": "static_plugin_metadata_no_runtime_clock",
...
def _standalone_capability_list(section: str | None = None) -> dict[str, object]:
    ...
    if canonical_section:
        if canonical_section not in sections:
            raise ValueError(f"unknown capability section: {section}")
        return {
            "schema_version": "omj_capability_list/v1",
    ...
    for section_name, values in search.items():
        for item in _standalone_items(values):
            if _standalone_matches(wanted, item):
                return {
                    "schema_version": "omj_capability_inspect/v1",
...
def _standalone_sections() -> dict[str, object]:
    return {
        "omj_awareness": awareness_primer_payload(),
        "agent_roles": [
            {
        ...
        "hooks": {
            "schema_version": "omj_hook_manifest/v1",
            "plugin_tools": [
                {
                    "name": name,
                    ...
                    in {"omj_capabilities", "omj_context", "omj_probe", "omj_recommend", "omj_interact"},
...
def _standalone_hook_payload_fields(name: str) -> list[str]:
    if name == "pre_llm_call":
        return ["omj_awareness_primer", "omj_context_brief", "omj_route_hint", "bounded_status_context", "redacted"]
    if name == "pre_tool_call":
        return ["tool_name", "tool_family_hint", "omj_generic_tool_checkpoint", "claim_boundary", "redacted"]
...
def _cli_backend_surface(name: str) -> str:
    if name == "omj_interact":
        return "omj chat interact"
    if name == "omj_capabilities":
        return "omj capabilities"
    if name == "omj_context":
        return "omj context brief"
    if name == "omj_probe":
        return "omj probe"
            ...
            capabilities.append(
                {
                    ...
                    "workflow_routing_hint": (
                        ...
                        f"{lane.get('use_for') or 'OMJ workflow guidance'}. "
...
def _standalone_playbook_capabilities() -> list[dict[str, object]]:
    ...
    evidence_boundary = str(awareness.get("evidence_boundary") or "")
    context_rule = (
        "Use OMJ playbooks as situation-level workflow maps: choose the nearest playbook for the user's request, "
        ...
        "items": [
            {
                ...
                "source_refs": ["src/plugin_bundle/omj/tools/capability_tool.py"],
            }
...
def _standalone_evidence_boundaries() -> dict[str, str]:
    return {
        "prepared_is_not": (
            "Prepared OMJ capability, handoff, topology, or routing metadata is not execution, worker dispatch, "
