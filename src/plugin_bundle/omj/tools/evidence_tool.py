...
SCHEMA_VERSION = "omj_evidence_probe/v1"
_MAX_COMMANDS = 8
...
_DEFAULT_ALLOWLIST = (
    ...
    "omj docs workflows --check",
    "omj release checklist",
    "omj release product-readiness",
    "omj release evidence-bundle",
    "python -m unittest",
    "python3 -m unittest",
    "python -m compileall",
...
def omj_evidence_handler(args: dict, **kwargs) -> str:
    observation = observe_plugin_tool_call("omj_gather_evidence", args, kwargs)
...
def _run_command(command_text: str, tokens: list[str], *, workdir: Path, timeout: int, truncate: int) -> dict[str, object]:
    try:
        env = os.environ.copy()
        with tempfile.TemporaryDirectory(prefix="omj-evidence-pycache-") as pycache_dir:
