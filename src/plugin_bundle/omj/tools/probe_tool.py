...
OMJ_PROBE_SCHEMA = {
    "name": "omj_probe",
    "description": (
        "Read OMJ local capability probe and roadmap metadata. Capability presence is not "
...
def _package_probe(
    *,
    ...
    from omj.probe import probe_capabilities

    paths = resolve_paths(omj_home=omj_home, hermes_home=hermes_home)
...
def _standalone_probe(
    *,
    ...
    # Keep this fallback deliberately smaller than omj.probe: copied plugin bundles
    # must answer setup/status questions without importing the installed package.
    home = _expand_path(omj_home or os.environ.get("OMJ_HOME", "~/.omj"))
    hermes = _expand_path(hermes_home or os.environ.get("HERMES_HOME", "~/.hermes"))
    status = read_omj_status(omj_home=home, limit=3)
    hud = read_omj_hud(omj_home=home, hermes_home=hermes, preset="focused", limit=1)
    ...
    payload: dict[str, Any] = {
        ...
        "degraded": True,
        "plugin_tool": "omj_probe",
        "omj_home": str(home),
        "hermes_home": str(hermes),
        "capabilities": capabilities,
        "target_topology": hud.get("target_topology", {}),
        "plugin_distribution_ready": _capability_status(capabilities, "omj_plugin_bundle") == "available",
        ...
        "claim_boundary": (
            "This standalone plugin probe can read local OMJ files and the current plugin bundle metadata. "
    ...
    if include_parity:
        payload["parity_matrix"] = {
            "schema_version": "omj_parity_matrix/v1",
            "source": "standalone_plugin_bundle_fallback",
            "degraded": True,
            "status": "unavailable_without_package_backend",
            "message": "Install or refresh the OMJ package backend to compute the full parity matrix.",
...
def _standalone_capabilities(
    ...
    *,
    ...
    managed_skill = skills_dir / "oh-my-jeo" / "SKILL.md"
    hermes_config = hermes / "config.yaml"
    config_text = _read_text(hermes_config)
    plugin_dir = hermes / "plugins" / "omj"
    ...
    return [
        _capability(
            ...
            str(hermes_config),
            (
                "Hermes config references the managed OMJ skills directory"
                if str(skills_dir) in config_text
                else "Managed OMJ skills are not registered in this config from the standalone probe view"
            ),
        ),
        _capability(
            "managed_skills",
            ...
            str(managed_skill),
            "Managed oh-my-jeo skill is installed" if managed_skill.is_file() else "Managed OMJ skill files are missing",
        ),
        _capability(
            "plugin_dir_exists",
        ...
        _capability(
            ...
            str(plugin_dir),
            "Managed OMJ plugin bundle files are present" if _plugin_bundle_ready(plugin_dir) else "Managed OMJ plugin bundle files are missing",
        ),
        _capability(
            "plugin_tool_context",
            "available",
            "omj_probe",
            "This response was produced by the managed OMJ plugin tool; persisted host observation remains separate",
        ),
        ...
        _capability(
            ...
            "OMJ runtime state is present" if status.get("runtime_state_present") else "No OMJ runtime state has been recorded yet",
        ),
        _capability(
            "mcp_bridge_server",
            "unknown",
...
def _standalone_roadmap(probe_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    baseline = [
        ...
        for name in ("managed_skills", "external_skill_dirs", "omj_plugin_bundle", "target_registry")
        ...
        next_actions.append(
            {
                ...
                "boundary": "Setup prepares local OMJ surfaces; it is not workflow execution evidence.",
        ...
        next_actions.append(
            {
                ...
                "command": "omj plugin observe-host --host hermes-agent --session <session-id> --event plugin_load --evidence-ref <host-log-ref>",
        ...
        next_actions.append(
            {
                ...
                "kind": "evidence_gap",
                "priority": 30,
                "label": "Record a wrapper-visible OMJ workflow run",
                "operator_instruction": "Ask Hermes to use an OMJ workflow, then let the wrapper record route/session status.",
                "why": "No wrapper/session artifact has been observed in this OMJ home yet.",
    ...
    return {
        "schema_version": "omj_capability_gap_roadmap/v1",
        "source": "standalone_plugin_bundle_fallback",
        "degraded": True,
        "summary": {
...
def _latest_plugin_observation(home: Path) -> dict[str, Any] | None:
    ...
    for line in lines:
        ...
        try:
            ...
            continue
        if isinstance(record, dict) and record.get("schema_version") == "omj_plugin_host_observation/v1":
            latest = record
    return latest
