...
OMJ_INTERACT_SCHEMA = {
    "name": "omj_interact",
    "description": (
        "Build the OMJ chat_interaction/v1 envelope for a natural-language Hermes request and, "
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "mode": {
                ...
                "default": "auto",
                "description": "OMJ chat interaction mode.",
            },
            "limit": {
                "type": "integer",
                "minimum": 1,
            ...
            "record_session": {
                "type": "boolean",
                "default": True,
                "description": "Record or resume a metadata-only wrapper session under OMJ runtime state.",
            },
            "source_metadata": {
                "type": "object",
...
_CLAIM_BOUNDARY = (
    "A wrapper chat/session record proves OMJ chat orchestration metadata only. "
...
def omj_interact_handler(args: dict, **kwargs) -> str:
    observation = observe_plugin_tool_call("omj_interact", args, kwargs)
    message = str(args.get("message") or "").strip()
    if not message:
        payload = {
            "schema_version": "omj_interact_result/v1",
            "status": "error",
            "error": "omj_interact.message is required",
...
def _package_interaction(
    ...
    *,
    ...
    from omj.wrapper.sessions import create_or_resume_wrapper_session
    ...
    if bool(args.get("record_session", True)):
        result = create_or_resume_wrapper_session(
            ...
            record_provenance={
                ...
                "producer_detail": "omj_interact plugin tool",
        ...
        interaction["wrapper_session"] = {
            "schema_version": "omj_wrapper_session_ref/v1",
            "recorded": True,
...
def _fallback_interaction(args: dict[str, Any], message: str, *, error: str) -> dict[str, Any]:
    ...
    workflow = str(route_hint.get("primary_workflow") or "oh-my-jeo")
    message_hash = hashlib.sha256(message.encode("utf-8")).hexdigest()
    payload: dict[str, Any] = {
        ...
        "chat_response": {
            ...
            "kind": "route_hint",
            "headline": f"[omj] route - {workflow}",
            "body": "OMJ can suggest the nearest workflow here, but package-backed chat/session recording is unavailable.",
            "state": {
