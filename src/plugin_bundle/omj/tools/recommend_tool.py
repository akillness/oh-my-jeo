...
OMJ_RECOMMEND_SCHEMA = {
    "name": "omj_recommend",
    "description": (
        "Recommend OMJ workflows for a natural-language Hermes request without shell catalog approval. "
...
_FALLBACK_RECOMMENDATIONS = (
    {
        "skill": "oh-my-jeo",
        "description": "Use the OMJ router when the request crosses workflow lanes or needs a safe first step.",
        "category": "router",
        "phase": "route",
        "hermes_role": "guide",
        "handoff_policy": "hermes_retained",
        "next_action": "clarify_or_route",
        "wrapper_guidance": "Show the nearest OMJ workflow lane, one safe next action, and what is not evidence yet.",
    },
    {
...
def omj_recommend_handler(args: dict, **kwargs) -> str:
    observation = observe_plugin_tool_call("omj_recommend", args, kwargs)
    message = str(args.get("message") or "").strip()
    limit = _bounded_limit(args.get("limit"), default=5)
    if not message:
        payload = {
            "schema_version": "omj_recommend_result/v1",
            "status": "error",
            "error": "omj_recommend.message is required",
            "recommendations": [],
    ...
    payload = {
        ...
        "tool_guidance": (
            "Use this tool when Hermes needs the nearest OMJ workflow without asking the user to approve "
            "`omj recommend` or `omj list` shell commands."
        ),
        "claim_boundary": _claim_boundary(),
    }
    return json.dumps(attach_public_observation(payload, observation), sort_keys=True)
...
def _recommendation_from_hint(hint: dict[str, Any], route_hint: dict[str, Any]) -> dict[str, Any]:
    workflow = str(hint.get("workflow") or "oh-my-jeo")
    ...
    return {
        "skill": workflow,
        "description": str(hint.get("reason") or context_card.get("omj_pattern") or "OMJ workflow route hint."),
        ...
        "matched": matched,
        "why": str(hint.get("reason") or "Matched OMJ awareness route hint metadata."),
        ...
        "wrapper_guidance": str(
            ...
            or "Name the OMJ workflow, show one safe next action, and keep observed evidence separate."
        ),
...
def _static_fallback_recommendation(item: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        **item,
        ...
        "confidence": "low",
        "matched": [],
        "why": "No strong plugin route hint matched; start with general OMJ routing guidance.",
...
def _claim_boundary() -> str:
    ...
    return str(
        ...
        or "OMJ recommendations are routing guidance only, not workflow execution evidence."
    )
