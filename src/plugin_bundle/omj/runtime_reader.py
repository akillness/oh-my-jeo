...
STATUS_SCHEMA_VERSION = "omj_status/v1"
HUD_SCHEMA_VERSION = "omj_hud/v1"
...
def _default_omj_home() -> Path:
    return _expand_path(os.environ.get("OMJ_HOME", "~/.omj"))
...
def read_omj_hud(
    ...
    *,
    ...
    payload["display"] = {
        "line": format_omj_hud_line(payload, preset=safe_preset),
        "segments": _hud_segments(payload, preset=safe_preset),
    }
    return payload


def format_omj_hud_line(payload: dict[str, Any], *, preset: str = "focused") -> str:
...
def _plugin_capabilities(plugin_dir: Path, last_distribution: dict[str, Any]) -> dict[str, Any]:
    files = {
        ...
        "managed_manifest": (plugin_dir / ".omj-plugin-manifest.json").is_file(),
    }
    files.update(
        {
...
def read_omj_status(omj_home: str | Path | None = None, limit: int = 5) -> dict[str, Any]:
...
def _valid_progress_binding(binding: dict[str, Any], expected_target_type: str) -> bool:
    if not isinstance(binding, dict) or _has_raw_or_hidden_content(binding):
        return False
    if binding.get("schema_version") != "omj_executor_progress_binding/v1":
        return False
...
def _valid_progress_event(event: dict[str, Any], binding_id: str, instance_id: str) -> bool:
    if not isinstance(event, dict) or _has_raw_or_hidden_content(event):
        return False
    if event.get("schema_version") != "omj_progress_event/v1":
        return False
...
def _valid_progress_report(report: dict[str, Any], binding_id: str, instance_id: str) -> bool:
    if not isinstance(report, dict) or _has_raw_or_hidden_content(report):
        return False
    if report.get("schema_version") != "omj_progress_report/v1":
        return False
