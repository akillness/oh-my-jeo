...
from ..plugin_bundle.omj.awareness import awareness_lane_examples, awareness_primer_payload
...
def capability_summary() -> dict[str, object]:
    ...
    awareness = snapshot["omj_awareness"]
...
def evidence_boundaries() -> dict[str, object]:
    return {
        ...
        "claim_rule": "Capability presence means OMJ can prepare guidance or status; it does not prove host/plugin/runtime execution.",
    }
...
def _section_ids(section: str, payload: object) -> list[str]:
    if section == "omj_awareness":
        return ["omj_awareness", "first_turn_rule", "workflow_lanes", "fallback_rule", "evidence_boundary"]
