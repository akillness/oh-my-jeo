...
def build_routing_record(routing: dict[str, Any]) -> dict[str, Any]:
    ...
    record = {
        ...
        "action": action,
        "selected_skill": str(routing.get("selected_skill", "oh-my-jeo")),
...
def build_coding_delegation_record(delegation: dict[str, Any]) -> dict[str, Any]:
    ...
    record = {
        ...
        "recommended_workflow": str(nested.get("recommended_workflow", delegation.get("recommended_workflow", "oh-my-jeo"))),
...
def _compact_wrapper_session_provenance(provenance: Any) -> dict[str, Any]:
    ...
    producer_detail = str(provenance.get("producer_detail") or "omj wrapper session backend")
    return {
...
def _compact_hermes_coding_team_path(value: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "not_observed_by_omj": _compact_string_list(value.get("not_observed_by_omj", [])),
        "claim_boundary": str(value.get("claim_boundary", "")),
    }
...
def validate_coding_runtime_handoff(handoff: Any) -> list[str]:
    ...
    _require(
        handoff.get("dispatch_contract") == "wrapper_or_user_starts_runtime; omj_does_not_execute_runtime",
        errors,
    ...
    if isinstance(worktree_contract, dict):
        expected = {"policy", "isolation", "required_before", "not_observed_by_omj"}
        ...
        for key in ("required_before", "not_observed_by_omj"):
...
def validate_hermes_coding_team_path(path: Any, *, selected: str) -> list[str]:
    ...
    for key in ("leader_contract", "worker_contract", "status_ladder", "wrapper_actions", "not_observed_by_omj"):
...
def validate_isolation_plan(value: Any, label: str) -> list[str]:
    ...
    for key in ("reason_codes", "required_before", "recommended_when", "wrapper_actions", "observation_events", "not_observed_by_omj"):
    ...
    _require(
        "git worktree creation" in value.get("not_observed_by_omj", []),
        errors,
        f"{label} not_observed_by_omj must include git worktree creation",
    )
    return errors
