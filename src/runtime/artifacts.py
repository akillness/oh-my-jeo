...
def _unique_run_id(paths: OmjPaths, slug: str) -> str:
    for _ in range(100):
...
def read_state(paths: OmjPaths) -> dict[str, Any] | None:
    return read_json_object(paths.runtime_state_path)


def read_state_result(paths: OmjPaths) -> tuple[dict[str, Any] | None, str | None]:
    return read_json_object_result(paths.runtime_state_path)


def read_state_error(paths: OmjPaths) -> str | None:
    return read_state_result(paths)[1]


def update_state(paths: OmjPaths, patch: dict[str, Any]) -> dict[str, Any]:
...
def create_run(paths: OmjPaths, metadata: dict[str, Any]) -> dict[str, Any]:
...
def create_prepared_coding_delegation_run(paths: OmjPaths, metadata: dict[str, Any]) -> dict[str, Any]:
    prepared_metadata = {
        **metadata,
        "status": "prepared",
...
def append_journal_observation(paths: OmjPaths, observation: dict[str, Any]) -> dict[str, Any]:
...
def _append_runtime_observation_to_journal(target_dir: Path, record: dict[str, Any]) -> None:
    try:
        ...
        paths = OmjPaths(omj_home=runtime_dir.parent, hermes_home=Path("~/.hermes").expanduser())
        append_observation_event(
            paths,
            {
...
def _journal_events_for_run_result(paths: OmjPaths, run_id: str) -> tuple[list[dict[str, Any]], list[str]]:
...
def _legacy_lifecycle_projection(shown: dict[str, Any]) -> dict[str, Any]:
    ...
    if merge.get("observed"):
        ...
        observation_status = "prepared_not_observed"
    return {
        "schema_version": "omj_lifecycle_projection/v1",
        "run_id": str(run.get("run_id", "")),
...
def list_runs(paths: OmjPaths, *, limit: int | None = None) -> list[dict[str, Any]]:
...
def show_run(paths: OmjPaths, run_id: str) -> dict[str, Any]:
    run_dir = paths.runtime_runs_dir / run_id
...
def list_wrapper_session_records(paths: OmjPaths, *, limit: int | None = None) -> list[dict[str, Any]]:
...
def show_wrapper_session_record(paths: OmjPaths, session_id: str) -> dict[str, Any]:
...
def _show_executor_progress(target_dir: Path) -> dict[str, Any]:
    ...
    result = {
        "schema_version": "omj_executor_progress_show/v1",
...
def summarize_delegated_coding_status(paths: OmjPaths, run_id: str) -> dict[str, Any]:
...
def validate_runtime(paths: OmjPaths, run_id: str | None = None) -> dict[str, Any]:
    if run_id:
...
def _wrapper_session_dirs_for_run(paths: OmjPaths, run_id: str) -> list[Path]:
    if not paths.runtime_wrapper_sessions_dir.exists():
        return []
...
def _wrapper_session_records_for_run(paths: OmjPaths, run_id: str, *, limit: int | None = None) -> list[dict[str, Any]]:
    sessions: list[dict[str, Any]] = []
