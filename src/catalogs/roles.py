...
ROLE_CONTRACT_VERSION = "omj_role_surface/v1"
ROLE_WORKFLOW_CONTEXT_RULE = (
    "Use this role as OMJ workflow-layer responsibility context: route the user's request to the nearest skill, "
    "name adjacent OMJ workflows when the work crosses lanes, and keep status/evidence boundaries visible."
)
ROLE_CHAT_RULE = (
    "Normal users talk to Hermes; role names help Hermes explain ownership and next action without making the user "
    "learn backend OMJ commands."
)
ROLE_BOUNDARY_RULE = (
...
_ROLES = (
    RoleDefinition(
        ...
        primary_skills=("oh-my-jeo", "voice-operator", "gateway-intent-card"),
...
def role_summary_markdown() -> str:
    lines = [
        "OMJ role names are responsibility descriptors, not runtime agents. They help Hermes and wrappers explain who owns the next step without implying a hidden worker ran.",
        "",
    ]
    for role in _ROLES:
        lines.extend(
...
def roles_reference_markdown() -> str:
    lines = [
        "# OMJ Role Surface",
        "",
        "OMJ roles are responsibility descriptors, not runtime agents. They make chat responses, wrapper buttons, and status cards easier to read without claiming that a separate worker exists or ran.",
        "",
        ...
        "",
        "Use `omj setup --operating-model <id>` only when a profile should start from one",
        "of these postures. Use `omj setup --profile-pack <id>` only when you also want",
        ...
        "",
        '<p align="center">',
        '  <img src="../assets/omj-profile-interaction-map.svg" alt="OMJ request-to-handoff role interaction map" width="920">',
        "</p>",
        "",
        "## Roles",
        "",
