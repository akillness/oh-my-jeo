...
def workflow_state_path(paths: OmjPaths, workflow: str) -> Path:
    validate_workflow_name(workflow)
    return paths.workflow_state_dir / f"{workflow}-state.json"


def read_workflow_state(paths: OmjPaths, workflow: str) -> dict[str, Any] | None:
...
def read_workflow_state_result(paths: OmjPaths, workflow: str) -> tuple[dict[str, Any] | None, str | None]:
    try:
...
def list_workflow_states(paths: OmjPaths) -> tuple[list[dict[str, Any]], list[dict[str, str]]]:
...
def active_workflow_states(paths: OmjPaths) -> tuple[list[dict[str, Any]], list[dict[str, str]]]:
...
def finish_workflow_state(paths: OmjPaths, workflow: str, outcome: str = "finished", note: str = "") -> dict[str, Any]:
...
def start_workflow_state(paths: OmjPaths, workflow: str, note: str = "") -> dict[str, Any]:
    validate_workflow_name(workflow)
...
def clear_workflow_state(paths: OmjPaths, workflow: str) -> bool:
    path = workflow_state_path(paths, workflow)
    if not path.exists():
        return False
    path.unlink()
