...
from ..paths import OmjPaths, expand_path


TARGET_REGISTRY_SCHEMA_VERSION = "omj_target_registry/v1"
TARGET_OBSERVATION_SCHEMA_VERSION = "omj_target_observation/v1"
TARGET_TOPOLOGY_SCHEMA_VERSION = "omj_target_topology/v1"
TARGET_NOTICE_SCHEMA_VERSION = "omj_target_change_notice/v1"
...
def read_target_registry_result(paths: OmjPaths) -> tuple[dict[str, Any] | None, str | None]:
...
def summarize_target_registry(paths: OmjPaths) -> dict[str, object]:
    registry, error = read_target_registry_result(paths)
    if error:
        return {
...
def _empty_topology(paths: OmjPaths, error: str | None) -> dict[str, object]:
    return {
        "schema_version": TARGET_TOPOLOGY_SCHEMA_VERSION,
        "registry_path": str(paths.target_registry_path),
