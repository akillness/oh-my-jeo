...
TASK_CARD_SCHEMA_VERSION = "omj_task_card/v1"
...
def task_card_recommendation(card: dict[str, object]) -> dict[str, object]:
    ...
    selected = str(card.get("selected_workflow_rail", "oh-my-jeo"))
    return {
        "skill": selected,
    ...
    for alias in aliases:
        ...
        for surface in _MAINTENANCE_SURFACE_PHRASES:
            ...
            if f"{compact_surface}{compact_alias}" in compact:
                return True
        if normalized.startswith("omj") and (normalized_alias in normalized or compact_alias in compact):
            return True
    return command in tokens and "omj" in tokens
...
def _is_near_exact_maintenance_request(normalized: str, compact: str, tokens: set[str]) -> bool:
    ...
    if run_hit and len(meaningful) <= 8:
        return True
    return normalized.lstrip("`'\"“”‘’ ").startswith(("omj", "./omj", "/omj")) and len(meaningful) <= 4
...
def _runtime_portability_card() -> dict[str, object]:
    return {
        ...
        "workflow_rails": [
            ...
            {
                "skill": "doctor",
                "purpose": "Verify the restored OMJ/Hermes install after files and config are present.",
            },
            {
...
def _maintenance_command_card(command: str) -> dict[str, object]:
    selected_skill = "doctor" if command == "doctor" else "oh-my-jeo"
    command_argv = ["omj", command]
    if command == "list":
        first_safe_action = "Run `omj list` and summarize the installed workflow catalog without starting repo work."
    else:
        first_safe_action = f"Run `omj {command}` and report only the observed maintenance output."
    return {
        "schema_version": TASK_CARD_SCHEMA_VERSION,
        "task_type": "omj_cli_maintenance",
        "route_level": TASK_CARD_MAINTENANCE_ROUTE_LEVEL,
        "confidence": "high",
        "score": 80,
        "selected_workflow_rail": selected_skill,
        "recommended_next_action": f"run_omj_{command}",
        "command": command,
        "command_argv": command_argv,
        "user_facing_summary": (
            f"I will run the OMJ maintenance {command} path; code changes require a separate request."
        ),
        "not_a_workflow": [
            "coding_handoff",
        ...
        "workflow_rails": [
            {
                ...
                "purpose": "Keep the response scoped to the requested OMJ maintenance command and observed output.",
            },
            {
                "skill": "doctor",
        ...
        "routing_reason": (
            "Matched a short OMJ CLI maintenance command; route as operator maintenance before inheriting stale coding context."
        ),
        "wrapper_guidance": (
            "Render this as an operator maintenance command. Run the requested `omj` command, summarize observed output, "
...
def _router_design_feedback_card() -> dict[str, object]:
    return {
        ...
        "routing_reason": (
            "Matched OMJ routing/design feedback signals; route to workflow-learning before any domain-specific workflow."
        ),
        "wrapper_guidance": (
            "Treat this as router-design learning material. Do not route Korean or English OMJ feedback to customer feedback triage."
        ),
        "claim_boundary": (
