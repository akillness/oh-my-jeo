...
from .intent import classify_omj_quality_intent
...
OMJ_QUALITY_IMPROVEMENT_GUARD = RoutingGuardRule(
    id="omj_quality_improvement_loop_before_feedback_triage",
    rule=(
        "OMJ self-improvement requests about router quality, context loss, progress reporting, or coding-handoff "
        "reliability should route to the process/coding lane even when bug words are present."
    ),
    matched_label="guard:omj_quality_improvement_loop",
    preferred_skills=("ultraprocess",),
    score_boost=48,
    why=(
        "Matched semantic OMJ quality-improvement intent; bug/failure terms are evidence for improving OMJ "
...
MISSED_WORKFLOW_WEB_RESEARCH_GUARD = RoutingGuardRule(
    ...
    rule="Missed-OMJ feedback about research/source work should recover to web-research instead of broad router help.",
    matched_label="guard:missed_workflow_research_recovery",
    preferred_skills=("web-research",),
    score_boost=32,
    why="Matched guard/trigger metadata; missed OMJ research feedback should recover to source-backed Hermes research.",
    activation_status="active",
)
MISSED_WORKFLOW_OPERATING_RHYTHM_GUARD = RoutingGuardRule(
    id="missed_workflow_operating_record_recovery",
    rule="Missed-OMJ feedback about meeting notes or operating records should recover to operating-rhythm.",
    matched_label="guard:missed_workflow_operating_record_recovery",
    preferred_skills=("operating-rhythm",),
    score_boost=34,
    why="Matched guard/trigger metadata; missed OMJ meeting/record feedback should prepare an operating record with evidence boundaries.",
    activation_status="active",
)
...
DOCTOR_HEALTH_GUARD = RoutingGuardRule(
    ...
    rule="OMJ install, setup, update, stale skill, or registration health confusion should route to doctor.",
...
def active_routing_guard_rules(
    ...
    *,
    ...
    if not workflow_learning_applies and _omj_quality_improvement_guard_applies(normalized_query):
        rules.append(OMJ_QUALITY_IMPROVEMENT_GUARD)
...
def _omj_quality_improvement_guard_applies(normalized_query: str) -> bool:
    return classify_omj_quality_intent(normalized_query).applies
...
def _missed_workflow_research_guard_applies(normalized_query: str, query_tokens: set[str]) -> bool:
    ...
    if _research_department_guard_applies(normalized_query, query_tokens):
        return False
    if not _missed_omj_workflow_context_applies(normalized_query):
        return False
...
def _memory_curation_guard_applies(normalized_query: str, query_tokens: set[str]) -> bool:
    ...
    return context and (hermes_context or omj_context or stale or capability_intent) and cleanup
...
def _doctor_health_guard_applies(normalized_query: str, query_tokens: set[str]) -> bool:
    ...
    if _contains_phrase(
        ...
        (
            "after omj update",
            "omj update says setup",
            "setup is next",
            "skills still look stale",
            "skills look stale",
            "hermes skills still",
            "hermes skills list does not show",
            "hermes skills list does not show omj",
            "skills list does not show omj",
    ...
    return omj_context and maintenance and confusion
...
def _visual_summary_guard_applies(normalized_query: str, query_tokens: set[str]) -> bool:
    ...
    if explicit_visual_phrase:
        return True
    if (
        _missed_omj_workflow_context_applies(normalized_query)
