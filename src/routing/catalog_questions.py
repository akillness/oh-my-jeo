...
_EXPLICIT_OMJ_CAPABILITY_PHRASES = (
    ...
    "how can oh-my-jeo help",
    "can oh-my-jeo help with",
    "what is oh-my-jeo useful for",
    "how should i use oh-my-jeo",
    "oh-my-jeo로 뭐 할 수",
    "oh-my-jeo로 무엇을 할 수",
    "oh-my-jeo 기능 뭐",
)
...
def is_skill_catalog_question(message: str) -> bool:
    ...
    if _is_missed_workflow_feedback(search_texts):
        return False
    if _contains_catalog_token(search_texts, _EXPLICIT_OMJ_CAPABILITY_PHRASES):
        return True
...
def _is_named_workflow_catalog_question(search_texts: tuple[str, ...]) -> bool:
    return (
        _contains_catalog_token(search_texts, _OMJ_CONTEXT_MARKERS)
...
def _is_context_capability_question(search_texts: tuple[str, ...]) -> bool:
    return _contains_catalog_token(search_texts, _OMJ_CONTEXT_MARKERS) and _contains_catalog_token(
