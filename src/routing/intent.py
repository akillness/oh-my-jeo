...
def classify_workflow_intent(message: str) -> WorkflowIntent:
    ...
    diagnostic_evaluation_context = _diagnostic_omj_evaluation_context(normalized, compact)
    matching_normalized = (
        normalized
...
def classify_omj_quality_intent(message: str) -> OmjQualityIntent:
    """Classify deterministic OMJ self-improvement loop intent.

    This is a semantic feature layer over local cues: product/customer bug words
    are only decisive when the request subject is customer feedback; OMJ bug
    words become evidence for improving routing, context, progress, or handoff
    quality when the request is about OMJ itself.
    """
    normalized = normalized_phrase(message)
    compact = normalized.replace(" ", "")

    system_target_cues = _matched_omj_system_target_cues(normalized)
    quality_domain_cues = _matched_omj_quality_cues(_OMJ_QUALITY_DOMAIN_CUES, normalized, compact)
    improvement_cues = _matched_omj_quality_cues(_OMJ_IMPROVEMENT_CUES, normalized, compact)
    quality_cues = _matched_omj_quality_cues(_OMJ_QUALITY_PROBLEM_CUES, normalized, compact)
    loop_cues = _matched_omj_quality_cues(_OMJ_LOOP_CUES, normalized, compact)
    handoff_cues = _matched_omj_quality_cues(_OMJ_HANDOFF_CUES, normalized, compact)
    customer_feedback_cues = _matched_omj_quality_cues(_CUSTOMER_FEEDBACK_CUES, normalized, compact)

    target_cues = _compact_tuple((*system_target_cues, *quality_domain_cues))
    has_omj_subject = bool(system_target_cues)
    ...
    customer_feedback_subject = bool(customer_feedback_cues) and not has_omj_subject
    applies = (
        has_omj_subject
...
def _matched_omj_quality_cues(cues: tuple[str, ...], normalized: str, compact: str) -> tuple[str, ...]:
    matches: list[str] = []
    for cue in cues:
...
def _diagnostic_omj_evaluation_context(normalized: str, compact: str) -> bool:
    ...
    has_user_omj_subject = bool(_matched_omj_system_target_cues(user_region))
    ...
    if not (has_user_omj_subject or has_user_router_subject):
        return False
    return bool(_matched_cues(_OMJ_DIAGNOSTIC_EVALUATION_CUES, user_region, user_compact))
...
def scrub_diagnostic_status_text(message: str) -> str:
    """Remove pasted/generated OMJ status fragments while preserving user text."""
    normalized = normalized_phrase(message)
    compact = normalized.replace(" ", "")
    if not _diagnostic_status_context(normalized, compact):
        return message
