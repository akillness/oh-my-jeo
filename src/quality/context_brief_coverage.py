...
# protect the moment where Hermes should remember OMJ before reaching for
# generic image, search, file, chat, or coding tools. Each workflow below is a
# cue the OMJ route-hint engine actually recognizes, so the gate verifies the
# real deterministic routing the wrapper ships, not an aspirational corpus.
CONTEXT_BRIEF_COVERAGE_CASES: tuple[ContextBriefCoverageCase, ...] = (
    ...
    ContextBriefCoverageCase(
        "missed-route-learning",
        "Missed route routes to workflow learning",
        "this is a missed route, the router picked the wrong workflow",
        "workflow-learning",
        "record_missed_route",
    ),
    ContextBriefCoverageCase(
        "scheduled-automation-blueprint",
        "Scheduled ops routes to automation blueprint",
        "run this every morning as a scheduled ops blueprint with cron",
    ...
    ContextBriefCoverageCase(
        "one-cycle-delivery",
        "Pull-request delivery routes to ultraprocess",
        "take this pull request through one full delivery cycle",
        "ultraprocess",
...
def build_context_brief_coverage_demo(*, source: str = "discord") -> dict[str, object]:
    ...
    return {
        ...
        "check_basis": [
            ...
            "Prompt context includes bounded OMJ awareness and message-specific route hints when available.",
        ...
        "cases": rows,
        "claim_boundary": (
            "Context brief coverage proves deterministic local OMJ mental-model and route-hint contracts only. "
...
def _evaluate_context_brief_case(case: ContextBriefCoverageCase, *, source: str) -> dict[str, object]:
    ...
    observed = {
        ...
        "prompt_context_has_primer": "[OMJ Awareness]" in prompt_context,
        "prompt_context_has_route_hint": "[OMJ Route Hint]" in prompt_context,
    ...
    issues: list[str] = []
    if observed["schema_version"] != "omj_context_brief/v1":
    ...
    if observed["generic_tool_checkpoint"] != "omj_generic_tool_checkpoint/v1":
        issues.append("missing generic tool checkpoint")
    if observed["normal_response_contract"] != "omj_context_response_contract/v1":
    ...
    if case.expected_catalog_question:
        ...
        if observed["catalog_recommended_tool"] != "omj_capabilities":
            issues.append(f"expected omj_capabilities, observed {observed['catalog_recommended_tool']}")
    else:
