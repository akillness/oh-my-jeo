...
CAPABILITY_GAP_ROADMAP_SCHEMA_VERSION = "omj_capability_gap_roadmap/v1"
BASELINE_PRODUCT_CAPABILITIES = ("managed_skills", "external_skill_dirs", "target_registry", "omj_plugin_bundle")
...
def build_capability_gap_roadmap(probe_payload: dict[str, object]) -> dict[str, object]:
        ...
        actions.append(
            {
                ...
                "command": "omj setup",
                "boundary": "Setup prepares and registers local OMJ workflow surfaces; it is not workflow execution evidence.",
        ...
        actions.append(
            {
                ...
                "why": "The OMJ plugin bundle is locally installed, but no host-supplied active Hermes plugin event is recorded yet.",
                "command": "omj plugin observe-host --event plugin_load --host hermes --evidence-ref <host-log-or-wrapper-event>",
        ...
        actions.append(
            {
                ...
                "why": "OMJ can prepare chat-first workflows, but no wrapper/session artifact has been observed in this OMJ home yet.",
                "operator_instruction": "Ask Hermes to use an OMJ workflow, then let the wrapper record the route/session status.",
            ...
            actions.append(
                {
                    ...
                    "command": "omj mcp config-recipe --host <host> && omj mcp serve",
        ...
        actions.append(
            {
                ...
                "priority": 70,
                "label": "Optionally configure OMJ as an MCP bridge",
                "why": "Normal Hermes chat workflows do not require MCP, but MCP-capable hosts can use it for local OMJ status/recommend/probe tools.",
                "command": "omj setup --with-mcp",
    ...
    return {
        ...
        "claim_boundary": (
            "This roadmap separates missing OMJ product setup from missing host/runtime evidence. "
