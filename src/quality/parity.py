...
PARITY_MATRIX_SCHEMA_VERSION = "omj_parity_matrix/v1"
...
PARITY_CAPABILITIES: tuple[ParityCapability, ...] = (
    ParityCapability(
        ...
        omj_surface="`hermes skills ...` compatible skill pack, `omj setup`, and the managed `~/.hermes/plugins/omj` bridge.",
        status="available",
        evidence=(
            "skills/*/SKILL.md",
            "src/plugin_bundle/omj/plugin.yaml",
            "src/install/plugin_pack.py",
            "src/install/plugin_observations.py",
        ...
        user_value="Users install OMJ once and then talk to Hermes; operators can still verify the local payload.",
        claim_boundary=(
            "Plugin install/import/register smoke is not proof that Hermes loaded or used the plugin; "
            "omj_plugin_host_observation/v1 proves only the recorded host plugin event."
        ),
    ),
    ParityCapability(
        id="specialist_roles",
        ...
        omj_surface="Skill catalog role metadata, operating models, optional visible profile packs, wrapper role narration, and plugin `omj_role` / `[omj-role:name]` context injection.",
        status="available",
        evidence=("src/catalogs/roles.py", "src/plugin_bundle/omj/omj_roles.py", "src/plugin_bundle/omj/tools/role_tool.py", "src/plugin_bundle/omj/hooks/tool_hooks.py"),
    ...
    ParityCapability(
        ...
        omj_surface="Plugin `omj_gather_evidence` runs shell-free allowlisted local probes such as OMJ doctor, harness validation, docs checks, unittest, compileall, and git diff whitespace checks.",
        status="available",
        evidence=("src/plugin_bundle/omj/tools/evidence_tool.py", "src/plugin_bundle/omj/config.yaml", "tests/test_plugin_distribution.py"),
    ...
    ParityCapability(
        ...
        omj_surface="`team`, `ultrawork`, `omj runtime team-readiness`, runtime handoff payloads, worker-protocol guidance, wrapper sessions, and runtime observations.",
        status="available",
        evidence=(
            "skills/team/SKILL.md",
            "skills/ultrawork/SKILL.md",
            "src/coding/team_readiness.py",
    ...
    ParityCapability(
        ...
        omj_surface="`worktree_session_isolation/v1` plans inside coding handoffs, wrapper Prepare worktree actions, `omj worktree prepare/list/bind`, executor-session status cards, loop queue metadata, and runtime observations for worktree creation.",
        status="available",
        ...
        missing_piece="OMJ can explicitly create a local Git worktree and return binding recipes, but it does not auto-launch executors or claim host agent sessions without wrapper/runtime evidence.",
        ...
        claim_boundary=(
            "A worktree plan is not a created worktree; an OMJ-created worktree is "
    ...
    ParityCapability(
        ...
        omj_surface="`omj hud`, plugin `omj_recommend`/`omj_hud`/`omj_status` tools, wrapper sessions, runtime runs, memory inspect, and status cards.",
        status="available",
        evidence=("src/surfaces/hud.py", "src/plugin_bundle/omj/tools/hud_tool.py", "src/wrapper/sessions.py", "src/runtime/artifacts.py"),
    ...
    ParityCapability(
        ...
        omj_surface="`omj setup --with-mcp`, `omj mcp manifest`, `omj mcp config-recipe`, `omj mcp serve`, `omj probe`, and MCP preference/host-config/runtime-call separation.",
        status="available",
        evidence=("src/mcp/bridge.py", "src/commands/mcp.py", "src/maintenance/probe.py", "tests/test_mcp_bridge.py"),
        missing_piece="OMJ does not auto-enable a host MCP config or independently inspect live host sessions; the host or wrapper must record load/session evidence.",
        v1_decision="Ship a dependency-free stdio bridge with only allowlisted local status, recommendation, and probe tools.",
        user_value="Operators can connect OMJ to MCP-capable hosts with copy-paste host recipes without exposing arbitrary shell or hidden execution.",
    ...
    ParityCapability(
        ...
        omj_surface="`loop`, `ultraprocess`, `ralplan`, `ultragoal`, loop queue ticks, verification tiers, and failure-mode status cards.",
        status="available",
        evidence=("src/workflows/goal_loop.py", "src/commands/loop.py", "skills/loop/SKILL.md", "skills/ultraprocess/SKILL.md"),
    ...
    ParityCapability(
        ...
        omj_surface="`omj setup`, `omj doctor`, `omj update`, `omj uninstall`, `omj release checklist`, `omj release product-readiness`, and `omj release hermes-smoke`.",
        status="available",
        evidence=("src/commands/setup.py", "src/maintenance/doctor.py", "src/maintenance/release.py", "install.sh"),
...
def build_parity_matrix(probe_payload: dict[str, object] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "basis": "OMX/OMC/OMO common public capability patterns, translated into OMJ's Hermes-native evidence model.",
        "summary": {
...
def _probe_alignment(probe_payload: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "omj_plugin_bundle": by_name.get("omj_plugin_bundle", "unknown"),
