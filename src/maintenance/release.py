...
from ..system.paths import OmjPaths
...
INSTALLED_COMMAND_SMOKE_SCHEMA = "installed_omj_command_smoke/v1"
...
ROUTER_CONTENT_MARKERS = ("OMJ Awareness Primer", "img-summary", "Normal users should talk to Hermes Agent")
WORKFLOW_CONTEXT_MARKERS = ("OMJ Context Rail", "not a standalone executor", "Prepared OMJ routing")
ROLE_CONTEXT_MARKERS = ("OMJ Role Context", "OMJ workflow-layer responsibility context", "prepared guidance only")
...
def package_url_for(channel: str, version: str = "", package_url: str = "") -> ReleaseSelection:
    ...
    if channel == "stable":
        if not version:
            raise ValueError("stable channel requires --version or OMJ_VERSION")
...
def release_readiness_checklist(
    *,
    ...
    wheel = f"dist/oh_my_jeo-{release_version}-py3-none-any.whl"
    omj_display = _shell_word(omj_command or "omj")
    items = [
        ...
        ReleaseChecklistItem(
            ...
            "uv run python -m omj.cli docs workflows --check",
            "contract-quality",
            True,
            False,
        ...
        ReleaseChecklistItem(
            ...
            "uv run python -m omj.cli harness validate",
            "contract-quality",
            True,
            False,
        ...
        ReleaseChecklistItem(
            ...
            "uv run python -m omj.cli cases demo --all --json",
            "contract-quality",
            True,
            False,
        ...
        ReleaseChecklistItem(
            ...
            "uv run python -m omj.cli cases artifact --all --json",
            "contract-quality",
            True,
            False,
        ...
        ReleaseChecklistItem(
            ...
            "uv run python -m omj.cli cases replay --json",
            "contract-quality",
            True,
            False,
        ...
        ReleaseChecklistItem(
            ...
            "uv run python -m omj.cli cases readiness --json",
            "contract-quality",
            True,
            False,
        ...
        ReleaseChecklistItem(
            ...
            f"{omj_display} release evidence-bundle --version {release_version} --write --json",
            "evidence-packaging",
            True,
            False,
            "A local `omj_release_evidence_bundle/v1` artifact is written with checklist, product readiness, skill content, use-case readiness, and parity snapshots.",
        ...
        ReleaseChecklistItem(
            ...
            (
                "uv run python -m omj.cli --omj-home /tmp/omj-smoke --hermes-home /tmp/hermes-smoke "
        ...
        ReleaseChecklistItem(
            ...
            "uv run python -m omj.cli --omj-home /tmp/omj-smoke --hermes-home /tmp/hermes-smoke probe",
            "local-quality",
            True,
            False,
        ...
        ReleaseChecklistItem(
            ...
            "uv run python -m omj.cli release hermes-smoke",
            "release-smoke",
            True,
            False,
        ...
        ReleaseChecklistItem(
            ...
            "Check installed omj command is on PATH",
            f"command -v {omj_display}",
            "installed-command",
            True,
            False,
            "The shell resolves the installed OMJ command before any nested smoke uses it.",
            "PATH resolution proves command discoverability only; it does not prove console-script importability.",
        ),
        ReleaseChecklistItem(
            "installed_command_help",
            "Check installed omj command help",
            f"{omj_display} --help",
            "installed-command",
            True,
            False,
        ...
        ReleaseChecklistItem(
            ...
            f"{omj_display} release skill-content-smoke --json",
            "installed-command",
            True,
            False,
            ...
            "This proves the installed OMJ command package can render expected skill guidance; it does not prove Hermes loaded or selected it in chat.",
        ),
        ReleaseChecklistItem(
            "installed_command_smoke",
            "Observe installed command smoke without Hermes mutation",
            (
                f"{omj_display} --omj-home /tmp/omj-smoke --hermes-home /tmp/hermes-smoke "
                f"release hermes-smoke --install-path setup --omj-command {omj_display} --include-command-smoke"
            ),
            "installed-command",
            True,
            False,
            "Nested installed_command_smoke is mode=live, observed=true, ok=true, and includes installed skill content smoke.",
            "This observes the installed OMJ command path and generated skill guidance while keeping the outer Hermes profile smoke plan-only.",
        ),
        ReleaseChecklistItem(
            "build_artifacts",
        ...
        ReleaseChecklistItem(
            ...
            (
                ...
                f"/tmp/omj-wheel-smoke/bin/python -m pip install --upgrade {wheel}"
            ),
            "package-build",
            True,
            False,
        ...
        ReleaseChecklistItem(
            ...
            (
                "/tmp/omj-wheel-smoke/bin/omj --omj-home /tmp/omj-wheel-home --hermes-home /tmp/hermes-wheel-home "
                "release hermes-smoke --install-path setup --omj-command /tmp/omj-wheel-smoke/bin/omj --include-command-smoke"
            ),
            "package-build",
            True,
            False,
        ...
        ReleaseChecklistItem(
            ...
            f"{omj_display} release install-smoke --live --repo-root \"$PWD\" --install-script \"$PWD/install.sh\"",
            "installer",
            True,
            False,
        ...
        ReleaseChecklistItem(
            ...
            f"{omj_display} release hermes-smoke --live --install-path tap --target-confirmed",
...
def product_readiness_report(
    *,
    ...
    release_version = _normalize_release_version(version)
    omj_display = _shell_word(omj_command)
    skill_content = skill_content_smoke()
    parity = build_parity_matrix()
    checklist = release_readiness_checklist(version=release_version, omj_command=omj_command)
    ...
    gates = [
        ...
        _product_readiness_gate(
            ...
            f"{omj_display} release checklist --version {release_version} --json",
    ...
    return {
        ...
        "boundary": (
            "Product readiness proves deterministic local OMJ package and product contracts only. "
...
def hermes_release_smoke_steps(
    *,
    install_path: str,
    skill: str = HERMES_TAP_SKILL,
    tap: str = DEFAULT_HERMES_TAP,
    omj_command: str = "omj",
    omj_home: str | Path | None = None,
    ...
    doctor_command = _omj_scoped_command(
        omj_command,
        "doctor",
        omj_home=omj_home,
        hermes_home=hermes_home,
    ...
    install_steps = (
        [
            HermesSmokeStep(
                ...
                "install",
                True,
                proof_boundary="Registers the OMJ GitHub tap in the current Hermes profile; this is setup evidence only.",
            ),
            HermesSmokeStep(
                "skill_install",
    ...
    check_steps = [
        ...
        HermesSmokeStep(
            ...
            "verify",
            False,
            proof_boundary="Shows enabled Hermes skills; the OMJ router should be visible after install/reload.",
        ),
        HermesSmokeStep(
            "skill_check",
            ("hermes", "skills", "check", check_skill_name),
            "verify",
            False,
            proof_boundary="Runs Hermes skill validation for the OMJ router skill.",
        ),
    ]
    if install_path == "tap":
        check_steps.append(
        ...
        check_steps.append(
            HermesSmokeStep(
                ...
                False,
                proof_boundary=(
                    "Checks OMJ-managed local skill registration. Hermes v0.15.1 does not reliably inspect "
                    "skills.external_dirs local skills by short name, so setup-path smoke uses list/check plus OMJ doctor."
                ),
            )
        )
...
def _shell_word(value: str) -> str:
    ...
    if not stripped:
        raise ValueError("release checklist omj command must not be empty")
...
def hermes_release_smoke_plan(
    *,
    ...
    target = _target_binding(omj_home=omj_home, hermes_home=hermes_home)
    steps = hermes_release_smoke_steps(
        install_path=install_path,
        skill=skill,
        tap=tap,
        omj_command=omj_command,
        omj_home=target["omj_home"],
    ...
    return {
        ...
        "proof_boundary": (
            ...
            "installed, loaded, or used OMJ. Run with --live against the target profile for observed smoke evidence."
        ),
    ...
    *,
    ...
    steps = [
        HermesSmokeStep(
            ...
            "verify",
            False,
            proof_boundary="Verifies the installed OMJ console script is importable and runnable from the current PATH.",
        ),
        HermesSmokeStep(
            "installed_omj_skill_content",
            (omj_command, "release", "skill-content-smoke", "--json"),
            "verify",
            False,
            proof_boundary=(
                "Verifies the installed OMJ command package can render router awareness and workflow context rails. "
                "This is package-content evidence only, not Hermes runtime-load evidence."
            ),
        ),
        HermesSmokeStep(
            "installed_omj_setup_plan",
            _omj_release_plan_command(
                omj_command,
                install_path="setup",
                omj_home=target["omj_home"],
                hermes_home=target["hermes_home"],
            ...
            False,
            proof_boundary=(
                "Verifies the installed OMJ console script can render the setup-path Hermes smoke plan. "
    ...
    return {
        ...
        "ok": True,
        "observed": False,
        "command_under_test": omj_command,
        "target_binding": target,
        "path_check": installed_command_path_check_plan(omj_command),
        "proof_boundary": (
            "Plan mode lists installed-command checks only. Run release hermes-smoke with "
            "--include-command-smoke to observe PATH resolution and the installed OMJ executable."
        ),
        "steps": [step.to_payload() for step in steps],
    }
...
def skill_content_smoke() -> dict[str, object]:
    ...
    return {
        ...
        "proof_boundary": (
            "This validates generated skill guidance inside the current OMJ command package. "
...
def _product_readiness_next_actions(blocking_failures: Sequence[Mapping[str, object]], warning_count: int) -> list[str]:
    if blocking_failures:
        return [
            "Fix blocking product readiness gates, then rerun `omj release product-readiness --json`.",
            "Use the per-gate command to inspect the failing contract before tagging.",
        ]
    actions = [
        "Write `omj release evidence-bundle --version <version> --write --json` and attach it to the release notes or PR.",
        ...
        "Use `omj release product-readiness --json` when a wrapper or release note needs the full machine-readable payload.",
    ]
    if warning_count:
...
def release_evidence_bundle(
    *,
    version: str = __version__,
    omj_command: str = "omj",
    paths: OmjPaths | None = None,
    write: bool = False,
) -> dict[str, object]:
    release_version = _normalize_release_version(version)
    resolved_paths = paths or OmjPaths(omj_home=Path("~/.omj").expanduser(), hermes_home=Path("~/.hermes").expanduser())
    checklist = release_readiness_checklist(version=release_version, omj_command=omj_command)
    product = product_readiness_report(version=release_version, omj_command=omj_command)
    skill_content = skill_content_smoke()
    ...
    payload: dict[str, object] = {
        ...
        "boundary": (
            "A release evidence bundle packages deterministic local OMJ evidence and optional local artifact-store state. "
    ...
    if write:
        artifact_path = resolved_paths.release_evidence_dir / f"omj-release-evidence-{release_version}.json"
        payload["written"] = True
        payload["artifact_path"] = str(artifact_path)
        atomic_write_json(artifact_path, payload, private=True)
        _write_release_evidence_index(resolved_paths, payload)
    else:
        payload["artifact_path"] = str(resolved_paths.release_evidence_dir / f"omj-release-evidence-{release_version}.json")
    return payload
...
def _release_evidence_next_actions(blocking_failures: Sequence[str], local_store_status: str) -> list[str]:
    if blocking_failures:
        return [
            "Fix blocking bundle gates, then rerun `omj release evidence-bundle --write --json`.",
    ...
    if local_store_status != "passed":
        actions.insert(0, "Run `omj cases artifact --all --write --json` if you want the optional local use-case artifact store populated.")
    return actions
...
def _write_release_evidence_index(paths: OmjPaths, payload: Mapping[str, object]) -> None:
    ...
    index = {
        "schema_version": "omj_release_evidence_index/v1",
...
def _use_case_demo_card_failures(payload: Mapping[str, object]) -> list[str]:
    failures: list[str] = []
    if payload.get("schema_version") != "omj_use_case_demo_collection/v1":
    ...
    for index, card in enumerate(cards):
        ...
        if card.get("schema_version") != "omj_use_case_demo_card/v1":
        ...
        if not isinstance(wrapper_card, Mapping) or wrapper_card.get("component") != "omj_use_case_card":
        ...
        if not isinstance(chat_surface, Mapping) or not str(chat_surface.get("headline") or "").startswith("[omj] "):
...
def _use_case_artifact_failures(payload: Mapping[str, object]) -> list[str]:
    failures: list[str] = []
    if payload.get("schema_version") != "omj_use_case_artifact_collection/v1":
        failures.append("collection_schema")
    ...
    for index, artifact in enumerate(artifacts):
        ...
        elif "omj cases validate --json" not in proof_surfaces:
...
def _use_case_replay_failures(payload: Mapping[str, object]) -> list[str]:
    failures: list[str] = []
    if payload.get("schema_version") != "omj_use_case_replay/v1":
...
def _bundled_role_contexts() -> dict[str, str]:
    try:
        root = resources.files("omj.plugin_bundle.omj.references")
    ...
    *,
    ...
    return {
        ...
        "proof_boundary": (
            ...
            "a live chat selected OMJ unless the wrapper records that chat response."
        ),
        "steps": [
            {
    ...
    *,
    ...
    return {
        ...
        "proof_boundary": (
            ...
            "It still does not prove a later chat session selected OMJ unless that session is observed separately."
        ),
    }
...
def run_installed_command_smoke(
    *,
    ...
    plan = installed_command_smoke_plan(omj_command=omj_command, omj_home=omj_home, hermes_home=hermes_home)
    target = plan["target_binding"]
    execute = runner or subprocess_runner
    path_check = inspect_installed_command_path(omj_command)
    if not bool(path_check["ok"]):
        return {
            ...
            "results": [],
            "failed_step": "installed_omj_path",
            ...
            "proof_boundary": (
                "Installed command smoke did not execute because the OMJ command was not discoverable or "
    ...
    return {
        ...
        "proof_boundary": (
            "Installed command smoke observes the OMJ console script, generated skill guidance, and plan rendering only. "
            "It does not mutate Hermes or prove live chat usage."
        ),
    }


def _target_binding(*, omj_home: str | Path | None = None, hermes_home: str | Path | None = None) -> dict[str, object]:
    omj = expand_home(omj_home, "OMJ_HOME", "~/.omj")
    hermes = expand_home(hermes_home, "HERMES_HOME", "~/.hermes")
    return {
        "omj_home": str(omj),
        "hermes_home": str(hermes),
        "explicit_omj_home": omj_home is not None,
...
def _omj_scoped_command(
    omj_command: str,
    *command_parts: str,
    omj_home: str | Path | None = None,
    hermes_home: str | Path | None = None,
) -> tuple[str, ...]:
    command = list(_omj_base_command(omj_command, omj_home=omj_home, hermes_home=hermes_home))
    command.extend(command_parts)
    return tuple(command)


def _omj_base_command(
    omj_command: str,
    *,
    omj_home: str | Path | None = None,
    hermes_home: str | Path | None = None,
    ...
    command = [omj_command]
    if omj_home is not None:
        command.extend(["--omj-home", str(omj_home)])
    if hermes_home is not None:
        command.extend(["--hermes-home", str(hermes_home)])
    return tuple(command)


def _omj_release_plan_command(
    omj_command: str,
    *,
    install_path: str,
    omj_home: str | Path | None = None,
    ...
    return (
        *_omj_base_command(omj_command, omj_home=omj_home, hermes_home=hermes_home),
        "release",
        "hermes-smoke",
        "--install-path",
        install_path,
...
def _live_smoke_command(install_path: str, target: Mapping[str, object]) -> list[str]:
    command = ["omj"]
    if target["explicit_omj_home"]:
        command.extend(["--omj-home", str(target["omj_home"])])
...
def _hermes_smoke_next_action(ok: bool, failed_step: str) -> str:
    if ok:
        return "Restart or refresh Hermes Agent if required, then try the first OMJ Hermes prompt and record the observed response."
    ...
    if failed_step == "setup_doctor":
        return "Run `omj doctor` manually and repair the OMJ-managed skill registration reported there."
    if failed_step == "installed_command_smoke":
        return (
            "Inspect installed_command_smoke.failed_step, then repair the installed `omj` command path, "
    ...
    if failed_step in {"tap_list", "skills_list", "skill_check", "skill_inspect"}:
        return "Inspect the failing Hermes skills command output and confirm the target Hermes profile is the one OMJ was installed into."
...
def _installed_command_smoke_next_action(
    ...
    failed_step: str,
    *,
    command_under_test: str = "omj",
) -> str:
    command = str(command_under_test or "omj").strip() or "omj"
    if ok:
        return (
            f"Installed `{command}` command path is runnable; "
            "continue with Hermes profile smoke or release tagging."
        )
    if failed_step == "installed_omj_path":
        if path_check_kind(command) == "direct_path":
            return f"Make {shlex.quote(command)} executable, or pass --omj-command with an executable OMJ path."
        return (
            f"Install OMJ so `command -v {shlex.quote(command)}` resolves, "
            "or pass --omj-command with an executable OMJ path."
        )
    if failed_step == "installed_omj_help":
        return "Check PATH, package installation, and console-script importability for `omj`."
    if failed_step == "installed_omj_skill_content":
        return "Run `omj release skill-content-smoke --json` directly and inspect missing router awareness or workflow context rail markers."
    if failed_step == "installed_omj_setup_plan":
        return "Run `omj release hermes-smoke --install-path setup` directly and inspect the console-script error."
