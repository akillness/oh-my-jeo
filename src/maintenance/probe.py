...
def _wrapper_artifacts(paths: OmjPaths) -> list[Path]:
    artifacts: list[Path] = []
...
def _plugin_smoke_message(plugin: dict) -> str:
    ...
    if missing_tools or missing_hooks:
        ...
        return "Installed OMJ plugin register smoke is incomplete: " + "; ".join(details)
    return "Installed OMJ plugin has not passed fake Hermes register smoke"
...
def _mcp_preference_capability(paths: OmjPaths) -> Capability:
    ...
    if error:
        return Capability(
            ...
            "unknown",
            evidence,
            f"Could not read OMJ setup state for MCP preference: {error}",
        )
    if not isinstance(state, dict):
        return Capability(
            ...
            "unknown",
            evidence,
            "No OMJ setup state has recorded an MCP bridge preference",
        )
    ...
    if not requested or mode == "none":
        return Capability(
            ...
            "unknown",
            evidence,
            "OMJ setup state says no optional MCP bridge preference was requested",
        )
    if observed:
        return Capability(
            "mcp_preference",
            "available",
            evidence,
            f"OMJ MCP bridge preference was requested and observed by setup state (mode={mode})",
        )
    return Capability(
        "mcp_preference",
        "unverified",
        evidence,
        f"OMJ MCP bridge preference was requested (mode={mode}), but no MCP host session or tool-call evidence is recorded",
    )
...
def _worktree_creator_capability() -> Capability:
    return Capability(
        "worktree_creator",
        "available",
        "omj worktree prepare; omj worktree list; omj worktree bind",
        (
            "OMJ can explicitly create local Git worktrees, record omj_worktree_observation/v1 "
...
def _team_worker_readiness_capability(paths: OmjPaths) -> tuple[Capability, dict]:
    ...
    if status == "available":
        message = (
            "OMJ ships team/swarm worker contract readiness, wrapper actions, and runtime observation ledger support; "
            f"presentation={presentation_status}, Hermes visibility={hermes_visibility}, observed runtime={observed_status}"
        )
        capability_status = "available"
    else:
        message = "OMJ team/swarm worker contract readiness is missing required local workflow surfaces"
...
def _mcp_bridge_runtime_capability(paths: OmjPaths) -> Capability:
    state, error = read_state_result(paths)
    evidence = str(paths.runtime_state_path)
    if error:
        return Capability(
            ...
            "unknown",
            evidence,
            f"Could not read OMJ MCP bridge runtime observation state: {error}",
        )
    ...
    if not isinstance(bridge, dict) or not bridge.get("observed"):
        return Capability(
            ...
            "unverified",
            evidence,
            "No OMJ MCP bridge tool call has been observed in local OMJ state",
        )
    ...
    return Capability(
        ...
        "available",
        evidence,
        f"OMJ MCP bridge observed a local tool call ({tool}); host-specific load evidence remains separate",
    )


def _mcp_host_session_capability(paths: OmjPaths) -> Capability:
    ...
    if errors:
        return Capability(
            ...
            "unknown",
            evidence,
            f"Could not read OMJ MCP host session observations: {'; '.join(errors[:3])}",
        )
...
def _plugin_runtime_observed_capability(paths: OmjPaths) -> tuple[Capability, bool, bool]:
    ...
    if errors:
        return (
            Capability(
                ...
                "unknown",
                evidence,
                f"Could not read OMJ plugin host observations: {'; '.join(errors[:3])}",
            ),
            False,
            False,
        )
    ...
    if latest and latest.get("observed"):
        ...
        return (
            Capability(
                ...
                evidence,
                (
                    f"OMJ plugin active runtime observed by {host} ({event}, session={session_id})"
                    if active
                    else f"OMJ plugin historical runtime event observed by {host} ({event}, session={session_id}); active readiness requires one of {', '.join(PLUGIN_HOST_ACTIVE_OBSERVATION_EVENTS)}"
                ),
            ),
            True,
            active,
...
def probe_capabilities(paths: OmjPaths, *, include_parity: bool = False, include_roadmap: bool = False) -> dict:
    ...
    managed_skill_path = paths.skills_dir / "oh-my-jeo" / "SKILL.md"
    ...
    capabilities.append(
        Capability(
            ...
            "Managed oh-my-jeo skill is installed" if _has_file(managed_skill_path) else "Managed skills are not installed",
        )
    )
    ...
    capabilities.append(
        _marker_capability(
            "native_hooks",
            hooks_markers,
            "Hook-like files exist, but omj has no stable Hermes hook contract to claim native integration",
    ...
    capabilities.append(
        _dir_capability(
            ...
            (
                "Hermes plugin directory exists; managed OMJ bundle install, import/register smoke, "
    ...
    capabilities.append(
        _dir_capability(
            ...
            "Apps directory exists, but omj has no stable Hermes app contract",
    ...
    capabilities.append(
        _marker_capability(
            "mcp_host_config",
            mcp_markers,
            "MCP host config exists, but OMJ has not verified a Hermes MCP extension contract or host load event",
    ...
    capabilities.append(
        Capability(
            ...
            "unknown",
            str(paths.hermes_config_path),
            "No stable Hermes-native skill metadata contract is known to omj yet",
        )
    )
    ...
    capabilities.extend(
        [
            ...
            Capability(
                ...
                "Managed OMJ plugin bundle is installed" if plugin["plugin_dir_installed"] else "Managed OMJ plugin bundle is not installed",
            ),
            Capability(
                "plugin_import_smoke",
                ...
                "Installed OMJ plugin imports locally" if plugin["plugin_import_smoke"] else "Installed OMJ plugin has not passed local import smoke",
            ),
            Capability(
                ...
                str(paths.hermes_plugin_dir),
                (
                    f"Installed OMJ plugin registers tools={plugin['registered_tools']} hooks={plugin['registered_hooks']}"
        ...
        [
            Capability(
                ...
                (
                    ...
                    else "No OMJ target registry has been recorded yet"
                ),
            ),
            Capability(
                "target_topology",
