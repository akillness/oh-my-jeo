...
WORKTREE_PREPARE_SCHEMA_VERSION = "omj_worktree_prepare/v1"
WORKTREE_OBSERVATION_SCHEMA_VERSION = "omj_worktree_observation/v1"

WORKTREE_CLAIM_BOUNDARY = (
    "An OMJ-created Git worktree is observed workspace-isolation evidence only. "
...
RUNTIME_OBSERVATION_FOLLOWUP = {
    ...
    "record_with": (
        "omj runtime observe --run <run-id> --runtime-profile <runtime-profile> --event worktree_creation "
...
def list_worktree_records(paths: OmjPaths, *, limit: int = 20) -> tuple[list[dict[str, Any]], list[str]]:
    records, errors = read_jsonl_objects(paths.runtime_worktrees_path)
    return list(reversed(records))[:limit], errors


def latest_observed_worktree_record(paths: OmjPaths, worktree_path: str | Path) -> dict[str, Any]:
...
def _append_worktree_record(paths: OmjPaths, record: dict[str, Any]) -> None:
...
def _branch_name(branch: str, task: str) -> str:
    ...
    return f"omj/{slug or 'worktree'}-{stamp}"
