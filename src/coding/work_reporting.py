...
def sanitize_user_report_text(value: Any, *, locale: str = "", voice: str = "") -> str:
    """Remove prompt-only OMJ rails from user-facing report text."""
...
def _internal_context_user_summary(*, locale: str, voice: str) -> str:
    if _friendly_ko(locale=locale, voice=voice):
        return "내부 OMJ 상태는 확인했고, 사용자에게는 필요한 진행 상황만 짧게 전할게."
    if _polite_ko(locale=locale, voice=voice):
        return "내부 OMJ 상태는 확인했으며, 사용자에게는 필요한 진행 상황만 요약하겠습니다."
    return "Internal OMJ status context was observed; user-facing output should summarize only the relevant status."
...
def _summary_ref(work_id: str, title: str, report_kind: str, status: str) -> str:
    ...
    return f"omj-work-observation:{hashlib.sha256(payload.encode('utf-8')).hexdigest()[:16]}"
