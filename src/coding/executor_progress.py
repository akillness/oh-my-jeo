...
EXECUTOR_PROGRESS_BINDING_SCHEMA_VERSION = "omj_executor_progress_binding/v1"
EXECUTOR_PROGRESS_EVENT_SCHEMA_VERSION = "omj_progress_event/v1"
EXECUTOR_PROGRESS_REPORT_SCHEMA_VERSION = "omj_progress_report/v1"
...
def progress_dir_for_target(paths: OmjPaths, target_type: str, target_id: str) -> Path:
    if target_type == "run":
...
def write_progress_binding(paths: OmjPaths, binding: dict[str, Any]) -> dict[str, Any]:
    _require_valid("binding", validate_progress_binding(binding))
...
def read_progress_binding(paths: OmjPaths, target_type: str, target_id: str) -> dict[str, Any] | None:
...
def list_progress_bindings(paths: OmjPaths, *, limit: int | None = None) -> list[dict[str, Any]]:
...
def observe_executor_progress(
    ...
    *,
    ...
    if not should_report:
        ...
        return {
            "schema_version": "omj_executor_progress_observation/v1",
            "binding": updated,
            "event": {},
            "report": {},
            "reported": False,
...
def append_progress_event(paths: OmjPaths, binding: dict[str, Any], event: dict[str, Any]) -> dict[str, Any]:
    _require_valid("binding", validate_progress_binding(binding))
    _require_valid("event", validate_progress_event(event))
    target = _binding_target(binding)
    progress_dir = progress_dir_for_target(paths, target["type"], target["id"])
...
def latest_progress_event(paths: OmjPaths, binding: dict[str, Any]) -> dict[str, Any]:
    target = _binding_target(binding)
...
def append_progress_report(paths: OmjPaths, binding: dict[str, Any], report: dict[str, Any]) -> dict[str, Any]:
    _require_valid("binding", validate_progress_binding(binding))
    _require_valid("report", validate_progress_report(report))
    target = _binding_target(binding)
    progress_dir = progress_dir_for_target(paths, target["type"], target["id"])
...
def latest_progress_report(paths: OmjPaths, binding: dict[str, Any]) -> dict[str, Any]:
    target = _binding_target(binding)
...
def project_active_executor_status(paths: OmjPaths, *, limit: int | None = 50, now: str | None = None) -> dict[str, Any]:
    bindings = [
    ...
    return {
        "schema_version": "omj_executor_progress_projection/v1",
...
def validate_progress_binding(record: dict[str, Any]) -> list[str]:
    ...
    if record.get("schema_version") != EXECUTOR_PROGRESS_BINDING_SCHEMA_VERSION:
        errors.append("schema_version must be omj_executor_progress_binding/v1")
...
def validate_progress_event(record: dict[str, Any]) -> list[str]:
    ...
    if record.get("schema_version") != EXECUTOR_PROGRESS_EVENT_SCHEMA_VERSION:
        errors.append("schema_version must be omj_progress_event/v1")
...
def validate_progress_report(record: dict[str, Any]) -> list[str]:
    ...
    if record.get("schema_version") != EXECUTOR_PROGRESS_REPORT_SCHEMA_VERSION:
        errors.append("schema_version must be omj_progress_report/v1")
    if str(record.get("executor_profile", "")) not in ALLOWED_EXECUTOR_PROFILES:
        errors.append("executor_profile must be codex, claude_code, or hermes_local")
    if not str(record.get("binding_id", "")):
        errors.append("binding_id is required")
...
def _latest_group_payload(paths: OmjPaths, group: list[dict[str, Any]], *, kind: str) -> dict[str, Any]:
    payloads = []
...
def _terminal_result_status(paths: OmjPaths, binding: dict[str, Any]) -> str:
    target = _binding_target(binding)
