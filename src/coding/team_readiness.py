...
TEAM_WORKER_READINESS_SCHEMA_VERSION = "omj_team_worker_readiness/v1"
...
def build_team_worker_readiness(paths: OmjPaths, *, target_limit: int | None = DEFAULT_RUNTIME_TARGET_SCAN_LIMIT) -> dict[str, Any]:
    ...
    return {
        ...
        "runtime_observation_contract": {
            ...
            "record_command_template": (
                "omj runtime observe --session <session-id> --runtime-profile <hermes|omx-runtime|omo-runtime|omc-runtime> "
        ...
        "claim_boundary": (
            "OMJ can prepare and verify the team/swarm worker contract, wrapper actions, and observation ledger. "
...
def _skill_surfaces(paths: OmjPaths) -> list[dict[str, Any]]:
...
def _skill_purpose(name: str) -> str:
    ...
    return purposes.get(name, "OMJ workflow surface.")


def _runtime_observation_readiness(paths: OmjPaths, *, target_limit: int | None) -> dict[str, Any]:
...
def _runtime_target_files(paths: OmjPaths) -> list[tuple[str, Path]]:
    targets = [
...
def _next_actions(*, hermes_visible: bool, runtime_observation: dict[str, Any]) -> list[str]:
    ...
    if not hermes_visible:
        actions.append("Run `omj setup` so Hermes can discover the team and ultrawork skills.")
    actions.extend(
        [
