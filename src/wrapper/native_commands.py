...
NATIVE_COMMAND_SURFACE_SCHEMA_VERSION = "omj_native_command_surface/v1"
NATIVE_COMMAND_RENDER_SCHEMA_VERSION = "omj_native_command_render/v1"
FALLBACK_CARD_SCHEMA_VERSION = "omj_command_fallback_card/v1"

NATIVE_COMMAND_SOURCES = ("generic", "discord", "slack", "telegram", "hermes")
COMMAND_NAME = "omj"
PARTIAL_PREFIXES = ("./", "/", "./o", "/om")
PICKER_ALIASES = ("./omj", "/omj", "./skills", "/skills")
...
def build_native_command_surface(source: str = "generic") -> dict[str, object]:
    ...
    surface = {
        ...
        "purpose": "Open the OMJ workflow picker from a Hermes chat surface.",
        "entrypoints": {
        ...
        "preview_contract": {
            ...
            "schema_version": "omj_command_preview/v1",
        ...
        "rendering_steps": [
            ...
            "When partial input cannot open native autocomplete, pass the message to `omj chat interact`.",
            "If the response kind is `command_preview`, render the fallback card and primary `Open omj` action.",
            "When the user selects the action, submit `./omj` or `/omj` back through the same wrapper.",
...
def render_native_command_response(interaction: dict[str, object], *, source: str = "generic") -> dict[str, object]:
    ...
    if kind == "command_preview":
        ...
        insert_text = str(suggestion.get("insert_text") or "./omj")
        payload.update(
            {
        ...
        payload.update(
            {
                ...
                "picker_schema": picker.get("schema_version", "omj_skill_picker/v1"),
...
def _fallback_card(source: str, *, insert_text: str | None = None) -> dict[str, object]:
    command = insert_text or ("./omj" if source in {"discord", "generic", "hermes"} else "/omj")
    return {
        "schema_version": FALLBACK_CARD_SCHEMA_VERSION,
        "headline": "Open OMJ",
        "body": "Choose `omj` to open the workflow picker. The picker appears before any workflow, plan, handoff, or execution is selected.",
        "primary_action": {
            "id": "show_skill_picker",
            "label": "Open omj",
            "submit_text": command,
            "opens_schema": "omj_skill_picker/v1",
        },
            ...
            "commands": [
                {
                    ...
                    "options": [
                        {
                            "name": "request",
                            "description": "Optional request to route after OMJ opens.",
                            "type": "string",
                            "required": False,
                        }
                    ],
            ...
            "slash_commands": [
                {
                    ...
                    "description": "Open the OMJ workflow picker.",
                    "usage_hint": "[request]",
                    "payload_target": "omj chat interact --source slack",
                }
            ],
            "shortcuts": [
                {
                    "name": "Open OMJ",
                    "callback_id": "omj.open_picker",
                    "description": "Open the OMJ workflow picker in the current thread.",
                }
            ],
        }
    if source == "telegram":
...
def _component_for_source(source: str, *, insert_text: str) -> dict[str, object]:
    ...
    if source == "slack":
        return {
            "kind": "slack_blocks",
            "blocks": [
                {"type": "section", "text": {"type": "mrkdwn", "text": "*Open OMJ*"}},
                {
                    "type": "actions",
                    "elements": [
                        {
                            "type": "button",
                            "action_id": "omj.open_picker",
                            "text": {"type": "plain_text", "text": "Open omj"},
                            "value": insert_text,
                        }
                    ],
                },
    ...
    if source == "telegram":
        return {
            ...
            "reply_markup": {
                "inline_keyboard": [[{"text": "Open omj", "callback_data": "omj.open_picker"}]],
            },
            "submit_text": insert_text,
        }
    if source == "hermes":
        return {
            ...
            "rows": [{"label": COMMAND_NAME, "submit_text": insert_text, "opens": "omj_skill_picker/v1"}],
        }
    return {
        "kind": "generic_button_card",
        "buttons": [{"label": "Open omj", "submit_text": insert_text}],
    }
...
def _telegram_button_for_option(option: dict[str, str]) -> dict[str, str]:
    return {"text": option["label"], "callback_data": f"omj.workflow:{option['value']}"}
