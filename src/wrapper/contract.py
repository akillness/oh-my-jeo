...
from ..paths import OmjPaths, resolve_paths
from ..plugin_bundle.omj.awareness import workflow_context_card_for_workflow, workflow_context_cards
...
SKILL_PICKER_SCHEMA_VERSION = "omj_skill_picker/v1"
COMMAND_PREVIEW_SCHEMA_VERSION = "omj_command_preview/v1"
CONTEXT_PRIMER_SCHEMA_VERSION = "omj_context_primer/v1"
USAGE_TRACE_SCHEMA_VERSION = "omj_usage_trace/v1"
WORKFLOW_EXPLANATION_SCHEMA_VERSION = "omj_workflow_explanation/v1"
MESSENGER_RENDERING_SCHEMA_VERSION = "omj_messenger_rendering/v1"
...
_SKILL_PICKER_ENTRIES = (
    ("oh-my-jeo", "Route for me", "Let Hermes choose the safest workflow.", "./omj <request>"),
    ...
    ("doctor", "Doctor", "Check OMJ install and Hermes registration health.", "./doctor"),
)
...
_STATUS_COPY = {
    ...
    "show_prompt_handoff": (
        ...
        "The selected prompt handoff is prepared, but OMJ has not dispatched it to an executor.",
        "Prompt handoff is prepared only; OMJ has not dispatched it to an executor.",
    ),
    "show_runtime_handoff": (
        "handoff",
...
_ACK_PRIMARY_ACTIONS_BY_NEXT_ACTION = {
    ...
    "run_omj_update": ("run_omj_update", "Run omj update"),
    "run_omj_setup": ("run_omj_setup", "Run omj setup"),
    "run_omj_doctor": ("run_omj_doctor", "Run omj doctor"),
    "run_omj_install": ("run_omj_install", "Run omj install"),
    "run_omj_list": ("run_omj_list", "Run omj list"),
...
def build_chat_interaction_payload(
    ...
    *,
    ...
    if _is_omj_intro_question(message) and resolved_mode in {"route", "plan", "clarify"}:
        base["mode"] = "route"
        base["route"] = _omj_intro_route_payload(route_payload)
        base["chat_response"] = build_chat_response_from_omj_context_brief(
            message,
            source=source,
            thread_key=str(base["thread_key"]),
        )
    ...
    if _is_omj_quickstart_question(message) and resolved_mode in {"route", "plan", "clarify"}:
        base["mode"] = "status"
        base["route"] = _omj_quickstart_route_payload(route_payload)
        base["chat_response"] = build_chat_response_from_omj_quickstart(
    ...
    if (
        _is_omj_status_question(message)
        and str(route_payload.get("selected_skill", "")) == _ROUTER_SKILL
        and resolved_mode in {"route", "plan", "clarify"}
    ):
        base["mode"] = "status"
        base["route"] = _omj_status_route_payload(route_payload)
        base["chat_response"] = build_chat_response_from_omj_status_roadmap(
            paths or resolve_paths(),
            thread_key=str(base["thread_key"]),
        )
    ...
    *,
    ...
    if action == "dispatch":
        ...
        if selected == "workflow-learning":
            ...
            headline = (
                "I can record this missed OMJ route."
    ...
    if action == "fallback" and _is_file_lookup_fallback(decision):
        return _chat_response(
            ...
            claim_boundary="No OMJ workflow, execution, or file inspection has started.",
            extra_state={
                "route_action": action,
                "confidence": decision.get("confidence", "low"),
    ...
    *,
    ...
    if target == "skill_candidate":
        ...
        body = (
            ...
            "OMJ has not run /learn, created a skill, or changed memory. "
            f"Candidate: {summary}"
        )
...
def build_chat_response_from_delegation(delegation_payload: dict[str, object], *, thread_key: str = "") -> dict[str, object]:
            ...
            actions.append(
                _action(
                    ...
                    payload={
                        ...
                        "disabled_reason": "A wrapper session id is required before OMJ can attach observed coding-session evidence.",
                    },
                )
            )
        actions.append(_action("show_status", "Show status", "secondary"))
    ...
    if action == "delegate" and delegation_payload.get("prompt_handoff"):
        ...
        return _chat_response(
            ...
            thread_key=thread_key,
            actions=actions,
            claim_boundary="Prompt handoff is prepared only; OMJ has not dispatched it to an executor.",
            extra_state={
                "delegation_action": action,
                "intent": delegation.get("intent", "unknown"),
                "selected_workflow": delegation.get("recommended_workflow", ""),
                ...
                "prepared_handoff_boundary": "Prompt handoff is prepared only; OMJ has not dispatched it to an executor.",
                "executor_readiness": delegation_payload.get("executor_readiness", {}),
                "executor_resolution": executor_resolution,
                "coding_status_request": status_request,
                "route_context": delegation_payload.get("route_context", {}),
    ...
    if action == "delegate" and delegation_payload.get("runtime_handoff"):
        ...
        return _chat_response(
            ...
            claim_boundary=(
                hermes_coding_team_claim_boundary()
                if selected == "hermes"
                else "Runtime handoff is prepared only; OMJ has not started Hermes, OMX, OMO, OMC, workers, tmux, or worktrees."
            ),
            extra_state={
                "delegation_action": action,
                "intent": delegation.get("intent", "unknown"),
...
def build_chat_response_from_omj_status_roadmap(paths: OmjPaths, *, thread_key: str = "") -> dict[str, object]:
    ...
    return _chat_response(
        kind="status",
        headline="Here is the current OMJ status and next action.",
        body="\n".join(body_lines),
        phase="status",
        ...
        extra_state={
            ...
            "workflow_explanation_reason": "The user asked for OMJ status or the next operational action, so the wrapper shows local probe and roadmap evidence instead of drafting a new plan.",
            "evidence_not_observed": [
...
def build_chat_response_from_omj_quickstart(
    paths: OmjPaths,
    *,
    source: str = "generic",
    thread_key: str = "",
    ...
    body_lines = [
        (
            f"OMJ quickstart is {card.get('status', 'unknown')}: "
        ...
        "",
        "Next in Hermes:",
        f"- {first_prompt}" if first_prompt else "- Ask Hermes what you want to do with OMJ.",
        "- Open the workflow picker with ./omj when you want to choose manually.",
    ...
    return _chat_response(
        kind="quickstart",
        headline="Here is the OMJ first-use path.",
        body="\n".join(body_lines),
        phase="status",
        ...
        extra_state={
            "status_source": "omj_quickstart",
            "quickstart_card": card,
            "capability_gap_roadmap": roadmap,
            "roadmap_next_actions": next_actions,
            "workflow_explanation_reason": "The user asked what to do after OMJ setup or install, so the wrapper shows the first-use quickstart card instead of asking for shell command approval.",
            "evidence_not_observed": observed_gaps,
        },
    )


def build_chat_response_from_omj_context_brief(
    message: str = "",
    *,
    source: str = "generic",
    thread_key: str = "",
    ...
    body_lines = [
        "OMJ is the Hermes workflow layer: install once, then ask Hermes in chat instead of memorizing backend commands.",
        "",
        "Use it for:",
    ...
    return _chat_response(
        kind="context_brief",
        headline="Here is how OMJ fits into Hermes.",
        body="\n".join(body_lines),
        phase="route",
        next_action="show_context_brief",
        thread_key=thread_key,
        actions=[
            _action("show_context_brief", "Show OMJ overview", "primary"),
            _action("show_skill_picker", "Open workflow picker", "secondary"),
            _action("show_quickstart", "Show quickstart", "secondary"),
        ],
        claim_boundary=str(brief.get("claim_boundary") or _default_overclaim_guard()),
        extra_state={
            "status_source": "omj_context_brief",
            "context_brief": brief,
            "workflow_explanation_reason": "The user asked what OMJ is or how to use it, so the wrapper explains the Hermes-facing mental model before opening the full picker.",
            "evidence_not_observed": [
                "workflow selection",
                "plan acceptance",
                "executor dispatch",
...
def build_status_card_from_status(status_payload: dict[str, Any]) -> dict[str, object]:
    ...
    if progress_events:
        card["progress_reporting"] = {
            "schema_version": "omj_progress_reporting/v1",
    ...
    updated.update(
        {
            ...
            "confidence": "high",
            "reason": "Catalog question; show the OMJ workflow picker instead of asking for shell command approval.",
            "recommendations": [
                {
                    "confidence": "high",
                    ...
                    "skill": _ROUTER_SKILL,
                    "wrapper_guidance": "Render the OMJ workflow picker in chat; do not ask the user to approve `omj list` for catalog discovery.",
                }
            ],
            "routing_instruction": "Show the OMJ workflow picker for this catalog question.",
            "routing_prompt_template": "Show the OMJ workflow picker for this catalog question.\n\nUser message:\n{message}",
            "score": max(10, _intish(updated.get("score", 0))),
...
def _omj_status_route_payload(route_payload: dict[str, object]) -> dict[str, object]:
    updated = dict(route_payload)
    harness = primary_harness_for_skill(_ROUTER_SKILL)
    updated.update(
        {
            ...
            "candidate_skill": _ROUTER_SKILL,
            "confidence": "high",
            "reason": "OMJ status or next-action question; show probe-backed status instead of drafting a plan.",
            "recommendations": [
                {
                    "confidence": "high",
                    ...
                    "matched": ["omj_status_question"],
                    "next_action": "show_status",
                    "score": max(10, _intish(updated.get("score", 0))),
                    "skill": _ROUTER_SKILL,
                    "wrapper_guidance": "Render the OMJ status and capability roadmap in chat; do not ask for shell approval just to answer status.",
                }
            ],
            "routing_instruction": "Show OMJ status and next actions from the local probe roadmap.",
            "routing_prompt_template": "Show OMJ status and next actions from the local probe roadmap.\n\nUser message:\n{message}",
            "score": max(10, _intish(updated.get("score", 0))),
            "selected_harness": harness,
            "selected_skill": _ROUTER_SKILL,
            "threshold": "high",
...
def _omj_quickstart_route_payload(route_payload: dict[str, object]) -> dict[str, object]:
    updated = dict(route_payload)
    harness = primary_harness_for_skill(_ROUTER_SKILL)
    updated.update(
        {
            ...
            "candidate_skill": _ROUTER_SKILL,
            "confidence": "high",
            "reason": "OMJ first-use or post-setup question; show the quickstart card instead of drafting a plan.",
            "recommendations": [
                {
                    "confidence": "high",
                    ...
                    "matched": ["omj_quickstart_question"],
                    "next_action": "show_quickstart",
                    "score": max(10, _intish(updated.get("score", 0))),
                    "skill": _ROUTER_SKILL,
                    "wrapper_guidance": "Render the OMJ quickstart card in chat; do not ask for shell approval just to answer first-use setup guidance.",
                }
            ],
            "routing_instruction": "Show the OMJ quickstart card and first-use Hermes prompts.",
            "routing_prompt_template": "Show the OMJ quickstart card and first-use Hermes prompts.\n\nUser message:\n{message}",
            "score": max(10, _intish(updated.get("score", 0))),
            "selected_harness": harness,
            "selected_skill": _ROUTER_SKILL,
            "threshold": "high",
...
def _omj_intro_route_payload(route_payload: dict[str, object]) -> dict[str, object]:
    updated = dict(route_payload)
    harness = primary_harness_for_skill(_ROUTER_SKILL)
    updated.update(
        {
            ...
            "candidate_skill": _ROUTER_SKILL,
            "confidence": "high",
            "reason": "OMJ intro or usage question; show the compact Hermes-facing mental model before the full picker.",
            "recommendations": [
                {
                    "confidence": "high",
                    ...
                    "matched": ["omj_intro_question"],
                    "next_action": "show_context_brief",
                    "score": max(10, _intish(updated.get("score", 0))),
                    "skill": _ROUTER_SKILL,
                    "wrapper_guidance": "Render the OMJ context brief first, then offer the workflow picker or quickstart card.",
                }
            ],
            "routing_instruction": "Show the OMJ context brief and offer the workflow picker as the next action.",
            "routing_prompt_template": "Show the OMJ context brief and offer the workflow picker as the next action.\n\nUser message:\n{message}",
            "score": max(10, _intish(updated.get("score", 0))),
            "selected_harness": harness,
            "selected_skill": _ROUTER_SKILL,
            "threshold": "high",
...
def _is_omj_intro_question(message: str) -> bool:
...
def _is_omj_quickstart_question(message: str) -> bool:
    text = message.strip().lower()
    if not text:
        return False
    omj_markers = ("omj", "oh-my-jeo", "oh my jeo")
...
def _is_omj_status_question(message: str) -> bool:
    text = message.strip().lower()
    if not text:
        return False
    omj_markers = ("omj", "oh-my-jeo", "oh my jeo")
...
def _roadmap_next_action_sentence(actions: list[dict[str, object]]) -> str:
    if not actions:
        return "Next: no blocking OMJ setup action is currently required."
    first = actions[0]
    label = str(first.get("label", "Check OMJ status")).strip() or "Check OMJ status"
...
def _command_preview_response(decision: dict[str, object], *, thread_key: str = "", message: str = "") -> dict[str, object]:
    ...
    insert_text = str(suggestions[0].get("insert_text", "./omj")) if suggestions else "./omj"
    return _chat_response(
        kind="command_preview",
        headline="Open OMJ.",
        body="Complete or choose `omj` to open the OMJ workflow picker. The preview shows one top-level entry instead of every installed workflow.",
...
def _command_preview_state(message: str, *, source: str) -> dict[str, object]:
    ...
    insert_text = f"{prefix}{_COMMAND_PREVIEW_ALIAS}" if prefix else "./omj"
    return {
        "schema_version": COMMAND_PREVIEW_SCHEMA_VERSION,
        "trigger": token,
        "source": source,
        ...
        "suggestions": [
            {
                ...
                "insert_text": insert_text,
                "description": "Open the OMJ workflow picker.",
                "opens_schema": SKILL_PICKER_SCHEMA_VERSION,
        ...
        "rendering_hints": {
            "discord": "When the user types `./`, show only `omj`; selecting it inserts `./omj` and opens the picker.",
            "slack": "When the user types `/`, show only `omj`; selecting it inserts `/omj` and opens the picker.",
            "telegram": "When the user sends `/` or `/om`, show the `omj` bot command or an Open omj inline button.",
            "hermes_tui": "Render one preview row: `omj` opens the OMJ workflow picker.",
        },
...
def _skill_picker_response(decision: dict[str, object], *, thread_key: str = "", message: str = "") -> dict[str, object]:
    ...
    extra_state = {
        ...
        "skill_picker": picker,
        "direct_invocation_aliases": ["./omj", "/omj", "./skills", "/skills"],
    }
    if catalog_question:
        extra_state["capability_summary"] = _catalog_capability_summary()
    return _chat_response(
        kind="skill_picker",
        headline="Here are the OMJ workflows." if catalog_question else "Choose an OMJ workflow.",
...
def _skill_picker_body(*, catalog_question: bool) -> str:
    ...
    if catalog_question:
        return "\n".join(
            [
                "You do not need to run a shell command for this. OMJ covers planning, ops, deliverables, coding handoffs, loops, and status.",
                "",
                "Start here:",
                "- Route for me: let Hermes choose the safest workflow from your message.",
                "- Choose workflow: pick from the OMJ capability families.",
...
def _context_primer_state() -> dict[str, object]:
    ...
    return {
        ...
        "summary": (
            "OMJ is a Hermes workflow layer. It helps Hermes route natural-language work into skills, plans, "
...
def _catalog_capability_summary() -> dict[str, object]:
    ...
    return {
        "schema_version": summary.get("schema_version", "omj_capability_summary/v1"),
