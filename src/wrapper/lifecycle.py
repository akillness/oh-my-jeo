    ...
    *,
    ...
    run = create_prepared_coding_delegation_run(
        paths,
        {
            ...
            "verification_summary": "prepared_not_observed; Codex work is not observed by omj",
        },
    )
...
def record_codex_dispatch(paths: OmjPaths, run_id: str) -> dict[str, object]:
...
def report_codex_delegation_lifecycle(paths: OmjPaths, run_id: str) -> dict[str, object]:
    status = summarize_delegated_coding_status(paths, run_id)
...
def _run_progress_status(paths: OmjPaths, run_id: str) -> dict[str, object]:
    try:
        binding = read_progress_binding(paths, "run", run_id)
    except (ExecutorProgressError, OSError, ValueError) as exc:
...
def _append_run_progress_warning(paths: OmjPaths, run_id: str, exc: Exception) -> None:
    try:
        append_event(
...
def _existing_run_dir(paths: OmjPaths, run_id: str) -> Path:
...
def _artifact_paths(paths: OmjPaths, run_id: str) -> dict[str, str]:
    run_dir = paths.runtime_runs_dir / run_id
    return {
