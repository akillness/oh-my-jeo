...
def read_executor_session(paths: OmjPaths, session_id: str) -> dict[str, Any] | None:
...
def _existing_session(paths: OmjPaths, session_id: str) -> dict[str, Any]:
...
def _merge_executor_session(paths: OmjPaths, session: dict[str, Any], patch: dict[str, Any]) -> dict[str, Any]:
    record = _build_executor_session_record(paths, session, patch)
    return _write_executor_session(paths, record)


def _build_executor_session_record(paths: OmjPaths, session: dict[str, Any], patch: dict[str, Any]) -> dict[str, Any]:
    session_id = str(session.get("session_id", ""))
    current = read_executor_session(paths, session_id) or _default_executor_session(session)
    merged = {
        **current,
...
def _write_executor_session(paths: OmjPaths, record: dict[str, Any]) -> dict[str, Any]:
...
def _executor_progress_status(paths: OmjPaths, session_id: str) -> dict[str, object]:
    try:
...
def _linked_status_result(paths: OmjPaths, session: dict[str, Any]) -> tuple[dict[str, Any], str | None]:
    run_id = str(session.get("current_run_id", ""))
    if not run_id:
        return {}, None
    try:
...
def _runtime_status(paths: OmjPaths, session: dict[str, Any]) -> dict[str, Any]:
    if session.get("status") != "runtime_handoff_prepared":
        return {}
...
def _require_no_observed_result(paths: OmjPaths, session: dict[str, Any]) -> None:
...
def _executor_launch_contract(
    ...
    *,
    ...
    return {
        ...
        "not_omj_backend_execution": True,
        "omj_execution_role": "contract_and_observation_only",
    ...
    return [
        {
            ...
            "when_to_use": "Use when this executor does not have a deterministic local launch command in OMJ.",
        }
    ]
...
def _display_status_lines(
    *,
    ...
    if status_blocker:
        lines.append(f"Action is blocked until OMJ can read valid evidence: {status_blocker}")
    return lines
...
def _executor_session_path(paths: OmjPaths, session_id: str) -> Path:
    return _session_dir(paths, session_id) / "executor_session.json"


def _session_dir(paths: OmjPaths, session_id: str) -> Path:
