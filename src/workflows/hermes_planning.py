...
def build_hermes_plan_payload(
    ...
    *,
    ...
    top = recommendations[0] if recommendations else {"skill": "oh-my-jeo", "score": 0, "confidence": "low"}
...
def write_hermes_plan(paths: OmjPaths, payload: dict[str, object]) -> dict[str, object]:
...
def _plan_for(task: str, top: dict[str, object]) -> HermesPlan:
    ...
    if weak:
        return HermesPlan(
            ...
            execution_handoff="Ask the smallest blocking clarification, then rerun `omj hermes plan` with the clarified task.",
    ...
    harness = "planning"
    handoff = (
        "Use `omj coding delegate --executor codex --record --from-plan <accepted-plan.md>` after this plan is accepted for a run-backed Codex handoff."
        if coding_shaped
    ...
    return HermesPlan(
        ...
        verification_plan=(
            "Run the targeted CLI test for `omj hermes plan`.",
