...
STAGED_GAP_MAP_STAGE_CONTRACT = {
    "implemented": "The OMJ feature surface exists in local skill, playbook, CLI, or artifact guidance; this is not observed host/runtime execution evidence.",
    ...
    "supported_elsewhere": "The capability is covered by a different OMJ surface and is not claimed by this scheduled ops blueprint.",
}
    ...
    *,
    ...
    record = {
        ...
        "optional_runtime_capabilities": [
            ...
            "Plugin hook status card if OMJ plugin is installed and observed by Hermes",
...
def write_hermes_ops_blueprint(paths: OmjPaths, record: dict[str, Any]) -> dict[str, Any]:
    errors = validate_hermes_ops_blueprint(record)
    if errors:
        raise ValueError("; ".join(errors))
...
def list_hermes_ops_blueprints(paths: OmjPaths, *, limit: int | None = None) -> list[dict[str, Any]]:
...
def show_hermes_ops_blueprint(paths: OmjPaths, blueprint_id: str) -> dict[str, Any]:
...
def validate_hermes_ops_store(paths: OmjPaths) -> dict[str, Any]:
    records: list[dict[str, Any]] = []
    errors: list[str] = []
    seen: set[str] = set()
    for blueprint_path in sorted(paths.hermes_ops_blueprints_dir.glob("*.json")):
...
def hermes_ops_blueprint_exists(paths: OmjPaths, blueprint_id: str) -> bool:
...
def _blueprint_path(paths: OmjPaths, blueprint_id: str) -> Path:
    if not _valid_blueprint_id(blueprint_id):
...
def _write_index_cache(paths: OmjPaths) -> None:
