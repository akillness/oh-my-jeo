...
USE_CASE_CATALOG_SCHEMA_VERSION = "omj_use_case_catalog/v1"
USE_CASE_DEMO_CARD_SCHEMA_VERSION = "omj_use_case_demo_card/v1"
USE_CASE_DEMO_COLLECTION_SCHEMA_VERSION = "omj_use_case_demo_collection/v1"
USE_CASE_RECOMMENDATION_SCHEMA_VERSION = "omj_use_case_recommendation/v1"
USE_CASE_VALIDATION_SCHEMA_VERSION = "omj_use_case_validation/v1"
USE_CASE_ARTIFACT_SCHEMA_VERSION = "omj_use_case_artifact/v1"
USE_CASE_ARTIFACT_COLLECTION_SCHEMA_VERSION = "omj_use_case_artifact_collection/v1"
USE_CASE_ARTIFACT_WRITE_SCHEMA_VERSION = "omj_use_case_artifact_write/v1"
USE_CASE_ARTIFACT_INDEX_SCHEMA_VERSION = "omj_use_case_artifact_index/v1"
USE_CASE_REPLAY_SCHEMA_VERSION = "omj_use_case_replay/v1"
USE_CASE_READINESS_SCHEMA_VERSION = "omj_use_case_readiness/v1"
...
USE_CASES: tuple[UseCase, ...] = (
    UseCase(
        ...
        hermes_chat_prompt="Use OMJ automation-blueprint for: Every morning, research competitor updates and send a digest only if something changed.",
        ...
        proof_surfaces=("omj ops blueprint", "omj playbook inspect scheduled-ops-blueprint", "omj harness inspect scheduled-ops-blueprint"),
    ...
    UseCase(
        ...
        current_gap="OMJ had handoff/status strength, but GitHub event recipes were not first-class.",
        ...
        hermes_chat_prompt="Use OMJ github-event-ops for: PR opened with failing CI; decide whether to review, label, or prepare a fix handoff.",
        ...
        proof_surfaces=("omj recommend", "omj playbook inspect github-event-ops", "omj harness inspect github-event-ops"),
    ...
    UseCase(
        ...
        current_gap="OMJ had target topology and team profiles, but not a Hermes board contract.",
        primary_skill="agent-board",
        playbook="agent-board",
        harness="agent-board",
        feature_surface="agent-board agent context: task/handoff/heartbeat/block/complete cards connected to OMJ status.",
        direct_skill_invocation="$agent-board Coordinate CTO, PM, QA, and release agents on this launch checklist.",
        hermes_chat_prompt="Use OMJ agent-board for: Coordinate CTO, PM, QA, and release agents on this launch checklist.",
        ...
        proof_surfaces=("omj recommend", "omj playbook inspect agent-board", "omj harness inspect agent-board"),
    ...
    UseCase(
        ...
        current_gap="OMJ had memory inspect, but not a deep Hermes MEMORY.md/USER.md/curator flow.",
        ...
        hermes_chat_prompt="Use OMJ memory-curation-review for: Inspect stale project memories and ask me what to keep.",
        ...
        proof_surfaces=("omj recommend", "omj playbook inspect memory-curation-review", "omj harness inspect memory-curation-review"),
    ...
    UseCase(
        ...
        current_gap="OMJ had wrapper contracts, but gateway-native target and delivery models were shallow.",
        ...
        hermes_chat_prompt="Use OMJ gateway-intent-card for: Route this Discord thread update silently unless action is needed.",
        next_action="prepare_gateway_intent_card",
        user_value="Gateway builders get a platform-neutral card Hermes can use without hardcoding a bot SDK into OMJ.",
        ...
        proof_surfaces=("omj recommend", "omj playbook inspect gateway-intent-card", "omj harness inspect gateway-intent-card"),
    ...
    UseCase(
        ...
        current_gap="OMJ had executor-neutral handoff, but runtime migration readiness was weak.",
        ...
        hermes_chat_prompt="Use OMJ executor-runtime-readiness for: Can this task run in Codex, Claude Code, or Hermes coding?",
        ...
        proof_surfaces=("omj recommend", "omj playbook inspect executor-runtime-readiness", "omj harness inspect executor-runtime-readiness"),
    ...
    UseCase(
        ...
        current_gap="OMJ had materials-package, but not a Hermes deliverable attachment UX lane.",
        ...
        hermes_chat_prompt="Use OMJ deliverable-package for: Turn this research into PPT and PDF with attachment status.",
        ...
        proof_surfaces=("omj recommend", "omj playbook inspect deliverable-package", "omj harness inspect deliverable-package"),
    ...
    UseCase(
        ...
        current_gap="OMJ had setup language UX, but no voice-first workflow guidance.",
        ...
        hermes_chat_prompt="Use OMJ voice-operator for: 'release before lunch, check risky parts' from mobile.",
        ...
        proof_surfaces=("omj recommend", "omj playbook inspect voice-operator", "omj harness inspect voice-operator"),
    ...
    UseCase(
        ...
        current_gap="OMJ setup had MCP preference, but workflow-specific MCP/tool recommendation and verification was weak.",
        ...
        hermes_chat_prompt="Use OMJ toolbelt-readiness for: What MCP or CLI tools do I need for weekly Linear and GitHub triage?",
        ...
        proof_surfaces=("omj recommend", "omj playbook inspect toolbelt-readiness", "omj harness inspect toolbelt-readiness"),
    ...
    UseCase(
        ...
        current_gap="OMJ evidence boundaries were strong, but runtime cost/latency telemetry contract was weak.",
        ...
        hermes_chat_prompt="Use OMJ ops-observability-card for: Show token, cost, latency, and last run status for this loop.",
        ...
        proof_surfaces=("omj recommend", "omj playbook inspect ops-observability-card", "omj harness inspect ops-observability-card"),
...
def demo_all_use_cases() -> dict[str, Any]:
    return {
        ...
        "boundary": (
            "Use-case demo cards are wrapper rendering artifacts. They prove OMJ "
...
def build_use_case_artifact(case_id: str) -> dict[str, Any]:
    ...
    artifact = {
        ...
        "proof_surfaces": [
            *case.proof_surfaces,
            f"omj cases demo {case.goal} --json",
            f"omj cases artifact {case.goal} --json",
            "omj cases validate --json",
        ],
        "evidence": {
        ...
        "release_quality": {
            ...
            "recommended_gate": "omj cases artifact --all --json",
        },
        "boundary": (
...
def write_use_case_artifact(paths: OmjPaths, artifact: dict[str, Any], *, force: bool = False) -> dict[str, Any]:
    errors = validate_use_case_artifact(artifact)
    if errors:
        raise ValueError("; ".join(errors))
...
def write_all_use_case_artifacts(paths: OmjPaths, *, force: bool = False) -> dict[str, Any]:
    written = []
...
def list_use_case_artifacts(paths: OmjPaths, *, limit: int | None = None) -> list[dict[str, Any]]:
...
def show_use_case_artifact(paths: OmjPaths, artifact_id: str) -> dict[str, Any]:
...
def validate_use_case_artifact(record: dict[str, Any]) -> list[str]:
    ...
    if record.get("schema_version") != USE_CASE_ARTIFACT_SCHEMA_VERSION:
        errors.append("schema_version must be omj_use_case_artifact/v1")
    ...
    if wrapper.get("component") != "omj_use_case_card":
        errors.append("wrapper_card.component must be omj_use_case_card")
...
def validate_use_case_artifact_store(paths: OmjPaths) -> dict[str, Any]:
    records: list[dict[str, Any]] = []
    errors: list[str] = []
    seen: set[str] = set()
    for artifact_path in sorted(paths.use_case_artifacts_dir.glob("*.json")):
    ...
    return {
        "schema_version": "omj_use_case_artifact_validation/v1",
        "ok": not errors,
...
def use_case_readiness(paths: OmjPaths | None = None) -> dict[str, Any]:
    ...
    if blocking_failures:
        next_actions.append("Fix blocking gates, then rerun `omj cases readiness`.")
    if any(gate["id"] == "local_artifact_store" and gate["status"] != "passed" for gate in gates):
        next_actions.append("Optional: write local runbook artifacts with `omj cases artifact --all --write`.")
    next_actions.append("Use `omj cases readiness --json` when a wrapper or release check needs the full payload.")
    return {
...
def validate_use_cases() -> dict[str, Any]:
    ...
    return {
        ...
        "errors": errors,
        "boundary": "Validation proves the 10 OMJ feature surfaces are routable and registered with the right exposure, playbook, harness, and invocation metadata; it is not proof that any external runtime action happened.",
    }
...
def _validate_demo_collection(payload: dict[str, Any]) -> list[str]:
    ...
    if payload.get("schema_version") != USE_CASE_DEMO_COLLECTION_SCHEMA_VERSION:
        errors.append("schema_version must be omj_use_case_demo_collection/v1")
    ...
    for card in cards:
        ...
        if card.get("schema_version") != USE_CASE_DEMO_CARD_SCHEMA_VERSION:
            errors.append(f"{prefix}: schema_version must be omj_use_case_demo_card/v1")
        ...
        if wrapper.get("component") != "omj_use_case_card":
            errors.append(f"{prefix}: wrapper_card.component must be omj_use_case_card")
        if wrapper.get("status") != "prepared_not_observed":
...
def _validate_artifact_collection(payload: dict[str, Any]) -> list[str]:
    ...
    if payload.get("schema_version") != USE_CASE_ARTIFACT_COLLECTION_SCHEMA_VERSION:
        errors.append("schema_version must be omj_use_case_artifact_collection/v1")
...
def _validate_replay_result(payload: dict[str, Any]) -> list[str]:
    ...
    if payload.get("schema_version") != USE_CASE_REPLAY_SCHEMA_VERSION:
        errors.append("schema_version must be omj_use_case_replay/v1")
...
def _local_artifact_store_gate(paths: OmjPaths | None) -> dict[str, Any]:
    if paths is None:
        return _readiness_gate(
            ...
            "not_checked",
            False,
            "Local artifact store was not checked because no OMJ paths were supplied",
            "omj cases artifact-validate --json",
        )
...
def _proof_surface_supported(surface: str, *, playbook_ids: set[str], harness_names: set[str]) -> bool:
    ...
    if clean in {"omj recommend", "omj ops blueprint"}:
        return True
    if clean.startswith("omj playbook inspect "):
        return clean.removeprefix("omj playbook inspect ") in playbook_ids
    if clean.startswith("omj harness inspect "):
        return clean.removeprefix("omj harness inspect ") in harness_names
    if clean.startswith("omj cases inspect "):
        return _find_case(clean.removeprefix("omj cases inspect ")) is not None
    return False
    ...
    return {
        ...
        "wrapper_card": {
            ...
            "sections": [
                ...
                {
                    "id": "what_omj_prepares",
                    "title": "What OMJ prepares",
        ...
        "actions": [
            ...
            },
            {
                ...
                "style": "secondary",
                "value": f"omj playbook inspect {case.playbook} --json",
            },
            {
                "id": "inspect_harness",
                ...
                "kind": "operator_command",
                "style": "secondary",
                "value": f"omj harness inspect {case.harness} --json",
            },
            {
                "id": "show_boundary",
        ...
        "operator_commands": [
            f"omj cases inspect {case.goal} --json",
            f"omj playbook inspect {case.playbook} --json",
            f"omj harness inspect {case.harness} --json",
        ],
        "evidence": {
            "state": "prepared_not_observed",
            "claim_boundary": case.evidence_boundary,
    ...
    return [
        {
            ...
            "value": f"omj cases inspect {case.goal} --json",
        },
        {
            "id": "render_demo_card",
            "kind": "operator_command",
            "label": "Render wrapper card",
            "value": f"omj cases demo {case.goal} --json",
        },
        {
            "id": "inspect_playbook",
            "kind": "operator_command",
...
def _use_case_artifact_path(paths: OmjPaths, artifact_id: str) -> Path:
    if not _valid_artifact_id(artifact_id):
...
def _write_use_case_artifact_index(paths: OmjPaths) -> None:
