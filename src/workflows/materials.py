...
MATERIAL_ARTIFACT_SCHEMA_VERSION = "omj_material_artifact/v1"
MATERIALS_INDEX_SCHEMA_VERSION = "omj_materials_index/v1"
...
def validate_material_artifact(record: dict[str, Any]) -> list[str]:
    ...
    if record.get("schema_version") != MATERIAL_ARTIFACT_SCHEMA_VERSION:
        errors.append("schema_version must be omj_material_artifact/v1")
...
def write_material_artifact(paths: OmjPaths, record: dict[str, Any]) -> dict[str, Any]:
    errors = validate_material_artifact(record)
    if errors:
        raise ValueError("; ".join(errors))
...
def list_material_artifacts(paths: OmjPaths, *, kind: str | None = None, limit: int | None = None) -> list[dict[str, Any]]:
    if kind is not None:
        validate_material_kind(kind)
...
def show_material_artifact(paths: OmjPaths, material_id: str) -> dict[str, Any]:
...
def material_artifact_exists(paths: OmjPaths, material_id: str) -> bool:
    if not _valid_material_id(material_id):
        return False
...
def validate_materials_store(paths: OmjPaths) -> dict[str, Any]:
    errors: list[str] = []
    records: list[dict[str, Any]] = []
    seen: set[str] = set()
    for material_path in sorted(paths.materials_dir.glob("*.json")):
    ...
    return {
        "schema_version": "omj_materials_validation/v1",
        "ok": not errors,
...
def export_materials_bundle(
    ...
    *,
    ...
    if format == "json":
        return {
            "schema_version": "omj_materials_export/v1",
            "kind": kind or "all",
...
def material_qa_ladder(formats: list[str] | None = None) -> dict[str, Any]:
    ...
    return {
        "schema_version": "omj_material_qa_ladder/v1",
        "formats": {
            item: {
...
def _material_path(paths: OmjPaths, material_id: str) -> Path:
    if not _valid_material_id(material_id):
