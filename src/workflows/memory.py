...
MEMORY_SCOPE_SCHEMA_VERSION = "omj_memory_scope/v1"
MEMORY_INDEX_SCHEMA_VERSION = "omj_memory_index/v1"
...
def build_project_memory_policy(paths: OmjPaths, *, mode: str | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "claim_boundary": "Project memory configures OMJ-local prepared context only; it does not mutate Hermes global or internal memory.",
    }


def read_project_memory_policy(paths: OmjPaths) -> dict[str, object]:
...
def build_project_memory_status(paths: OmjPaths) -> dict[str, object]:
    ...
    *,
    ...
    if not bool(policy.get("capture_enabled", True)):
        return {
            ...
            "claim_boundary": "Memory capture is disabled by OMJ project policy; Hermes global or internal memory is not mutated.",
        }
    ...
    return {
        ...
        "policy": policy,
        "claim_boundary": (
            "Captured project memory is an OMJ-local candidate or reviewed record only; "
...
def approve_project_memory_candidate(paths: OmjPaths, candidate_id: str, *, approved_by: str = "operator") -> dict[str, object]:
...
def reject_project_memory_candidate(
    ...
    *,
    ...
    return {
        ...
        "review": review,
        "claim_boundary": (
            "Rejected project memory is an OMJ-local review decision only; "
            "it is not execution, review, CI, merge, or Hermes internal-memory evidence."
        ),
    }
...
def build_project_memory_recall_pack(
    ...
    *,
    ...
    return {
        ...
        "redaction_policy": "metadata_only",
        "claim_boundary": (
            "Memory recall packs contain reviewed OMJ project summaries only; "
...
def build_memory_inspection(
    ...
    *,
    ...
    payload: dict[str, object] = {
        ...
        "redaction_policy": "metadata_only",
        "claim_boundary": (
            "Memory inspection reviews OMJ-local or wrapper-supplied context only; it is not proof that Hermes internal memory was read or changed."
        ),
    }
...
def apply_memory_update_batch(paths: OmjPaths, batch: dict[str, Any], *, dry_run: bool = False) -> dict[str, object]:
    ...
    return {
        ...
        "claim_boundary": "Approved updates write OMJ-local memory only; Hermes internal memory is not mutated.",
    }
...
def _build_project_memory_candidate(
    ...
    *,
    ...
    return {
        ...
        "safety": safety,
        "redaction_policy": "metadata_only",
        "claim_boundary": "Memory candidates are OMJ-local prepared context only; they are not approved memory or execution/review/CI/merge evidence.",
    }
...
def _record_from_candidate(candidate: dict[str, Any], *, approved_by: str, approved_at: str) -> dict[str, object]:
    ...
    return {
        ...
        "redaction_policy": "metadata_only",
        "claim_boundary": "Reviewed OMJ project memory is prepared context only; it is not execution, review, CI, merge, or Hermes internal-memory evidence.",
    }
...
def _read_project_memory_candidate(paths: OmjPaths, candidate_id: str) -> dict[str, Any] | None:
...
def _read_project_memory_candidates(paths: OmjPaths) -> list[dict[str, Any]]:
    return _read_memory_json_files(paths, _memory_candidates_dir(paths))


def _read_project_memory_records(paths: OmjPaths) -> list[dict[str, Any]]:
    records = []
...
def _read_project_memory_reviews(paths: OmjPaths) -> list[dict[str, Any]]:
    return _read_memory_json_files(paths, _memory_reviews_dir(paths))


def _read_memory_json_files(paths: OmjPaths, directory: Path) -> list[dict[str, Any]]:
...
def _write_project_memory_candidate(paths: OmjPaths, candidate: dict[str, object]) -> None:
    path = _memory_candidate_path(paths, str(candidate.get("candidate_id", "")))
    atomic_write_json(path, candidate, private=True)
    _write_memory_index(paths)


def _write_project_memory_record(paths: OmjPaths, record: dict[str, object]) -> None:
...
def _memory_candidates_dir(paths: OmjPaths) -> Path:
    return paths.memory_dir / "candidates"


def _memory_records_dir(paths: OmjPaths) -> Path:
    return paths.memory_dir / "records"


def _memory_reviews_dir(paths: OmjPaths) -> Path:
    return paths.memory_dir / "reviews"


def _memory_candidate_path(paths: OmjPaths, candidate_id: str) -> Path:
    if not _SAFE_REF.match(candidate_id):
        raise ValueError(f"unsafe memory candidate id: {candidate_id!r}")
...
def _memory_record_path(paths: OmjPaths, record_id: str) -> Path:
...
def _memory_review_path(paths: OmjPaths, candidate_id: str) -> Path:
    if not _SAFE_REF.match(candidate_id):
        raise ValueError(f"unsafe memory candidate id: {candidate_id!r}")
...
def _memory_snapshots(paths: OmjPaths) -> list[dict[str, object]]:
    snapshots: list[dict[str, object]] = []
    ...
    if reviewed_items:
        snapshots.append(_snapshot("omj_memory", _scope("project", "default"), reviewed_items))
    for path in _memory_scope_paths(paths):
        ...
        snapshots.append(_snapshot("omj_memory", data.get("scope", _scope("project", "default")), items))
    return snapshots


def _wrapper_session_snapshots(paths: OmjPaths, *, limit: int | None = None) -> list[dict[str, object]]:
        ...
        [
            {
                ...
                "key": "memory_boundary",
                "summary": "OMJ can inspect local state and wrapper snapshots; opaque Hermes memory requires explicit source evidence.",
            }
        ],
    )
...
def _prepare_update(paths: OmjPaths, update: Any, touched: dict[Path, dict[str, Any]]) -> dict[str, object]:
...
def _write_memory_index(paths: OmjPaths) -> None:
    ...
    atomic_write_json(
        paths.memory_index_path,
        {
            ...
            "claim_boundary": "OMJ local memory only; this index is not Hermes internal memory.",
        },
        private=True,
    )


def _safe_memory_files(paths: OmjPaths, directory: Path) -> list[Path]:
    if not directory.exists():
        return []
...
def _memory_scope_paths(paths: OmjPaths) -> list[Path]:
...
def _scope_path(paths: OmjPaths, scope: dict[str, str]) -> Path:
    kind = scope["kind"]
    ref = scope["ref"]
    if kind == "project":
...
def _assert_under_memory_root(paths: OmjPaths, path: Path) -> None:
    root = _memory_root(paths)
    candidate = path.resolve(strict=False)
    if root != candidate and root not in candidate.parents:
        raise ValueError(f"memory write path escapes .omj/memory: {path}")


def _memory_root(paths: OmjPaths) -> Path:
...
def _claim_boundary_for_source(source: str) -> str:
    return {
        ...
        "runtime_state": "Runtime state is an index of local OMJ activity, not execution/review/CI/merge evidence.",
        ...
        "omj_memory": "OMJ memory is user-approved local context only.",
