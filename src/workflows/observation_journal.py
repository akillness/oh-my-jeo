...
def append_observation_event(paths: OmjPaths, event: dict[str, Any]) -> dict[str, Any]:
...
def validate_observation_journal(paths: OmjPaths, *, run_id: str | None = None) -> dict[str, Any]:
    ...
    return {
        "schema_version": "omj_observation_journal_validation/v1",
...
def _prior_events_for_record(paths: OmjPaths, record: dict[str, Any]) -> list[dict[str, Any]]:
...
def _validate_observation_event_sequence(paths: OmjPaths, indexed_events: list[tuple[int, dict[str, Any]]]) -> list[str]:
    errors: list[str] = []
...
def _journal_review_required(paths: OmjPaths, run_id: str) -> bool:
    try:
