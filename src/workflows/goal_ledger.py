...
def _goal_dir(paths: OmjPaths, goal_id: str) -> Path:
...
def goal_ledger_path(paths: OmjPaths, goal_id: str) -> Path:
...
def _read_goal(paths: OmjPaths, goal_id: str) -> dict[str, Any]:
...
def _write_goal(paths: OmjPaths, goal: dict[str, Any]) -> dict[str, Any]:
...
def read_goal_ledger(paths: OmjPaths, goal_id: str) -> dict[str, Any]:
...
def list_goal_ledgers(paths: OmjPaths) -> list[dict[str, Any]]:
...
def build_goal_completion_gate(paths: OmjPaths, goal_id: str) -> dict[str, Any]:
    goal = read_goal_ledger(paths, goal_id)
    missing_required = [
        {
            "id": criterion["id"],
...
def build_goal_continuation(paths: OmjPaths, goal_id: str) -> dict[str, Any]:
    goal = read_goal_ledger(paths, goal_id)
...
def build_goal_status_card(paths: OmjPaths, goal_id: str) -> dict[str, Any]:
    goal = read_goal_ledger(paths, goal_id)
    gate = build_goal_completion_gate(paths, goal_id)
...
def _linked_runtime_check(paths: OmjPaths, run_id: str) -> dict[str, Any]:
...
def _delegated_runtime_status(paths: OmjPaths, run_id: str) -> dict[str, Any] | None:
    try:
...
def _goal_action_plan(goal_id: str, actions: list[str], status_card: dict[str, Any]) -> list[dict[str, str]]:
    commands = {
        "continue_goal": f"omj goal continue --goal {goal_id}",
        "show_status": f"omj goal status --goal {goal_id}",
        "record_checkpoint": f"omj goal checkpoint --goal {goal_id} --summary \"<summary>\" --criterion <AC-id> --evidence-ref <evidence>",
        "record_blocker": f"omj goal blocker --goal {goal_id} --summary \"<blocker>\" --evidence-ref <evidence>",
        "record_completion": f"omj goal complete --goal {goal_id} --evidence-ref <evidence>",
    }
    descriptions = {
