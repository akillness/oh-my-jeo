...
OPERATIONS_ARTIFACT_SCHEMA_VERSION = "omj_operation_artifact/v1"
OPERATIONS_INDEX_SCHEMA_VERSION = "omj_operations_index/v1"
...
def validate_operation_artifact(record: dict[str, Any]) -> list[str]:
    ...
    if record.get("schema_version") != OPERATIONS_ARTIFACT_SCHEMA_VERSION:
        errors.append("schema_version must be omj_operation_artifact/v1")
...
def write_operation_artifact(paths: OmjPaths, record: dict[str, Any]) -> dict[str, Any]:
    errors = validate_operation_artifact(record)
    if errors:
        raise ValueError("; ".join(errors))
...
def list_operation_artifacts(paths: OmjPaths, *, surface: str | None = None, limit: int | None = None) -> list[dict[str, Any]]:
...
def show_operation_artifact(paths: OmjPaths, artifact_id: str) -> dict[str, Any]:
...
def operation_artifact_exists(paths: OmjPaths, artifact_id: str) -> bool:
    if not _valid_artifact_id(artifact_id):
        return False
...
def validate_operations_store(paths: OmjPaths) -> dict[str, Any]:
    records: list[dict[str, Any]] = []
    errors: list[str] = []
    seen: set[str] = set()
    for surface in SURFACES:
    ...
    return {
        "schema_version": "omj_operations_validation/v1",
        "ok": not errors,
...
def export_operations_bundle(
    ...
    *,
    ...
    if format == "json":
        return {
            "schema_version": "omj_operations_export/v1",
...
def _artifact_path(paths: OmjPaths, surface: str, artifact_id: str) -> Path:
    if not _valid_artifact_id(artifact_id):
