...
OMJ_LOOP_COMMAND_RE = re.compile(r"^omj\s+(?:loop|루프)(?:\s|$)")
...
def loop_executor_capability(executor: str) -> dict[str, Any]:
    ...
    if selected == "claude-code":
        ...
        loop_mode = "omj_managed"
        dispatch_owner = "omj_wrapper"
        ...
        next_action = "continue_omj_managed_loop"
...
def read_loop_cycle(paths: OmjPaths, loop_id: str) -> dict[str, Any]:
...
def list_loop_cycles(paths: OmjPaths) -> list[dict[str, Any]]:
...
def explicit_loop_invocation_signal(goal_summary: str) -> bool:
    ...
    if LOOP_COMMAND_RE.match(normalized):
        return True
    if OMJ_LOOP_COMMAND_RE.match(normalized):
        return True
    return False
...
def _loop_invocation_payload(goal_summary: str) -> str:
    ...
    match = OMJ_LOOP_COMMAND_RE.match(normalized)
    if match:
        return normalized[match.end() :].strip()
    return normalized
...
def run_loop_once(paths: OmjPaths, loop_id: str) -> dict[str, Any]:
    ...
    return tick_loop_runtime(
        ...
        note=(
            ...
            "connector, executor, network, or code execution was performed by OMJ."
        ),
    )


def run_loop_once_result(paths: OmjPaths, loop_id: str) -> dict[str, Any]:
...
def list_loop_queue(paths: OmjPaths, loop_id: str, *, include_observed: bool = False) -> dict[str, Any]:
    cycle = read_loop_cycle(paths, loop_id)
    runtime = _runtime_state(cycle.get("runtime"))
    queue = [item for item in runtime.get("queue", []) if isinstance(item, dict)]
    visible = [
...
def inspect_loop_queue_item(paths: OmjPaths, loop_id: str, queue_id: str) -> dict[str, Any]:
    cycle = read_loop_cycle(paths, loop_id)
...
def build_loop_queue_handoff(paths: OmjPaths, loop_id: str, queue_id: str) -> dict[str, Any]:
    cycle = read_loop_cycle(paths, loop_id)
    item = _queue_item_ref(cycle, queue_id)[1]
...
def build_loop_cycle_narration(paths: OmjPaths, loop_id: str, queue_id: str = "") -> dict[str, Any]:
    cycle = read_loop_cycle(paths, loop_id)
    item: dict[str, Any] = {}
    if queue_id:
        item = _queue_item_ref(cycle, queue_id)[1]
...
def build_loop_status_card(paths: OmjPaths, loop_id: str) -> dict[str, Any]:
    cycle = read_loop_cycle(paths, loop_id)
    envelope = _dict_value(cycle, "authority_envelope")
...
def loop_cycle_path(paths: OmjPaths, loop_id: str) -> Path:
    return _loop_dir(paths, loop_id) / "cycle.json"


def _write_loop(paths: OmjPaths, cycle: dict[str, Any]) -> dict[str, Any]:
...
def _loop_dir(paths: OmjPaths, loop_id: str) -> Path:
...
def _runtime_queue_item(
    ...
    *,
    ...
    branch_hint = worktree_branch.strip() or f"omj-loop/{cycle.get('loop_id', 'loop')}/{planned_action}"
...
def _worktree_plan(cycle: dict[str, Any], planned_action: str, worktree_base: str, branch_hint: str) -> dict[str, Any]:
    ...
    loop_id = str(cycle.get("loop_id", "loop"))
    path_hint = f"{base.rstrip('/')}/omj-loop-{_slugify(loop_id)}-{_slugify(planned_action)}"
    return {
        ...
        "observed": False,
        "evidence_refs": [],
        "boundary": "OMJ records the worktree plan; an authorized wrapper or executor must create and observe it.",
    }
...
def _subagent_plan(
    ...
    return {
        ...
        "observed": False,
        "evidence_refs": [],
        "boundary": "OMJ prepares the subagent handoff; the wrapper/runtime records dispatch evidence separately.",
    }
...
def _connector_plan(connector: str, connector_action: str, planned_action: str) -> dict[str, Any]:
    ...
    return {
        ...
        "observed": False,
        "evidence_refs": [],
        "boundary": "OMJ records connector intent only; connector I/O requires a separate observed wrapper action.",
    }
...
def _queue_handoff_text(cycle: dict[str, Any], item: dict[str, Any]) -> str:
    ...
    lines = [
        f"Continue OMJ loop `{cycle.get('loop_id', '')}`.",
