...
def write_research_department_plan(paths: OmjPaths, record: dict[str, Any]) -> dict[str, Any]:
    errors = validate_research_department_plan(record)
    if errors:
        raise ValueError("; ".join(errors))
...
def list_research_department_plans(paths: OmjPaths, *, limit: int | None = None) -> list[dict[str, Any]]:
...
def show_research_department_plan(paths: OmjPaths, plan_id: str) -> dict[str, Any]:
...
def validate_research_department_store(paths: OmjPaths) -> dict[str, Any]:
    records: list[dict[str, Any]] = []
    errors: list[str] = []
    seen: set[str] = set()
    for plan_path in sorted(paths.research_department_plans_dir.glob("*.json")):
...
def research_department_plan_exists(paths: OmjPaths, plan_id: str) -> bool:
...
def _plan_path(paths: OmjPaths, plan_id: str):
    if not _valid_plan_id(plan_id):
