...
TRACE_REF_PREFIX = "omj-learning-trace"
EXPORT_REF_PREFIX = "omj-learning-export"
...
def write_learning_trace(paths: OmjPaths, trace: dict[str, Any]) -> dict[str, Any]:
    validate_workflow_learning_trace(trace)
...
def list_learning_traces(paths: OmjPaths, *, limit: int | None = None) -> list[dict[str, Any]]:
...
def show_learning_trace(paths: OmjPaths, trace_id: str) -> dict[str, Any]:
...
def write_workflow_eval(paths: OmjPaths, result: dict[str, Any]) -> dict[str, Any]:
    validate_workflow_eval_result(result)
...
def latest_workflow_eval_result(paths: OmjPaths, trace_id: str, *, rubric_id: str = "default") -> dict[str, Any] | None:
    matches = [
        result
...
def write_improvement_candidate(paths: OmjPaths, candidate: dict[str, Any]) -> dict[str, Any]:
    validate_improvement_candidate(candidate)
...
def show_improvement_candidate(paths: OmjPaths, candidate_id: str) -> dict[str, Any]:
...
def write_improvement_patch_proposal(paths: OmjPaths, proposal: dict[str, Any]) -> dict[str, Any]:
    ...
    case = {
        ...
        "fixture": {
            ...
            "privacy": {
                ...
                "redaction_provable_by_omj": False,
            },
        },
        "expected": expected,
        "replay_policy": {
        ...
        "claim_boundary": (
            ...
            "OMJ cannot prove that the fixture is redacted, and it must not be treated "
...
def write_regression_case(paths: OmjPaths, case: dict[str, Any]) -> dict[str, Any]:
    validate_regression_case(case)
...
def record_missed_route(
    ...
    *,
    ...
    trace = build_trace_from_chat_interaction(
        ...
        source_ref=source_ref,
        outcome="failed",
        feedback_summary="Operator reported that Hermes did not use the intended OMJ workflow.",
    )
    ...
    candidate = build_improvement_candidate(
        ...
        eval_result,
        target_type=target_type,
        title=title or "Review missed OMJ workflow route",
    )
    if not dry_run:
...
def replay_regression_cases(paths: OmjPaths, *, limit: int | None = None) -> dict[str, Any]:
...
def check_learning_index(paths: OmjPaths) -> dict[str, Any]:
...
def rebuild_learning_index(paths: OmjPaths, *, dry_run: bool = False) -> dict[str, Any]:
    scanned = _scan_learning_records(paths)
...
def write_learning_export(paths: OmjPaths, bundle: dict[str, Any]) -> dict[str, Any]:
    validate_workflow_learning_export(bundle)
...
def build_workflow_learning_audit(paths: OmjPaths, *, limit: int | None = 20) -> dict[str, Any]:
    scanned = _scan_learning_records(paths)
...
def build_workflow_learning_review_queue(paths: OmjPaths, *, limit: int | None = 20) -> dict[str, Any]:
    scanned = _scan_learning_records(paths)
    candidates = _read_valid_learning_records(paths.learning_candidates_dir, validate_improvement_candidate)
    patch_proposals = _read_valid_learning_records(paths.learning_patch_proposals_dir, validate_improvement_patch_proposal)
    ...
    for invalid in _list(scanned.get("invalid_records")):
        ...
        entries.append(
            _learning_review_queue_entry(
                ...
                next_command="omj learning index check",
...
def trace_path(paths: OmjPaths, trace_id: str) -> Path:
    return paths.learning_traces_dir / f"{_safe_id(trace_id)}.json"


def eval_path(paths: OmjPaths, eval_id: str) -> Path:
    return paths.learning_evals_dir / f"{_safe_id(eval_id)}.json"


def candidate_path(paths: OmjPaths, candidate_id: str) -> Path:
    return paths.learning_candidates_dir / f"{_safe_id(candidate_id)}.json"


def patch_proposal_path(paths: OmjPaths, proposal_id: str) -> Path:
    return paths.learning_patch_proposals_dir / f"{_safe_id(proposal_id)}.json"


def regression_case_path(paths: OmjPaths, case_id: str) -> Path:
    return paths.learning_regressions_dir / f"{_safe_id(case_id)}.json"


def learning_export_path(paths: OmjPaths, export_id: str) -> Path:
...
def _patch_proposal_steps(status: str) -> list[dict[str, str]]:
    return [
        ...
        _improvement_review_step(
            ...
            "Use the proposal as handoff material; OMJ has not edited files.",
        ),
    ]
...
def _select_export_traces(paths: OmjPaths, *, trace_ids: list[str], limit: int | None) -> list[dict[str, Any]]:
    if trace_ids:
        traces = []
...
def _read_regression_cases(paths: OmjPaths) -> list[dict[str, Any]]:
...
def _read_related_regression_cases(paths: OmjPaths, trace_id: str) -> list[dict[str, Any]]:
    return sorted(
        ...
        issues.append(
            {
                ...
                "next_action": "omj learning index rebuild",
            }
        )
        ...
        issues.append(
            {
                ...
                "next_action": "omj learning candidate <trace-id>",
            }
        )
    return issues
        ...
        warnings.append(
            {
                ...
                "next_action": "omj learning eval <trace-id>",
            }
        )
        ...
        warnings.append(
            {
                ...
                "next_action": "omj learning regression add <trace-id> --fixture-message <redacted fixture>",
            }
        )
        ...
        warnings.append(
            {
                ...
                "next_action": "omj learning export",
            }
        )
    return warnings
...
def _learning_audit_next_actions(
    *,
    ...
    if index_check.get("ok") is not True:
        actions.append({"id": "rebuild_learning_index", "command": "omj learning index rebuild"})
    if missing_eval:
        actions.append({"id": "evaluate_trace", "trace_id": missing_eval[0], "command": f"omj learning eval {missing_eval[0]}"})
    if missing_regression:
        actions.append(
            {
                "id": "add_regression_case",
                "trace_id": missing_regression[0],
                "command": f"omj learning regression add {missing_regression[0]} --fixture-message <redacted fixture>",
            }
        )
    if missing_candidate:
        actions.append({"id": "create_candidate", "trace_id": missing_candidate[0], "command": f"omj learning candidate {missing_candidate[0]}"})
    if pending_candidates:
        actions.append(
            {
                "id": "review_candidate",
    ...
    if not exports and status != "no_records":
        actions.append({"id": "write_learning_export", "command": "omj learning export"})
    if not actions:
        actions.append({"id": "continue_recording", "command": "omj learning record <message or --from-runtime-run>"})
    return actions
    ...
    *,
    ...
    if decision == "pending":
        return _learning_review_queue_entry(
            ...
            next_command=f"omj learning review-candidate {candidate_id} --decision approve|revise|reject",
            refs=[
    ...
    if decision == "revise":
        return _learning_review_queue_entry(
            ...
            next_command=f"omj learning candidate {trace_id} --target-type {candidate.get('target_type', 'workflow_rubric')}",
            refs=[
                _record_ref("candidate", candidate_id),
                learning_trace_ref(trace_id),
                _record_ref("eval", eval_id),
    ...
    if decision == "approve":
        if latest_proposal is None:
            return _learning_review_queue_entry(
                ...
                next_command=f"omj learning proposal {candidate_id}",
                refs=[
                    _record_ref("candidate", candidate_id),
                    learning_trace_ref(trace_id),
                    _record_ref("eval", eval_id),
...
def _patch_proposal_queue_entry(proposal: dict[str, Any], *, candidate_title: str = "") -> dict[str, Any]:
    ...
    next_command = {
        "review_improvement": f"omj learning review-candidate {proposal.get('candidate_id', '')} --decision approve|revise|reject",
        "add_regression_case": f"omj learning regression add {proposal.get('trace_id', '')} --fixture-message <redacted fixture>",
        "replay_regression_cases": "omj learning regression replay",
        "copy_patch_handoff": f"omj learning proposal {proposal.get('candidate_id', '')}",
    }.get(primary_action, "omj learning audit")
    return _learning_review_queue_entry(
        kind="patch_proposal",
        status=status,
        severity=severity,
...
def _records_from_index(
    paths: OmjPaths,
    kind: str,
    path_for_id: Callable[[OmjPaths, str], Path],
    validator: Callable[[dict[str, Any]], None],
) -> list[dict[str, Any]]:
...
def _update_learning_index(paths: OmjPaths, record: dict[str, Any], kind: str) -> None:
...
def _scan_learning_records(paths: OmjPaths) -> dict[str, Any]:
    specs = [
...
def _learning_index_check_payload(
    ...
    *,
    ...
    return {
        ...
        "repair_hint": (
            "Run `omj learning index rebuild` to rewrite the local learning index."
...
def _record_ref(kind: str, identifier: str) -> str:
    ...
    return f"omj-learning-{kind}:{identifier}"
