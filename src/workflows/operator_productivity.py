...
PROJECTION_SOURCE_REFS = (
    "src/workflows/operator_productivity.py",
...
def write_agent_operator_productivity_card(paths: OmjPaths, record: dict[str, Any]) -> dict[str, Any]:
...
def list_agent_operator_productivity_cards(paths: OmjPaths, *, limit: int | None = None) -> list[dict[str, Any]]:
...
def show_agent_operator_productivity_card(paths: OmjPaths, card_id: str) -> dict[str, Any]:
...
def validate_agent_operator_productivity_store(paths: OmjPaths) -> dict[str, Any]:
    errors: list[str] = []
    count = 0
    for card_path in sorted(paths.agent_operator_productivity_cards_dir.glob("*.json")):
...
def _write_index_cache(paths: OmjPaths) -> dict[str, Any]:
...
def _skill_chain(focus: str) -> list[dict[str, str]]:
    chains = {
        ...
        "mixed": ("oh-my-jeo", "executor-runtime-readiness", "web-research", "code-review", "ops-observability-card"),
    }
...
def _card_path(paths: OmjPaths, card_id: str):
