...
COMMAND_PATH_SCHEMA_VERSION = "omj_command_path/v1"
INSTALLED_COMMAND_PATH_CHECK_SCHEMA = "installed_omj_path_check/v1"
COMMAND_PATH_FOUND_NEXT_ACTION = "Run `omj doctor` after setup to verify Hermes registration."
COMMAND_PATH_MISSING_NEXT_ACTION = "Use the absolute command path printed by the installer, or add that directory to PATH."


def inspect_omj_command_path(command: str = "omj") -> dict[str, object]:
...
def installed_command_path_check_plan(omj_command: str) -> dict[str, object]:
    command = _normalized_command(omj_command)
    return {
...
def inspect_installed_command_path(omj_command: str) -> dict[str, object]:
    command = _normalized_command(omj_command)
    check = path_check_kind(command)
...
def _normalized_command(command: str) -> str:
    return str(command or "omj").strip() or "omj"
