...
def install_script_smoke_plan(
    *,
    ...
    command_under_test = str(paths["bin_dir"] / "omj")
    ...
    first_use_status = first_use_status_smoke_plan(
        omj_command=command_under_test,
        omj_home=paths["home"] / ".omj",
    ...
    return {
        ...
        "steps": [
            ...
            },
            {
                ...
                "proof_boundary": (
                    ...
                    "This still does not prove live Hermes chat selected OMJ."
                ),
            },
        ],
        ...
        "recommended_next_action": (
            ...
            if script.is_file()
            else "Run from the OMJ source checkout or pass --install-script pointing at install.sh."
        ),
        "proof_boundary": (
...
def run_install_script_smoke(
    *,
    ...
    if keep_work_dir:
        tmp = Path(tempfile.mkdtemp(prefix="omj-install-smoke-")).resolve()
        return _run_install_script_smoke_in_dir(
            repo_root=repo_root,
            install_script=install_script,
            package_url=package_url,
    ...
    with tempfile.TemporaryDirectory(prefix="omj-install-smoke-") as tmp:
        return _run_install_script_smoke_in_dir(
            repo_root=repo_root,
            install_script=install_script,
            package_url=package_url,
    ...
    *,
    ...
    if not bool(plan["install_script_found"]):
        return {
            **plan,
            ...
            "recommended_next_action": "Run from the OMJ source checkout or pass --install-script pointing at install.sh.",
        }
    ...
    if ok:
        ...
        command_smoke = run_installed_command_smoke(
            omj_command=str(plan["command_under_test"]),
            omj_home=paths["home"] / ".omj",
            hermes_home=paths["home"] / ".hermes",
            timeout_seconds=timeout_seconds,
            runner=smoke_runner,
...
def _install_smoke_env(
    *,
    ...
    return {
        "HOME": str(home),
        "OMJ_HOME": str(home / ".omj"),
        "HERMES_HOME": str(home / ".hermes"),
        "PATH": f"{bin_dir}{os.pathsep}{path}" if path else str(bin_dir),
        "NO_COLOR": "1",
        "OMJ_CHANNEL": "local",
        "OMJ_SOURCE_REF": "install-smoke-local",
        "OMJ_PACKAGE_URL": str(package_url),
        "OMJ_PYTHON": str(python_command),
        "OMJ_INSTALL_MODE": "venv",
        "OMJ_VENV_DIR": str(venv_dir),
        "OMJ_BIN_DIR": str(bin_dir),
        "OMJ_LINK_COMMAND": "1",
        "OMJ_FORCE_LINK": "1",
        ...
        "OMJ_LANG": "en",
        "OMJ_SETUP_ARGS": " ".join(str(item) for item in setup_args),
    }
...
def _install_script_smoke_next_action(ok: bool, failed_step: str) -> str:
    if ok:
        return (
            "Run the smoke-installed `omj setup` explicitly if you want profile registration evidence, then restart or "
    ...
    if failed_step == "install_script_missing":
        return "Run from the OMJ source checkout or pass --install-script pointing at install.sh."
    ...
    if failed_step == "installed_command_smoke":
        return "Repair the smoke-installed `omj` command path or setup-plan rendering, then rerun release install-smoke."
