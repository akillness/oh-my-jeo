...
from ..core.errors import OmjError
...
def _write_skill(skills_dir: Path, template: SkillTemplate, force: bool = False, managed: bool = False) -> None:
    ...
    if target_file.exists() and not force and not managed:
        ...
        if existing != template.content:
            raise OmjError(f"local skill differs, refusing to overwrite without --force: {target_file}")
...
def _write_skill_reference(
    ...
    if target_file.exists() and not force and not managed:
        existing = target_file.read_text(encoding="utf-8")
        if existing != template.content:
            raise OmjError(f"local skill reference differs, refusing to overwrite without --force: {target_file}")
    atomic_write_text(target_file, template.content)
...
def install_skill_pack(
    ...
    *,
    ...
    if modified and not force:
        raise OmjError("local modifications detected; rerun with --force or resolve: " + ", ".join(modified))
    if dry_run:
        return {
            "dry_run": True,
...
def uninstall_skill_pack(
    ...
    *,
    ...
    """Remove OMJ-managed local files without deleting unrelated Hermes state."""
    removed: list[str] = []
    ...
    if remove_files or remove_all:
        _collect_removal(paths.omj_home, removed=removed, would_remove=would_remove, kept=kept, dry_run=dry_run, force=True)
    ...
    return {
        "schema_version": "omj_uninstall/v1",
...
def _collect_removal(
    ...
    *,
    ...
    if managed_plugin and not force and not _looks_like_managed_plugin(path):
        kept.append({"path": str(path), "reason": "plugin dir is not an OMJ-managed bundle; rerun with --force to remove it"})
        return
    if dry_run:
...
def _looks_like_managed_plugin(path: Path) -> bool:
    return (path / ".omj-plugin-manifest.json").exists()
...
def _collect_command_package_removal(
    *,
    ...
    if venv_dir is None:
        kept.append({"path": "omj", "reason": "HOME is not available, so the install.sh-managed command venv cannot be located"})
        return
...
def _managed_command_links(venv_dir: Path) -> list[Path]:
    ...
    explicit_bin = os.environ.get("OMJ_BIN_DIR")
    home = os.environ.get("HOME")
    if explicit_bin:
        candidates.append(Path(explicit_bin).expanduser() / "omj")
    elif home:
        candidates.append(Path(home).expanduser() / ".local" / "bin" / "omj")
...
def _managed_team_profile_files(paths: OmjPaths) -> list[Path]:
...
def _manifest_team_profile_files(paths: OmjPaths) -> list[Path]:
...
def _legacy_managed_team_profile_files(paths: OmjPaths) -> list[Path]:
    if not paths.hermes_agents_dir.exists():
        return []
    files: list[Path] = []
    for path in sorted(paths.hermes_agents_dir.glob("omj-*.md")):
        if not path.is_file():
            continue
        try:
            ...
            continue
        if "schema_version: omj_team_profile_pack/v1" in text:
            files.append(path)
    return files
