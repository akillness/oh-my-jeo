...
def _hermes_agent_rows(paths: OmjPaths) -> list[dict[str, Any]]:
...
def _external_executor_rows(paths: OmjPaths, *, limit: int) -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
...
def _recent_run_ids(paths: OmjPaths, *, limit: int) -> list[str]:
...
def _run_coding_record(paths: OmjPaths, run_id: str) -> dict[str, Any]:
    try:
...
def _versions(hud: dict[str, Any]) -> dict[str, Any]:
    omj_version = str(hud.get("version", "") or "unknown")
    return {
        "omj": {
            "value": omj_version,
            "label": f"OMJ version: {omj_version}",
            "observed": omj_version != "unknown",
        },
        "hermes": {
            "value": "unknown",
...
def _display(
    ...
    headline = "OMJ ready" if settings["omj_connection"]["value"] == "ready" else "OMJ needs attention"
...
def _evidence_next_action(current_executor_row: dict[str, Any]) -> str:
    ...
    return "open Hermes or run omj doctor"
    ...
    *,
    ...
    return {
        ...
        "claim_boundary": (
            ...
            "Plain `omj menubar status` does not scan processes or infer PID state."
        ),
    }
...
def _plugin_status_label(status: str) -> str:
    labels = {
        "ready": "OMJ connection: Ready",
        "stale": "OMJ connection: Update needed",
        "installed": "OMJ connection: Installed",
        "missing": "OMJ connection: Not installed",
