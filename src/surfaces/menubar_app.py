...
def setup_menubar_app(
    ...
    *,
    ...
    resolved_command = command_path or _resolved_omj_command()
    if not resolved_command:
        return {**base_payload, "reason": "`omj` command path is not available yet; rerun setup after the command is on PATH."}
...
def start_menubar_app(paths: OmjPaths, *, platform_name: str | None = None) -> dict[str, Any]:
    system = platform_name or platform.system()
    app_paths = menubar_app_paths(paths)
    payload = {
        "schema_version": MENUBAR_APP_SCHEMA_VERSION,
    ...
    if not app_paths["launch_agent"].exists():
        return {**payload, "status": "missing", "message": "LaunchAgent is not installed; run `omj menubar install`."}
    ...
    return {**payload, "status": "running", "started": True, "message": "OMJ menu bar helper started."}


def stop_menubar_app(paths: OmjPaths, *, platform_name: str | None = None) -> dict[str, Any]:
    system = platform_name or platform.system()
    app_paths = menubar_app_paths(paths)
    payload = {
        "schema_version": MENUBAR_APP_SCHEMA_VERSION,
    ...
    return {**payload, "status": "stopped", "stopped": True, "message": "OMJ menu bar helper stopped."}


def uninstall_menubar_app(paths: OmjPaths, *, dry_run: bool = False, platform_name: str | None = None) -> dict[str, Any]:
    system = platform_name or platform.system()
    app_paths = menubar_app_paths(paths)
...
def menubar_app_paths(paths: OmjPaths) -> dict[str, Path]:
    app_dir = paths.omj_home / "menubar"
    return {
        "app_dir": app_dir,
        "source": app_dir / "OMJMenuBar.swift",
        "executable": app_dir / "omj-menubar",
        "launch_agent": Path.home() / "Library" / "LaunchAgents" / f"{MENUBAR_LABEL}.plist",
    }


def _resolved_omj_command() -> str:
    command = inspect_omj_command_path()
    if command.get("found") and command.get("path"):
        return str(command["path"])
    which = shutil.which("omj")
    return which or ""
...
def _write_launch_agent(plist_path: Path, executable: Path, omj_command: str, paths: OmjPaths) -> None:
...
final class OMJMenuBarDelegate: NSObject, NSApplicationDelegate {
    private let statusItem = NSStatusBar.system.statusItem(withLength: NSStatusItem.variableLength)
    private var timer: Timer?
    private var omjCommand = "omj"
    private var omjHome = ""
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        parseArguments()
        configureMenu(headline: "OMJ", summary: "Loading status", cards: [])
        refresh()
    ...
    private func refresh() {
        guard let payload = readStatusPayload() else {
            statusItem.button?.title = "omj !"
            configureMenu(
                headline: "OMJ needs attention",
                summary: "Status unavailable",
                cards: [
                    [
                        "title": "Recovery",
                        "rows": [
                            ["label": "Next", "value": "Run omj doctor"],
                            ["label": "Then", "value": "Run omj setup if registration needs repair"]
                        ]
                    ]
                ]
            )
        ...
        let title = (display?["menu_title"] as? String) ?? "omj"
        let headline = (display?["headline"] as? String) ?? "OMJ ready"
        let summary = (display?["summary_line"] as? String) ?? "OMJ ready"
        let connection = ((settings?["omj_connection"] as? [String: Any])?["value"] as? String) ?? "unknown"
    ...
    private func configureMenu(headline: String, summary: String, cards: [[String: Any]]) {
        ...
        menu.addItem(NSMenuItem(title: "Quit OMJ Menu Bar", action: #selector(quitClicked(_:)), keyEquivalent: "q"))
    ...
    private func readStatusPayload() -> [String: Any]? {
        let process = Process()
        process.executableURL = URL(fileURLWithPath: omjCommand)
        var args: [String] = []
        if !omjHome.isEmpty {
            args.append(contentsOf: ["--omj-home", omjHome])
        }
...
let delegate = OMJMenuBarDelegate()
