...
QUICKSTART_CARD_SCHEMA_VERSION = "omj_quickstart_card/v1"


def build_quickstart_card(paths: OmjPaths, *, source: str = "hermes") -> dict[str, object]:
    ...
    return {
        ...
        "first_five_minutes": [
            ...
            "Ask Hermes what OMJ can do, or paste a plain request and let Hermes route it.",
        ...
        "chat_prompts": [
            {
                ...
                "prompt": "Use OMJ request-to-handoff for: I want to safely add a feature to this repo.",
                "expected_workflow": "request-to-handoff",
            },
            {
                "label": "image summary card",
                "prompt": "Use OMJ img-summary for: summarize this PR as a shareable image card.",
                "expected_workflow": "img-summary",
            },
            {
                "label": "ambitious loop",
                "prompt": "Use OMJ loop for: improve this repo's first-run experience until the next bottleneck is verified.",
        ...
        "wrapper_actions": [
            {
                ...
                "label": "Open OMJ picker",
                "surface": str(source or "hermes"),
                "copy": "Show ./omj or the compact OMJ workflow picker.",
                "records_evidence": False,
            },
            {
                "id": "record_wrapper_usage",
                "label": "Record wrapper usage when Hermes uses OMJ",
                "surface": str(source or "hermes"),
                "copy": "Record route/session metadata only after the wrapper has actually used an OMJ workflow.",
...
def _plugin_bridge_status(probe: dict[str, object], capabilities: dict[str, dict[str, object]]) -> dict[str, object]:
    bundle = _capability_status(capabilities, "omj_plugin_bundle")
