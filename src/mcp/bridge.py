...
MCP_BRIDGE_SCHEMA_VERSION = "omj_mcp_bridge/v1"
MCP_TOOL_RESULT_SCHEMA_VERSION = "omj_mcp_tool_result/v1"
MCP_OBSERVATION_SCHEMA_VERSION = "omj_mcp_observation/v1"
MCP_HOST_SESSION_SCHEMA_VERSION = "omj_mcp_host_session/v1"
MCP_HOST_CONFIG_RECIPE_SCHEMA_VERSION = "omj_mcp_host_config_recipe/v1"
...
MCP_BRIDGE_CLAIM_BOUNDARY = (
    "The OMJ MCP bridge exposes allowlisted local status, recommendation, and probe tools only. "
...
MCP_HOST_SESSION_CLAIM_BOUNDARY = (
    "An OMJ MCP host session record is metadata supplied by a host or wrapper that observed bridge load/use. "
    ...
    return [
        {
            "name": "omj_status",
            "title": "OMJ Status",
            "description": "Return the compact OMJ HUD/status payload for Hermes-facing status narration.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "preset": {
            ...
            "outputSchema": _tool_result_schema("omj_status_result/v1"),
        },
        {
            "name": "omj_recommend",
            "title": "OMJ Workflow Recommendation",
            "description": "Recommend OMJ workflows for a natural-language operator request without storing the raw request.",
            "inputSchema": {
                "type": "object",
                "properties": {
            ...
            "outputSchema": _tool_result_schema("omj_recommend_result/v1"),
        },
        {
            "name": "omj_probe",
            "title": "OMJ Capability Probe",
            "description": "Return local OMJ capability probe data, optionally with the parity matrix and capability roadmap.",
            "inputSchema": {
                "type": "object",
                "properties": {
...
def _mcp_server_args(paths: OmjPaths, *, include_absolute_homes: bool) -> list[str]:
    ...
    if include_absolute_homes:
        args.extend(["--omj-home", str(paths.omj_home), "--hermes-home", str(paths.hermes_home)])
...
def build_mcp_manifest(paths: OmjPaths, *, command: str = "omj", include_absolute_homes: bool = True) -> dict[str, Any]:
    ...
    return {
        ...
        "setup": {
            ...
            "normal_hermes_surface": "Use installed OMJ skills in Hermes chat first; use this MCP bridge only when the host supports MCP tools.",
        },
...
def build_mcp_host_config_recipe(
    ...
    *,
    ...
    if host == "generic":
        snippet: dict[str, Any] | str = {"mcpServers": {"omj": generic_server}}
        ...
        apply_steps = [
            ...
            "Merge the mcpServers.omj entry into the existing config.",
            "Restart or reload the host, then record observed load with omj mcp observe-host.",
        ]
        source_urls = ["https://modelcontextprotocol.io"]
    elif host == "claude-code":
        snippet = {"mcpServers": {"omj": typed_stdio_server}}
        snippet_text = _pretty_json(snippet)
        config_format = "json"
        ...
        apply_steps = [
            ...
            "Open Claude Code and approve the project MCP server if prompted, then record observed load with omj mcp observe-host.",
        ]
        ...
        apply_steps = [
            "Merge the [mcp_servers.omj] table into the target Codex config.toml.",
            "Restart or reload Codex so it can start the stdio MCP server.",
            "Record observed load or tool use with omj mcp observe-host after the host actually uses the bridge.",
        ]
        source_urls = [
        ...
        apply_steps = [
            "Merge the mcp.omj entry into the OpenCode config.",
            "Restart or reload OpenCode so it can start the local MCP server.",
            "Record observed load or tool use with omj mcp observe-host after OpenCode actually connects.",
        ]
        ...
        apply_steps = [
            ...
            "Merge the mcpServers.omj entry into the existing config.",
            "Reload Cursor and record observed load or tool use with omj mcp observe-host after it actually connects.",
        ]
    ...
    return {
        ...
        "verify": {
            ...
            "boundary": "A pasted host config is not evidence that the host loaded OMJ; record a host observation only after the host actually loads or uses the bridge.",
        },
...
def read_mcp_host_sessions(paths: OmjPaths, *, limit: int | None = 20) -> tuple[list[dict[str, Any]], list[str]]:
...
def latest_observed_mcp_host_session(paths: OmjPaths) -> tuple[dict[str, Any] | None, list[str]]:
    records, errors = read_jsonl_objects(paths.runtime_mcp_host_sessions_path)
...
def run_stdio_mcp_server(paths: OmjPaths, *, stdin: TextIO | None = None, stdout: TextIO | None = None, stderr: TextIO | None = None) -> int:
    ...
    for line in input_stream:
        ...
        try:
            ...
            print(f"omj mcp bridge error: {exc}", file=error_stream)
...
def _append_mcp_host_session(paths: OmjPaths, record: dict[str, Any]) -> None:
...
def handle_mcp_message(paths: OmjPaths, message: Any) -> dict[str, Any] | None:
    ...
    if method == "initialize":
        return _result_response(
            request_id,
            {
                ...
                "serverInfo": {"name": "omj", "version": __version__},
                "instructions": (
                    "Use OMJ tools for local status, workflow recommendation, and capability probes only. "
...
def _handle_tool_call(paths: OmjPaths, request_id: Any, params: dict[str, Any]) -> dict[str, Any]:
...
def _call_tool(paths: OmjPaths, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    if name == "omj_status":
        preset = str(arguments.get("preset") or "focused")
        if preset not in {"minimal", "focused", "full"}:
            raise ValueError("omj_status.preset must be minimal, focused, or full")
    ...
    if name == "omj_recommend":
        message = str(arguments.get("message") or "").strip()
        if not message:
            raise ValueError("omj_recommend.message is required")
...
def _error_tool_result(tool: str, message: str) -> dict[str, Any]:
    return {
        "schema_version": MCP_TOOL_RESULT_SCHEMA_VERSION,
        "result_schema_version": "omj_tool_error/v1",
...
def _record_tool_call(paths: OmjPaths, tool: str) -> None:
    update_state(
        paths,
        {
            "last_mcp_bridge": {
                ...
                "claim_boundary": (
                    "A local MCP bridge tool call was observed by OMJ. This is not proof that a specific Hermes host "
