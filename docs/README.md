...
This directory is the public operating map for oh-my-jeo.
...
| Goal | Read |
| --- | --- |
| Understand what OMJ is and is not | [Direction](DIRECTION.md) |
...
OMJ is a Hermes-native wrapper orchestration layer.
...
leaves Hermes. OMJ should own the deterministic local contract between those
...
coding session. When a wrapper or operator chooses to proceed, `omj worktree
prepare` can create the local Git worktree and write
`omj_worktree_observation/v1`. That record proves workspace isolation only; the
...
![OMJ flagship command sets poster](../assets/omj-flagship-workflows-poster.png)
...
- Installation docs should lead with Hermes skill tap/install when available.
  `omj setup` should be described as a bootstrap, repair, validation, and
...
- Harness catalog changes should pass `omj harness validate`, and user-facing
  harness examples should stay backed by conformance tests.
- Release checks should include `omj release checklist --json`,
  ...
  `omj release hermes-smoke`, `omj release install-smoke`, and installed command
  smoke (`omj --help`). Use `release install-smoke --live` for an isolated
...
- Worktree/session isolation docs should describe `worktree_session_isolation/v1`
  as prepared workspace guidance and wrapper UX, and `omj worktree prepare` as
  explicit opt-in Git worktree creation that records workspace-isolation
  evidence only. `omj worktree bind` may describe wrapper launch/attach recipes
...
- Workflow learning docs should state that `workflow_learning_trace/v1` records
  ...
  fixed.
- Parity docs should map common oh-my runtime capability axes to OMJ's
  ...
  recorded as `omj_mcp_host_session/v1`.
- Goal execution docs should describe `.omj/goals` metadata-only ledgers,
...
- Material processing docs should describe `.omj/materials`
  `material_artifact/v1` records, target-format QA ladders, generation
  handoffs, and observed file evidence without implying OMJ rendered PPTX, PDF,
  Keynote, DOCX, XLSX, HWP, or uploaded files itself.
- Loop docs should describe `loopability_assessment/v1`,
  `.omj/loops` metadata-only `loop_cycle/v1` state, `loop_runtime/v1` tick
...
- The GitHub Pages site should stay a short public entry point that links back
  ...
  should stay grounded in inspected Hermes Agent files and OMJ evidence
  boundaries.
