...
OMJ memory context review helps Hermes and wrappers stop reusing stale
assumptions. It is a local, deterministic review surface for OMJ-managed state
...
- what context OMJ found locally
...
| Schema | Purpose |
| --- | --- |
| `memory_snapshot/v1` | A wrapper-supplied or OMJ-local context source. |
...
omj memory pack --fixture wrapper-memory.json --executor codex --session-id "$session_id"
omj chat session prepare-handoff "$session_id" --context-pack handoff-context.json "risky refactor"
```

The fixture is optional. Without it, OMJ inspects local setup profile, target
topology, runtime state, wrapper sessions, approved `.omj/memory` context, and
catalog hints.

Apply user-approved changes:

```sh
omj memory apply --batch memory-update-batch.json --dry-run
omj memory apply --batch memory-update-batch.json
```

Approved writes stay under `.omj/memory/`. Unsafe scope refs, path traversal,
...
If conflicts remain, OMJ writes `context_pack_blocked` with the conflict list
...
OMJ treats sources conservatively:
