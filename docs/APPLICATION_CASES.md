...
This guide documents the G1-G10 OMJ feature surfaces that make Hermes Agent
...
wrappers and operators can inspect through `omj cases`.

## G1-G10 Hermes Use-Case Map

These are implemented OMJ feature surfaces, not just example situations. Each
...
```sh
omj cases list --json
omj cases inspect G10 --json
omj cases demo G10 --json
omj cases demo --all --json
omj cases artifact G10 --json
omj cases artifact --all --write
omj cases artifact-validate --json
omj cases readiness --json
omj cases replay --json
omj cases recommend "PR opened with failing CI" --json
...
uses `omj_use_case_demo_card/v1` and contains:
...
`omj cases demo --all --json` exports all ten cards as
`omj_use_case_demo_collection/v1`. This gives wrapper authors and release checks
...
single artifact uses `omj_use_case_artifact/v1`; the full bundle uses
`omj_use_case_artifact_collection/v1`. These artifacts include:
...
```sh
omj cases artifact --all --write
omj cases artifact-validate
```

This writes JSON under `.omj/use-cases/artifacts/` plus a cache-only index at
`.omj/use-cases/index.json`. It is useful for wrapper QA, release review, demos,
...
`omj cases replay` runs English and Korean synthetic operator fixtures for every
...
Hermes should use the installed `oh-my-jeo` router guidance and the
...
```sh
run_json="$(omj runtime record --skill ultragoal --harness goal-execution --status started)"
run_id="$(printf '%s' "$run_json" | python3 -c 'import json,sys; print(json.load(sys.stdin)["run"]["run_id"])')"
omj runtime delegate --run "$run_id" --requested --not-observed --result not_observed
omj runtime show "$run_id"
```
...
```sh
run_json="$(omj runtime record --skill code-review --harness critic --status completed)"
run_id="$(printf '%s' "$run_json" | python3 -c 'import json,sys; print(json.load(sys.stdin)["run"]["run_id"])')"
omj runtime delegate --run "$run_id" --requested --not-observed --result not_observed --evidence-ref run.json
omj runtime show "$run_id"
```
...
OMJ skills. Wrapper operators can use a working `omj` command to inspect the
...
```sh
omj playbook list
omj playbook inspect safe-feature-change
omj playbook recommend "I want to safely add a feature to this repo"
omj playbook recommend "take this product idea from plan to deploy and monitor safely"
omj playbook recommend "turn the revenue spreadsheet into an Excel and PDF package with render QA"
omj playbook recommend "run a CTO loop for roadmap architecture tradeoffs delivery risk and release readiness"
omj playbook recommend "deploy and monitor this release with rollback and health checks"
```
...
Normal users should then talk to Hermes, not manually run `omj recommend` or
`omj playbook recommend`.
...
```sh
omj chat interact --source discord "결제 실패 피드백을 모아서 회의 주제와 다음 전략을 정리해줘"
omj coding delegate --executor codex --source discord "prepare weekly ops review from customer feedback and release risks"
```
...
OMJ supplies deterministic routing, plan, handoff, and status contracts for
...
```sh
omj chat interact --source discord "<message>"
omj playbook recommend "<message>" --limit 1
omj coding delegate --executor codex --source discord "<message>"
omj demo grounded-score
```
...
`omj demo grounded-score` is a deterministic contract-compliance demo over 28
...
- `omj doctor` reports the managed skill directory as healthy after setup.
- The generated router includes the representative harness registry.
- `omj docs workflows --json` exposes `harness_quality/v1` style quality data
  for wrapper rendering and status decisions.
- `omj cases demo --all --json` exposes `omj_use_case_demo_collection/v1` for
  every G1-G10 wrapper card and preserves `prepared_not_observed`.
- `omj cases artifact --all --json` exposes
  `omj_use_case_artifact_collection/v1`, and `omj cases artifact --all --write`
  can create `.omj/use-cases/artifacts/*.json` without turning prepared
  runbooks into observed runtime claims.
- `omj cases replay --json` exposes `omj_use_case_replay/v1` and must pass the
  English/Korean G1-G10 synthetic operator fixture set before the use-case map is
  treated as release-ready.
- `omj cases readiness --json` exposes `omj_use_case_readiness/v1` and rolls the
  catalog, card, artifact, replay, and optional local-store states into a single
  operator-readable readiness card.
- `omj playbook recommend` returns situation-level pipelines for safe coding,
...
- Runtime-backed cases above can create `.omj/runtime/runs/<run-id>/`
  artifacts.
- `delegation.json` separates requested delegation from observed delegation.
- `omj probe` output is captured before any native hook, plugin, app, MCP, or
