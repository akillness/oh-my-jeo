...
```sh
omj playbook list
omj playbook inspect request-to-handoff
omj playbook recommend "I want to safely add a feature to this repo"
omj playbook recommend "research latest official sources"
omj playbook recommend "prepare weekly ops review from customer feedback and release risks"
omj playbook recommend "every morning check competitor news and send a Slack digest only if something changed"
omj playbook recommend "set up a research department with Scout, Analyst, and Briefer for market papers"
omj playbook recommend "organize meeting history, scrum, sprint planning, retro decisions, and follow-up actions"
omj playbook recommend "create a monthly leadership PPT report package from current status and risks"
omj playbook recommend "turn the revenue spreadsheet into an Excel and PDF package with render QA"
omj playbook recommend "run an incident postmortem with SLO, error budget, remediation, and service reliability evidence"
...
| `deploy-and-monitor` | A user wants release operations and monitoring status without OMJ running infrastructure. | Release scope -> checks -> deploy decision -> monitor signals -> rollback gate -> post-deploy record. |
...
- OMJ owns deterministic local contracts, playbook selection, prepared handoff
...
Use `omj playbook inspect <id>` when the wrapper needs full stage-level
...
status boundary without making OMJ an infrastructure executor.
