...
- Optional `~/.hermes/plugins/omj` bridge hardening after v1 install smoke
...
- Plugin-native `omj_interact` chat/session observation for Hermes-hosted
  surfaces
- Fixture-backed Hermes Agent wrapper examples that consume plugin
  `omj_interact` and render the resulting `chat_interaction/v1` without
  touching real user state
- G1-G10 use-case demo cards through `omj cases demo`, including a checked-in
  `omj_use_case_demo_collection/v1` fixture for wrapper rendering
...
- G1-G10 use-case artifact bundles through `omj cases artifact`, including
  local `.omj/use-cases/artifacts/` writes, cache-only validation, and release
  smoke coverage
- G1-G10 use-case replay through `omj cases replay`, including English/Korean
  synthetic operator fixtures and release smoke coverage for deterministic
  recommendation routing
- G1-G10 use-case readiness through `omj cases readiness`, rolling catalog,
...
- Review, CI, merge-readiness, and merge observation records for delegated
  coding lifecycles
- Harness catalog inspection and validation through `omj harness list`,
  `omj harness inspect`, and `omj harness validate`
- GitHub Pages source for the public OMJ entry point
- Situation playbooks exposed through `omj playbook list`, `inspect`, and
  `recommend`
- Default plugin distribution path through `omj setup`, with local
  import/register smoke and conservative runtime-claim boundaries
- Host-observed plugin load/use records through `omj plugin observe-host`, kept
