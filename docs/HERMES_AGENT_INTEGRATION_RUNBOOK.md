...
This is an operator reference, not an `omj` command.

Use this runbook when you operate a Hermes-agent wrapper that receives natural
language from Discord, Slack, or another hosted chat surface and renders OMJ's
local deterministic contracts as native chat UX.

The chat user should not need to know command names. The wrapper can use OMJ
...
- Which local OMJ contract should the Hermes-agent wrapper consume?
...
| OMJ | Deterministic local routing, playbook, planning, handoff, session, status, and fixture contracts. | Hermes core patches, hidden LLM calls, or executor launch. |
| Selected coding executor | Main implementation work, verification output, review fixes, and execution evidence after dispatch or prompt handoff. | Chat UX or OMJ contract generation. |
...
The wrapper normally starts with `chat_interaction/v1`. If the OMJ plugin is
available, use `omj_interact` so Hermes can receive the same envelope and record
...
- `target_topology`: optional `omj_target_topology/v1` summary with
...
2. Ask OMJ for a platform-neutral interaction envelope.
3. Render `chat_response.headline`, `chat_response.body`,
   ...
   includes the visible OMJ usage marker, such as `[omj] web-research`; use
...
4. Apply `chat_response.messenger_rendering` according to the target surface's
  ...
  plugin `omj_interact` or `omj chat interact`. Render the prefix once per response, not on every
...
7. If a coding handoff is prepared, check `executor_readiness/v1` before first
   ...
   `omj coding executor-readiness --executor <profile>` once, cache the result,
...
8. Dispatch the `coding_executor_handoff/v1`
   payload to the external executor outside OMJ. For Codex targets, use
...
10. Re-render status from OMJ after each observed transition.
...
```sh
omj chat session start --source discord --source-event-id "$MESSAGE_ID" --channel-ref "$CHANNEL_ID" "risky refactor"
omj chat session accept-plan "$SESSION_ID"
omj chat session prepare-handoff "$SESSION_ID" "risky refactor"
...
shell-safe paths. OMJ itself still does not execute the coding agent; Hermes or
...
```sh
omj chat codex-progress --jsonl "$CODEX_JSONL" --evidence-ref codex-jsonl
omj chat session open-executor "$SESSION_ID" --observed \
...
omj chat session record-executor "$SESSION_ID" --result completed \
  --evidence-ref codex-summary \
  --codex-log-jsonl "$CODEX_JSONL" \
  --codex-log-ref codex-final-jsonl
omj chat session request-verification "$SESSION_ID"
omj chat session status "$SESSION_ID"
```
...
```sh
omj runtime progress bind --run "$RUN_ID" \
  --executor-profile codex \
  --process-session-id "$CODEX_PROCESS_SESSION" \
  --codex-session-ref "$CODEX_SESSION" \
  --evidence-ref codex-jsonl

omj runtime progress observe --run "$RUN_ID" \
  --codex-log-jsonl "$CODEX_JSONL" \
  --process-status running \
  --evidence-ref codex-jsonl

omj runtime progress status
```
...
`omj chat codex-progress` is an adapter helper for the raw-output gap: hosted
...
`raw_output_artifact` (`omj_context_artifact_ref/v1`), `context_budget`, capped
...
events instead of relying on 3-5 minute polling. Use `omj_progress_event/v1` for
...
OMJ core still does not launch Codex or claim that fixes were applied.
...
```sh
omj chat codex-followup "$FOLLOW_UP" \
...
operator action. OMJ records the reference and summary metadata only; it does
...
`coding_briefing/v1` object from `omj chat session status`. Render
