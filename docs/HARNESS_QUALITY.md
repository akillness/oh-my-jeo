...
Local operators can inspect the same contract with `omj harness list`,
`omj harness inspect <name>`, and `omj harness validate`.
...
- `omj docs workflows --json` exposes the full local workflow and harness
  catalog, including `workflow_catalog/v1.harnesses[].harness_quality`.
- `omj coding delegate` includes `harness_quality` beside the prepared
  delegation. Dispatch actions are removed unless the payload also includes a
  prepared executor handoff.
- `omj coding delegate --executor codex` includes the dispatch-capable contract
...
- `omj coding delegate --executor claude-code`, `--executor omx-runtime`, or
...
- `omj hermes plan` includes `wrapper_contract.harness_quality` so wrappers can
...
- `omj learning record`, `omj learning eval`, and `omj learning regression
  ...
  approval.
- `omj learning missed-route` is the wrapper-friendly shortcut for "Hermes did
  not use the expected OMJ workflow." It records a metadata-only trace, eval,
...
- `omj learning index check` and `omj learning index rebuild` keep the local
...
- `omj learning audit` reads the local learning corpus and returns
...
- `omj learning candidate <trace-id>` returns an
...
- `omj learning review` returns `workflow_learning_review_queue/v1`: the
...
- `omj learning review-candidate <candidate-id> --decision approve|revise|reject`
  ...
  raw text.
- `omj learning proposal <candidate-id>` records an
...
- `omj learning export` creates a redacted `workflow_learning_export/v1` review
