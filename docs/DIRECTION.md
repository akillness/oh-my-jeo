...
OMJ should keep familiar workflow names only when they help users and wrappers
...
OMJ should optimize for helping Hermes observe, summarize, and explain work,
...
Internal awareness rails such as `[OMJ Awareness]`, native bridge status
...
1. Keep users command-agnostic in chat.
   ...
   skill, plan, status, and handoff UX without requiring users to know OMJ
   commands.
...
5. Prefer local deterministic artifacts over hidden magic.
   ...
   Memory/context review follows the same rule. OMJ may inspect OMJ-local
...
6. Keep Hermes Agent as the chat surface.
   OMJ should not describe missing Discord/Slack bot layers as product gaps.
   The product story is that Hermes Agent receives natural language and consumes
   OMJ skills, workflows, and local status contracts.
...
6. Skill-first distribution.
   ...
   supports it. Keep `omj setup` as the bootstrap, repair, validation, and
