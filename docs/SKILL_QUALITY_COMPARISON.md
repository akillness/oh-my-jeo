# OMJ Skill Quality Comparison Against Oh My Codex

Date: 2026-06-11

## Scope

This report compares the repository-root OMJ skill set against the open-source
Oh My Codex (OMX) skill set.

Evidence used:

- OMJ repository: `67aa040` plus this skill-rubric working tree
- OMJ skill source: `skills/*/SKILL.md`, `src/skills/catalog.py`,
...
OMJ skills are more consistent and safer as Hermes-native wrapper guidance.
...
The main OMJ gap was not safety or catalog quality. The gap was per-skill
...
| --- | ---: | ---: | ---: | ---: | ---: |
| OMJ repo-root skills | 30 | 128.8 | 125 | 120 | 217 |
...
`meeting-brief`, `oh-my-jeo`, `ops-review`, `research-brief`,
...
| Quality feature | OMJ | OMX | Interpretation |
| --- | ---: | ---: | --- |
| Frontmatter description | 30 / 30 | 46 / 46 | Both are discoverable. OMJ descriptions are additionally namespaced with `[omj]`. |
| `Use When` style activation guidance | 30 / 30 | 11 / 46 | OMJ is now consistent for skill picker routing. |
| `Do Not Use When` guidance | 30 / 30 | 14 / 46 | OMJ now has broader negative routing coverage; OMX remains deeper in some hand-authored modes. |
| Explicit purpose or why section | 30 / 30 | 16 / 46 | OMJ now exposes a generated reason for every skill. |
| Step or execution procedure | 29 / 30 | 17 / 46 | OMJ has procedure stubs everywhere; OMX has deeper procedures in fewer skills. |
| State management guidance | 30 / 30 | 26 / 46 | OMJ is broad; OMX is deeper for stateful modes. |
| Examples or good/bad patterns | 30 / 30 | 5 / 46 | OMJ now has examples everywhere; flagship skills have bespoke good/bad examples. |
| Final checklist | 0 / 30 | 7 / 46 | OMX is stronger for completion discipline in major workflows. |
| Quality bar | 29 / 30 | 1 / 46 | OMJ is much stronger at machine-readable quality contracts. |
| Harness/runtime evidence language | 30 / 30 | 15 / 46 | OMJ is stronger for wrapper-safe evidence rendering. |
| Safety rules | 29 / 30 | 0 / 46 | OMJ is stronger at anti-overclaim framing. |
| Artifact expectations | 30 / 30 | 18 / 46 | OMJ is stronger at prepared vs observed artifact boundaries. |

## Quality Scorecard

Scores are relative to the intended product shape, not generic skill length.

| Dimension | OMJ | OMX | Notes |
| --- | ---: | ---: | --- |
| TUI and skill-picker discoverability | 9 | 7 | OMJ has uniform metadata, `[omj]` descriptions, category, phase, role, and quality tier. |
| Routing clarity | 8 | 7 | OMJ has strong positive triggers; OMX has better negative boundaries. |
| Execution specificity | 5 | 9 | OMX deep modes such as `deep-interview`, `team`, and `skill` are operational runbooks. OMJ often stays generic. |
| Evidence discipline | 9 | 7 | OMJ consistently separates prepared, requested, observed, and not-observed states. |
| Runtime-state depth | 6 | 9 | OMJ names state artifacts; OMX often gives exact state commands, schemas, phases, and recovery rules. |
| Maintainability | 9 | 6 | OMJ generated catalog plus tests prevents drift. OMX is powerful but uneven and hand-authored. |
| User/operator explanation | 7 | 8 | OMJ is clearer for wrapper status; OMX is clearer for "what do I do now" in advanced modes. |
| Example quality | 6 | 7 | OMJ now has generated examples everywhere and bespoke examples for flagship skills; OMX still has deeper long-form examples in some execution modes. |
| Coverage breadth | 7 | 9 | OMX covers more Codex-native roles and utility modes. OMJ adds business/Hermes wrapper lanes OMX does not target. |
| Product fit for Hermes Agent | 9 | 5 | OMJ avoids pretending Hermes is a hidden executor and adapts to Hermes-native surfaces. |

Overall:

- OMJ: strong B+ today, A- for wrapper safety and catalog discipline.
- OMX: A- for Codex-native execution protocol, B for consistency across the full catalog.

## What OMJ Does Better

### 1. Generated consistency

OMJ skills are generated from catalog data and locked by tests. The repo has a
...
OMJ skills repeatedly state that Hermes should orchestrate, clarify, research,
...
OMJ has `harness_quality/v1` and a workflow reference that exposes quality
...
OMJ includes skills such as `meeting-brief`, `feedback-triage`,
...
Impact: OMJ has a broader non-coding product story than OMX.
...
OMJ equivalents are safer for Hermes but usually shorter and more generic.

Impact: OMX is easier for an execution agent to follow without hidden context.

### 2. Negative routing boundaries

OMX frequently says when not to use a skill. Before this PR, OMJ mostly said
when to use it. That created ambiguity when two OMJ workflows were plausible,
for example:
...
Impact: the new generated `Do Not Use When` sections give OMJ broader
negative routing boundaries while preserving its catalog consistency.

### 3. Examples and completion checklists

OMX includes good/bad examples and final checklists in important modes. OMJ
...
Impact: OMJ now gives much better first-read intuition in the TUI, while the
...
runtime preconditions directly inside the skill. OMJ mentions runtime evidence
and wrapper state, but many skills do not yet explain recovery paths in
mode-specific terms.

Impact: OMJ has the right evidence model but weaker operator recovery guidance.
...
managed-skill counts. OMJ should document which skills are public, internal,
...
OMJ does not need to copy OMX's full Codex-native execution depth. The target
should be:

```text
OMX-level operational clarity
+ OMJ-level wrapper safety
+ Hermes-native evidence boundaries
+ business workflow coverage
```

For each flagship OMJ skill, the reader should understand within 60 seconds:
...
OMJ's strongest difference: Hermes-native wrapper discipline.
