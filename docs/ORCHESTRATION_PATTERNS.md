# OMJ Orchestration Patterns

OMJ orchestration patterns describe how Hermes should shape work before any
...
```sh
omj capabilities export --section orchestration_patterns --json
omj capabilities inspect executor_session_handoff --json
```
...
`worktree_isolated_workers` can recommend a worktree policy, but OMJ cannot say a
