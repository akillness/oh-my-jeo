...
oh-my-jeo should feel like a native Hermes workflow layer, not a pile
...
UX, OMJ produces deterministic local contracts, Hermes keeps user-facing
...
flowchart LR
  ...
  skills["Installed OMJ skills\nHermes skill tap or omj setup"]
  plugin["Optional OMJ plugin\n~/.hermes/plugins/omj"]
  wrapper["Hermes chat surface\nbuttons, threads, edits"]
  omj["OMJ local contract layer\nplaybooks, routing, plan, handoff, status"]
  hermes["Hermes Agent\nclarify, research, plan, narrate"]
  executor["Selected coding executor\nimplementation, verification"]
  memory["Project memory\nreviewed .omj/memory summaries"]
  runtime["Local runtime artifacts\nprepared and observed evidence"]
  site["Docs and status UI\ncards, examples, reports"]

  user --> hermes
  skills --> hermes
  plugin -->|"omj_interact, omj_recommend, omj_probe, omj_hud, omj_role, omj_status, evidence, hooks"| hermes
  user --> wrapper
  wrapper -->|"chat_interaction/v1"| omj
  omj -->|"answer, clarify, plan, or status"| wrapper
  wrapper --> hermes
  hermes -->|"accepted plan"| omj
  omj -->|"review, recall, prepared context only"| memory
  memory -->|"memory_recall_pack/v1"| omj
  omj -->|"prepared handoff, not execution proof"| executor
  executor -->|"dispatch, result, verification"| runtime
  runtime -->|"status_card/v1"| omj
  omj --> wrapper
  runtime --> site
```
...
Chat user
  ...
  -> Installed OMJ skills provide workflow and evidence guidance
  -> Hermes chat surface asks OMJ for backend contracts
...
src/
  omj/
    __init__.py              # public package shim; maps source folders below into omj.*
    cli/                     # module entry point package for omj.cli and python -m omj.cli
...
`~/.hermes/plugins/omj`. The v1 plugin registers deterministic
`omj_interact` chat/session interaction, `omj_recommend` route hints,
metadata-only `omj_probe` capability status/roadmap, compact metadata-only
`omj_hud`, detailed metadata-only `omj_status`, `omj_role` role context, a
bounded `omj_gather_evidence` local verification probe, and passive lifecycle
...
`omj_context_brief/v1` plus `omj_route_hint/v1` for messages that look like
...
the context brief adds `omj_catalog_question_hint/v1` so Hermes can show the
workflow picker or capability summary without shell approval. The `pre_tool_call` hook mirrors
the generic-tool checkpoint as structured `omj_generic_tool_checkpoint/v1`
...
tool labels and tool-family hints, never raw tool input. `omj hud`
...
by `omj menubar status`. Its `menubar_status/v1` payload is a UI projection over
...
`workflows/memory.py` owns OMJ project memory. It stores candidates, reviewed
records, review decisions, and recall packs under `.omj/memory/` using local
...
payload or an explicit `omj menubar status --observe-local-processes` request
from the native macOS helper. That observation is app-local, expires after a
short TTL, and applies restarting state only inside its restart window. OMJ does
...
`omj_target_topology/v1`, and keeps single-target versus multi-target behavior
...
the wrapper reports an observed coding-session start/attach event. OMJ still
...
remain missing evidence; OMJ does not infer them from prepared handoff text.
...
`omj learning missed-route` composes those primitives for the common wrapper
case where Hermes did not use the expected OMJ workflow; it records review
...
1. Hermes-native installed skills. The tap-compatible `skills/` directory and
   the managed `~/.omj/skills` bootstrap directory expose the same generated
   guidance to Hermes.
2. Prompt-level guidance. The router skill gives Hermes a structured map of
   ...
   core behavior.
3. Situation playbooks. `omj playbook recommend` lets wrappers map a natural
   request to a higher-level pipeline before they choose a lower-level skill,
   plan, research lane, or handoff.
4. Task abstraction cards. `omj_task_card/v1` lets wrappers classify work such
...
5. Wrapper-native chat orchestration. Plugin `omj_interact` and
   `omj chat interact` let Discord, Slack, or hosted Hermes wrappers receive
   one platform-neutral `chat_interaction/v1` envelope with renderable chat
   copy, state, action buttons, and a thread key.
6. Wrapper session persistence. `omj chat session` lets wrappers persist
...
`omj_interact` is the plugin-native Hermes-facing entry point for this
contract, and `omj chat interact` is the CLI/backend equivalent. They compose
...
`omj` command. The surrounding envelope preserves source metadata, message hash
...
keeping the raw message out of persisted OMJ artifacts. Claude Code and generic
...
the explicit workspace action, `omj worktree prepare` creates a local Git
worktree and records `omj_worktree_observation/v1`; that observation is
workspace-isolation evidence only. `omj worktree bind` can then return a
...
than runtime JSON under `.omj/runtime/`. The artifact is user-facing: it includes
...
after plan acceptance. `omj coding delegate --from-plan` rejects draft plans by
...
`omj chat session` is the recovery layer for adapters that need button/thread
...
OMJ feel more complete without turning Hermes into the main coding executor.

## Hermes Capability Boundary

`omj probe` is the non-mutating capability inspection surface. It reports
...
OMJ setup state only, `mcp_bridge_runtime` is a local OMJ-observed bridge tool
...
`omj_mcp_host_session/v1` through `omj mcp observe-host`; observed records
...
record `omj_plugin_host_observation/v1` with `omj plugin observe-host` after it
actually sees plugin load, status query, session end, or unload. Invoked OMJ
...
`plugin_runtime_observed` available in `omj probe`, but it still proves only the
...
For terminal operators, `omj probe` prints a compact status summary by default.
Wrappers and automation should request the full capability payload with
`omj probe --json` or `OMJ_OUTPUT=json`.

`omj probe --parity` adds `omj_parity_matrix/v1`. That matrix compares common
oh-my runtime capability axes with OMJ's actual surfaces: skill/plugin
...
claim. Team/swarm worker support is exposed as `omj_team_worker_readiness/v1`
through `omj runtime team-readiness`: OMJ can show the worker protocol, runtime
...
`omj worktree prepare/list/bind` backend and its local
`omj_worktree_observation/v1` ledger plus wrapper binding recipes; it does not
...
evidence under `.omj/runtime/`, but the artifact must separate requested
...
Runtime artifacts are local JSON/JSONL files under `.omj/runtime/`.
...
- CLI inspection through `omj runtime status`, `omj runtime runs`, and
  `omj runtime show <run-id>`
- schema validation through `omj runtime validate`
- redacted export through `omj runtime export`

Bot wrappers can call `omj chat route --record` before invoking Hermes. The
record stores the selected skill, confidence, score, message length, and message
hash without storing the raw prompt body.

Bot wrappers can call `omj coding delegate --executor codex --record` for
...
Bot wrappers can still call `omj runtime delegate` after the response if
delegation metadata is available. If not, they should record `not_observed`
rather than guessing.

Wrappers can also call `omj runtime wrapper` to record whether a prompt was
...
Wrappers can call `omj runtime delegation-status --run <run-id>` to combine the
...
`omj runtime progress bind|observe|status` is the live executor progress
...
incremental Codex JSONL or process-output snapshots. OMJ summarizes the snapshot
...
`omj coding lifecycle start|dispatch|result|verify|report`. These commands are
...
`omj chat session status` also exposes `coding_briefing/v1` as a sibling to the
...
`omj hermes plan --record` writes Markdown, not runtime JSON. The plan frontmatter
...
metadata. Runtime runs under `.omj/runtime/` remain evidence envelopes for what a
wrapper or operator observed.

The CLI exposes the state layer through:

- `omj state start --workflow <name>`
- `omj state status`
- `omj state finish --workflow <name> --outcome finished`
- `omj state clear --workflow <name>`
...
- Workspace guidance is printed by `omj snippet`; it is not applied by default.
