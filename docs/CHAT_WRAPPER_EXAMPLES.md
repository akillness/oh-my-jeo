...
This page shows how a Discord-style Hermes Agent surface can render OMJ output
...
uv run python -m omj.cli chat route-hint --source discord "make an image explaining the cron feature"
uv run python -m omj.cli chat native-command --source discord
uv run python -m omj.cli chat native-command --source slack
uv run python -m omj.cli chat native-command --source telegram
uv run python -m omj.cli demo orchestration
uv run python -m omj.cli demo orchestration --executor hermes
```
...
By default `omj demo orchestration` leaves the coding owner unselected and shows
...
## Messenger-Native OMJ Entry

For platforms that support registered commands or bot menus, wrappers can load
the deterministic OMJ command contract:

```sh
omj chat native-command --source discord
omj chat native-command --source slack
omj chat native-command --source telegram
```

Those commands return `omj_native_command_surface/v1` with the platform's `/omj`
...
card returned by `omj_native_command_render/v1`:

```text
Hermes Agent  BOT
Open OMJ

Choose `omj` to open the workflow picker. The picker appears before any
...
- Discord can register `/omj`, Slack can register `/omj` or a shortcut, and
  Telegram can register the `omj` bot command menu.
- If native command preview is limited by the platform, the user still gets an
  `Open omj` card in the same Hermes conversation.
- The card only opens the picker; it does not claim that a workflow started.

## Catalog Questions Without Shell Approval

If the user asks "what OMJ commands are available?" or "skill들은 뭐 있어?",
wrappers should call `omj chat interact` and render
`chat_response.kind == skill_picker`:

```text
Hermes Agent  BOT
[omj] oh-my-jeo - Here are the OMJ workflows.
...
## Compact OMJ Context For Hermes

When a wrapper only needs to prime Hermes before ordinary chat, image, file,
search, or coding tools, use `omj_context_brief/v1` instead of asking the user
to approve a shell command. The backend form is:

```sh
omj context brief --source discord --json "make an image card for this PR"
```

The plugin-native form is the `omj_context` tool. Both return the same compact
shape: OMJ lanes, common cues, a generic-tool checkpoint, an optional route
hint, catalog-question guidance, and the response contract. The raw prompt is
represented by hash/length metadata only.

When the message asks what OMJ commands, workflows, or skills are available,
the payload includes `catalog_question.schema_version ==
omj_catalog_question_hint/v1`. Hermes can then summarize the lanes and offer
`omj_skill_picker/v1` or call `omj_capabilities` with `{"action":"summary"}`
without asking the user to approve `omj list`.

For an image request, Hermes can then say:

```text
Hermes Agent  BOT
[omj] img-summary looks relevant.

This sounds like a visual summary request, so I will prepare the OMJ
...
The wrapper should not ask the user to approve `omj list` merely to show the
catalog. `omj_skill_picker/v1` carries the workflow labels, direct invocation
text, harness names, and routing-only claim boundary. Catalog-question
responses also include `omj_capability_summary/v1`, so Hermes can summarize the
...
the local install, `omj list --json` also includes
`omj_installed_skill_catalog_context/v1` plus per-skill descriptions, routing
...
make sure only one Discord bot answers", OMJ should not invent a narrow
`migration` workflow. The wrapper can render the `omj_task_card/v1` that sits
above the workflow rails:

```text
Hermes Agent  BOT
[omj] task-card - I will frame this as a high-level task first.
...
such as "피드백인데 OMJ 라우팅 설계가 잘못됐어", the same abstraction layer routes
to `workflow-learning` instead of customer `feedback-triage`.

## Plugin-Native Chat Interaction

When the managed OMJ plugin is loaded, a Hermes wrapper can call the plugin
tool `omj_interact` directly. That path returns the same renderable
`chat_interaction/v1` envelope as `omj chat interact`, and it can also record a
metadata-only wrapper session with `record_provenance.producer == plugin_tool`.

The transport-free shims can render this without touching your real `~/.omj` or
`~/.hermes` state:
...
You do not need to run a shell command for this. OMJ covers planning, ops,
...
```text
Hermes Agent  BOT
[omj] ralplan - I routed this to `ralplan` because it needs a safe plan first.
...
When the managed OMJ plugin bridge is loaded, `pre_llm_call` can add a bounded
`omj_context_brief/v1` payload plus an `omj_route_hint/v1` context block for
...
or gateway may still show the old injected hint until the installed OMJ package
and managed plugin are refreshed with `omj update` or `omj setup`, then the
Hermes Agent or gateway process is restarted or reloaded.

OMJ-owned wrapper and progress reports must render only bounded summaries from
...
own notification outside OMJ control; adapters should treat that as a separate
Hermes/gateway surface, not as OMJ's report body.
...
```sh
omj chat route-hint --source discord "make an image explaining the cron feature"
```
...
These routes are advisory. They tell Hermes which OMJ workflow to consider
...
When the managed OMJ plugin is loaded, its `pre_tool_call` hook can emit the
same checkpoint before image, file, search, or coding tool calls. It returns the
legacy text context plus a structured `omj_generic_tool_checkpoint/v1` payload
...
"does OMJ support image generation?", or "generate an image", the wrapper should
...
falling back to the generic OMJ workflow picker or a generic clarification.
...
catalog immediately. If the user asks "what is OMJ and how do I use it?" or
"OMJ가 뭐야? 어떻게 써?", `omj chat interact` returns
`chat_response.kind == context_brief` with `[omj] context`, a compact mental
model, the structured `omj_context_brief/v1`, and actions to open the picker or
...
such as "OMJ 기능 뭐 있어?" still opens `omj_skill_picker/v1`, but "does OMJ
support scheduled automation?", "can OMJ help with MCP setup?", "does OMJ
support memory cleanup?", "does OMJ support voice commands?", or "OMJ로 GitHub
...
generic tools, check OMJ prep/status/learning; if relevant, name the workflow
first.

[ Open img-summary ] [ Route for me ] [ Open omj ]

State
- Hint only: no workflow has been selected or executed.
- OMJ-first checkpoint: visible before image/file/search/coding tools.
...
`[OMJ Route Hint]` text into Hermes context itself. The default response is the
...
For missed OMJ usage feedback, the wrapper does not need to teach the user a
backend command first. Phrases such as "OMJ 안 썼어", "missed route: Hermes
skipped OMJ", or "Hermes did not use OMJ for my image request; record this as
...
## Missed OMJ Route Capture

If Hermes or the user says a response did not use the expected OMJ workflow, the
wrapper can record that as review material without storing the raw chat prompt:

```sh
omj learning missed-route \
  --source discord \
...
fixed. If the wrapper omits `--fixture-message`, OMJ records a non-replayable
...
Hermes Agent  BOT
[omj] plan - I routed this to `plan` because it needs a safe plan first.
...
- The user does not need to know an `omj` command.
- Hermes Agent explains why the request became a plan-first workflow and shows
  the visible OMJ trace prefix on the first line.
...
## Visible OMJ Trace And Messenger Formatting
...
```text
[omj] web-research - I know which workflow should handle this.
```
...
Slack, Telegram, Hermes TUI, web, and other adapters which rendering profile OMJ
...
`[omj] <workflow>` on every paragraph or every line. If an adapter splits a
...
```text
Hermes Agent  BOT
[omj] status - A coding-runtime handoff is ready.
...
- `Start Codex session` and `Start Claude Code session` include an
  ...
  copyable_instruction_only` do not mean OMJ launched the executor. New renderers
...
The following examples were run through `omj chat interact --source discord`.
...
Hermes Agent  BOT
[omj] feedback-triage - I know which workflow should handle this.
...
Hermes Agent  BOT
[omj] ultraqa - I know which workflow should handle this.
...
- `omj coding delegate` does not emit a dispatchable executor or runtime
...
Hermes Agent  BOT
[omj] deep-interview - This needs a clarification workflow before planning.
...
Hermes Agent  BOT
[omj] code-review - I know which workflow should handle this.
...
| Visible OMJ workflow marker | `chat_response.usage_trace.visible_prefix` |
...
Hermes Agent surfaces should render these fields natively and keep OMJ focused
