...
This guide is for users and operators who want Hermes Agent to see the OMJ
skill pack. Normal users should talk to Hermes through Hermes' skill and chat
surfaces. The `omj` command is bootstrap, repair, verification, and wrapper
...
```text
what should I do next with OMJ setup?
```
...
`omj quickstart` prints the compact first-use card instead of a deep diagnostic
...
observed by Hermes or a wrapper. The JSON form is `omj_quickstart_card/v1` and
is the same card that `omj chat interact` returns as
...
If the user asks what OMJ is or how to use it, wrappers can answer with
`chat_response.kind == context_brief` and `[omj] context` before opening the
...
command package and `omj` executable only. `omj setup` is the explicit,
repairable step that installs generated managed skills and registers them with
Hermes through `skills.external_dirs`.
When `omj setup` is run in a real terminal, it opens a small colored wizard that
chooses the setup language, connects OMJ to the target Hermes profile, asks for
one simple default coding agent suggestion (`Codex`, `Claude Code`, or
`Hermes`), installs the OMJ status helper, and then prints a human-readable
...
`omj setup --json` or `OMJ_OUTPUT=json omj setup` for the full
machine-readable payload.

Setup also records OMJ project-memory policy in `.omj/setup-profile.json`.
The default is safe `review-first` memory: OMJ can capture local candidates,
...
for review or rejection. This is OMJ project memory under `.omj/memory/`; setup
does not mutate Hermes global or internal memory.

The default user scope writes `~/.omj` and `~/.hermes`. Use project scope when
one repository needs isolated local OMJ skills and Hermes config:

```sh
omj setup --scope project
omj --scope project doctor
```

The installer also prints the installed `omj` command path. By default it uses
an isolated OMJ virtual environment and links `omj` into a user bin directory
when possible. If that directory is not on `PATH`, add the printed directory to
`PATH` or run the printed absolute `omj` path directly. `omj doctor` includes a
...
Plugin support is installed by `omj setup` by default. It provides a thin
Hermes plugin bridge in addition to the skill pack:

That installs `~/.hermes/plugins/omj` with deterministic workflow
recommendation, metadata-only HUD/status/role support, and a bounded evidence
probe.
`omj hud` prints the same compact status line a Hermes TUI or plugin surface can
render. It shows only operationally useful status: OMJ version, plugin
...
left to `omj doctor`, `omj_status`, and machine-readable HUD JSON. A quiet idle
line looks like
`[omj] v1.0.1 | plugin:ready | target:single | coding-agent:idle(ask)`.
The plugin also exposes `omj_context` for a compact OMJ mental model plus
generic-tool checkpoint, `omj_interact` for shell-free chat responses and
metadata-only wrapper session records, `omj_recommend` for route hints without
session recording, `omj_probe` for local setup/runtime status and
capability-roadmap cards, `omj_role`, validates `[omj-role:name]` markers for
delegated subagent prompts, and records a metadata-only session-end checkpoint
when OMJ runtime state exists. It also exposes `omj_gather_evidence` for
explicit allowlisted local verification probes such as OMJ doctor, harness
...
`omj menubar status` itself emits `menubar_status/v1` JSON with separate
`hermes_agents` and `external_coding_executors` sections, friendly labels such
as `OMJ connection: Ready`, `Hermes targets: 2`, `Coding agent: Codex`, and
...
On macOS, a normal user-scope `omj setup` also attempts to build and start the
small OMJ menu bar helper when `swiftc` is available. The helper lives under
`~/.omj/menubar`, is started with a user LaunchAgent, and refreshes the same
`omj menubar status --observe-local-processes` payload. The visible menu is
...
omj menubar start
omj menubar stop
omj menubar uninstall
```

Set `OMJ_MENUBAR=0` or run `omj setup --no-menubar` to skip the helper. Run
`omj setup --with-menubar` to request it explicitly. Missing `swiftc` or a
failed helper start does not make the OMJ workflow setup fail; setup reports the
menu bar step separately.

A native macOS MenuBarExtra app, the OMJ menu bar helper, or a test harness can
...
```sh
omj menubar status --overlay /path/to/overlay.json
omj menubar status --observe-local-processes
```

The overlay and local observation are app-local and expire by TTL. OMJ does not
...
omj setup --with-mcp
omj mcp manifest
omj mcp config-recipe --host claude-code
omj mcp config-recipe --host codex
omj mcp config-recipe --host opencode
omj mcp config-recipe --host cursor
# wrapper/host adapters can record observed host load when they see it:
omj mcp observe-host --host hermes-agent --session <session-id> --event host_load --evidence-ref <host-log-ref>
```
...
event. `omj mcp manifest` prints the generic stdio MCP bridge contract, and
`omj mcp config-recipe --host ...` prints host-shaped copy-paste snippets for
common MCP-capable environments without mutating those host config files. The
bridge exposes only local `omj_status`,
`omj_recommend`, and `omj_probe` tools; it is not arbitrary shell access,
connector execution, coding dispatch, or proof that an MCP runtime is active.
`omj mcp observe-host` is for host/wrapper adapters that already observed
bridge load or use and can attach a stable evidence reference. It records
`omj_mcp_host_session/v1` metadata; it does not discover or force host loading.

The OMJ plugin follows the same evidence split. `omj setup` installs the plugin
bundle and `omj doctor` can prove local import/register smoke. A Hermes host or
wrapper that actually sees the plugin load can record that runtime event:

```sh
omj plugin observe-host --host hermes-agent --session <session-id> --event plugin_load --evidence-ref <host-log-ref>
omj plugin observations
```

This writes `omj_plugin_host_observation/v1`. It is plugin load/use evidence
...
`observation` metadata to OMJ plugin tools/hooks. The plugin then records the
...
role, and show the next action without asking the chat user to run `omj`
commands.

## Hermes CLI Release Smoke

For release candidates, OMJ provides a dedicated smoke contract for the real
...
```sh
omj release install-smoke
```

When you want observed evidence that `install.sh` itself works from a checkout,
run it live. This still does not mutate your real Hermes profile; OMJ creates a
...
`first_use_status_smoke`. The first checks that the installed `omj` command is
...
After installing OMJ into the target runtime, verify the command path too:
...
OMJ workflow context to route well.
...
Use `omj release product-readiness --version 1.0.1 --json` when you want a
...
Use `omj release evidence-bundle --version 1.0.1 --write --json` when you want
that local release-candidate evidence written under
`.omj/runtime/release-evidence/` for a release PR or release note. The bundle is
...
list/check plus `omj doctor`, because current Hermes CLI releases do not
reliably inspect local external-dir skills by short name. Neither path proves
that a later Hermes chat session selected OMJ unless that chat response is
observed separately.

## Install Path B: OMJ Bootstrap Setup
...
surfaces such as `omj recommend`, `omj playbook recommend`, and
`omj chat interact` use a deterministic local phrase layer for tested Japanese,
...
path: Hermes can discover OMJ skills and the user talks to Hermes. `omj setup`
...
omj list
omj runtime status
omj runtime team-readiness
omj probe
omj probe --parity
omj probe --roadmap
```

`omj setup` should report a human-readable setup summary by default, including
the setup location, overall readiness, terminal command availability, OMJ
workflow path, Hermes connection path, coding request preference, profile
check, and OMJ status helper state. It should not require first-time users to
...
`omj doctor` should report a grouped health summary by default: managed skills,
...
state-log entry when the runtime directory is writable. `omj doctor --json`
returns the full check payload plus `doctor_summary/v1`. `omj list` should show
a concise managed skill summary plus workflow lanes by default. `omj list
--json` returns the managed manifest plus `omj_installed_skill_catalog_context/v1`
...
by default: `omj install`, `omj update`, `omj uninstall`, `omj apply`,
`omj list`, `omj recommend`, `omj playbook ...`, `omj profile ...`,
`omj probe`, and `omj snippet --output`. Use `--json` on those commands, or set
`OMJ_OUTPUT=json`, when a wrapper or automation needs the complete payload.
Short chat requests such as `omj update`, `omj setup`, `omj doctor`, `omj
install`, and `omj list` should stay in that maintenance lane: run the requested
...
surface.
`omj runtime status` should show the local runtime artifact directory and the
latest install/apply/doctor state when those commands have run. `omj probe`
reports observable Hermes capability surfaces without mutating Hermes internals.
For MCP, `omj probe` reports the bridge server, setup preference, runtime tool
call observation, host session observation, and host config separately:
`mcp_preference` means `omj setup --with-mcp` was requested in OMJ local state,
`mcp_bridge_server` means the installed command package exposes `omj mcp serve`,
`mcp_bridge_runtime` means OMJ has observed a local MCP bridge tool call, and
`mcp_host_session` means a host or wrapper recorded load/session evidence with
`omj mcp observe-host`. `mcp_host_config` only means a Hermes MCP config file
such as `.mcp.json` or `mcp.json` exists. `omj mcp config-recipe --host
...
records that event.
After `omj setup` has run, `omj doctor` also checks the managed plugin manifest
plus local import/register smoke. `omj probe` reports
...
use. When a host or wrapper records `omj plugin observe-host`, or invokes an OMJ
...
Use `omj runtime team-readiness` when an operator or wrapper wants to know
whether Hermes/team/swarm coding paths are ready to present. It returns
`omj_team_worker_readiness/v1` with the installed skill visibility, runtime
...
difference between "OMJ ships this contract" and "Hermes can currently see the
installed team/ultrawork skill surface."

Use `omj probe --parity` when an operator wants the broader comparison against
common oh-my runtime capability axes. It returns `omj_parity_matrix/v1` with
...
maintenance. Available means OMJ has the deterministic contract or backend
...
`omj worktree prepare` can create the local Git worktree and record
`omj_worktree_observation/v1`; that still proves workspace isolation only, not
executor dispatch or implementation.

Use `omj probe --roadmap` when the question is "what should I do next?" rather
than "what does OMJ support?" It returns
`omj_capability_gap_roadmap/v1` and separates baseline product/setup gaps from
host or wrapper evidence gaps. For example, missing managed skills or Hermes
registration points to `omj setup`; missing plugin runtime, MCP host-session,
or wrapper-session observations points to the host or wrapper evidence that
must be recorded before OMJ can claim those runtime states. `omj probe
...
1. The wrapper receives a user message in Discord, Slack, Telegram, or another chat
   surface.
2. The wrapper calls `omj chat interact` with the platform source and either a
   plain message or event JSON.
3. `omj` returns one `chat_interaction/v1` envelope with a renderable
...
4. At startup or deploy time, the wrapper can call
   `omj chat native-command --source discord`, `--source slack`, or
   `--source telegram` to obtain the platform registration contract for `/omj`
...
5. If the message is a partial command prefix such as `./`, `/`, `./o`, or
   ...
   `omj_command_fallback_card/v1` style card with a single `Open omj` action.
   Selecting it submits `./omj` or `/omj` and opens the workflow picker.
6. If the message is `./omj`, `/omj`, `./skills`, or `/skills`, the wrapper
   ...
   names clean; the skills do not need an `omj-` prefix.
7. If the user asks what OMJ commands, skills, or workflows are available, the
   wrapper still renders `chat_response.kind == skill_picker`. Do not ask the
   user to approve `omj list` for a catalog question; `omj_skill_picker/v1`
   ...
   `omj_capability_summary/v1`, which lets Hermes explain the larger
   ...
   it can use `omj context brief --json` or plugin tool `omj_context` to fetch
   `omj_context_brief/v1`: lanes, common cues, generic-tool checkpoint, optional
   route hint, and response rules without storing or echoing the raw prompt.
   If the user asks "what is OMJ?" or "how do I use OMJ?", `omj chat interact`
   can now return `chat_response.kind == context_brief` directly, with the same
   structured `omj_context_brief/v1` under `chat_response.state.context_brief`.
8. If the user asks what to do next after setup or install, the wrapper returns
   `chat_response.kind == quickstart` with `[omj] quickstart`, the
   `omj_quickstart_card/v1` payload, first-use Hermes prompts, and the same
   capability roadmap metadata. If the user explicitly asks for detailed status,
   the wrapper returns `chat_response.kind == status` with `[omj] status` and
   ...
   approve shell commands just to understand OMJ health.
9. The wrapper renders `chat_response.headline`, `body`, `state`, `actions`, and
   ...
   already starts with the visible OMJ marker, such as `[omj] web-research`;
   ...
   why/next/not-evidence card so Hermes can explain why OMJ selected this
...
10. Adapters apply `chat_response.messenger_rendering` for the selected surface:
   ...
   `omj chat interact --render-profile limited_markdown`. The prefix appears
...
11. If the interaction asks for clarification, the wrapper keeps the answer in
   the same thread and calls `omj chat interact` again with the updated message.
...
14. For a coding profile that has not been observed before, the wrapper can run
   `omj coding executor-readiness --executor <profile>` once and cache
...
16. If the wrapper has local memory-like context candidates, it can run
   `omj memory inspect` and attach a conflict-free `handoff_context_pack/v1` to
   the later handoff. Conflicting or stale assumptions must be shown as memory
   review, not silently reused.
17. Status updates use `omj coding lifecycle report` or
   `omj chat interact --run <run-id>` and stay in the same thread.
18. Hermes still starts with its normal config and reads `skills.external_dirs`;
   `omj apply` makes sure `~/.omj/skills` is included in that discovery list.

`omj` provides deterministic local contracts for command registration, fallback
...
```sh
omj chat interact --source discord --event-json event.json
omj chat interact --source slack "risky refactor"
printf '%s' "$SLACK_TEXT" | omj chat interact --source slack --stdin
```
...
With `--auto-apply-target-change`, OMJ persists the observed target registry
...
omj chat session select-executor "$session_id" claude-code
omj chat session select-executor "$session_id" generic
```

Check the selected coding agent once before first dispatch:

```sh
omj coding executor-readiness --executor codex
omj coding executor-readiness --executor claude-code
omj coding executor-readiness --executor omx-runtime
```
...
```sh
omj memory inspect --fixture wrapper-memory.json
omj memory pack --fixture wrapper-memory.json --executor codex --session-id "$session_id" > handoff-context.json
omj chat session prepare-handoff "$session_id" --context-pack handoff-context.json "risky refactor"
```
...
written only to `.omj/memory/`; OMJ does not read or mutate opaque Hermes
internal memory.

Codex lifecycle calls after the wrapper has an accepted Codex coding handoff:

```sh
start_json="$(omj coding lifecycle start --executor codex --record "risky refactor")"
run_id="$(printf '%s' "$start_json" | python3 -c 'import json,sys; print(json.load(sys.stdin)["run"]["run_id"])')"

# Dispatch to the external Codex executor outside OMJ, then record the
...
omj runtime review --run "$run_id" --status passed --reviewer code-review --evidence-ref review-comment
omj runtime ci --run "$run_id" --status passed --check "unit:passed"
omj runtime merge --run "$run_id" --ready --target-branch main
...
omj hermes plan --source discord --record "risky refactor with review"
omj hermes plan-accept .hermes/plans/<accepted-plan.md>
omj coding delegate --executor codex --source discord --record --from-plan .hermes/plans/<accepted-plan.md>
omj coding delegate --executor claude-code --source discord --record "risky refactor"
omj runtime delegation-status --run <run-id>
```

`omj hermes plan --record` writes a draft `hermes_plan/v1` Markdown artifact
...
```sh
omj docs workflows --json
omj harness list
omj harness inspect planning
omj harness validate
```

Use `omj runtime export --redacted` when you need a portable support artifact.
...
`omj` records runtime metadata only by default:

- setup/install/apply/doctor summaries in `~/.omj/runtime/state.json`
- workflow run envelopes in `~/.omj/runtime/runs/<run-id>/run.json`
- append-only run events in `events.jsonl`
- append-only lifecycle observations in `~/.omj/runtime/journal/events.jsonl`
- wrapper chat sessions in `~/.omj/runtime/wrapper_sessions/<session-id>/`
...
- The installer ran in the same runtime context as the Discord, Slack, or hosted
  chat wrapper.
- `omj doctor` reports the managed skill directory as installed and registered.
- The bot process can read the same Hermes home/config that `omj apply` updated.
- The bot was restarted after installation or update.
- plugin `omj_interact` returns the same `chat_interaction/v1` envelope and can
  record a metadata-only wrapper session when Hermes supplies host/session
  metadata.
- `omj chat interact --source discord "<message>"` or
  `omj chat interact --source slack "<message>"` returns a
...
- `omj coding delegate --executor codex --source discord --record "<message>"`
  ...
  `executor_handoff`.
- `omj coding delegate --executor codex --record --from-plan <accepted-plan.md>`
...
- `omj coding lifecycle start --executor codex --record "<message>"` creates a
  prepared Codex handoff lifecycle without storing the raw prompt body by
  default.
- `omj coding delegate --executor claude-code --record "<message>"` and generic
  profiles return a `coding_prompt_handoff/v1` prompt-only payload without
  creating a lifecycle run.
- `omj coding delegate --executor hermes --record "<message>"` and
...
- Coding handoffs include `worktree_session_isolation/v1` so wrappers can show
  ...
  or observes the workspace action. `omj worktree prepare --repo <repo> --task
  "<task>"` is the explicit local backend action for creating the Git worktree;
  `omj worktree bind --path <worktree> --executor codex --session <session-id>`
...
- Codex handoff payloads expose `codex_skill` plus
  ...
  dispatches to Codex.
- `omj memory pack` attaches `context_pack` only when no unresolved memory
  conflict remains; otherwise the handoff contains `context_pack_blocked`.
- `omj memory apply --batch <file> --dry-run` previews approved memory updates
  without writing, and the real apply writes only under `.omj/memory/`.
- `omj coding lifecycle result --run <run-id> --result completed` is rejected
  until `omj coding lifecycle dispatch --run <run-id>` records dispatch
  observation.
- `omj coding lifecycle report --run <run-id>` does not claim final completion
  while executor, verification, review, CI, or merge-readiness evidence is
  missing.
- `omj hermes plan --source discord --record "<message>"` writes a
...
- `omj runtime record` can create a run and `omj runtime show <run-id>` can read
  it from the same runtime context.
- `omj probe` reports managed skills and external skill directory registration
  as available before any deeper integration claim is made.
- If skills do not appear, run `omj setup`, then `omj doctor`, then restart the
  bot again.

Current limitation: plugin `omj_interact`, `omj chat interact`,
`omj chat route`, `omj coding delegate`, and `omj coding lifecycle` choose
...
and merge evidence that OMJ later records.

## Update

Update the installed `omj` command package and then refresh the managed skill
pack:
...
Most users should run only `omj update`. When `omj` is running from the default
...
managed skills, and records a concise update log. If `omj` is running from a
pip, pipx, distro, or custom Python install that OMJ cannot safely mutate, the
update still refreshes workflows but prints
`OMJ command: not updated (workflows only)` plus the installer command needed to
update the CLI itself. Successful command package updates print a compact line
such as `OMJ command: 1.0.0 -> 1.0.1 (updated)` or
`OMJ command: main@old -> main@new (updated)` before the workflow summary.

Advanced operators can still pin or test a different source with
`omj update --channel stable --version <version>` or
`omj update --channel local --from-skills-dir ./skills`, but those flags are for
release validation, fixtures, or intentional rollback testing. Local
modifications block updates unless `--force` is supplied.

Run `omj doctor` after an update. Use `omj setup` only when doctor reports that
Hermes registration needs repair, then restart Hermes Agent. Rerun the installer
manually only when `omj update` says the command package was not updated, or
when you intentionally want a one-shot reinstall from a specific source ref. The
installer passes command-package update evidence into OMJ so the state log can
...
under `~/.omj/runtime/state.json` as `last_setup`, `last_install`,
...
Use project-local OMJ/Hermes paths during setup:
...
```sh
omj setup --profile-pack cto-loop --profile-pack startup-delivery
```
...
```sh
omj setup --operating-model coding-runtime-team
```
...
```sh
omj profile list
omj profile inspect coding-runtime-team
omj profile inspect cto-loop
```
...
directly to setup with `omj setup --language zh`.

Skip Hermes config registration during setup:

```sh
omj setup --skip-apply
```
...
`omj setup ...` as a separate command.

## Uninstall

Remove OMJ-managed local state and Hermes integration files:

```sh
omj uninstall
```

This unregisters `~/.omj/skills` from Hermes config, removes `~/.omj`, removes
the managed `~/.hermes/plugins/omj` plugin bundle when it has an OMJ manifest,
removes generated team role files recorded in OMJ team-profile manifests, and
removes the install.sh-managed `omj` command venv/link when the current command
...
that OMJ cannot safely identify as install.sh-managed.
If `omj` still runs after uninstall, that means the command package is still on
...
```sh
omj uninstall --dry-run
```

Only remove the Hermes config registration:

```sh
omj uninstall --registration-only
```

Legacy cleanup for just the registration plus managed `~/.omj` directory:

```sh
omj uninstall --remove-files
```

`omj uninstall --all` and `omj uninstall --purge` are explicit aliases for the
default full cleanup. Add `--force` only when you intentionally want to remove
an unmanaged `~/.hermes/plugins/omj` directory. Add `--keep-command` when you
