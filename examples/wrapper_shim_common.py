...
from omj.ingress import extract_message_text, extract_source_metadata  # noqa: E402
from omj.wrapper.contract import (  # noqa: E402
    INTERACTION_MODES,
    build_chat_interaction_payload,
    build_chat_status_interaction,
    usage_trace_payload,
)
from omj.wrapper.native_commands import build_native_command_surface, render_native_command_response  # noqa: E402
from omj.wrapper.route_hints import build_chat_route_hint_payload  # noqa: E402
from omj.plugin_bundle.omj.tools.chat_tool import omj_interact_handler  # noqa: E402
...
def run_shim(source: str, argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Render the fixture through the managed plugin's omj_interact tool. "
            "By default this uses temporary OMJ/Hermes homes so the example does not mutate real user state."
        ),
    )
    parser.add_argument(
        "--status-json",
    ...
    parser.add_argument(
        "--omj-home",
        default=None,
        help="Optional OMJ home for --plugin-interact. Defaults to an isolated temporary directory.",
    )
    parser.add_argument(
        "--hermes-home",
        default=None,
...
def _plugin_interaction(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="omj-plugin-interact.") as temp_root:
        root = Path(temp_root)
        return _plugin_interaction_with_homes(
            source,
            message,
        ...
        omj_interact_handler(
            {
                ...
                "observation": {
                    ...
                    "event": "tool_call",
                    "evidence_ref": f"fixture:{fixture.as_posix()}#omj_interact",
                },
            }
        )
    )
    payload["example_runtime"] = {
        "schema_version": "omj_plugin_interact_example_runtime/v1",
...
def _render_plugin_interaction(source: str, interaction: dict[str, Any]) -> dict[str, object]:
    ...
    rendered["plugin_interact"] = {
        "schema_version": "omj_plugin_interact_render/v1",
        "tool": "omj_interact",
